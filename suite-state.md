...
_Last verified: 2026-09-03 (nightly cron)_
...
## System Health (as of 2026-09-03)
...
## Field Data Plane — SKS tenant (as of 2026-09-03)
...
| Operational | app_data.field_timesheets | 286 | ✓ 286 |
...
## Product Pulse (as of 2026-09-03)
...
| Active users | blocked | `service.profiles.last_login_at` never populated by Shell SSO (0-of-5, verified 2026-09-03) — see `ops/pending.md` |
...
- **EQ Field mobile-centering sprint closed same-day — 12 of the 16 audited items were already shipped, not open** — a 2026-08-07 OneDrive audit doc listed 12 P1 gaps; live-code verification before starting any build found all 12 already fixed in `v3.5.469`, the audit having never been updated after that release shipped. The follow-on P2 scope (a worker/supervisor split for Safety + Leave) was also partly stale once checked: Leave's supervisor view already existed and was already mobile-responsive, and Safety's Prestart/Toolbox/Diary/Incident list pages are already supervisor-only by permission grant (`reports.*.view`/`.create`) — there was no worker view to split from. Real gaps found and shipped instead: Safety list-header counts made visually prominent + Leave approve/reject buttons bumped to the 44px tap-target standard (`v3.5.484`, field PR #680); Calendar's desktop-only month grid gained a mobile agenda list + bottom-sheet day detail, converted using the same pattern as Timesheets' Job Numbers panel (`v3.5.485`, field PR #681); the Teams drawer's missing active-page highlight (a gap noted but left unfixed at v3.5.469) plus 2 more manager-only voice-mic tap targets (`v3.5.486`, field PR #682) — a third candidate (a photo-remove badge) was explicitly declined rather than rushed, since bumping it to 44px would have made it bigger than the 84px thumbnail it sits on. All three merged and confirmed live via `field.eq.solutions/sw.js`'s `CACHE` const. Timesheets mobile work explicitly deferred (Royce's call — not a priority without usage data). Full record: `eq-context/eq/sprints/2026-08-12-field-mobile-centering.md`. (field PRs #680/#681/#682, 2026-08-12) **Correction (2026-08-30, failure ledger F14):** the "supervisor-only by permission grant" clause above went stale on 2026-08-24, when a permission-matrix change opened `reports.prestart.create`/`reports.toolbox.create` to every Field role except subcontractor — this entry was never updated. The staleness went unnoticed until it caused a real bug: Prestart/Toolbox "+ New" buttons still carried a manager-only CSS class nobody removed when the policy changed, so a supervisor-permitted worker (Cameron Tregoning) couldn't see the button at all for 6 days. Root-caused and fixed live (eq-field PR #839, v3.5.610) — current state: Prestart/Toolbox create is open to every Field role except subcontractor, not supervisor-only.
- **eq-ui gains Tooltip, EmptyState, and Pagination components** — closes three real gaps found while building the kitchen-sink reference against the EQ design system. Originated as a Claude Design handoff (real TSX/CSS, not mockups); before merge it was run against the actual toolchain and two issues fixed: a `jsx-a11y/no-static-element-interactions` lint failure in Tooltip (Escape handling moved to a document-level listener, matching `DropdownMenu`'s existing pattern) and a props-shape mismatch (EmptyState/Pagination now extend native HTML attributes + forward refs/rest props, matching `Card`/`StatusBadge`). All three use existing `--eq-*` tokens only, no new tokens. `npm run check` green (43/43 tests), manually verified live in the kitchen-sink dev preview. Published as `@eq-solutions/ui@1.12.0`. (eq-ui PR #33, 2026-07-26)
- **eq-shell's QuotesModule DropdownMenu was likely rendering unstyled in production** — bumping the `@eq-solutions/ui` pin to v1.12.0 (to pick up the components above) surfaced that the barrel stylesheet consumed via `@eq-solutions/ui/styles` had been missing `DropdownMenu.css` since v1.11.1, and this repo's only CSS import path is that barrel — confirmed directly against the live `v1.11.1` git tag, not assumed. `build:packages && tsc -b` clean, 247/247 tests, `check:css` (549 `eq-*` classes resolve) confirms the fix. (eq-shell PR #1027, 2026-07-26)
- **eq-service bumped `@eq-solutions/ui` to v1.12.0 too** — purely additive, this repo has no `DropdownMenu` usage so the barrel-CSS bug above never affected it. (eq-service PR #604, 2026-07-26)
- **eq-service's `sharp`/`uuid` npm-audit findings closed via `overrides`, not a `next`/`exceljs` major bump** — investigated whether untrusted image bytes ever reach the vulnerable `sharp`/libvips path via `next/image`: they don't (`next/image`'s `<Image>` is never imported anywhere in this repo; attachment/defect-photo thumbnails render via a plain `<img>` from signed Supabase Storage URLs). `exceljs`'s only `uuid` usage is one `require('uuid').v4` call, confirmed identical under the patched version's CJS export. Fix pins just `sharp@^0.35.3` + `uuid@^11.1.1` via `package.json` `overrides`, leaving `next`/`exceljs` untouched — avoids the breaking downgrade `npm audit fix --force` would have forced. (eq-service PR #605, 2026-07-26)
