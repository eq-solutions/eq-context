...
5. **Freshness gate — verify what you loaded is actually current.** MANDATORY. Compare the `last_updated` frontmatter of `system/TODAY.md` and the generation stamp of `digest.md` against today's date.
   ...
   | `products.md` `last_updated` > 14 days old | Flag it: product status/version numbers are leads, not facts — check the file's own staleness banners before quoting a version number. |
...
- **Decision quality on demand** — "steelman this" / "no mistakes" / "high value?" / "will this work" / `/decide`. Six-step pass (frame, steelman, pre-mortem, value, feasibility, the call) for choosing *between* options, before anything is finalized — see `rules/decision-protocol.md`. Scale the output to the decision; don't run full ceremony on a trivial call.
...
| Triage protocol (clickable pass through digest.md's curated Needs-you/Waiting-on-you/Aging buckets, batched HTML cards) | [rules/triage-protocol.md](https://raw.githubusercontent.com/eq-solutions/eq-context/main/rules/triage-protocol.md) |
| Deploy topology protocol (which repo actually serves a URL, verified live, never assumed from naming) | [rules/deploy-topology-protocol.md](https://raw.githubusercontent.com/eq-solutions/eq-context/main/rules/deploy-topology-protocol.md) |
...
| Substrate tidy protocol (on-demand pressure-release pass — prune rule/file bloat, Royce-triggered only) | [rules/tidy-protocol.md](https://raw.githubusercontent.com/eq-solutions/eq-context/main/rules/tidy-protocol.md) |
...
| **Claude Chat** (claude.ai) | Memory + pasted `CHAT-PROMPT.md`; substrate via **GitHub connector** | **GitHub MCP — confirmed working 2026-09-14** (updates the prior "None"/403 assumption; see `ops/security-register.md` SEC-78) | Read substrate through the GitHub connector, never web fetch (can't construct URLs; returns previews, not raw text). Connector missing → fresh session after Royce connects it. Write access exists but bypasses every local-clone guardrail (`pre_tool_use.py`, the 200-line edit block, the NUL-fill scan) — treat any Chat-session write with the same caution as an ungated `git push` until SEC-78 is resolved. |
