...
**Deferred:**
- [ ] Same `tender_enrichment`/`nominations`/`pending_schedule` truncation-stopgap pattern likely also exists in `eq-field/scripts/sks-pipeline.js` + `sks-pipeline-resource.js` (already has `limit=1000-5000` guards, not yet swept the way SKS's own pipeline files were) — not checked this session, worth a follow-up sweep if eq-field's pipeline tables approach those caps. _(added 2026-07-10)_
