...
**Deferred:**
- [ ] **eq-field bulk-export path has the same unbounded-fetch pattern** — `_loadFullDataForExport()` still does unscoped `schedule?select=*` / `timesheets?select=*` for the export feature. Same truncation risk once a tenant's table passes 1000 rows, but shows up as an incomplete export rather than a save/display failure — no active report. Flagged as `task_69a6ff0f`, spawned into its own session by Royce, running independently as of session close. _(added 2026-07-10)_
