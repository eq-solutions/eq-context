...
- [ ] **Dispatch both migrations to ehow** — PR #705 merged (read fix + write fix, both deliberately `NOT APPLIED`). The write-side migration was corrected same session: its first draft scoped supervisor writes to "their own crew only", which would have broken two real tenant-wide features (bulk roster-prefill, CSV import) — fixed to "own row, or any approver, tenant-wide" instead, matching how those features actually work today. Still blocked on the same gap either way: dispatching now would lock the ~44 SKS staff who haven't signed in yet out of their own timesheet/leave the moment they do, since the fix depends on their login already being linked to their staff record. That population is bounded (hasn't grown since 2026-07-05) and resolves itself as Royce's QR-code onboarding rollout (apprentices first) gets people signed in — needs a call on whether to wait for that to land or dispatch anyway with a documented gap. _(added 2026-08-16, updated 2026-08-16)_
