...
*Continuation of the same-night "birthday still broken" investigation (background task `task_bb6cab43`) — Aiden Crowley's birthday visibly selected (Day=18, Month=Feb) in EQ Field's Edit Person modal, saved with no error, came back null — twice. Root-caused to an undocumented database trigger, not any app code — every client-side layer (the form, the save function, the write payload) checked out correct on inspection, in both this session and the one before it.*

- [x] **Fixed + shipped live.** `app_data.staff` carried an out-of-band trigger (`staff_derive_dob_from_cards`, never recorded in any eq-field migration) that unconditionally re-derived day/month from a separate, unrelated "full date of birth" field — which is null for anyone who's never gone through the licence-verification wizard. Every save silently wiped the birthday back out, in the same write that had just set it — no error, the request genuinely succeeded, it just also got quietly undone. Fixed with a one-line guard, applied directly to ehow and verified end-to-end before landing the paper trail: eq-field [PR #767](https://github.com/eq-solutions/eq-field/pull/767), merged.
...
- [ ] **Open product question, not decided**: for a Cards-linked person who has both a full date of birth AND a differently-valued directly-edited day/month, the fix still lets the full date win on the next save. Probably correct (one source of truth) but Royce's call, not resolved. _(added 2026-08-24)_
