...
- [ ] **The 24 unlinked names need a human pass before any resend/nudge action gets built** — this list already overlaps a name this substrate previously excluded for good reason (Thomas Cavanough, 2026-07-05 entry below: left the company, explicitly excluded from any future invite/roster work). Worth Royce reviewing the live list on `/sks/staff` (filtered to "Not signed in") before anyone automates a nudge against it. _(added 2026-08-20)_
...
- [ ] **This divergence is still live today and now includes timesheets too — checked directly, not assumed.** Live proof (2026-09-01): joined 12 real SKS workers across both DBs by `people.canonical_id = workers.id = field_people.worker_id` (100% match — the identity bridge is sound). Found named, dated, ongoing one-way drift: Aiden Crowley's whole week was rewritten in EQ Field on 2026-09-01 03:39 (marked sick Monday, moved Wed–Fri to a different site) while sks-labour still showed the prior day's version; Richard Brooks' Wednesday changed in EQ Field on 2026-08-31 21:48, never synced back. Widened the check to timesheets (out of scope on 2026-08-14): EQ Field's capture for the same real people is thin (5–11 rows/person over ~2 months) against sks-labour's dense daily entries (779 rows, 46 in the last 7 days) — EQ Field is not a working timesheet replacement yet. **Matters more now than on 2026-08-14**: sks-nsw-labour's login screen was changed this same session (v3.10.112, PR #79) to actively point staff at `core.eq.solutions/sks/field` — so people are now being sent toward the diverging, thinner copy. Needs Royce's call on reconciling, or at least deciding which system is authoritative, before that notice drives real traffic over. _(added 2026-09-01)_
...
- [ ] **Structure now built (2026-09-07, see `eq/pending/eq-shell.md`)** — a real `manager_id` field + Staff-page UI exists now, restricted to Royce only until the backfill runs. What's still missing is exactly what this line originally flagged: a fresh export from the SKS interactive org-chart tool's own Export/Import JSON function, to seed it for real. Still needs Royce to supply that file.
