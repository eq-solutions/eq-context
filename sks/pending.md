...
- [ ] **Empty "Pipeline" nav header in employee view** (spawned as background task `task_dcd8df1b`, running) — the sidebar section wrapper `#nav-section-pipeline` isn't role-gated but all 3 of its items (Pipeline/Resources/Accounts) are `edit-only` (`.nav-item.edit-only` → `display:none` without `body.manager-mode`, base.css:210-211). An employee-view Core login never gets `manager-mode`, so the items hide and only the orphaned "PIPELINE" label shows. Fix: hide a `.nav-section` when all its nav-items are hidden (group-level `edit-only`, or a post-`applyTierVisibility` JS sweep). Surfaced 2026-07-11 when Royce hit it after logging into Core as an employee-only account — NOT a bug in this session's work, just role-gated nav behaving as designed minus the empty-header polish. _(added 2026-07-11)_
