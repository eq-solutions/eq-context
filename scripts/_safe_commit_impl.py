...
Normally invoked via the scripts/safe_commit.py bootstrap sitting next to this
file, not directly -- that bootstrap fetches origin/main and always execs the
CURRENT copy of this file from there, so a caller's own on-disk copy of THIS
file no longer needs to be current for its checks to apply (see
system/failures.md F17, second occurrence, for why that distinction exists).
Invoke this file directly only to test a local, uncommitted edit to it before
pushing -- that intentionally bypasses the bootstrap and runs exactly what's
on disk here, same as any other script.
...
Usage (normally via the bootstrap -- see scripts/safe_commit.py):
...
Before even that, it checks whether THIS FILE's own on-disk copy is current
(see check_script_currency) -- a defense-in-depth backstop for the case where
something invokes this file directly rather than through the bootstrap. When
invoked normally (via the bootstrap), this check always trivially passes: the
bytes running here were fetched fresh from origin/main moments ago, and don't
even resolve as "inside" a checkout the comparison can reach (see the
function's own ValueError handling) -- so it costs nothing in the common case
and only matters for a direct, bootstrap-bypassing invocation.
"""
...
def check_script_currency(root: Path, force: bool) -> bool:
    """Compare THIS FILE's own on-disk bytes against origin/main's current copy
    of itself (already fetched by the time this runs). Only matters when this
    file is invoked directly, bypassing the scripts/safe_commit.py bootstrap
    (see the module docstring) -- the bootstrap always execs a copy fetched
    fresh from origin/main, which lives outside this checkout entirely, so
    the comparison below no-ops via the ValueError path for every bootstrap
    invocation. Direct invocation is how development/testing of THIS file
    itself works, deliberately -- but a caller who forgot they were doing
    that, or who has an old direct-invocation muscle-memory habit from before
    the bootstrap existed, still gets a clear warning instead of running
    silently-stale logic. See system/failures.md F17 (second occurrence):
    a checkout sat two hours stale relative to a fix here and silently ran
    the old, no-guard version for the rest of that day -- this and the
    bootstrap are the two-layer response to that."""
    ...
    print(
        ...
        "on the old code anyway. (Or: use scripts/safe_commit.py -- the bootstrap next "
        "to this file -- which always runs origin/main's current copy regardless.)",
        file=sys.stderr,
    )
    if force:
