...
|------|-------------|--------|---------------|---------|--------------|
| EQ Shell | `eq-solutions/eq-shell` | `main` | GitHub push → Netlify CD — **merging to `main` IS the deploy** (auto, unattended — "live 2-4s later" was wrong, corrected to ~minutes; see note below) | dev@eq.solutions | Explicit instruction only — **and the approval gate bites at the merge click** |
...
- **`eq-shell` is the opposite trap, and it is the one that bites harder.** Merging a PR to `main` **does** trigger a Netlify production build, unattended, no separate deploy step ever. **The "2-4 seconds" timing above was wrong — corrected 2026-09-09.** Directly measured on a real merge: [eq-shell PR #1858](https://github.com/eq-solutions/eq-shell/pull/1858) merged 11:17:44Z, confirmed live on core.eq.solutions (via the served JS chunk, not a deploy-status proxy) at 11:28:12Z — a 10m28s gap. Two more deploys pulled from the Netlify API the same session showed the same shape: one created 11:07:28Z / published 11:20:53Z (13m25s), another created 11:05:02Z / published 11:17:48Z (12m46s). All three land in the same several-minutes range, not seconds. **The deploy is still automatic, unattended, and there is still no separate "safe" intermediate step — that part of the original claim was never wrong.** What was wrong is treating it as *instant*: in a busy merge window (6 merges landed in the ~10 minutes around this check, which is normal for this repo, not exceptional) an individual commit's own build routinely finishes several minutes after the merge click, sometimes after being superseded by a later commit's build first (see the concurrent-merge note below). **Never say something is "merged but not live" as if that's surprising or a bug — for several minutes after every eq-shell merge, it's the expected, normal state.** Treat every eq-shell merge approval as a production-deploy approval regardless of this latency — the auth-change review rule still bites at the *merge* click, since there is no later "actually deploy it" step to gate on instead. Trigger is Netlify's own GitHub App (installation `121276861`), not a repo webhook and not a workflow. The original 2026-08-15 verification ("13 consecutive merges -> 13 production deploys") isn't necessarily wrong on ITS OWN claim (that merging reliably triggers a production deploy, no manual step) — only on the timing. Its methodology for the timing figure specifically isn't recorded in enough detail to know how "2-4s" was arrived at; most likely it measured how fast a new deploy record appears (the webhook firing) rather than how long that deploy takes to finish and publish, which is a different and much larger number. Don't cite "2-4s" for anything going forward.
