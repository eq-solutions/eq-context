...
| 2026-09-07 (d) | 317 | 430 | 1,429 | 4,807/5865 | 405 | 260 | 3,262 | 96 | Fourth pass, same day, run inside a live `/triage` session rather than standalone — kept deliberately tight given three passes already ran today. Before touching anything: found this protocol's own committed copy (pass (c)'s row + footnote) missing from the local working tree, and `system/failures.md` similarly missing a Budget line and F14's 5th-recurrence update that pass (c)/PR #219 had already merged — both reconciled onto origin/main's current content first (not this pass's finding, but blocking correctness for it: writing a new row on a stale base would have silently reverted merged work). `eq/pending/eq-cards.md` had the same shape (a `/triage` note from a different concurrent session, plus a local-only unmerged bullet) — also reconciled, merging both sides rather than picking one. rules/*.md +133 vs (c) is ~entirely `rules/triage-protocol.md` (134 lines, new this session, already in the preferred `/gap`-shaped pointer form — not bloat). **Step 4 finding:** `digest-refresh.yml`'s own comment describes its `repository_dispatch` trigger as conditional ("once `EQ_CONTEXT_PAT` is added... and sender jobs are wired") — but that secret has existed since 2026-09-05 (confirmed via `gh secret list`) and digest.md's commit history (8 commits in the trailing 13h) shows it firing live, nowhere near "nightly." Comment now describes a precondition that's been true for 2 days as if still pending. Same undersell shape in `suite-state.md`'s own Crons table ("Nightly ~9pm" for itself, contradicting the accurate merge-triggered description two sections up in this repo's own root `CLAUDE.md`) — the real fix belongs in whichever script writes that table, not a hand-edit here. Both flagged to Royce, not yet fixed — pending confirmation on where the suite-state.md fix should land. **Step 5, closed for real this time:** `eq/pending/*.md` (11 shards) given the ~500-line budget Step 5's own text already suggested as an example — written into each shard's frontmatter `scope` line. 3 shards already over it at write time: `cross-repo.md` (912), `eq-field.md` (1284), `eq-shell.md` (1394) — flagged for a dedicated prune pass, not curated here; deciding which specific bullets are stale enough to archive is real editorial judgment, not a line-count mechanical task. `eq/pending/*` column now reports both the strict-directory figure and the wider family total (sks/pending.md + ops/pending.md included) since Step 5's fix only reaches the former. **failures.md** (257→260 net of the reconciliation) picked up 3 lines from the reconciled Budget block. **Step 6:** lighter in the two doc-accuracy spots once Royce confirms; heavier in `rules/*.md` by design (triage protocol, real new capability, correctly shaped); the pending-shard mechanism gap Step 5 flagged twice before is now closed with a stated number, not just noted a third time. |

† Every tracked column now carries a stated budget somewhere in its own file(s), not just a Pass Log entry: `system/worktree-registry.md` ~150 lines (2026-09-07); `system/lessons.md` ~500 lines and `system/failures.md` ~400 lines (both 2026-09-07 (c)); `eq/pending/<repo>.md` shards ~500 lines each, stated in each shard's own frontmatter (2026-09-07 (d)). Closes the gap this footnote named through passes (b) and (c).
