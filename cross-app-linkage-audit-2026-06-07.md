...
| S6 | **CLOSED/SUPERSEDED** | `urjhmkhbgaxrofurpbgc` eq-solves-service | DELETED 2026-06-22 — finding no longer actionable | `urjhmkhbgaxrofurpbgc` (eq-solves-service-dev) was DELETED 2026-06-22. EQ Service migrated to ehow (`ehowgjardagevnrluult`). The eq-context substrate cache (`context_files` table) was retired at the same time — substrate is now served via GitHub raw CDN. This finding is no longer actionable. |
...
4. **Security severity was inverted — corrected.** **`nspb` (SKS LIVE) anon-write on live PII is the top item**, not
   ...
   structural** risk. (Note: `urjhmkhbgaxrofurpbgc` was DELETED 2026-06-22; EQ Service is now on ehow; substrate moved to GitHub raw CDN.)
...
| ~~`urjhmkhbgaxrofurpbgc`~~ | ~~eq-solves-service host (+ substrate)~~ | **DELETED 2026-06-22** — EQ Service migrated to ehow (`ehowgjardagevnrluult`). Substrate cache retired — substrate is now served from GitHub raw CDN. | n/a — deleted |
...
| **eq-shell** | `claude/worker-linker-schema-fix` | `jvkn` (`shell_control`) + every tenant plane via `tenant_routing` | tenants, users, memberships, routing; tenant `app_data.*`; `canonical_events` | `canonical-api` writes `customers/contacts/sites/staff/licences/jobs/assets` into tenant `app_data`; **`cards-approve-staff` sets `field_approved` flag on `app_data.staff` (tenant data plane via tenant_routing) + records in `shell_control.cards_field_approvals`. ktmj write removed 2026-06-08.** | **SSO authority.** Mints short-lived Supabase JWT via `token-exchange.ts` (HS256, SUPABASE_JWT_SECRET, 60s TTL). `mint-iframe-token.ts` removed; `#sh=` HMAC path retired (PR #430). Hosts the `canonical_events` POST endpoint |
| **eq-quotes-port** | `claude/mobile-fixes` | **RETIRED** — EQ Quotes (Flask) retired 2026; replaced by EQ Ops (eq-shell repo, core.eq.solutions/ops). The canonical-api dual-write, HMAC Field sites pull, and event emission paths described in this row are no longer active. | — | — | — |
| **eq-solves-service** | `claude/posthog-canonical-distinct-id` | **`ehowgjardagevnrluult`** (ehow/sks-canonical — migrated 2026-06-22; formerly off-topology on the now-deleted urjh) via anon+service key | own `customers/sites/assets/*_tests/defects/maintenance_checks` | same; stamps `canonical_id` after sync | **HTTP PUT/POST** to `canonical-api` (`eq-service:*` external_ids); emits `defect.created/resolved`, `maintenance_check.completed/overdue`. No DB-to-DB link |
...
| **4** | **CLOSED 2026-06-08** — `cards-approve-staff` repointed to `app_data.staff` via tenant_routing; ktmj write removed. This item no longer requires action. | — | — | — |
