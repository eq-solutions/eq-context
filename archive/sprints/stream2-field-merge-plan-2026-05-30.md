...
> **✅ EXECUTED + MERGED 2026-05-30 — PR #138 → main (v3.5.26).** Ported tenant-gated to `sks`, EQ-safe (verified: no duplicate IDs, sks-only reveal via `applyTierVisibility()`, `TENANT_DISABLED_TABLES.eq` lists `prestarts`/`toolbox_talks`, div-balanced, `node --check` clean). Review caught + fixed a duplicate-ID defect (the safety panel reused `#page-prestart-list`/`#page-toolbox-list` that EQ's site-reports owns — namespaced to `#safety-*`). **Remaining B5-cutover TODOs:** (1) **safety form modals were NOT transplanted** — `safety.js` references EQ's site-reports modal IDs (`#modal-prestart`, `#prestart-form-body`, `#prestart-modal-title`, + toolbox equivalents); at cutover SKS needs its own namespaced safety modals, or reconcile onto site-reports'. Until then the SKS prestart/toolbox **forms won't open correctly**. (2) `toolbox_talks` not in `ORG_TABLES` — add if it carries `org_id`. (3) SKS path unsmoked — exercise `loadSafety`/`loadSafetyDashboard` against sks-labour at cutover. (4) `safety.js` list-id refs now diverge from SKS source (namespaced). The spec below is retained for reference / the cutover modal work.
