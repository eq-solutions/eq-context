...
failures:
  ...
  - id: F10
    ...
    rung: 1
    target_rung: 4
    guard: "system/lessons.md's 2026-05-24 entry (rung 1, prose only) plus, as of 2026-08-05, a human manually running `git config --get core.hooksPath` while investigating something else entirely — how mechanism (3) below was actually caught. hooks/session_start.py gains a HOOKS check in this same fix (prints local/--worktree/effective scope and warns loudly on any mismatch, every session, unprompted, same mechanism F1's FRESHNESS check uses) — real signal, but it warns rather than blocks, so it does not by itself close the gap to target_rung 4. A wrong or shadowed value can still be set and take effect; the check only surfaces it after the fact. Honestly rung 1, not self-promoted."
