...
| C:\Projects\eq-field-bootperf (eq-field) | claude/field-boot-perf | Claude (foreman: per-app nav-speed — Field boot) | 2026-07-11 | ACTIVE — **PR #452 OPEN** (defer JSZip off the render-blocking `<head>` + index.html `no-store`→`no-cache` + print.css media="print" + jvkn preconnect; v3.5.296). CI green (Tests+lint, header/redirect rules, deploy-preview). Branch = Netlify preview only; NO deploy until Royce merges. Non-`-wt` path deliberately (EnterWorktree can't cross repos from an eq-context session; `block-worktree-write` pattern-matches `*-wt`). |
| C:\Projects\eq-svc-bootperf (eq-solves-service) | claude/service-boot-perf | Claude (foreman: per-app nav-speed — Service boot) | 2026-07-11 | ACTIVE — **PR #494 OPEN** (lazy-load posthog-js off the critical bundle + Sentry `bundleSizeOptimizations`). `next build` green locally + Typecheck+audit CI pass. Branch = preview only; NO deploy until Royce merges. Non-`-wt` path (same reason). |
