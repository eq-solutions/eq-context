...
|--------|--------|-----------------|---------|--------|
| C:\Projects\eq-shell-tenant-drift-wt (eq-shell) | fix/tenant-drift-resilience | Claude (harden check-tenant-drift.mjs against one unreachable tenant killing the required gate for every PR) | 2026-07-16 | DONE — **PR [#878](https://github.com/eq-solutions/eq-shell/pull/878) OPEN**. Root eq-shell checkout was mid-collision with a concurrent session (branch had switched to `chore/remove-dead-vendored-platform-apps` under me) — isolated my edit off into this worktree via stash rather than disturb the shared root. Worktree prunable after merge. |
...
| C:\Projects\eq-shell-dupes-merge-wt (eq-shell) | claude/dupes-tab-merge | Claude (wire merge into the generic Sites Dupes tab — flag-pair RPC + UI) | 2026-07-16 | DONE — **PR [#876](https://github.com/eq-solutions/eq-shell/pull/876) OPEN**, all real gates green (typecheck·test·lint, migration ledger hygiene, Plan, Netlify preview). Only red check is the pre-existing tenant-drift gate (red on `main` itself since 2026-07-15 22:09Z, unrelated Supabase project removal — chip task_3c4ee4fa). Migration **0186** (`eq_site_advisory_flag_pair`) validated live via BEGIN…ROLLBACK on ehow. **NOT merged — Royce's call.** |
| C:\Projects\eq-intake-dupes-merge-wt (eq-intake) | claude/dupes-tab-merge-client | Claude (flagSitePairForMerge client wrapper) | 2026-07-16 | DONE — **PR [#71](https://github.com/eq-solutions/eq-solves-intake/pull/71) OPEN**, fully green (Build, typecheck, test). **NOT merged — Royce's call.** |
