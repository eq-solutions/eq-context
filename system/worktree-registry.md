...
`C:\Projects\eq-shell\.claude\worktrees\invite-precheck-audit-log (eq-shell)` | `fix/invite-precheck-audit-log` | session 040c535e — audit-logs the two pre-check "already has an account" refusals in `create-worker-invite.ts` | 2026-09-15 | active — [PR #1938](https://github.com/eq-solutions/eq-shell/pull/1938) OPEN, **stacked on [#1935](https://github.com/eq-solutions/eq-shell/pull/1935)** (based on `fix/invite-claimed-email-check`, not `main`). Gets no automatic CI while stacked — `ci.yml` gates on `branches: [main]`; dispatched manually, green. Remove once both merge.
