...
|--------|--------|-----------------|---------|--------|
| C:\Projects\eq-shell-comms-polish-wt (eq-shell) | claude/nsw-comms-polish-f0fe33 | Claude (NSW Comms polish — perf on the jobs-GET waterfall, resource-dashboard landing view, creative extras) | 2026-07-17 | ACTIVE — off origin/main (332d426). Committed `bc37aec`: parallelized the 3-deep serial comms-jobs.ts all-jobs GET waterfall (jobs/PO-totals/roster-crew now Promise.all'd) + new "Dashboard" landing view (default, deep-linkable `?tab=dashboard`) — capacity card + crew-gaps + this-week agenda, every row click-through to the job's edit card. Build+tests+CSS-coverage green. NOT pushed, NOT deployed per Royce's explicit "don't push anything this session." Creative extras (Monday present-mode, ICS feed, etc.) proposed to Royce, not built pending his pick. |
...
| C:\Projects\eq-shell-accessmodel-c1-wt (eq-shell) | claude/accessmodel-cluster1-shell | Claude (access-model cluster 1 — mirror v2.5.2 sensitive-read keys, gate PII/margins/financial, Access Control columns, server requirePerm) | 2026-07-17 | ACTIVE — off origin/main (332d426). eq-roles v2.5.2 LIVE (d7dcdc4). Phases 1–2 DONE → **PR [#885](https://github.com/eq-solutions/eq-shell/pull/885) OPEN**: mirror + Access Control surfacing + `tenant-role-perms` allowlist-derive fix + server redaction (entity.view_pii) + financial gate + UI gates (StaffPage/gm-reports/QuotesModule margins). check:perms + build + 142 tests green. **Phase 4 margin RLS DONE too** — migration 0188 (`4ad96ff`) gates margin/cost in eq_list_quotes/eq_get_quote_detail (the direct browser-RPC read path), validated on ehow BEGIN…ROLLBACK. Phase-4 remainder (PII/financial defense-in-depth) = chip task_02a64782. NO deploy/dispatch until Royce (app auto-deploys on merge; migration 0188 dispatched separately via One Pipe). |
