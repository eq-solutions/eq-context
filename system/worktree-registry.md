...
|--------|--------|-----------------|---------|--------|
| C:\Projects\eq-shell-signals-wt (eq-shell) | claude/dashboard-signals | Claude (Core dashboard signals board — Phase 1) | 2026-07-17 | ACTIVE — off origin/main (71fac54). Committed `148a89d`: `signals.ts` endpoint + 3 permission-gated bands (compliance / outstanding works / deployable crew) + crew-math tests. `pnpm run build` green. Phase 0 server `can()` was already on main; quote-stage source is the concurrent `claude/briefing-quote-status-align`. Gates on interim base perms — will adopt cluster-1's `field.view_licences` etc. NOT pushed, NOT deployed. |
| C:\Projects\eq-shell-accessmodel-c1-wt (eq-shell) | claude/accessmodel-cluster1-shell | Claude (access-model cluster 1 — mirror v2.5.2 sensitive-read keys, gate PII/margins/financial, Access Control columns, server requirePerm) | 2026-07-17 | ACTIVE — off origin/main (332d426). Depends on eq-roles v2.5.2 (tagged d7dcdc4). Phase 1 (pin bump #v2.5.2 + mirror) DONE — check:perms + build green. Phase 2 (gating) in progress. NO deploy until Royce merges. |
