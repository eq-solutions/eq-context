...
|---|------|---------|------------------|--------|
| 1 | eq-service | `getCanonicalMembers()`/`getCanonicalMemberMap()` default to SKS's roster; ~17 call sites (incl. the pre-visit-brief email cron) never pass a tenant | Silent wrong-tenant data today, not a future risk | **Fix built and MERGED, LIVE 2026-09-09:** [eq-service#838](https://github.com/eq-solutions/eq-service/pull/838) — closes `ops/security-register.md` SEC-76. Merged via admin override past 2 pre-existing unrelated CI failures (the real gate, `tsc + next build`, was clean). Live-verified on service.eq.solutions via Netlify + commit ancestry. |
| 2 | eq-service | SKS's tenant ID hardcoded into 2 RLS `WITH CHECK` policies (`acknowledgments`, `app_config`); true footprint may be larger and isn't fully knowable from the repo | Silent security-layer denial — Postgres just refuses the write, no app error | **Investigated 2026-09-09, CLOSED — wrong repo, not a bug.** Tracked as `ops/security-register.md` SEC-77. Both tables belong to eq-field, not eq-service — eq-service's own `0146`/`0146b` migrations were stale orphaned duplicates (removed in [eq-service#839](https://github.com/eq-solutions/eq-service/pull/839)), and no eq-service app code touches either table. On their real home (eq-field/ehow) the hardcode is a **deliberate, already-shipped fix** for a real Demo↔SKS cross-tenant leak — future-tenant provisioning already handled correctly by eq-field's own generator tool ([§6.4](#64-concurrent-work-already-in-flight-eq-field-tenant-provision-generator)). See SEC-77 for full detail, including a live-verified ~31-table footprint for the underlying hardcoded-literal pattern that's larger than this row's original 2-table estimate — split ~15 eq-field / ~10 eq-shell / 6 shared / 7 unresolved. **eq-shell's own ~10-table share is NOT covered by this closure — checked 2026-09-10, `tender_enrichment` specifically.** Confirmed: `tenant-migrate.yml` doesn't handle this table at all, by deliberate design — its creating migration was misfiled into the control-plane tree and tombstoned 2026-07-11 with an explicit "do not move into `supabase/tenant-migrations/`" header, precisely to keep the One Pipe off it. Its whole RLS lifecycle has always been hand-applied per plane. Live-checked all 3 real tenants: all currently correct (ehow hardcoded to its own literals, zaap dynamic via eq-shell PR #1863/migration `0314`, madagins hardcoded but correctly substituted to its own UUIDs — verified against jvkn). No active bug, but no guardrail either — the next tenant that skips the manual step lands on the exact zaap-style "RLS enabled, zero policies" lockout already found and fixed today. **Checked 2026-09-10: `nominations`/`tender_phases` did NOT share a clean bill of health.** `tender_phases` was fine everywhere it exists (doesn't exist on zaap at all). But `nominations` on zaap had the identical live silent-lockout bug as `tender_enrichment` — RLS enabled, zero policies, not hypothetical — plus a superfluous (non-exploitable) `anon` grant on madagins's `nomination_clashes` view. Both fixed and dispatched: [eq-shell#1878](https://github.com/eq-solutions/eq-shell/pull/1878) (migrations `0316`/`0317`), live-verified post-dispatch. **Checked 2026-09-10: `apprentice_profiles`/`tenders` — both clean, no fix needed, but same ownership-attribution trap as `tender_enrichment`** (the `CREATE TABLE` grep matched a dead, 0-row `app_data.*` table; the real `public.*` tables are correct on all 3 planes — `tenders` on zaap is even fully dynamic, no hardcoding at all; `apprentice_profiles` has no `authenticated` grant anywhere, consistent with the item-4 finding that access currently goes through eq-field's service-role path, not RLS). **Checked 2026-09-10: `tender_import_runs`/`tender_review_decisions`/`tender_nominations` — closes out all 6 originally-named tables.** First two clean on all 3 planes, same shape as the rest of the family. `tender_nominations` doesn't exist live anywhere (ehow/zaap/madagins) — superseded by `public.nominations`, the one table in this family that was actually broken. **Final tally across all 6: only `nominations` had a real live bug (fixed); the ownership-attribution trap held on all 6 without exception.** Full detail: `ops/security-register.md` SEC-77. |
| 3 | eq-cards | 3 comments in the just-merged PR #348 describe "shared DB by default" as a settled decision you explicitly reversed the same session; the tracked follow-up to fix them never landed | Code behavior is already correct — only the comments misdescribe a non-negotiable | **Fix built and MERGED, LIVE 2026-09-09:** [eq-cards#351](https://github.com/eq-solutions/eq-cards/pull/351) — "correct stale tenant-isolation policy in code comments." |
| 4 | eq-field | Apprentice module: an unrecognized tenant falls back to the **pre-fix, unfiltered legacy read** — the exact hole an August security fix closed for SKS. Backwards fail-safe. | Dormant today (only SKS + disposable demo content exist); becomes a real vulnerability the moment a real tenant turns Apprentices on | **Investigated 2026-09-09, NOT built — scope is bigger than this row states, see Detail.** `apprentice-data.js`/`apprentice-write.js` don't just gate on `tenant_slug !== 'sks'` — the whole endpoint (via `_shared/field-person.js`) is hardcoded to `EHOW_URL`/`EHOW_SERVICE_ROLE_KEY` (SKS's own Supabase project), for every caller regardless of tenant. Widening the gate alone, without also resolving the correct per-tenant Supabase project, would mint a REAL cross-tenant leak (another tenant's apprentice reads/writes landing in SKS's database) — strictly worse than today's per-tenant-only privacy gap. Proper fix needs per-tenant Supabase client resolution (mirroring whatever pattern `verify-pin.js`'s `DATA_TENANT_IDS`/`TENANT_JWT_SECRETS` already use elsewhere in this repo), not a one-line change. Flagging back to you rather than building the narrow-and-wrong version. |
| 5 | eq-field | `sites.js`/`managers.js` (~11 sites) gate Shell-canonical-ownership write-protection on the literal string `'sks'`, not "is this tenant Shell-integrated" | A second Shell-integrated tenant gets **full write access** to Field-side sites/managers Shell is supposed to own — a data-corruption path, not just a missing feature | **Fix built and MERGED, LIVE 2026-09-09:** [eq-field#970](https://github.com/eq-solutions/eq-field/pull/970) — both files now key off `TENANT.CORE_ONLY` (already live, canonical-driven, `organisations.branding.coreOnly`) matching `auth.js`'s own already-established `_isCoreOnly()` pattern, `'sks'` kept only as the same hard fallback that pattern already uses. Scope correction found while building: this is a **client-side UX guard only** (toast + no-op) — no server-side RLS/API enforcement lives in this file, so "data-corruption path" as originally phrased overstates what this specific fix closes; any real server-side gap is a separate concern, likely overlapping SEC-77's ~31-table hardcoded-tenant-ID RLS finding. Confirmed live on field.eq.solutions via commit ancestry. |
| 6 | eq-shell | `scripts/check-tenant-drift.mjs`'s own `CANONICAL_PROJECTS` list is fixed at 3 entries; an unset project ref is **skipped, not failed** | Madagins' own dedicated Supabase project currently gets **zero** of this script's anon-grant/RLS/policy-lint security checks until someone hand-adds a 4th entry — the security guard has the exact disease it's guarding against. (Independently corroborated: today's live `digest.md` separately flags an active Supabase-advisor RLS gap on madagins's own `app_data._eq_migrations` table — a second, more urgent open security gap on the same project, not fixed by anything in this doc.) | **Fix built and MERGED, LIVE 2026-09-09:** [eq-shell#1854](https://github.com/eq-solutions/eq-shell/pull/1854) — new CHECK 15 queries live jvkn (via this file's own `loadActiveTenants()`) and fails loud on any active tenant with no matching `CANONICAL_PROJECTS`/`TENANT_DATA_PLANES` entry. Doesn't add the missing checks themselves (still needs a human to add the ref + CI secret). First draft's query was wrong (queried `shell_control.tenants.supabase_project_ref`, which is NULL for sks/madagins live) — caught via Supabase MCP before relying on it, rewritten to reuse the file's own proven `tenant_routing`-backed helper. **This PR's own CI then caught a second real bug**: the check shipped immediately-blocking instead of informational-first (this doc's own §9 recommendation for a brand-new check), so it correctly detected the live Madagins gap and would have red-lined the required drift-check gate for the whole repo. Fixed same session — added `--strict-canonical-completeness`, off by default, matching this file's own strict-flag sequencing. Confirmed live on core.eq.solutions via commit ancestry, after eq-shell's deploy queue (backlogged behind ~10 other same-window merges from other sessions) caught up. |
...
|---|-----------|-------|-------------------|----------|
| 1 | [`netlify/functions/token-exchange.ts:42`](https://github.com/eq-solutions/eq-shell/blob/main/netlify/functions/token-exchange.ts#L42) `ALLOWED_FIELD_TENANT_SLUGS` | Which slugs may mint a Field/Service Supabase JWT | `['eq','demo-trades','melbourne','sks','madagins']` (now fixed) | **Completeness gap, strongest table-backed candidate in the sweep.** The same function already reads `field_tenant_slug` and a dynamic entitlement check (`isModuleEnabledForTenant`) *before* this array is consulted — the array is a redundant second, static gate on facts already sourced dynamically. **Fix built and MERGED, LIVE 2026-09-09:** [eq-shell#1850](https://github.com/eq-solutions/eq-shell/pull/1850) drops the array check on the non-admin path (the caller's own `field_tenant_slug`/`slug` now trusted directly from `shell_control.tenants`, already scoped to the signed session — not attacker-controlled). The array's *other* use — validating `body.tenant_slug` on the platform-admin cross-tenant picker, where the value genuinely is caller input — is deliberately left as a static list, a smaller, lower-frequency instance of the same gap. `tsc -b` + `eslint` clean, 2 new regression tests, 9/9 passing. Auth-adjacent code — the edit was flagged by the Claude Code auto-mode classifier and only applied after explicit confirmation. Confirmed live on core.eq.solutions via commit ancestry, not just merge success. |
