...
| 2 | eq-service | SKS's tenant ID hardcoded into 2 RLS `WITH CHECK` policies (`acknowledgments`, `app_config`); true footprint may be larger and isn't fully knowable from the repo | Silent security-layer denial — Postgres just refuses the write, no app error | **Investigated 2026-09-09, CLOSED — wrong repo, not a bug.** Tracked as `ops/security-register.md` SEC-77. Both tables belong to eq-field, not eq-service — eq-service's own `0146`/`0146b` migrations were stale orphaned duplicates (removed in [eq-service#839](https://github.com/eq-solutions/eq-service/pull/839)), and no eq-service app code touches either table. On their real home (eq-field/ehow) the hardcode is a **deliberate, already-shipped fix** for a real Demo↔SKS cross-tenant leak — future-tenant provisioning already handled correctly by eq-field's own generator tool ([§6.4](#64-concurrent-work-already-in-flight-eq-field-tenant-provision-generator)). See SEC-77 for full detail, including a live-verified ~31-table footprint for the underlying hardcoded-literal pattern that's larger than this row's original 2-table estimate — split ~15 eq-field / ~10 eq-shell / 6 shared / 7 unresolved. **eq-shell's own ~10-table share is NOT covered by this closure and remains genuinely open** — not checked whether `tenant-migrate.yml` already handles literal substitution for them. |
