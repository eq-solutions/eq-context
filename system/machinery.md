...
| `refresh_suite_state.py` | Refreshes `suite-state.md` from live systems; evicts Key Decisions past the 30 most recent. Since 2026-09-01: computes the Product Pulse section (F4) — 6 signals (incl. a windowed `maintenance_checks_pulse()` RPC, Royce-approved live-ehow schema change) + flip detection; gave the Field Data Plane section a self-healing heading/anchor after its old one silently stopped matching anything for three weeks; entire body now runs under `if __name__ == "__main__":` so its pure functions are safely importable for tests. Public-schema pulse signals count rows via HEAD + `Prefer: count=exact` (`Content-Range` parsing), not `select=count()` — the aggregate form 400'd (`PGRST123`) on first live run, since this project's PostgREST has `db-aggregates-enabled` off. |
...
| `shared-object-drift.yml` — live-definition diff for shared ehow app_data objects (eq-field/eq-shell registry) | cron + push |
