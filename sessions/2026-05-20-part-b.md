...
**Follow-up (later same day):** the first two of the three items above were closed out. A parallel Claude Code session renamed the `eq-solves-field` local clone from `demo` to `main`, tracking `origin/main` (0/0), with the previous SKS-Live `main` preserved as a local `main-pre-split-archive` branch (more conservative than the original "delete it" plan) — the 50 prior uncommitted entries were handled in that pass, three merged branches deleted locally, and a stale safe-cleanups worktree removed. Separately, Royce did the `eq-solves-field.netlify.app` dashboard rewire himself mid-session — the site now builds from `eq-solutions/eq-field` `main` (renamed from `demo`), verified via the Netlify REST API plus a manual build trigger. The third item (Royce's personal `CLAUDE.md` deployment table) remained outstanding — Royce-manual edit, not substrate-visible.
