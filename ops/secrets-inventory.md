...
|---|---|---|
| Shell's session-signing JWT secret | eq-shell `SUPABASE_JWT_SECRET` = eq-shell `EQ_SHELL_JWT_SECRET` = eq-service `EQ_SHELL_JWT_SECRET` = eq-service `SUPABASE_JWT_SECRET` = eq-field `SUPABASE_JWT_SECRET` | Forge a valid login session for **any** user, on any app in the suite — the top secret in the whole inventory. **SEC-63 (2026-08-24):** also exists as a Netlify **account-scope** var (team `milmlow`) — eq-service and eq-field don't hold their own site-level copy of `SUPABASE_JWT_SECRET` at all, they inherit this one directly. Confirmed live it does **not** reach sks-nsw-labour or any other site on the account (checked all 10 sites individually) — this was the open question in the original finding, resolved P1 not P0. This is the first account-scope secret this inventory has ever tracked; the methodology before this only ever enumerated per-site vars. |
