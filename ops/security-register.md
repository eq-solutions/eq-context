...
| SEC-30 | **P0 — live, unauthenticated, confirmed on BOTH tenant planes. Proposed fix REJECTED 2026-08-21 — do not dispatch #1509.** | `public.app_config` grants unconditional SELECT (`qual=true`) to `anon` (zaap: policy role `{anon}`) / to everyone (ehow: policy role `{public}`, which includes `anon`) on role-elevation codes + integration secrets, keyed by `org_id`. zaap: `manager_password`/`staff_code`/`supervisor_code`, 2 of 3 org_ids real. ehow: `staff_code`/`supervisor_code` **plus `tafe_fn_token` (208 chars, shaped like a real bearer token, not a PIN)** — only 1 org_id in the table and it's SKS's real, only tenant, not a demo row. | eq-canonical-internal (zaap) + **sks-canonical (ehow, LIVE PRODUCTION — same hole, worse: 100% real data, includes what looks like a live third-party credential, not just internal PINs)** | **OPEN — the "reasoned, not traced" consumer this row originally flagged has now been traced, and the original recommended fix doesn't survive it.** `eq-field/scripts/app-state.js:481-502` does an unconditional, unauthenticated fetch of this exact table, scoped to the CALLING tenant's own `org_id`, on every page load, before any session exists, on both planes — this is real and load-bearing, not the legacy/dead path the "reasoned, not traced" hedge left open as a possibility. eq-shell [PR #1509](https://github.com/eq-solutions/eq-shell/pull/1509) (`REVOKE ALL FROM anon`) is now confirmed-unsafe, not just undispatched — merging it breaks that boot fetch for every tenant on whichever plane it lands on. **Do not merge #1509.** Commented on the PR 2026-08-21 explaining why. Real fix needs the enumerated list of org_ids with a live EQ Field deployment on each plane (zaap hosts more than one real non-demo tenant org — see Detail), scoped as an allowlist; a single hardcoded org_id (the shape this register's SEC-33 fix used for a different table) would just break OTHER real tenants instead of anon. That enumeration hasn't been done — tracked as a follow-up, not rushed. See Detail. |
| SEC-31 | **P1** | `public.organisations` on jvkn — any authenticated user, any tenant, reads every organisation's row incl. `supabase_url`/`supabase_anon_key`, cross-tenant (`tenant_id` column exists, no policy references it) | eq-canonical (jvkn) | **OPEN — found 2026-08-20. Fix drafted + dry-run verified 2026-08-21, not yet applied — no CI apply path for jvkn control-plane migrations (hand-applied only).** Already a known, deliberately-deferred gap in `eq-shell/scripts/check-tenant-drift.mjs`'s own code comment, but never entered into this register. Unaffected by SEC-30/32's boot-fetch complication — this is a different `organisations` table on a different project, `authenticated`-only (anon confirmed 401), no pre-auth read dependency. See Detail. |
| SEC-32 | **P1, confirmed on BOTH tenant planes. Bundled fix REJECTED alongside SEC-30 2026-08-21 — do not dispatch #1509.** | `public.organisations` — `anon` reads the full tenant registry. zaap: `id`/`slug`/`name`/`tier`, 3 rows. ehow: same shape, 1 row (SKS's own real tenant) — policy role `{public}`, same class as SEC-30. | eq-canonical-internal (zaap) + sks-canonical (ehow) | **OPEN — found 2026-08-20. Same #1509 rejection as SEC-30 applies (bundled in the same migration file) — see SEC-30's Status for detail, not repeated here.** Low severity alone; the recon step that turns SEC-30 from "some UUID's password leaked" into naming the real org. See Detail. |
| SEC-33 | **P1 — fix ready, isolated, dry-run verified on BOTH planes.** | `app_data.staff` on zaap — any authenticated tenant member (no role required) can SELECT/INSERT/DELETE every coworker's PII; tenant-only policy never references the `staff_id`/`user_id`/`created_by` columns that exist on the table | eq-canonical-internal (zaap) | **OPEN — found 2026-08-20. Fix ready: eq-shell [PR #1510](https://github.com/eq-solutions/eq-shell/pull/1510), dry-run verified live on zaap, CI green.** Originally bundled with SEC-30/32 in the same migration file (`0256_*.sql`); pulled those back out 2026-08-21 once the app_config boot-fetch issue surfaced (see SEC-30) — SEC-33 has no dependency on that question and ships alone. Awaiting dispatch approval. SEC-27's exact shape, but live (real grant), not latent. See Detail. |
