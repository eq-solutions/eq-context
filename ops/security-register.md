...
Diff: 23 files, +92/-57, sitting uncommitted in the working tree at
`C:\Projects\eq-service`. **Not committed, not pushed, not deployed** —
waiting on Royce's explicit go, per the hard rule (no commit without
explicit instruction). No live click-test possible: no second tenant
exists to compare against, and no Shell/demo credentials in this
environment either way. `lib/canonical-sync.ts` also reads
