...
| SEC-75 | P3 | 4 tables in `wipe_backup` schema (ehow) have RLS disabled — Supabase advisor CRITICAL, anon/authenticated exposed; NOT empty (914/323/2636/758 rows, see Detail), no code path references the schema | sks-canonical (ehow) | **OPEN — merged, not yet applied.** eq-shell [#1833](https://github.com/eq-solutions/eq-shell/pull/1833) (migration 0309, RLS lockdown no policy) merged `e0f815bd`; RLS still disabled live on ehow as of 2026-09-09 — awaiting Royce's `tenant-migrate.yml` dispatch. See Detail. |
...
### SEC-75 — 4 wipe_backup.*_20260712 tables on ehow have RLS disabled (P3)
Supabase's security advisor flags `wipe_backup.assets_20260712`,
...
**Corrected 2026-09-09 (same session, later, once Supabase MCP access
became available): NOT empty.** Live `SELECT count(*)` on ehow:
assets_20260712 914, check_assets_20260712 323, check_items_20260712 2636,
...
one-off CTAS creation, so the estimate silently reads 0 while real data
sits underneath. Worth remembering if a `list_tables` row count is ever
relied on for an emptiness claim elsewhere. P3 not P1 still stands on
reachability grounds (no known code path reaches this schema, matching this
register's own SEC-5/SEC-48/SEC-49 judgment), but the blast radius if that
reachability assumption is ever wrong is now known to be real production
data, not nothing.

**2026-09-09 -- merged, not yet applied.** eq-shell
[#1833](https://github.com/eq-solutions/eq-shell/pull/1833) adds
`supabase/tenant-migrations/0309_wipe_backup_schema_rls_lockdown.sql`
...
than drop, to keep the data available if it's ever needed -- looks like the
right call now that the tables are confirmed non-empty. Merged `e0f815bd`.
Confirmed live via `list_migrations` on ehow: 0309 is not in the ledger yet,
and `list_tables`'s own advisory still reports all 4 CRITICAL -- nothing
touches ehow until Royce dispatches `tenant-migrate.yml` per the One Pipe
rule.
