...
| SEC-75 | P3 | 4 tables in `wipe_backup` schema (ehow) have RLS disabled — Supabase advisor CRITICAL, anon/authenticated exposed; 0 known rows, no code path references the schema | sks-canonical (ehow) | **OPEN — found 2026-09-09, fix drafted.** eq-shell [#1833](https://github.com/eq-solutions/eq-shell/pull/1833) (migration 0309, RLS lockdown no policy) — PR open, not yet merged. See Detail. |
...
them to a specific event. 0 rows as of 2026-09-09 per Royce's own check —
not independently re-verified live this session (no Supabase MCP or CLI
available). P3 not P1: no known code path reaches this schema, matching this
register's own SEC-5/SEC-48/SEC-49 judgment for a latent-but-currently-
unreachable exposure.

**2026-09-09 — fix drafted, not yet applied.** eq-shell
...
than drop, to keep the data available if it's ever needed. PR open, not yet
merged; nothing touches ehow until Royce dispatches `tenant-migrate.yml` per
the One Pipe rule.
