...
| SEC-30 | **P0 — live, unauthenticated, confirmed on BOTH tenant planes. Real fix ready 2026-08-21, verified against jvkn's canonical tenant registry, awaiting dispatch approval.** | `public.app_config` grants unconditional SELECT (`qual=true`) to `anon` (zaap: policy role `{anon}`) / to everyone (ehow: policy role `{public}`, which includes `anon`) on role-elevation codes + integration secrets, keyed by `org_id`. zaap: `manager_password`/`staff_code`/`supervisor_code`, 2 of 3 org_ids real. ehow: `staff_code`/`supervisor_code` **plus `tafe_fn_token` (208 chars, shaped like a real bearer token, not a PIN)** — only 1 org_id in the table and it's SKS's real, only tenant, not a demo row. | eq-canonical-internal (zaap) + **sks-canonical (ehow, LIVE PRODUCTION — same hole, worse: 100% real data, includes what looks like a live third-party credential, not just internal PINs)** | **OPEN — fix ready.** The enumeration this row called for is done: `TENANT.ORG_UUID` is always resolved from jvkn's canonical `organisations` table (hostname match or a same-hostname-gated `?tenant=` override — traced in `eq-field/scripts/app-state.js`), never from a tenant's own local rows. Queried jvkn live: only 3 orgs exist suite-wide — `eq`→zaap (active), `sks`→ehow (active), `favour-perfect`→a deleted project (suspended). The two zaap-local orgs that blocked the earlier draft ("Melbourne Construction Group", "Demo Trades Pty Ltd") aren't in canonical at all — orphaned rows, no `tenant_routing` entry, unreachable through any real user path. So each plane has exactly one real org_id that can ever hit this anon read. eq-shell [PR #1509](https://github.com/eq-solutions/eq-shell/pull/1509) rewritten accordingly (`0257_close_sec30_32_both_planes.sql`, replaces the withdrawn `REVOKE ALL`/single-org drafts) — org-scoped per plane, guarded on each plane's actual policy names (zaap and ehow name them differently), dry-run verified end-to-end on both zaap and ehow including the migration's own post-condition check. CI green, mergeable. Not dispatched — same approval gate as everything else here. See Detail. |
| SEC-31 | **P1 — fix ready, blocked on Royce's hands, not on design.** | `public.organisations` on jvkn — any authenticated user, any tenant, reads every organisation's row incl. `supabase_url`/`supabase_anon_key`, cross-tenant (`tenant_id` column exists, no policy references it) | eq-canonical (jvkn) | **OPEN — found 2026-08-20. Fix drafted + dry-run verified 2026-08-21. Live apply attempted 2026-08-21 and deliberately blocked by the Claude Code auto-mode classifier** (direct DDL against a live project outside a governed pipeline — same class of block as SEC-29's dispatch trigger, confirmed on retry, not transient) — **per instruction, no workaround attempted; needs Royce to run it himself.** Statement + full context: `eq-shell/supabase/migrations/2026_08_21_organisations_tenant_scope_read.sql`. No CI apply path exists for jvkn control-plane migrations regardless (hand-applied only, per CLAUDE.md). Unaffected by SEC-30/32's boot-fetch complication — different `organisations` table, different project, `authenticated`-only (anon confirmed 401), no pre-auth read dependency. See Detail. |
| SEC-32 | **P1, confirmed on BOTH tenant planes. Fix ready alongside SEC-30 2026-08-21, awaiting dispatch approval.** | `public.organisations` — `anon` reads the full tenant registry. zaap: `id`/`slug`/`name`/`tier`, 3 rows. ehow: same shape, 1 row (SKS's own real tenant) — policy role `{public}`, same class as SEC-30. | eq-canonical-internal (zaap) + sks-canonical (ehow) | **OPEN — fix ready.** Same #1509 rewrite as SEC-30 applies (bundled in the same migration file, `0257_close_sec30_32_both_planes.sql`) — see SEC-30's Status for the full verification detail, not repeated here. Low severity alone; the recon step that turns SEC-30 from "some UUID's password leaked" into naming the real org. See Detail. |
