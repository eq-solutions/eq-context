...
| SEC-45 | **P2 — fix merged, not applied.** | `eq_cards_find_or_create_worker_for_invite` (jvkn) — same zero-caller-check shape as SEC-44; its only legitimate caller is a server-role function, so the `authenticated` grant buys nothing. Doubles as a cross-org existence oracle and lets a caller rewrite an unclaimed worker's phone/email — the setup step that makes SEC-44 work without knowing a real target's phone | eq-canonical (jvkn) | **OPEN — found 2026-08-20, reasoned.** Fix: eq-cards [PR #291](https://github.com/eq-solutions/eq-cards/pull/291) — `REVOKE EXECUTE ... FROM authenticated`. Confirmed zero direct app-layer callers before writing. **Merged 2026-08-23.** Not applied to jvkn (Supabase MCP disconnected mid-session -- apply_migration unavailable to this session right now).
| SEC-46 | **P2 — fix merged, not dispatched.** | eq-field's `_purgeTenantRows()` (Sites/Supervision CSV import) issues the DELETE with the user's own JWT against tables whose RLS is tenant-only (no role condition) — the SEC-21/22 fix (`canManageData()`) lives entirely in browser JS, so any authenticated non-manager can still wipe Sites/Supervision from devtools | eq-field, sks-canonical (ehow) | **OPEN — found 2026-08-20, proved via live `pg_policies` read.** Sibling sweep otherwise clean — all 6 eq-field importers now gate correctly except these two tables' DB layer. Fix: eq-shell [PR #1541](https://github.com/eq-solutions/eq-shell/pull/1541), migration `0267` — RESTRICTIVE INSERT/UPDATE/DELETE policies, manager+supervisor tier, mirroring SEC-33's own applied pattern (`app_data.staff`, this finding's sibling gap, already closed). **Merged 2026-08-23.** Not dry-run verified live this pass -- Supabase MCP disconnected mid-session; built from source, not a live read. Not dispatched -- recommend a live-DB re-check before dispatch given that. See Detail.
| SEC-47 | **P2 — fix drafted, not yet merged (found already in progress, not this pass's work).** | `app_data.approve_safety_record` (ehow) — tenant-checked, no role check and no check that the approver isn't the submitter; any tenant member can approve their own prestart/toolbox-talk submission | sks-canonical (ehow) | **OPEN — found 2026-08-20, reasoned.** 35 prestarts + 1 toolbox talk live behind it — **in `public.prestarts`/`public.toolbox_talks`, a different table pair this function has never touched; the live-exploitable population is actually zaap's 50 draft-status rows in the `app_data` tables this function does touch (ehow's `app_data` twins are empty).** Fix: eq-shell [PR pending — migration `0265`, already merged to `eq-shell` main by a concurrent session] — role-gates to manager/supervisor plus a self-approval block, using `app_data.eq__caller_actor_uid()` (not the older, JWT-incompatible identity helpers) after live-checking it resolves correctly on both planes. Not dispatched. Found while building SEC-45/46 — register was out of sync with what's already merged, corrected here rather than left stale.
