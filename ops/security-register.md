...
| SEC-75 | ~~P1~~ **CLOSED 2026-09-07** | `app_data.field_people_iud()` (ehow) — the SECURITY DEFINER trigger behind `field_people`'s real write path — had no role/self check on its UPDATE branch; any authenticated tenant member could overwrite any coworker's name, email, phone, DOB, and emergency contact, bypassing RLS entirely (owner `postgres`, `rolbypassrls=true`). Found re-verifying SEC-33: `app_data.staff` has zero write grant for `authenticated`, as SEC-33 found, but real writes never use that grant — they go through this trigger, which SEC-33's own note incorrectly assumed was service-role-only. | sks-canonical (ehow) — confirmed absent on zaap, nothing to fix there | **CLOSED 2026-09-07.** Gates the UPDATE branch on `v_can_people or user_id = v_actor_id` — SEC-33's own self-or-manager shape, applied one layer down at the function that actually writes. 3-line diff, dry-run tested live twice before dispatch. `apply_migration` blocked twice by the auto-mode classifier (SEC-31's exact precedent) — Royce applied via the Supabase SQL editor, independently re-verified live after. Shared-object snapshot rebaselined same session. See Detail. |
...
**CORRECTION, found 2026-09-07 while re-verifying this closure before answering an unrelated
question:** the "no write path is affected" claim two sentences up is incomplete. `authenticated`
genuinely holds no INSERT/UPDATE/DELETE grant on `app_data.staff` (re-confirmed live, unchanged)
— but real application writes never use that grant. They go through
`app_data.field_people_iud()`, a `SECURITY DEFINER` trigger (owner `postgres`, bypasses RLS)
behind the `field_people` view's INSTEAD OF trigger, invoked under the calling user's own JWT —
not the service-role key this note assumed was the only writer. That function's own UPDATE
branch had no role/self gate on core PII fields — a second, distinct mechanism from anything
this entry checked. See SEC-75.

### SEC-75 — ehow app_data.field_people_iud(): SECURITY DEFINER write path had no role/self gate (P1)
Found 2026-09-07 re-verifying SEC-33's closure — SEC-33 closed the RLS/grant-layer exposure on
`app_data.staff` correctly, but `authenticated` holding zero write grant on that table doesn't
mean the table can't be written by an ordinary session. The `field_people` view's INSTEAD OF
trigger, `app_data.field_people_iud()`, is `SECURITY DEFINER` owned by `postgres`
(`rolbypassrls=true`) — it never evaluates `app_data.staff`'s RLS at all, grant or no grant, and
it's what Field's own identity-edit UI actually writes through, under the calling user's real JWT.

Live-read via `pg_get_functiondef` (2026-09-07): the UPDATE branch's existing logic already
nulled out `licence`/`agency`/`hire_company`/`rating`/`archived` for a caller without the
matching permission flag (`v_can_licences`/`v_can_labour_hire`/`v_can_people`) — deliberate,
pre-existing design. But the UPDATE statement's own WHERE clause
(`where staff_id = OLD.id and tenant_id = coalesce(v_tid, tenant_id)`) carried no equivalent
role-or-self check, unlike the DELETE branch a few lines below it in the same function, which
already required `v_can_people`. Any authenticated tenant member could therefore UPDATE any
other staff row's `first_name`/`last_name`/`email`/`phone`/`date_of_birth`/
`emergency_contact_name`/`emergency_contact_mobile`/`emergency_contact_relationship` — reachable
through the ordinary app path (Field's identity-edit screen calls this same view), not only a
devtools/direct-REST bypass. The 2026-08-23/24 shared-object collision on this exact function
(this repo's PR #761 vs eq-shell's 0270/0271, restored by eq-shell's 0273/[PR #1567](https://github.com/eq-solutions/eq-shell/pull/1567)) is unrelated in cause — same object, different gap.

Checked zaap before scoping the fix, not after (SEC-33's own 2026-08-21 "scope correction"
note above is the reason this got checked rather than assumed): `app_data.field_people_iud()`
does not exist on zaap at all — confirmed live, the identical `pg_proc` lookup returns zero
rows. ehow-only, not a both-planes fix.

**Fix, dry-run tested live twice before dispatch:** add `v_actor_id uuid` (assigned from JWT
`app_metadata.actor_id`, the same claim this plane's SEC-33 fix already reads) and extend the
UPDATE's WHERE clause to `and (v_can_people or user_id = v_actor_id)` — self-or-manager, the
identical shape SEC-33 already applies to the base table's RLS policy, now also enforced at the
function that actually executes the write. Nothing else in the ~130-line function touched —
verified via `diff -u` against the live pre-fix definition immediately before drafting, and
again immediately before the real dispatch (re-pulled the live def a second time to rule out a
concurrent edit — byte-identical both times, consistent with the heavy concurrent-session
activity this same session separately found and logged under F9). Grants checked before
(`{postgres=X/postgres,service_role=X/postgres}` — no `authenticated`/`anon`/`PUBLIC` to lose)
and confirmed unchanged after — `eq_enforce_function_privacy`'s restore logic (see eq-field
CLAUDE.md's own gotcha on this trigger) had nothing to do here.

**Dispatch:** the live `apply_migration` call was blocked twice by the Claude Code auto-mode
classifier, consistently, not flaky — SEC-31's exact precedent. Stopped rather than routed
around it via a different tool for the same DDL. Royce applied the statement directly via the
Supabase SQL editor on ehow. Independently re-verified live afterward, not just taking the
"applied" report at face value: `pg_get_functiondef` re-pulled fresh, `has_self_check` and
`has_actor_assignment` both `true`, full definition matches the drafted fix exactly (modulo
`\r\n` line endings from the SQL editor, immaterial to behaviour). Grants re-checked post-apply,
unchanged. Shared-object registry (`eq/identity/shared-db-objects.snapshot.json`,
`app_data.field_people_iud` entry) rebaselined in the same session so `shared-object-drift.yml`'s
next scheduled run doesn't flag this as unexpected drift.
