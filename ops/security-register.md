...
| SEC-63 | ~~P1~~ **CLOSED 2026-09-05** | An uninventoried Netlify **account-scope** (team `milmlow`) secret, `SUPABASE_JWT_SECRET`, is the same value that signs every session in the suite (matches 5 per-site vars) — appears nowhere in `ops/secrets-inventory.md`, which only ever enumerated per-site vars. `dev` context unmasked (same as SEC-61) | Netlify account scope (team `milmlow`) | **CLOSED 2026-09-05.** Site-scope resolved 2026-08-24 (reaches only eq-shell/eq-service/eq-field, not sks-nsw-labour; inventory updated). Royce deleted the `dev`-context value himself via the Netlify dashboard (Team settings → Shared environment variables), per the plan recorded 2026-08-24 after the coding session's own classifier blocked the automated delete. Confirmed directly by Royce 2026-09-05; not independently re-verified live — a same-session read attempt via the Netlify MCP (`manage-env-vars`, read-only flags) was itself blocked by the permission classifier before returning data. See Detail. |
...
| SEC-76 | ~~P2~~ **CLOSED 2026-09-09** | `lib/canonical-members.ts` defaulted an unset tenant slug to `'sks'`, and `getCanonicalMembers()`/`getCanonicalMemberMap()` used that as their default arg — 19 call sites across 15 files called them bare, so every one silently rendered SKS's staff roster regardless of which tenant was actually viewing | eq-service | **CLOSED — merged & deployed.** [PR #838](https://github.com/eq-solutions/eq-service/pull/838), merged via admin override past 2 pre-existing unrelated CI failures (`tsc + next build`, the real gate, was clean), live-verified on service.eq.solutions via Netlify + commit ancestry. Was not live-exploitable at the time (only SKS existed as a tenant) but was a primed landmine for the next one. See Detail. |
...
**Note 2026-09-08:** Live re-checked at Royce's request (he asked to delete
this row, believing it was already fixed). `sks-nsw-labour.netlify.app` is
still fully live and reachable by anyone with the URL — not decommissioned.
One real piece of progress: the anon key can no longer read `people` (401
`permission denied for table people`, confirmed via an ordinary unauthenticated
page load — this table was wide open at the original 2026-06-05 finding).
Could not confirm `timesheets`/`leave_requests`/`audit_log` — two independent
read-only checks (a direct Supabase project lookup, and a count-only query
through the page's own already-loaded client) were both blocked by the
Claude Code permission classifier; not pursued further. Consistent with, not
a contradiction of, the two 2026-09-05 notes above — the 2026-09-19 target
hasn't been reached. **Downgraded the Status cell from OPEN to SCHEDULED**
(this file's own established distinction — see `security_open_critical()` in
`.github/scripts/refresh_digest.py`, which only suppresses NEEDS-YOU
surfacing for a leading status of SCHEDULED/VERIFIED, never OPEN/STILL OPEN)
so this stops nagging on every session without falsely marking it closed —
same intent as the 2026-08-21 de-escalation note above, done properly this
time: that note's own prose never actually changed what the generator reads,
which is why this kept surfacing daily regardless. Re-open condition
unchanged: active surfacing resumes if 2026-09-19 passes with
`nspbmirochztcjijmcrx` still live.
...
**2026-09-05 — closed.** Royce deleted the `dev`-context value directly via the Netlify
dashboard, the manual path this row already named as the fallback. A same-session
attempt to re-verify live via the Netlify MCP (`manage-env-vars` with only the
read-flag `getAllEnvVars` set, no delete/upsert) was blocked outright by the permission
classifier before it returned any data — the classifier does not appear to distinguish
a read from a write on this operation. This closure therefore rests on Royce's direct
confirmation, not an independent live re-check; worth a real re-verification (Netlify
dashboard or a permission rule that lets a future session read this specific var) if
that's ever wanted.
...
### SEC-75 — 4 wipe_backup.*_20260712 tables on ehow had RLS disabled (P3, CLOSED 2026-09-09)
Supabase's security advisor flagged `wipe_backup.assets_20260712`,
`check_assets_20260712`, `check_items_20260712`, and `sy3_planless_20260712`
(ehow) CRITICAL — RLS disabled, exposed to anon/authenticated. No migration
or app code anywhere references the `wipe_backup` schema (grepped eq-shell
in full). `assets_20260712` ties to a real, closed incident: eq-shell's
`0176_assets_delete_guard_protect_plant_equipment.sql` documents a
2026-07-12 09:53:26 UTC service-role customer-asset import that
collateral-deleted 16 SKS plant_equipment rows, restored from
`audit_log.old_record` (not from this backup) — root cause permanently
closed via trigger guards (0154 -> 0176 -> 0256). The other 3 tables share
the date suffix (same ehow incident-response day, which separately also had
a live staff-dedup DML hotfix) but have no documentation of their own tying
them to a specific event.

**Corrected 2026-09-09 (mid-session, once Supabase MCP access became
available): NOT empty.** Live `SELECT count(*)` on ehow: assets_20260712
914, check_assets_20260712 323, check_items_20260712 2636,
sy3_planless_20260712 758. assets_20260712's 914 exactly matches 0176's own
incident count, confirming this table is a full pre-delete snapshot of that
day's DELETE, not just the 16 plant_equipment stragglers. The earlier
"0 rows" claim came from the Supabase MCP `list_tables` tool's row count,
which reads `pg_class.reltuples` (a planner estimate populated by ANALYZE)
rather than a real count -- these tables were never ANALYZEd after their
one-off CTAS creation, so the estimate silently read 0 over real data.
Worth remembering if a `list_tables` row count is ever relied on for an
emptiness claim elsewhere.

**Fix: eq-shell** [#1833](https://github.com/eq-solutions/eq-shell/pull/1833)
adds `supabase/tenant-migrations/0309_wipe_backup_schema_rls_lockdown.sql`
(`-- Plane: ehow ONLY`) -- enables RLS with no policy on all 4 tables,
service-role-only posture matching the SERVICE_ROLE_ONLY pattern used for
`audit_log`/`migration_baseline` elsewhere. Royce's call: lock down rather
than drop, to keep the data available if it's ever needed -- the right call
in hindsight, given the tables turned out to hold real data. Merged
`e0f815bd`.

**Dispatch near-miss, corrected same session.** First `tenant-migrate.yml`
dispatch used `--slug=ehow` (the Supabase project ref, used throughout this
repo's docs as ehow's nickname) -- the job exited green in 9s but its own
log read `No tenant routing for slug='ehow'` and silently applied nothing;
RLS was still disabled on re-check. `shell_control.tenant_routing` has no
`slug` column at all -- the actual slug lives in `shell_control.tenants`
(joined via `tenant_id`), confirmed live: ehow's real slug is `sks`.
Redispatched with `--slug=sks`; log confirmed `Targets: sks (active ->
ehowgjardagevnrluult)` and `[sks] applied 0309_wipe_backup_schema_rls_
lockdown.sql`, 1 applied / 336 correctly skipped (already on ledger),
0262 (a zaap-only migration) correctly stayed out of scope for sks. A green
tenant-migrate.yml run does not by itself mean anything was applied -- check
its own log line, or the ledger / advisor, before trusting it. Both green
runs are visible on the PR/Actions history for anyone auditing this later:
run 34317512446 (no-op, wrong slug) then 34317768298 (real apply).

**Closed, live-verified 2026-09-09 06:08:49 UTC**: all 4 tables
...
### SEC-76 — eq-service canonical-members.ts defaulted every unscoped roster lookup to SKS (P2, fix ready)
Found during a research pass scoping a suite-wide tenant-identity-drift doc
(eq-context, 2026-09-09). `lib/canonical-members.ts:35` defaulted its tenant
slug to `process.env.CANONICAL_TENANT_SLUG ?? 'sks'`, and `getCanonicalMembers()`
/ `getCanonicalMemberMap()` used that as their default argument. 19 call sites
across 15 files called them bare (no tenant argument) — every one silently
rendered SKS's staff roster regardless of which tenant was actually viewing:
assignee dropdowns, notification recipients (including the pre-visit-brief
cron), report "tested by"/"assigned to" names, the audit log, the admin user
roster. Independently re-derived against live code before touching anything —
grep found the exact same 19 sites, same line numbers, that the handed-in
report named. A related edge case in `lib/calendar/supervisor-digest.ts:178`:
on a tenant-slug resolution failure it fell through to the same unsafe
default instead of the local fallback its own docstring required.

Not live-exploitable today — only one tenant (SKS) exists on ehow (see
suite-state.md's tenant count) — so there's no second tenant's data to leak
yet. A primed landmine for the next tenant, same class as the
`field_field_*` view-grants finding (2026-07-19, above) and this repo's own
prior `fix/sec50-*` / `fix/tenant-scope-*` branches.

**Fix (code-complete, verified, not yet committed):** `getCanonicalMembers`
/ `getCanonicalMemberMap` now require their tenant-slug argument — no more
default, so a missed call site is a `tsc --noEmit` compile error, which is
how completeness was verified (clean compile + clean `eslint`, all 23
touched files). Added `getCanonicalMemberMapForTenantId` — a Map-returning
sibling of the existing `getCanonicalMembersForTenantId` — and used it to
fix the 19 sites plus the `supervisor-digest.ts` edge case plus one
cascading layer (`lib/reports/resolve-user-names.ts`, called from 5 report
builders that already carried `tenantId`). `canonical-members.ts` is
`"private": true`, not a published `@eq-solutions/*` package — zero
external blast radius from the signature change. Every fix site uses a
fresh `createAdminClient()` for the tenant-slug lookup rather than each
call site's own request-scoped client, matching the 3 pre-existing correct
reference sites (`supervisor-digest.ts`, `dispatch-notifications/route.ts`,
`defect-notifications.ts`) — the alternative (reusing each page's
RLS-scoped client) would make correctness depend on `service.tenants`'
RLS policy, unverified and unnecessary to depend on.

Diff: 23 files, +92/-57. **CLOSED 2026-09-09** — [PR #838](https://github.com/eq-solutions/eq-service/pull/838)
merged (admin override past 2 pre-existing unrelated failures: a chronic
`npm audit` finding on `js-yaml`/`next`/`sharp`, and this repo's
chronically-broken integration-test suite — `tsc + next build`, the real
gate, was clean) and confirmed live on service.eq.solutions via Netlify +
commit ancestry, not just merge success. No live click-test against a
second tenant was possible — none existed to compare against, and no
Shell/demo credentials in this environment either way. `lib/canonical-sync.ts`
also reads `CANONICAL_TENANT_SLUG` — a separate consumer, deliberately
...
identity" failure shape four times in one day across four other repos. SEC-76
above (`canonical-members.ts`) is a second, independent task from the same
sweep (§0 item 1) — different finding, different fix, no overlap in scope.
...
writes there. **Not investigated further this session — spun off as its own
task** (`task_53ac5191`, "Find why public.audit_log still gets writes, not
service.audit_logs") rather than folded in here — a genuinely separate
question from this row's own scope.
...
Ownership resolved by grepping `CREATE TABLE` across eq-shell's and
eq-field's migrations (not guessed by name): **eq-field owns ~15**
(likely already covered by its own tenant-provision generator's
substitution policy — see `system/tenant-identity-drift-scoping-2026-09-09.md`
§6.4/addendum for the full split), **eq-shell owns ~10** (not covered by
that generator at all — open question whether eq-shell's own
`tenant-migrate.yml` already handles this), **6 are shared** between both
...
**Renumbering note (2026-09-09):** this finding was originally written and
pushed as SEC-76 (commit `a59f79c0`). A concurrent session's own,
independent SEC-76 finding (`canonical-members.ts`, above) was pushed
shortly after via its own `safe_commit.py` run — that script copies a
file's bytes wholesale from the caller's own working tree into a fresh
scratch worktree off `origin/main`; it does not diff or merge against
what's changed on `origin/main` since the caller's local copy was last
read. The other session's local copy predated this row's push, so its
push silently overwrote this entire finding on `origin/main` — not a
rejected push, not a visible conflict, just gone. Caught only because this
session's own close-protocol read `origin/main` fresh before writing the
session log and found SEC-76 was a different finding than the one just
pushed. Restored here as SEC-77. **Worth a process fix, not applied here**:
`safe_commit.py`'s own docstring already names this exact risk ("this does
NOT resolve content conflicts... that's still the caller's call to make")
but nothing currently forces a caller to re-check a fast-moving,
frequently-appended file (like this register) immediately before its own
