...
| SEC-53 | P3 | core.eq.solutions and field.eq.solutions CSPs both still allow-list `ktmjmdzqrogauaevbktn.supabase.co` (deleted project, SEC-17) in `connect-src`; core's `frame-src` still lists `quotes.eq.solutions` (retired) | eq-shell, eq-field | OPEN — found 2026-08-20, proved live. No exploit path (dead refs are inert) — cleanup. **2026-09-04 — eq-shell half CLOSED, eq-field half still OPEN.** core.eq.solutions: [#1663](https://github.com/eq-solutions/eq-shell/pull/1663) (`3b832f73`, 2026-08-30) dropped `ktmjmdzqrogauaevbktn.supabase.co` and `quotes.eq.solutions` from the CSP — confirmed clean by the live response headers 2026-09-04. field.eq.solutions: eq-field's `_headers` on `main` (line 22, `Content-Security-Policy`) still allow-lists the deleted `ktmjmdzqrogauaevbktn` project — re-verified against eq-field `origin/main` 2026-09-04. What's left is a one-line `_headers` edit in eq-field. |
