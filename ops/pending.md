...
- [ ] **Root-cause default-privilege gap — attempted, does NOT actually fix it, do not retry the same approach.** Royce approved (`/decide` pass, "go") building `ALTER DEFAULT PRIVILEGES ... REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC` on ehow/zaap/jvkn to stop SEC-4/13/15 recurring. **Live-tested in a rolled-back transaction on ehow before shipping (per Rule 0.5) and it's a no-op**: the `pg_default_acl` catalog row updates correctly, but a freshly `CREATE FUNCTION`'d function's real ACL still carries `PUBLIC=EXECUTE` and `anon`/`authenticated` can still call it — confirmed 3 ways (plain schema, scratch schema, with an explicit companion `GRANT ... TO service_role` to force a non-null ACL, all still leaked PUBLIC). Compared against `rls_introspection()`'s own already-fixed ACL (clean, no PUBLIC) to confirm the difference: only an **explicit REVOKE/GRANT run directly on the function after creation** — the exact pattern SEC-4/13/15 already used — actually works. Why `ALTER DEFAULT PRIVILEGES` fails for functions specifically (works fine for tables, per the 2026-06-07 lockdown) is unexplained — not a known Supabase event-trigger (checked `pg_event_trigger`: only pg_cron/pg_net/pg_graphql hooks exist, none touch this). **Real conclusion: there may be no way to "close this at the source" for functions on this platform — the drift gate catching each new instance (as it already does, 3-for-3) may be the actual permanent mitigation, not a workaround.** Nothing shipped; migration file was written, tested, and deleted without committing. Needs someone with deeper Postgres/Supabase-internals knowledge to explain the mechanism before trying again, or accept the drift-gate-catches-it-quickly posture as the real answer.
