...
## New machine (NUCBOX_EVO-X2) bridged from the old Beelink — repos + global config relocated to C:\Projects (2026-09-07)

*Royce started using a new computer. This session (one of ~9 running concurrently on related new-PC-setup and eq-field work today, see `sessions/2026-09-07.md`) handled the machine-level bridge rather than any single product repo: confirmed this box replaces the Beelink as the canonical machine, relocated all 15 eq-solutions/personal repos off an earlier ad-hoc `C:\Users\EQ\code\eq-solutions\` layout onto the established flat `C:\Projects\<repo>` convention, restored `~/.claude` as a live git working copy of `Milmlow/claude-code-config`, and restored the `C:\Projects\CLAUDE.md` umbrella file from a OneDrive-synced copy rather than a manual old-machine transfer.*

- [ ] **`C:\Projects\CLAUDE.md`'s own repo-map table has drift, deliberately not fixed this session**: lists `eq-intake` (live org has `eq-solves-intake`), lists `eq-solves-assets` (no match in a live `gh repo list eq-solutions` — 15 repos checked), and its "Naming traps" section still describes `eq-solves-service` → `eq-service` as a "planned move" that's evidently already happened. Needs a live-verify pass + Royce's steer on genuine renames vs. repos that need re-adding. _(added 2026-09-07)_
- [ ] **"Beelink" is still hardcoded as the canonical machine** in several eq-context/claude-code-config docs (e.g. this repo's own `CLAUDE.md`, the "Claude Code / Cowork (Beelink)" local-clone rule) — stale now that this machine has taken over that role. Deliberately deferred to a dedicated substrate tidy-up pass rather than folded into this session. _(added 2026-09-07)_
- [ ] **`claude-code-config` repo root carries stray non-config files** (`notif_1.sql`, `notif_2.sql`, `notifications_insert.sql`, `sprint-wave2-identity.js`) that don't belong in a global-config repo — flagged, not cleaned. Also left a redundant checkout at `C:\Projects\claude-code-config` (the repo's real home is `~/.claude` itself, now git-tracked in place there) — harmless, not removed. _(added 2026-09-07)_
- [ ] **4 empty leftover directories** at `C:\Users\EQ\code\eq-solutions\{eq-cards,eq-field,eq-service,eq-shell}` — confirmed empty (0 items), repeatedly failed to delete (file-lock, likely antivirus/indexing on this new machine) across 2 separate attempts. Harmless; retry later or after a reboot. _(added 2026-09-07)_
- [ ] **VS Code Settings Sync** — suggested early in the bridge, never confirmed either done or unnecessary. _(added 2026-09-07)_

**Notes for whoever picks this up:** `.claude/worktrees/`-nested worktrees (e.g. eq-context's own `adversarial-test-python-fix`) survive a parent-repo relocation cleanly; older sibling-folder worktrees (like `eq-field-tenant-demo-fix`, found stranded mid-session with real uncommitted work on branch `claude/tenant-demo-fix` — recovered via `git worktree repair`, content intact throughout) do not, and need manual repair after any repo move. Separately: a bulk `robocopy /MOVE` of a freshly-cloned repo on this machine silently left `eq-shell`'s working tree with several files missing (showed as "deleted" in `git status`) despite a non-fatal exit code — `git fsck` confirmed the object database was intact throughout, `git restore .` fixed it cleanly. Worth checking `git status`/`git fsck` after any bulk repo move on this machine rather than trusting the copy tool's own exit code.
