...
**Correction, 2026-09-09 (second) — the Decision line's "Cards included" is wrong on mechanism.** Verified against `eq/identity/IDENTITY-MODEL.md` §9-10 and the 2026-06-15 decision below ("Cards is the Worker-Facing EQ Platform; Core is Employer-Only"): Cards does not mint sessions through `token-exchange.ts` at all — it authenticates workers directly via Supabase's own GoTrue phone-OTP flow, bridged to `shell_control.users` only via `custom_access_token_hook`. The single mandatory hub for **minting a session** covers Shell, Field, and Service (all three go through `token-exchange.ts`); Cards reaches jvkn by an entirely separate path. The genuine shared SPOF is **jvkn itself** — both paths ultimately depend on `shell_control.users` living there — not eq-shell's own Netlify function, which is a SPOF for Field/Service specifically, not for Cards. Practical effect on the accepted risk: unchanged for "jvkn is down" (still locks out everyone — both paths depend on the same project); narrower than stated for "eq-shell/Netlify specifically is down" (Cards would very likely keep working, since GoTrue is called directly, never proxied through eq-shell's own functions). Found while building an architecture walkthrough, not chased further — Cards' behavior during a live eq-shell-specific outage (e.g. the tenant-tile handoff back to Shell) hasn't been separately tested.
