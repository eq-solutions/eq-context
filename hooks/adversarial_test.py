...
te("Write a brand-new file under the bare root (F16, unlike F12, does not exempt new files)",
   {"tool_name": "Write", "tool_input": {"file_path": os.path.join(context_root, "system", "brand-new-file.md")}},
   2, CONTEXT_ENV)
te("MultiEdit under the bare root",
   {"tool_name": "MultiEdit", "tool_input": {"file_path": os.path.join(context_root, "system", "lessons.md")}},
   2, CONTEXT_ENV)
te("Windows backslash path form",
...
te("a manually created SIBLING worktree (eq-context-wt-<topic>) needs no special case",
   edit_at(os.path.join(os.path.dirname(context_root), "eq-context-wt-some-topic", "CLAUDE.md")), 0, CONTEXT_ENV)
te("EQ_CONTEXT_ROOT_OK=1 escape hatch for a deliberate root task",
   edit_at(os.path.join(context_root, "CLAUDE.md")), 0, dict(CONTEXT_ENV, EQ_CONTEXT_ROOT_OK="1"))
te("a sibling directory sharing the name as a PREFIX, not the real repo",
   edit_at(context_root + "-other-repo/CLAUDE.md"), 0, CONTEXT_ENV)
te("a completely different repo — control, F16 must stay dormant",
   edit_at(os.path.join(NOGIT_CWD, "CLAUDE.md")), 0, CONTEXT_ENV)
te("Bash git status against the bare root — F16 checks Edit/Write only, not git verbs",
   bash_at("git status", context_root), 0, CONTEXT_ENV)
