...
Blocks the failures prose could not stop:
  ...
  F16 The SHARED eq-context checkout itself — the repo this very file lives
      in — never got F15's treatment, despite being the single most
      concurrently-written repo in the suite (every EQ/SKS session writes
      here at close, CLAUDE.md Section 10). Live evidence found 2026-09-08:
      the bare root 100+ commits behind origin/main carrying a ~33-file
      uncommitted pile with no single owner; three separate orphaned
      worktrees in three different, ad-hoc naming conventions (nested
      .claude/worktrees/, <repo>-<topic>-wt, <repo>-wt-<topic>), two of them
      holding real never-pushed commits; and close.md's own documented Step 5
      (`git commit -m "..."` with no `--` pathspec) directly contradicted
      F9(a)'s existing bare-commit guard above, meaning every session
      following that doc verbatim was already being blocked and forced to
      improvise on the spot. Same mechanism as F15
      (eq_context_bare_root_path() mirrors eq_cards_bare_root_path()
      exactly). Shipped in two stages same day: built adversarially tested
      but gated inactive first (EQ_CONTEXT_ROOT_GUARD=1 opt-in), specifically
      because close.md's own Steps 2-4 still wrote the bare root directly at
      the time; activated unconditionally once tools/commands/close.md was
      restructured (a new Step 0: isolate into a worktree before Steps 1-4
      touch any file) so nothing in the documented close protocol still
      needs bare-root Edit/Write access. Royce's explicit choice via
      AskUserQuestion — "restructure close.md first, then activate" — over
      shipping inactive or activating immediately and accepting the
      close.md-Steps-2-4 breakage. See system/failures.md -> F16.
...
def main():
    ...
    # --- F16: eq-context's own bare-root guard -----------------------------
    # Same mechanism as F15 above, mirrored for the repo hosting this file.
    # Active unconditionally, same as F15 — see this file's module docstring
    # (F16) for the two-stage shipping history (built gated-off first, since
    # tools/commands/close.md's own Steps 2-4 still wrote the bare root
    # directly at the time; activated once that doc was restructured to
    # isolate first, per Royce's explicit choice).
    if tool in EDIT_TOOLS:
        ...
        if eq_context_bare_root_path(path16) and os.environ.get("EQ_CONTEXT_ROOT_OK") != "1":
            block(
                f"BLOCKED by pre_tool_use (F16, rung 4).\n\n"
                ...
                f"  a direct edit here can collide with a concurrent session, or\n"
                f"  simply be left uncommitted/unpushed if this session ends before\n"
                f"  the final push — the actual, evidenced failure mode: a\n"
                f"  100+-commit-behind root carrying a 30-file pile, and three\n"
                f"  abandoned worktrees in three different naming conventions, all\n"
                f"  found live on 2026-09-08.\n\n"
                f"  Work in your own worktree instead — one call, no clone/setup:\n"
                f"    EnterWorktree\n\n"
                f"  Following tools/commands/close.md? Its Step 0 isolates first —\n"
                f"  go back and run that, then Steps 1-4 land in the worktree it\n"
                f"  creates, never here. Landing any other substrate change (a doc\n"
                f"  fix, a one-off edit)? Use scripts/safe_commit.py, not a raw\n"
                f"  commit/push in this root — it fetches, branches off a fresh\n"
                f"  origin/main in a scratch worktree, copies in exactly the files\n"
                f"  you name, commits, and pushes with fetch+rebase retry on a race.\n\n"
