...
| Hook | Rung | Kills |
|---|---|---|
| `session_start.py` | 4 | **F1** — stale substrate read (8–12d, 200 OK, no error). Prints freshness, Needs-you, goals status, any guard overdue for promotion, and **core.hooksPath resolution** — flags F10, four distinct mechanisms that all produced the identical "pre-commit silently doesn't run" symptom (2026-05-24 wrong directory, 2026-08-04 shadow copy at `.git/hooks` via F8, 2026-08-05 a `--worktree` override shadowing a correct `--local` value, 2026-09-06 unset at every scope on a fresh clone/machine). Warn-only for a wrong-but-SET value — `pre_tool_use.py` blocks the SET itself instead (rung 4 since 2026-08-05, see its own row below); **self-heals** the unset-everywhere case directly (sets `--local core.hooksPath .githooks`, re-verifies) since a fresh `git clone` cannot populate local config for itself and there is no legitimate reading of "unset" to preserve. Reads the **local clone, never a URL** — the URL is what lied. Also prints an unconditional **ISOLATE** reminder (F16, 2026-09-08) — every session, any repo, pointing at `EnterWorktree` + `scripts/safe_commit.py` before any write to THIS shared checkout. Purely informational, rung 2; the enforcement half lives in `pre_tool_use.py` below. |
| `pre_tool_use.py` | 4 | **F2 / F6 / F7 / F9 / F10 / F12 / F13 / F15 / F16** — Edit/Write silently truncating (F2) and `>>` append NUL-filling (F6) long files on the virtiofs mount; a git merge/stash-pop round-trip NUL-filling a file by some other means (F7, 2026-07-31); concurrent-session git races corrupting the shared eq-context checkout (F9, 2026-08-04) — bare `git commit` sweeping up another session's staged files, and rebase/merge/pull racing on HEAD/index/refs; a `core.hooksPath` SET landing anywhere but `.githooks` in that same checkout (F10, 2026-08-05); a raw file copy blind-overwriting that checkout from elsewhere (F12, 2026-08-18); a false eq-shell deploy-posture claim entering any substrate .md (F13, 2026-08-15); concurrent-session Edit/Write corrupting the shared eq-cards checkout (F15, 2026-08-25) — no compound git op needed, an ordinary Edit/Write against the bare root lands in whatever tree the root's branch/HEAD happens to be at that instant; and the identical F15 mechanism mirrored for eq-context's own bare root (F16, 2026-09-08) — shipped inactive first (`EQ_CONTEXT_ROOT_GUARD=1` to opt in) while `tools/commands/close.md`'s own Steps 2-4 still wrote the bare root directly, then activated unconditionally once that doc was restructured (a new Step 0: isolate first) to close the gap fully, per Royce's explicit choice via `AskUserQuestion`. Also blocks `git` from the Cowork sandbox (orphan `index.lock`). **Fail-closed.** F2/F6/the git-lock block are Linux-sandbox-scoped; no-ops on Windows (on the Beelink `guard.js` is the active write-guard for those — though as of 2026-07-31 `guard.js` still carries no NUL-byte logic of its own, see `system/failures.md` → F7 for the open question that leaves). **F7, F9, F10, F12, F13, F15, and F16's own checks are NOT sandbox-scoped** — they run on any platform ahead of any Bash/PowerShell git verb (F9/F10/F12) or Edit/Write (F13/F15/F16), each scoped to its own ONE shared checkout by exact path, so the fixes they recommend (a private clone for F9; `EnterWorktree` for F15/F16) are never themselves blocked. Tool matching widened to `Bash` + `PowerShell` throughout (previously `Bash`-only). F15/F16 are narrower than F9 by design: they block Edit/Write only, not git verbs — a commit landing on the shared root's current branch is a real, smaller, stated-open gap (`system/failures.md` → F15), not an oversight. |
| `session_end.py` | 4 | Section 10 (Session End Protocol) sitting at rung 1 — an agent had to *remember* to commit, push, and log the session, the exact "read it and still didn't fire" failure class F2 already proved doesn't hold. Reports dirty tree / unpushed main / a day with commits but no `sessions/<date>.md`, and (F16, 2026-09-08) any registered eq-context worktree carrying uncommitted or unpushed work at session end. **Fail-open, loud** — informational only, never blocks Stop (no destroyed data at stake to justify fail-closed, and a Stop hook that traps someone mid-exit is its own Loop of Despair). Built 2026-07-20, ahead of a recorded ledger entry — see `system/failures.md` note before treating it as a normal rung-2→4 promotion. |
