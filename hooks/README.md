...
| `pre_tool_use.py` | 4 | **F2 / F6 / F7** — Edit/Write silently truncating (F2) and `>>` append NUL-filling (F6) long files on the virtiofs mount; a git merge/stash-pop round-trip NUL-filling a file by some other means (F7, 2026-07-31). Also blocks `git` from the Cowork sandbox (orphan `index.lock`). **Fail-closed.** F2/F6/the git-lock block are Linux-sandbox-scoped; no-ops on Windows (on the Beelink `guard.js` is the active write-guard for those — though as of 2026-07-31 `guard.js` still carries no NUL-byte logic of its own, see `system/failures.md` → F7 for the open question that leaves). **F7's own scan is NOT sandbox-scoped** — it runs on any platform ahead of any Bash/PowerShell git verb, checking the working tree for pre-existing NUL corruption regardless of whether this hook thinks it's in the sandbox. Tool matching widened to `Bash` + `PowerShell` throughout (previously `Bash`-only). |
