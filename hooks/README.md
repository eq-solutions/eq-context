...
| `pre_tool_use.py` | 4 | **F2 / F6 / F7 / F9 / F15** — Edit/Write silently truncating (F2) and `>>` append NUL-filling (F6) long files on the virtiofs mount; a git merge/stash-pop round-trip NUL-filling a file by some other means (F7, 2026-07-31); concurrent-session git races corrupting the shared eq-context checkout (F9, 2026-08-04) — bare `git commit` sweeping up another session's staged files, and rebase/merge/pull racing on HEAD/index/refs; concurrent-session Edit/Write corrupting the shared eq-cards checkout (F15, 2026-08-25) — no compound git op needed, an ordinary Edit/Write against the bare root lands in whatever tree the root's branch/HEAD happens to be at that instant. Also blocks `git` from the Cowork sandbox (orphan `index.lock`). **Fail-closed.** F2/F6/the git-lock block are Linux-sandbox-scoped; no-ops on Windows (on the Beelink `guard.js` is the active write-guard for those — though as of 2026-07-31 `guard.js` still carries no NUL-byte logic of its own, see `system/failures.md` → F7 for the open question that leaves). **F7, F9, and F15's own checks are NOT sandbox-scoped** — they run on any platform ahead of any Bash/PowerShell git verb (F9) or Edit/Write (F15), each scoped to its own ONE shared checkout by exact path, so the fixes they recommend (a private clone for F9; `EnterWorktree` for F15) are never themselves blocked. Tool matching widened to `Bash` + `PowerShell` throughout (previously `Bash`-only). F15 is narrower than F9 by design: it blocks Edit/Write only, not git verbs — a commit landing on the shared root's current branch is a real, smaller, stated-open gap (`system/failures.md` → F15), not an oversight. |
