...
- [x] **Tenant-rule audit complete (2026-08-04).** Read-only recon across eq-shell, eq-field, eq-solves-service for service-role/admin queries missing an explicit tenant filter, per `rules/non-negotiables.md` #11. **eq-shell: clean** — no unfiltered cross-tenant query found (~110 of 218 `netlify/functions/` files read directly, the remaining ~90 pattern-sampled and consistent, not individually confirmed — not a zero-gap claim for the whole repo, a strong one for the areas checked). Three real gaps found elsewhere, each currently inert only because the caller happens to pre-validate, not because the function defends itself — logged as separate items below.
- [x] **All 3 gaps fixed and merged to `main` (2026-08-04).**
  - **eq-field** [PR #646](https://github.com/eq-solutions/eq-field/pull/646) merged (`17c1f7f`). `ts_reminders_sent` dedup/cooldown check was keyed only on `person_name`+`week`, no tenant filter — a real cross-tenant coupling bug (one tenant's same-named employee could suppress another's reminder), not a data leak. New migration `20260804_ts_reminders_sent_tenant_id.sql` adds `tenant_id`, backfills every existing row to SKS's id, both queries in `ts-reminder/index.ts` now filtered. The audit's *other* eq-field finding — the global hourly send-cap having no tenant filter — was investigated directly and is **not** a bug: a deliberate anti-abuse control on an endpoint gated only by a display name; scoping it per-tenant would let an attacker exhaust the cap once per tenant instead of once total. Left untouched, commented. **Live now (2026-08-04): migration applied to `ehow` (verified via `information_schema.columns` — `tenant_id uuid NOT NULL` present, lookup index rebuilt tenant-first) and `ts-reminder` redeployed (v11 → v12) with the merged tenant-scoped code.** Royce's explicit go-ahead for this step. **Not run** locally — no Deno runtime available, no existing test coverage for this function.
  - **eq-solves-service** [PR #690](https://github.com/eq-solutions/eq-service/pull/690) merged (`40dea4c`). `send-pre-visit-brief.ts` now filters all 9 queries that have a `tenant_id` column (verified live against ehow's schema first — `service.profiles` has no such column and was correctly left alone). `archive/helpers.ts`'s `countDependencies()` gained a required `tenantId` parameter + filter on its one service-role branch, threaded through both call sites. `tsc --noEmit` (full non-incremental) and `eslint` both clean. **This repo auto-deploys `main` to `service.eq.solutions` via Netlify CD** — the fix goes live on the next build, no separate step needed, unlike eq-field. **Not run** — no test coverage exists for either function.
- [x] **Audit extended to eq-cards (2026-08-04) — found a real authorization bug, closed same day.** `generate-wallet-pass`'s ownership check was dead code: queried a nonexistent `workers.auth_user_id` column via a `worker_id` field never selected on the credential row, so it always no-opped. Any authenticated Cards user who obtained another worker's `worker_credentials.id` would have gotten that worker's full wallet-pass payload with no error. Checked directly before deploying: this function had **never been deployed to production on any version** — no changelog entry, no caller anywhere in the Cards app's own source — so the bug was real in the code but never actually reachable. Fixed: joined `workers.user_id` into the existing query, gated on it, 404 on mismatch. eq-cards [PR #214](https://github.com/eq-solutions/eq-cards/pull/214) merged (`c256cf8`), then deployed as the function's first-ever live version (`jvkn`, v1, fixed from the start) — Royce's explicit go-ahead, given nothing calls it yet and it's otherwise still pre-production. Tracked as [SEC-20](../ops/security-register.md), now closed.
...
- [x] **EQ Service URL disagreement resolved (2026-08-04).** Checked live via the Netlify API rather than guessing: `service.eq.solutions` is correct, `eq-solves-service.netlify.app` was never a valid alias. Fixed `suite-state.md` and, one level down, the same dead-URL bug in `.github/scripts/refresh_suite_state.py`'s `NETLIFY_SITES` matching (also caught the equivalent dead `eq-solves-field.netlify.app` key — both had been silently matching nothing, masked only because `NETLIFY_TOKEN` isn't set in CI).
...
- [x] **eq-field's app icon colour fixed (2026-08-04).** Confirmed real: `manifest.json`'s `theme_color` was `#1F335C`, the same stale SKS navy already caught in the transactional emails. eq-field [PR #647](https://github.com/eq-solutions/eq-field/pull/647) merged — corrected to `#203060`, plus a wider gap found while checking it: the whole "Add to Home Screen" identity (name/icons/colour) was SKS-only regardless of tenant, unlike the browser-tab favicon which already swaps by tenant. New `manifest-eq.json` gives EQ its own identity; the existing tenant-detection script now overrides the manifest link, theme-colour, and iOS app title for SKS too. Verified live on the deploy preview for both tenants before merge.
...
## ⏩ EQ Shell — admin licence backfill: back-photo support + OCR-hang diagnosis (2026-08-04)
