...
- [x] The home-page "ask anything" bar and the daily AI briefing now only show for roles who could already see that underlying data elsewhere — nobody loses anything they had, nobody gets a new shortcut around an existing gate.
- [x] The licence-photo scanning page now checks access properly before running — previously any signed-in role could open it directly and read back another person's name, licence number and date of birth.
