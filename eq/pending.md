...
- [x] **An admin changing someone's mobile number now actually moves their login, instead of quietly splitting them in two.** Previously only Shell's own copy was updated; the real login still had the old number, so the worker's next sign-in minted a fresh account with an empty wallet while every licence they'd uploaded stayed on the old one. Now both move together, and if the login change fails the copy is put back rather than left disagreeing. eq-shell PR [#1347](https://github.com/eq-solutions/eq-shell/pull/1347), squash `eb5a1348`, live.
- [x] **Workers who belong to no company can now be helped at all.** Every admin tool was scoped to your own company, so the 19 accounts with no employer — 18 of whom signed in within 90 days — could only be fixed by someone editing the database by hand, with no record of who did it or why. New platform-admin tool replaces that: requires a second factor, refuses self-edits, and records who confirmed the person's identity and the reason. Both platform admins already have the second factor set up, so it works from day one. eq-shell PR [#1352](https://github.com/eq-solutions/eq-shell/pull/1352), live.
- [x] **A read-only permission was gating a write to staff records.** `staff-update` let anyone holding the "can view Field" key — which every role holds, apprentices and subcontractors included — rewrite any colleague's name, email, phone, trade or level in their company. Now gated on manage-people (managers and supervisors only). eq-shell PR [#1353](https://github.com/eq-solutions/eq-shell/pull/1353), live. Two things found in the same pass drove the rest of that thread in later sessions the same day: the endpoint had never actually worked (wrong database schema, so every call errored), and the underlying database function was callable directly by any logged-in user regardless of the app's permission checks — closed by #1364, and the endpoint retired outright by #1366.
- [x] **Signing in with an email address now works, as a second door for a lost number.** One field takes either a mobile or an email and works out which; an unknown address is refused rather than turned into a new account. Shipped in eq-cards [#246](https://github.com/eq-solutions/eq-cards/pull/246) alongside the sign-in copy fix recorded further down this file — same branch, three commits.
- [x] **Both apps deployed and verified live** — Core on `7d0e065`, Cards on `e090418`, each smoke-checked (sites answering, auth-gated endpoints refusing unauthenticated calls). The Cards deploy also ran the edge-function step green in a real deploy for the first time — the gap that let a merged fix silently not ship.
...
- [x] **Deployed — automatically, on merge. The "not deployed / Royce to trigger" note that sat here was wrong and is retracted.** Merging to `main` on eq-shell **does** trigger a Netlify production build, live on core.eq.solutions 2-4 seconds later, unattended. PR [#1346](https://github.com/eq-solutions/eq-shell/pull/1346) (squash `25b4a0fb`) merged 2026-08-14 11:18:41 UTC and its production deploy started at 11:18:43 — it was live the moment it merged, and `25b4a0fb` is confirmed an ancestor of the currently-serving deploy. There was never anything to trigger. Verified 2026-08-15 across 13 consecutive merges → 13 production deploys via Netlify's own GitHub App (installation `121276861`); the old claim was mis-inferred from `deploy_source: "api"` + `cdp_enabled_contexts`, which do **not** mean "a human triggered it" — see [rules/deployment.md](../rules/deployment.md). _(added 2026-08-14, corrected 2026-08-15)_
