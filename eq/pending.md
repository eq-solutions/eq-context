...
- [ ] **PR #683 merge/deploy** — held pending Royce's explicit go-ahead, per the standing auth-change rule. **Correction 2026-08-13:** the TODAY.md overseas goal cited alongside this was killed (Royce's explicit call, see `system/TODAY.md`) — the standing CLAUDE.md auth-change-approval rule is the reason this stays held, independent of the goal's death. _(added 2026-08-12)_
- [x] **`task_de667109` (EQ Service) outcome: fixed and deployed, in two stages.** First pass: the reports dashboard (reachable by anyone signed in — no role check at all) and the audit log page (blocked supervisors who should have access, while being the *only* thing stopping anyone else reading it) both now enforce the correct role, plus the same check added at the database level so a direct API call can't bypass the page. eq-service [PR #707](https://github.com/eq-solutions/eq-service/pull/707), merged and applied live. That pass found the identical gap on two more pages reading the same `audit_logs` table (`/admin/imports`, `/admin/activity` — the latter a wall-display kiosk feed for onboarding days) — spun off as `task_9f6fca23`. Second pass closed those: gated both on the same `canViewAuditLog` helper, eq-service [PR #709](https://github.com/eq-solutions/eq-service/pull/709), merged and deployed to `service.eq.solutions` (Netlify deploy confirmed: commit `f7bcb1d`, secret scan clean). Royce confirmed the kiosk login is a manager/supervisor account so `/admin/activity` keeps working under the new gate. Also flagged but not touched: the report-download endpoint lets a supervisor download reports the dashboard itself won't show them (pre-existing, narrower business call, not mine to make silently) — noted in PR #707. _(closed 2026-08-13)_
- [ ] **`task_fd65aa59` (EQ Shell: `audit.rollback`/`entity.edit` gaps) outcome** — still unknown, running independently. _(added 2026-08-12)_
