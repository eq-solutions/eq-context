...
- [x] Fixed directly, same session — trivial text edit, no auth/deploy risk: table now names Opus 5, Opus 4.8 references removed.

---

## eq-shell: the daily duplicate-identity check is stale, and it surfaced a second misleading-error gap (2026-08-16)
*Sentry flagged `EQ-SHELL-1M` as a "new error" in the session-start NEEDS YOU list. Checked it rather than assuming it was the same known incident. It wasn't active — it's a scheduled cron (`check-dangling-staff-pointers.mjs`) that alerts once a day for as long as a duplicate `staff_id` exists on live jvkn. Queried live: zero duplicates exist as of 2026-08-16 — the one it flagged (Richard Brown, `staff_id ced7fabc-9f0f-4fca-8fc3-639227410477`) was already cleaned up after the alert fired. PR #1373 (2026-08-15) stops new ones from being created. Sentry issue itself couldn't be marked resolved from this session — blocked by the safety classifier as a monitoring-state change — needs a manual click.*

- [ ] **Resolve [EQ-SHELL-1M](https://eq-solutions.sentry.io/issues/EQ-SHELL-1M) in Sentry** — data's already clean, just needs the click so the daily cron doesn't keep re-surfacing a phantom alert. _(added 2026-08-16)_
