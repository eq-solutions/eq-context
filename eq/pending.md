...
- [x] **Found a strong lead via the error-tracking tool (Cards project, not Shell) — narrows it to a Cards-side self-signup attempt, not the invite-accept flow.** A Cards error fired twice, 17 seconds apart, at 9:01am UTC on 2026-07-06 — exactly one hour before Will's real invite was correctly accepted: someone on a Sydney mobile phone tried to "connect to a company" in Cards and was blocked because that session's profile had no name set yet. That symptom (a nameless profile) matches the duplicate account's own state before it was hand-repaired. Not proven as Will specifically — Sentry didn't tag a user ID on this event, only an IP/geo — but the timing, location, and symptom line up well enough to call this the likely origin: a second, incomplete Cards session attempting self-signup around the same time his real invite was being processed, separate from his established phone-sign-in account. Detail: [EQ-CARDS-10](https://eq-solutions.sentry.io/issues/EQ-CARDS-10). _(done 2026-07-20)_
- [ ] **Still open: confirming it was actually Will on that Cards session, and what code path turned the failed attempt into a real duplicate account.** Not urgent (his data is already repaired) but worth closing out if it recurs — next step would be comparing that Sydney IP/session against Will's own device history if he can confirm it, or tracing eq-cards' connect-request code path from a still-nameless session through to a created sign-in record. _(added 2026-07-20)_
