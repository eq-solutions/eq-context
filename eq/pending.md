...
*Royce: "the map we made is all squashed now" → `/decide` on a map-emoji idea → built as a Lucide-icon nav item instead → "go ahead and remove the staff reviews tab, this was moved to eq shell" → both shipped together. Then: "can we make it so that when you hover over a location it lists the people names?" → shipped. Then Royce caught a real bug from the outside: "weird - its showing 3.5.486 and the map view is there? Have we missed a few updates?" → investigated and fixed live.*

- [x] **Dashboard's embedded "Where people are today" map moved to its own dedicated "Map" page** (new sidebar nav item, next to Sites) — ~25-30 versions of squash/centering bugs (v3.5.400-490) traced to a live map canvas sharing a container with the rest of the Dashboard's own frequent re-renders. Own page = own lifecycle, problem structurally avoided rather than patched again. eq-field [#689](https://github.com/eq-solutions/eq-field/pull/689), merged, live (v3.5.491).
- [x] **Staff Reviews tab removed** — functionality moved to eq-shell, this was a dead pilot tab in eq-field. Same PR as above.
- [x] **Map hover tooltips now list who's actually rostered at a site**, not just a headcount number — both the new Map page and the Roster Overview card's mini-map. Database RPC (`get_site_headcount_for_map`) extended to return names alongside the count, applied live to the SKS database on Royce's explicit go-ahead. eq-field [#690](https://github.com/eq-solutions/eq-field/pull/690), merged, live (v3.5.492).
- [x] **Fixed a real bug Royce caught live: the version number shown in the app had been frozen for 6 releases.** A cache-busting tag in the app's HTML didn't get bumped when it should have (v3.5.487 through v3.5.492), so returning browsers kept serving an old cached copy of one core file forever (by design — that file is cached for a year). Confirmed nothing else was affected, just the version display. Same mistake has happened 3 times before in this app's history; still no automated check catches it. eq-field [#691](https://github.com/eq-solutions/eq-field/pull/691), merged, live (v3.5.493).
...
*Royce: "when someone was approved leave one of the executives didn't know about it" — turned out the original report was from the old SKS NSW Labour app, wanted EQ Field checked separately. Then: "check the wiring for the job numbers tab via timesheets." Then: "the digest only addresses leave next week... thinking the next four weeks would help?" Then: "explain what options we have to be able to edit these templates" → `/decide` → "then build this."*

- [x] **Found and fixed the real reason someone can approve leave and nobody else finds out: there are 3 separate ways to approve it in this app, and only 1 of them emailed anyone.** The dashboard's quick-approve button sent no email at all, to anyone — a bare database write with nothing else. The "Approve" button built into the notification email itself (probably the one people actually click) never copied in the configured notify-list either. Both fixed — every approval path now behaves the same way. eq-field [#692](https://github.com/eq-solutions/eq-field/pull/692), merged, live (v3.5.494).
- [x] **Checked who's actually on the notify list today: just Royce.** If the missing executive isn't already flagged as a Supervisor in the system, adding them to the list won't be possible until they are — Royce is adding them himself via the app.
- [x] **Job Numbers panel on the Timesheets tab was blank while the same numbers showed fine on their own page** — a loading-order bug (one small file wasn't being pulled in unless you'd visited the other page first in the same session). Fixed, same PR as above.
- [x] **Friday digest email now looks 4 weeks ahead for upcoming leave instead of just 1**, grouped into 4 separate week sections so a busy week doesn't bury a leave request starting sooner. eq-field [#693](https://github.com/eq-solutions/eq-field/pull/693), deployed live.
- [x] **New self-service screen for editing the wording of the 3 leave emails** (request submitted / approved-or-rejected / receipt) — no code change or Claude needed for a wording tweak going forward. Scoped deliberately small first (`/decide` pass): only these 3 emails, not the Friday digest or the other email types yet, holding those until this gets used at least once. Has a "send yourself a test" button before saving for real, and always falls back to today's wording if anything goes wrong — a bad edit can never stop an email going out. eq-field [#695](https://github.com/eq-solutions/eq-field/pull/695), merged, live (v3.5.496).
- [x] **Caught and resolved a real collision mid-session**: another change landed with the exact same version number while this was being built. Re-based cleanly onto the latest code, renumbered, nothing lost on either side.
