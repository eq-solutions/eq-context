...
- [ ] **Merge PR #958 (eq-shell) once reviewed** — closes a suite-wide gap where Field/Service/Cards iframe token minting never checked the per-tenant module entitlement (a user could get a valid token for a module their tenant had switched off; zero live impact today since those 3 modules are on for all 3 tenants, but closes the boundary for any future tenant). Ops backend deliberately left out of this PR — see next line. _(added 2026-07-22)_
- [ ] **Ops backend still has no entitlement enforcement** — `crm-write.ts`/`canonical-api.ts` are shared between Ops-exclusive endpoints and ungated Customers/Staff pages, so a blind entitlement check risks breaking Customers/Staff for `eq` (the only tenant with `ops` currently off). Needs a real design pass (split which endpoints are ops-only vs shared) before enforcing — not done in PR #958. _(added 2026-07-22)_
