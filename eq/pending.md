...
- [ ] **Dashboard map centering — built, PR open, awaiting merge and Royce's live confirmation.** Royce, after hard-refreshing to rule out stale cache: "still pinned to the bottom, dig into it" / "I am positive that the map (nominal values) thinks its 1600px tall but the actual window is only 800px - something like that". Real precedent found in this file's own history — v3.5.427 logged the identical complaint verbatim, patched then by raising maxZoom rather than the actual measurement race. `L.map()` measures its container at the exact sync tick it's constructed, which can still race ahead of the Shell-iframe's own layout pass finishing even with `invalidateSize()` immediately after. eq-field [PR #630](https://github.com/eq-solutions/eq-field/pull/630) (v3.5.437) defers the fit one `requestAnimationFrame` and logs real-vs-Leaflet height once for a hard number if it recurs. Ruled out a `height:1600px` rule in vendored `leaflet.css` — stock Leaflet's Safari-only tile-stretch workaround, inert on Chromium. **Not independently reproduced or verified** — no access to Royce's authenticated session to click-test; this is a well-reasoned, precedented fix for a documented failure class, not a confirmed-working claim. `node --check` + `npx eslint` (0 errors) clean. Not merged. _(added 2026-08-03)_
