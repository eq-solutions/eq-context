...
*Continuation of the mobile-dashboard work above. Royce reviewed 3 real phone screenshots ("still feels like a lot of information that's unorganised... have the changes been a success") and found a genuine duplicate-render bug, not just a design opinion. Then asked for the hero stats to be actionable tiles (people on leave / licences expiring / outstanding quotes value) instead of generic figures. Then found, live on his own phone, that none of it actually worked — three separate real bugs, not a caching issue as first suspected.*

- [x] **Today's Actions was rendering three times on mobile** — a dead CSS selector (`.eq-hub-cols` in `App.css`; the real class is `.eq-hub-col`) meant the desktop columns were never actually hidden on mobile. Fixed, plus trimmed the "Licences expiring" list out of `SignalsBoard` on mobile (same names Today's Actions already shows there), with a one-line fallback note so the card doesn't look empty when that was its only content. eq-shell [PR #1299](https://github.com/eq-solutions/eq-shell/pull/1299).
- [x] **Hero stats replaced with 3 actionable, colour-coded tiles** — people on leave (sky), licences expiring (amber), outstanding quotes value to the nearest thousand (green), each tapping through to the relevant screen. eq-shell [PR #1300](https://github.com/eq-solutions/eq-shell/pull/1300).
- [x] **The new tiles weren't showing on Royce's phone at all — root-caused live, not assumed cache.** `.eq-mob-hero`'s mobile media-query override set background/border/padding but never reset `display` off the base rule's `display: none`. The whole hero card — greeting, dateline, every stat tile, old design and new — has been invisible on mobile since the original build (#303, months old), not something this session broke. eq-shell [PR #1301](https://github.com/eq-solutions/eq-shell/pull/1301).
- [x] **Once visible, found 3 more real bugs testing live on Royce's phone, all fixed same session:** the logo was styled text instead of the real EQ mark; "Outstanding quotes" showed "-" for SKS because it read EQ's own native tenders pipeline (never populated for SKS) instead of real quote data — added a `quotes_outstanding_cents` figure summed from quotes genuinely out with a client; "On leave" showed 1 while Field's own Leave tab showed 4 for the same day — the tile read a capacity-planning number that nets out anyone also rostered elsewhere in the week, not a literal headcount, so a real leave-request count was added, sourced the same way Field is and verified live against ehow before writing the fix (1 vs 4, confirmed). Tapping the leave tile also didn't open Field on the Leave tab — Field is pre-warmed in the background 2.5s after login and never reloads, so any `?tab=` deep link clicked after that point silently failed to switch tabs; fixed with a route-aware reload. eq-shell [PR #1303](https://github.com/eq-solutions/eq-shell/pull/1303).
- [ ] **Royce to give final confirmation on his phone** that all four PR #1303 fixes hold: real logo, a real dollar figure on Outstanding quotes, "4" on On leave, and tapping it lands on Field's Leave tab. _(added 2026-08-11)_
