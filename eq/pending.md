...
- [ ] **The `eq` tenant map gap above — Royce: "fix it".** Built and PR'd: eq-field [PR #626](https://github.com/eq-solutions/eq-field/pull/626) (v3.5.433) adds lat/lng to `SEED.sites` and a `_dashMapSeedRows()` helper computing headcount straight from `SEED.schedule`; `_renderDashboardMapCard()` branches on `TENANT.ORG_SLUG === 'eq' || 'demo'` to render from that instead of the (always-failing) live RPC path, reusing the existing clustering/render code unchanged. Group-filter dropdown doesn't apply to the seed path (accepted demo-only simplification). `core-bundle-b1.js` now 1892/1900 — only 8 lines of headroom left, next touch will likely need the ceiling raised a 4th time. Not merged, not deployed — awaiting Royce's explicit go, same as #625. _(added 2026-08-03)_
...
**Deferred:**
- [x] ~~A rare licence-photo-scanning failure needs a credential check, not a code fix.~~ **CORRECTED 2026-08-02 (later same day) — root cause found live, was NOT a credential/key issue.** Reproduced by Royce during a real test session and traced directly against `jvknxcmbtrfnxfrwfimn` logs: `ocr-licence`'s manual JWT check hit GoTrue's `/auth/v1/user` and got back `403 user_not_found` ("User from sub claim in JWT does not exist") — the calling device held a cached session for an `auth.users` row that had already been deleted (an old leftover test-account session, not either of Royce's two active identities, both confirmed still present and healthy). The client's `ocrErrorMessage` mapping (`licences_list_helpers.dart`) labels any 401 here "sign-in expired," which surfaced as "OCR didn't run: sign-in expired." **Not fixed at the source** — signing out and back in clears it for that one device, but nothing yet detects a dead-user session app-wide and forces a clean re-auth before it reaches a confusing error deep in the OCR flow; will recur for any device holding a session for a since-deleted account. Auth-flow change, needs Royce's call before building. _(originally added 2026-08-02, corrected 2026-08-02)_
