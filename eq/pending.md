...
## EQ Service — NSX/ACB testing lists fixed in the Shell iframe + Field Run-Sheet now carries recorded breaker details (2026-07-15, BOTH MERGED + LIVE)
*Two separate bugs found and fixed same day. First: opening NSX or ACB Testing inside Shell showed "No checks yet" even when checks existed — Royce hit this live on a real SKS check (DigiCo Annual NSX). Root cause: those two screens (plus the Test Equipment cert-history panel) fetched data straight from the browser, but inside the Shell frame there's no login session for the browser to use, so the read silently came back empty. Moved those reads onto the server (same way the working screens already do it) — fixed, confirmed live. Second: the printable Field Run-Sheet was found to be dropping breaker nameplate details (brand/model/serial/etc.) that a tech had already recorded on-site — the data was being fetched from the database then thrown away before reaching the printout, a bug that would recur forever once discovered, not just today. Fixed + given a regression test so it can't silently break again. Also fixed printed asset order (was click-order from setup, now alphabetical/numeric).*
...
- [x] **Field Run-Sheet now prints recorded breaker nameplate data** (brand, model, serial, poles, rating, trip unit, fixed/withdrawable) instead of always-blank cells, once a tech has entered it via the on-site 3-step workflow. Visual/functional tick-boxes and electrical readings are unchanged — those are deliberately printed blank for hand-completion on site (Royce's existing "print empty, complete on site" design), not part of this fix. eq-service **PR #541 MERGED + LIVE** (`8fa1ac4`), regression test added. _(done 2026-07-15)_
...
- [ ] **APPLY (needs Royce): migration 0185 to ehow.** Code is merged and live on core.eq.solutions, but the RPCs themselves aren't callable on production data yet — same posture as 0183 was until its One Pipe apply. Nothing breaks by waiting; the console just won't show Preview/Confirm merge until it's applied. _(added 2026-07-15)_
- [ ] **Seed one realistic flagged pair on ehow to actually try Preview/Confirm merge end-to-end.** Still 0 real flagged rows — see the item above from 2026-07-15, still open. _(added 2026-07-14)_
