...
- [x] **Investigated the actual mechanism — inconclusive, but the leading theory was disproven.** The invite-accept code's phone-matching logic did NOT cause Will's specific case — the permanent activity record shows his real invite was matched to his correct existing record, exactly as designed. The duplicate account's creation doesn't fit any known code path in eq-shell; pinning the real trigger needs system logs from 2026-07-06, which are no longer available (aged out, and the deeper diagnostic tool isn't authorised in this environment). _(done 2026-07-20)_
...
- [ ] **Still open: what actually created Will's duplicate account.** Not urgent (his data is already repaired) but worth understanding if it recurs. Next step needs either the deeper error-tracking tool authorised for this environment, or Netlify's own function-run history for 2026-07-06 if it goes back that far. _(added 2026-07-20)_
