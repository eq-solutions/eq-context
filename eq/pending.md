...
- [x] **Audit extended to eq-solves-intake (2026-08-04) — 2 of 3 findings fixed, 1 flagged not fixed (2026-08-05).** This repo (product name "EQ Intake," status **In build**, not live) uses **physical per-tenant Supabase projects**, not one shared DB behind RLS. **(1) Fixed:** `edge-functions/api-intake/index.ts` trusted a caller-supplied `body.tenant_id` with no real ownership check — any authenticated user whose JWT simply lacked a tenant claim (not just a genuine service-role caller) got it trusted outright. Now explicitly compares the raw bearer token against `SUPABASE_SERVICE_ROLE` before trusting it. **(2) Fixed:** `approve-worker-assignment/index.ts`'s `staff`/`licences` upserts weren't tenant-scoped at the app layer (the underlying DB constraint isn't either, and that's owned by eq-shell's migration lineage — out of scope here) — added explicit ownership checks before each upsert. **(3) Flagged, not fixed:** `scripts/migrate-cards-to-canonical.mjs` genuinely reads the whole Cards pool unscoped, but hasn't been touched since 2026-05-28 and its column names no longer match the live schema confirmed elsewhere this session — likely doesn't even run as-is. Inventing a tenant filter for a schema this stale risked a wrong fix reading as a real one, so added a loud docstring warning instead of guessing. eq-solves-intake [PR #109](https://github.com/eq-solutions/eq-solves-intake/pull/109) — pushed, not merged. **Not run** — no Deno runtime available; `migrate-cards-to-canonical.mjs` is plain JS, `node --check` passes. No CI in this repo covers `edge-functions/` or `scripts/`.
