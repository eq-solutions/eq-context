...
- [ ] **Cross-repo attribution — partial**: eq-solves-service's admin archive/restore/cascade-archive path now attributes to a real actor (eq-service [#652](https://github.com/eq-solutions/eq-service/pull/652), merged + deployed live). eq-field still open and now known-blocked, not just untouched — its data-plane JWT `sub` is always the tenant id, never a person (`verify-pin.js:625`, "sub is not security-bearing"), so there's no verified safe id to attribute with yet. eq-cards' `workers-canonical-sync` (human-edit path carries a real actor in its webhook payload; the nightly cron reconcile correctly has none and should stay `source='system'`) not touched — the edge function lives outside all three app repos, needs locating first. _(updated 2026-07-30)_
