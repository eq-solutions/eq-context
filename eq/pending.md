...
- [ ] **The "PR #973" cited in the two bullets directly above is the wrong PR number.** Checked GitHub directly while independently verifying a different PR #973 question (see the quotes-pipeline entry near the top of this file): eq-shell's actual PR #973 is "perf(quotes): bound the Ops pipeline fetch, add a real counts RPC" — nothing to do with database list-loading or the "Overdue follow-up" filter. Whatever PR these two bullets are really describing needs its correct number identified; not investigated further here, out of scope for the migration-status question that surfaced it. _(added 2026-07-26)_
