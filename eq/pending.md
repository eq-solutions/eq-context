...
- [ ] **`crm-write.ts`/`canonical-api.ts` still have no entitlement gate** — deliberately left alone in #960 too: they're shared between Ops and the ungated Customers/Staff pages, so gating them needs their own design pass (e.g. a request-level entity-type/module hint), not a blind check. Only live exposure today is the `eq` tenant (the one tenant with `ops` off). _(added 2026-07-22)_
