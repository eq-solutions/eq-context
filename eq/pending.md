...
## Suite-wide permission audit closed out across all 4 apps — 1 real live security hole found + fixed (eq-cards), everything else merged (2026-08-12 → 2026-08-13)
...
## eq-solves-service: 2 parallel-session PRs merged + a broad repo survey found 2 more real gaps, fixed (2026-08-13)
*Direct continuation of the permission-audit thread above. Once that thread closed out, asked what else was outstanding — found 2 unrelated PRs from a concurrent session waiting for review, then ran a broad read-only survey (existing audit-tooling output, skipped tests, TODO markers, docs-vs-code drift, `npm audit`, a spot-check of 14 more action files not touched by the thread above) rather than guessing at what to build next.*

- [x] **Reviewed and merged 2 PRs a concurrent session had built and left open**: eq-service [PR #720](https://github.com/eq-solutions/eq-service/pull/720) — archiving a maintenance/testing check flipped `is_active` but never stamped `deleted_at`, unlike the sibling customers/sites/assets triggers; fixed both call sites plus a backfill migration (`0207`) for the 4 affected live rows (not applied yet — Royce-only dispatch, same as migration `0206` above). eq-service [PR #721](https://github.com/eq-solutions/eq-service/pull/721) — new `/admin/quality` page surfacing eq-intake's quality-guardian output (live nightly since July 3, zero admin visibility until now); resolves the open question in the 2026-08-08 stale-branch cleanup section below.
- [x] **Broad survey came back mostly clean** — 434/434 tests passing, 0 npm audit vulnerabilities, 0 stale TODO/FIXME markers, docs matched code on every claim spot-checked, 14 more action files checked with no gaps beyond the two below.
- [x] **Found + fixed: `bulkUpdateAssetNamesAction` was skipping the mandatory audit-log step** every other mutation in the app follows — had the right role check and tenant-scoping, just never called `logAuditEvent()`. eq-service [PR #722](https://github.com/eq-solutions/eq-service/pull/722), merged.
- [x] **Found + fixed: contact CSV import let a supervisor bulk-import up to 1,000 contacts** via the broader `canWrite`, while every single-record contact action (create/edit/delete/merge, on both customer and site contacts) is `isAdmin`-only — the same inconsistent-auth-surface shape already documented and fixed once in this codebase for contract-scope import (issue #451), apparently missed for contacts at the time. Confirmed with Royce before narrowing; no UI change needed since the Import button was already `isAdmin`-gated, only the server action lagged. eq-service [PR #723](https://github.com/eq-solutions/eq-service/pull/723), merged.
- [x] **Cleaned up 5 stale worktrees + their local branches**, each individually confirmed merged via `gh pr list --head <branch> --state all` before removal, not assumed from directory age. 2 hit git's known squash-merge false-negative on `branch -d` and needed a verified `-D`.
- [ ] **Migration `0207`** (the `deleted_at` backfill, 4 live rows) needs Royce to dispatch `apply-service-migrations.yml` — same standing blocker as migration `0206`. _(added 2026-08-13)_
- [ ] **Live click-through not done** for #720/#721/#722/#723 — same sandbox limitation as the rest of this thread. _(added 2026-08-13)_
...
*Started from Royce reviewing his own SKS HR review templates as candidates for a new feature. `/decide` landed on Shell (not Field — Field owns operational stuff, Shell owns the canonical staff record). Full history, data model, and every decision behind this thread: `eq/shell-conversations-scoping-2026-08-11.md`. Remaining work tracked as a sprint: [`eq/sprints/2026-08-13-resourcing-closeout.md`](sprints/2026-08-13-resourcing-closeout.md).*
...
*Royce reported "the navigation on ACBtest doesn't allow you to open the check" with 3 real phone screenshots (pulled from Google Drive, not attached directly) and asked to work through how a check gets created — including photographing a switchboard schedule and picking which breakers are RCDs. Also asked to confirm the RCD trip-time thresholds against AS/NZS 3000 while at it. Scoped with `/decide` (a real tension surfaced: the active overseas-hardening goal excludes new feature work — Royce explicitly overrode it, staying engaged through the build) before any of the RCD entry work started.*

- [x] **RCD 5×-rated-current threshold was genuinely wrong, not just unverified.** Migration 0203's 100ms limit cited "AS/NZS 3000 Table J1" — no such table checks out against any source found; IEC/AS 61008 (the actual RCD product standard) and NZ's Electricity (Safety) Regulations 2010 both put it at 40ms. Zero live rows were affected (checked before fixing — nothing had been tested since 0203 went live). Fixed via migration 0206, [eq-service PR #701](https://github.com/eq-solutions/eq-service/pull/701), merged.
- [x] **ACB nav bug root-caused — not a routing bug.** The checks-list table (both ACB/NSX's own hand-rolled table and the shared `@eq-solutions/ui` Table component used suite-wide) has no mobile column-width handling, so a long auto-generated check name wraps one word per line on a phone, blowing the row out to 8+ lines — looked broken enough to read as "can't open it" even though the tap handler was fine. Fixed the ACB/NSX instance: desktop table now truncates with an ellipsis, new mobile card layout below. [eq-service PR #702](https://github.com/eq-solutions/eq-service/pull/702), merged.
- [x] **Built the RCD in-app entry workflow** the page's own empty state had been anticipating since before this session ("once the in-app entry workflow lands... queued") — previously RCD had zero way to start a check on-site, xlsx import was the only path. New `/testing/rcd/create`: pick a site + board, then build the circuit list three ways — type rows in one at a time, upload a photo of the switchboard schedule, or pick the board's circuit count from a dropdown to generate all the rows at once (Royce: "make it easier" than adding per row). One confirm screen either way, RCD checkbox per row (pre-ticked from the photo's own asterisk/keyword detection when that's the source, always editable — only ticked rows become real data). [eq-service PR #703](https://github.com/eq-solutions/eq-service/pull/703) (flow), [#704](https://github.com/eq-solutions/eq-service/pull/704) (44px touch-target fix, caught when Royce asked directly "is mobile view finished as well" and it wasn't quite), [#705](https://github.com/eq-solutions/eq-service/pull/705) (bulk-generate dropdown) — all merged.
- [x] **New eq-solves-intake edge function `parse-rcd-switchboard-schedule`** — reuses the same proven vision-extraction pattern already powering the Maximo PDF importer (same live project, same `ANTHROPIC_API_KEY`), rather than building a bespoke parser into eq-service (the repo's own standing architecture rule). Smoke-tested against a synthetic schedule image — asterisk + keyword RCD detection both correct. [eq-solves-intake PR #115](https://github.com/eq-solutions/eq-solves-intake/pull/115), merged.
...
- [ ] **Migration 0206 not yet applied to the live database** — merged to code, needs Royce to manually dispatch `apply-service-migrations.yml`; Claude Code's own permission classifier blocks doing that directly (same wall as the Netlify secret above). _(added 2026-08-11)_
...
- [ ] **1 item still needs Royce's call, not a code fix** — `claude/service-canonical-identity-phase3-4` (eq-service): re-keys shell-auth JWT + remaps 5 SKS users' FK refs, explicitly marked "DO NOT DEPLOY without Royce's go" in its own commit, never landed — still wanted or shelved? _(added 2026-08-08)_
...
- [x] **eq-context PR #62 investigated properly, 2026-08-13 — legitimately superseded, correctly closed.** PR #62 (opened 2026-07-04) was never merged or commented on, closed 2026-08-07 with no explanation. Root cause found: a *different* fix for the exact same problem — commit `f25847c` (2026-07-12, pushed directly to main, not via PR) — broadened the frontmatter-check's exemption list (`CHAT-PROMPT.md`, `*/README.md`, `*/changelog/*`) and fixed frontmatter on the genuinely-governed docs #62 also touched, 8 days after #62 was opened. #62 got closed because it was made redundant, not abandoned by mistake — no reconstruction needed.
  - **But the check is red again right now** — confirmed live: `f25847c` fixed the violations that existed in July, but new drift since then broke it again. Ran the check's exact logic locally against current `main`: exactly one violation, `eq/sprints/2026-08-12-field-mobile-centering.md` missing `last_updated` (had `created` instead, not a recognized substitute). Fixed.
