...
- [x] **Watch-task run + false alarm cleared (2026-07-12 later).** Daily sentinel found EQ-SHELL-R/S quiet (0 events/24h) — expected, the fallback alarms retired with cookie mode. Investigated a suspected env-var parity break (eq-shell `SUPABASE_JWT_SECRET` prod 20-char ≠ eq-field `ZAAP_JWT_SECRET`, while the SKS secret pair matches exactly) — looked like broken EQ token-mode keys, but `/eq/field` **re-smoked live and loads fully** → false alarm: `ZAAP_JWT_SECRET` is NOT the verifier for the EQ handoff. **DO NOT touch either secret chasing this.** Memory updated. _(done 2026-07-12)_
