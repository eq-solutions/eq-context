...
- [ ] **Small follow-up still open, not done:** the Sentry alert script (`scripts/create-sentry-alerts.mjs`, eq-shell) was edited 2026-07-21 to reference `mint-cards-otp` instead of the deleted `mint-cards-iframe-token` function, but never re-run against live Sentry — the live alert rule still watches the old, now-nonexistent function name until someone re-runs the script. _(added 2026-07-22)_
