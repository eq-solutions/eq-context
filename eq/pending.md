...
- [ ] **eq-cards PR #221 and its eq-shell companion #1294 are now stale and effectively orphaned.** Both open since 2026-08-10, proposing to rewrite `shell-verify.js` into a broker that relays to a new eq-shell endpoint instead of self-signing — motivated by removing eq-cards' own copies of `SUPABASE_JWT_SECRET`/`SUPABASE_SERVICE_ROLE_KEY` (jvkn) and `EQ_SESSION_SALT`. That motivation still stands, but the file it modifies no longer exists on `main` after tonight's retirement. Worth closing both rather than resurrecting the broker pattern — deleting the file achieves the same secret-reduction goal more directly: those three secrets are now unused in eq-cards' Netlify project (nothing left references them) and can likely just be deleted outright, no broker needed. Not closed or actioned — Royce's call, both are his PRs to close and his Netlify project to edit. _(added 2026-08-16)_
