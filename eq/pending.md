...
- [ ] **`QuotesModule.tsx` canonical-api job-sync calls are dead (401, silent no-op)** — found while doing the entitlement design pass above. The browser-side `fetch('/.netlify/functions/canonical-api', …)` calls (job lookup ~line 1088, `syncJobToCanonical` PUT ~line 2521) carry no `Authorization` header, but `canonical-api.ts` requires a per-app Bearer key — so both calls 401 every time and silently no-op (`if (!res.ok) return`). Currently harmless because the scheduled `quote-job-consumer.ts` (every 15 min) covers the real job sync, but the instant-feedback UI path has probably never worked. Needs a decision: drop the dead browser calls, or add a session-authenticated proxy function for instant feedback. Task chip spawned (`task_8f8351a3`). _(added 2026-07-23)_
