...
**Deferred:**
- [x] **Merge eq-shell PR #704** to fully close Phase 0. _(merged 2026-07-08, `82c97cb`)_
...
- [ ] **Access-model Phase 1** — convert Shell `role === '...'` enforcement checks to `can()`; add a lint ratchet. **Corrected 2026-07-08: real count is 5 sites (not ~10), and it is NOT Shell-only** — none had a matching canonical PermKey, so a package prerequisite was needed first. **eq-roles v2.5.1 shipped** (`ops.view_rates`, `ops.manage_rates`, `entity.manage_activation` — additive, zero live behaviour change, matches each hand-rolled check's current grant set exactly). Session brief for the eq-shell conversion itself was presented but **not yet confirmed by Royce** — branch `claude/access-model-phase-1-enforcement` exists with zero commits, awaiting go-ahead. _(added 2026-07-08, updated 2026-07-08)_
