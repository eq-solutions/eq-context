...
- [ ] **eq-solves-service's matching work (the same kind of gap for two CMMS actions) is sitting in an open, unmerged PR (#551)** — separate repo, not touched this session, migration not applied yet.
- [ ] **No repo-side record of the security_invoker fix yet** — it was applied directly to the live database (same as the grant-restore above), but doesn't have a matching file committed to a repo the way the grant-restore does (eq-field PR #498). Small, not urgent since the live database is already correct — worth a quick follow-up PR so the fix is on record. _(added 2026-07-19)_
