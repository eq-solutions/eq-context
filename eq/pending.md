...
*Three findings handed off from a 2026-08-16 permission audit. Session also hit the shared `C:\Projects\eq-cards` checkout moving from its starting branch to `main` under a concurrent session mid-task — second occurrence of this failure mode — recovered via a dedicated worktree, nothing lost (the original branch was already squash-merged). Full detail in `sessions/2026-08-16.md`.*

- [x] **`netlify/functions/shell-verify.js` retired.** Confirmed undeployed by hitting the live endpoint directly (returns Netlify's SPA fallback, not a function response) rather than inferring from config — `deploy.yml` only ever zip-uploads `build/web`, never `netlify/functions/`. Deleted the file and the now-empty `[functions]` stanza in `netlify.toml`. eq-cards [PR #253](https://github.com/eq-solutions/eq-cards/pull/253), merged + deployed live.
- [x] **`AdminWorkerDetailScreen`'s credential list fixed.** `worker_credentials` had an owner-only RLS policy and no org-admin read path — the screen read the table directly via PostgREST and silently got `[]` for every worker but the admin's own. New org-scoped RPC (`eq_cards_admin_list_worker_credentials`, migration 0129) matches the existing write-path predicate (`is_org_admin` + `is_worker_in_org`) rather than widening RLS directly. `worker_credentials` carries 0 live rows today, so this closed the gap before it had real data to expose. Same PR #253, applied live to jvkn before merge.
- [x] **`org_join_requirements`'s admin policy fixed.** Was gated on the raw `org_memberships.role='admin'` check, reachable by only 3 of 17 real org admins — predates `is_org_admin()`'s canonicalisation (0105) by about an hour, never updated after. Aligned (migration 0130). Same PR #253, applied live to jvkn before merge.
- [x] **eq-cards PR #221 and its eq-shell companion #1294 closed** — RESOLVED 2026-08-16: both had proposed rewriting `shell-verify.js` into a broker relaying to a new eq-shell endpoint, motivated by removing eq-cards' own copies of `SUPABASE_JWT_SECRET`/`SUPABASE_SERVICE_ROLE_KEY` (jvkn) and `EQ_SESSION_SALT`. Moot after tonight's retirement — the file they modify no longer exists on `main`. Closed on Royce's explicit instruction, each with a comment pointing back to #253. _(added 2026-08-16, resolved 2026-08-16)_
- [ ] **eq-cards' Netlify project likely still holds 3 now-unused secrets** — `SUPABASE_JWT_SECRET`, `SUPABASE_SERVICE_ROLE_KEY` (jvkn), `EQ_SESSION_SALT` were only ever read by the now-deleted `shell-verify.js`. Worth deleting them outright (manual-hands-only, Claude Code is blocked from writing Netlify secrets by design) — not verified whether anything else in the project still references them. _(added 2026-08-16)_
