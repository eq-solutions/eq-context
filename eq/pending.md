...
- [ ] **Quotes-vs-jobs Kanban split itself — still not built.** The board→canonical-job sync gap and its backfill are both done (see changelog + `sessions/2026-08-03.md`), which was the prerequisite work; the actual two-board split Royce proposed (Quotes: Draft/Open, Sent, Archived; Jobs: Won, Job Created, In Progress, Complete, Invoiced) hasn't been started. Full architecture doc: [eq/ops/EQ-OPS-ARCHITECTURE.md](../ops/EQ-OPS-ARCHITECTURE.md). _(added 2026-08-03)_
- [ ] **`app_data.jobs.quote_id` still has no FK constraint** — blocked on one known orphan row (`job_id 7842b5ce…`, `quote_id 4f5b887c…`) that points at a hard-deleted quote. Needs Royce's call: delete the orphan job row, or null out its `quote_id`, before the constraint can be added. _(added 2026-08-03)_
- [ ] **Second write path into `app_data.jobs` found this session, not previously documented**: a scheduled function (`quote-job-consumer.ts`, every 15 min) independently upserts jobs from a `quote.accepted` canonical event feed, with a 7-day lookback window. It's event-driven only, not a backlog sweep — this is why 30 quotes stuck at job-stage status needed an explicit backfill rather than self-healing on their own. Worth knowing before assuming any future gap will just catch up on its own. _(added 2026-08-03)_
