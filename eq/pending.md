...
- [ ] **Red "Schema drift" gate on eq-shell main, PR #608 IN FLIGHT — root cause was worse than first diagnosed.** Original read: `service.customer_contacts`/`service.site_contacts` on ehow just had RLS disabled. A follow-up audit (chip `task_0b6bb8ad`) found the real state: the 2026-07-02 eq-intake "steward run 001" had renamed both tables to `*_legacy_20260702` (data frozen, RLS-on, service_role-only — safe) and replaced them with `security_invoker` VIEWS over `app_data.contacts` + link tables, writes going through 6 SECURITY DEFINER `INSTEAD OF` triggers that assert JWT tenant. Verified live: tenant-safe. The FIRST version of PR #608's migration (0155) assumed tables and would have **errored 42809 live** (`ALTER TABLE ... ENABLE ROW LEVEL SECURITY` on a view) — caught before dispatch, not deployed. As of this session's close, a session is actively rewriting `0155` (relkind-aware) + a new `0156` (codifies the view/trigger DDL into the governed migration lineage) + `check-tenant-drift.mjs` + `regen-tenant-baseline.mjs` directly in the shared `C:\Projects\eq-shell` working tree — **uncommitted, in progress**, not yet pushed to #608. _(added 2026-07-03, needs your call — once that session finishes and pushes, review the full #608 diff carefully before dispatching `tenant-migrate.yml`; do not dispatch the version currently on the open PR, it predates the fix)_
