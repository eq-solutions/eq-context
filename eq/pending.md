...
- [x] **`public.eq_update_staff`'s `authenticated` EXECUTE grant revoked** (both zaap + ehow — the grant migration 0172 re-asserted, wrong role). eq-shell PR [#1364](https://github.com/eq-solutions/eq-shell/pull/1364), all CI green. Migration only — **not yet dispatched** to the tenant planes (One Pipe, `tenant-migrate.yml`, production-gated).
- [x] **Re-audited the 12 sibling keys carrying the same "enforced downstream" claim** — found 2 more real holes, same shape: `field.view_hours` (`entity-rows.ts`'s `timesheet` entity) and `field.view_licences` (`entity-rows.ts`'s `licence` entity + `staff-canonical-licences.ts`, the endpoint `StaffPage.tsx` actually uses) were both gated on plain `field.view` instead of their split-out, manager+supervisor-only key. Live read exposure — any of the 4 excluded roles could see every worker's pay-adjacent timesheet data and licence/compliance status tenant-wide. eq-shell PR [#1365](https://github.com/eq-solutions/eq-shell/pull/1365). The other 4 field keys + 5 service keys + 2 db-layer/interim keys checked and held up (verified against live code, not taken on faith).
- [x] **`staff-update.ts` retired, not repaired** — confirmed fully superseded by `entity-patch.ts`'s `staff` entity (same fields, same table, better auth model, already had the identity-lock safeguard this file lacked). Zero frontend callers. Royce's explicit call via AskUserQuestion. eq-shell PR [#1366](https://github.com/eq-solutions/eq-shell/pull/1366).

**Deferred:**
- [ ] **3 PRs open, need Royce's merge decision** — [#1364](https://github.com/eq-solutions/eq-shell/pull/1364) (GRANT revoke, CI green), [#1365](https://github.com/eq-solutions/eq-shell/pull/1365) (field.view_hours/licences gate fix), [#1366](https://github.com/eq-solutions/eq-shell/pull/1366) (retire staff-update.ts). All independent, can merge in any order. Merging any of them to eq-shell `main` auto-deploys to production in 2-4s. _(added 2026-08-15)_
- [ ] **#1364's migration needs a `tenant-migrate.yml` dispatch after merge** — One Pipe governance, production-gated, Royce's explicit go (same as every prod migration). _(added 2026-08-15)_
