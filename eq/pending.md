...
- [x] **`QuotesModule.tsx` canonical-api job-sync calls — FIXED, PR [#969](https://github.com/eq-solutions/eq-shell/pull/969) OPEN.** Was dead code: the browser-side `fetch('/.netlify/functions/canonical-api', …)` calls (job lookup, `syncJobToCanonical` PUT) carried no `Authorization` header against a Bearer-key-only function, so both 401'd every time and silently no-op'd — `canonicalJobId` never populated, so Issues/Attachments on a won quote stayed filed under the quote instead of moving to the job. Added a session-authenticated `netlify/functions/quote-job-sync.ts` proxy, gated on `isModuleEnabledForTenant('ops')` (this one genuinely is ops-exclusive), and repointed `QuotesModule.tsx` at it. **Also fixed a latent duplicate-row bug found along the way:** the dead browser PUT used a different `external_id` prefix (`eq-ops:job:`) than the scheduled `quote-job-consumer.ts` (`eq-quotes:job:`) — the new endpoint upserts by `(tenant_id, quote_id)` and matches the consumer's convention so the two writers can't fork. Build/tsc/190 tests clean; lint matches the pre-existing `_ctx unused` pattern shared by all 60 sibling ops functions. NOT merged — Royce's call. _(done 2026-07-23)_
