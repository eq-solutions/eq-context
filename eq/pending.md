...
- [x] **Two silent failures fixed.** Agreeing to the privacy consent step used to just quietly stop and reset the button if the save failed, with no explanation. Loading licence types could show a raw system error on screen instead of plain English. Both now tell the person something went wrong and log it properly. _(done 2026-07-20)_
- [x] **Avoided building something that already existed.** Was about to build a "you have credentials expiring soon" summary for the wallet screen — checked the actual code first and found it's already there, live, at the top of the wallet screen. Saved a wasted build. _(done 2026-07-20)_
- [x] **Corrected a wrong belief in the record — including the official decision doc, not just working notes.** The 15 June decision that Cards and Core should never share a sign-in screen said the Shell-to-Cards shortcut was "retired." Real usage data shows it's actually the majority path — over half of all sign-ins in the last 3 months go through it. Checked how that happened: real work kept landing on that exact shortcut for two weeks *after* it was declared dead (a proper working version shipped 24 June, with two more rounds of polish after) — this wasn't neglect, someone made an active call to keep and fix it, the decision doc just never caught up. Corrected both the official decision record (`ops/decisions.md`, `eq/cards/worker-platform-direction-2026-06-15.md`) and working notes to say so. _(done 2026-07-20)_
- [x] **That same sign-in shortcut had zero visibility into whether it actually works.** A failed handshake was falling back silently with nothing logged anywhere — invisible to error tracking. Added proper tracking so this is now measured directly. Real data suggests roughly a third of these attempts don't cleanly land the person in their wallet, though nothing is crashing — worth watching now that it's actually measured instead of guessed at. _(done 2026-07-20)_
- [x] **Found and fixed two "looks like it works but doesn't" controls.** A "show my QR code to sign in on-site" button that nothing anywhere reads yet (checked EQ Field — no scanner exists). And a "biometric unlock" toggle in Settings that's done nothing for any real user since it was built, because EQ Cards only ships as a website — there's no phone-app-store version for it to matter on. Fixed both: the QR button is now honest about not being available yet (and tracks real taps, so future demand is measured, not guessed), and the biometric toggle is hidden on web instead of pretending to work. _(done 2026-07-20)_
- [x] **All four fixes shipped as one pull request, tests passed, merged live.** _(done 2026-07-20, PR #158 merged)_
...
- [x] **One more speed pass ("anything else while the bonnet's open?"), shipped and confirmed live.** Checked whether the error-tracking trim above could go further by stripping unused library files out of what gets deployed — confirmed that's not safely possible without risking error-tracking breaking outright, so left it alone rather than force it. Found two real wins instead: the main dashboard was waiting on one extra setting lookup before it started fetching anything else; and the Maintenance Checks page (used by every technician and supervisor, multiple times a day) was fetching 7 separate pieces of information one at a time instead of all together, plus quietly fetching one piece of data that was never even used. Fixed both, shipped, and confirmed live after deploy (dashboard and maintenance pages both load cleanly, no errors). _(done 2026-07-20)_
...
- [x] **PR #466 (Dependabot config) merged** — clean, CI green, no risk. Immediately opened 4 more version-bump PRs (#501-504), all checked and merged same session. _(done 2026-07-20)_
- [x] **PR #461 (close the `eq` tenant's demo login gate) — re-scoped, not just merged.** Its original fix (from 2026-07-12) hardcoded a check that main had since replaced with a general "Core-only" switch — merging as-is would have added a redundant, conflicting check. Rewrote it to close the actual remaining gap (a supervisor-password backdoor the general switch hadn't covered yet), merged, and flipped the one database setting needed to actually turn the gate on for `eq`. Live-verified: the `eq` tenant now requires signing in through Core, same as SKS. _(done 2026-07-20)_
- [x] **Full strategic audit run** (CEO / UI / engineering-quality lenses — 4th in a recurring series, last one 2026-06-14). Strongest security cycle of the four; real automated tests for the first time; a genuine mobile-usability push landed. One recurring problem got worse for the third review running: a handful of files keep growing past a healthy size with no rule stopping it. Also found the Melbourne enterprise-customer plan appears to have quietly gone quiet — worth Royce confirming whether that's still a target. Found and closed 4 already-stale to-do items along the way (things that were already fixed months ago but still listed as open). _(done 2026-07-20)_
- [x] **This week's sprint, executed same session (Mon-Thu):** adopted a rule capping file growth; fixed a leftover cosmetic bug the audit's own live-check found (login screen still said "for SKS" after the `eq` tenant became Core-only too); reviewed and documented the security model of the email-based leave-approval links (no gap found, one design trade-off flagged for Royce's call); wrote the first-ever automated tests for the app's single most critical file (every save in the app routes through it); then split that file from 2278 lines down to ~1000, verified safe via those new tests plus a live browser check, and fixed a real bug the tests turned up along the way (a failed save was supposed to give up retrying after 5 tries and never did). _(done 2026-07-21)_
...
- [x] **Built and live-validated the database-level checks for all three permissions** (with every test wrapped so nothing could accidentally stick before review) — roster edits, team management, and the licence/labour-hire-company fields. Also closed a real side-gap: the app's session token never carried the "grant this one permission to this one person" override, only their general role — added it. eq-field **PR #497 MERGED**, migration applied to the live database, Royce's go. _(done 2026-07-19)_
- [x] **Found and fixed an unrelated, already-live database gap while validating** — six tables the Roster/Timesheets/Leave/Safety screens are meant to write through had silently lost their database permission at some point. Confirmed it wasn't visibly breaking anything today (the app has been quietly routing around the broken tables via a newer path since an earlier fix), closed it anyway since it was a live landmine waiting to reappear. eq-field **PR #498 MERGED**, fix applied live. _(done 2026-07-19)_
- [x] **Found a second gap after the first merge, while cleaning up** — the labour-hire-company protection only covered one of two database columns holding that value; the second was still wide open to anyone with a session, any role. A leftover note claiming this was already fixed turned out to be wrong when checked against the live database — corrected it, fixed it for real, applied it live. eq-field **PR #499 MERGED**. _(done 2026-07-19)_
...
- [x] **Checked real numbers before building anything** — the busiest pages were taking 4–10 seconds to load for real users, over a third of usage is on mobile, and there was no way to measure "does a page feel fast" at all (the tool for it was switched off). _(done 2026-07-16)_
- [x] **Turned on real page-speed measurement** — was completely missing before; now it's tracking how fast pages actually feel for real users. _(done, live)_
- [x] **Added a "you'll lose this if you leave" warning on the quote form** — it now warns before closing the tab or navigating away with unsaved changes, on both new quotes and edits (previously only new quotes silently saved a draft; edits had no protection at all). _(done, live)_
- [x] **Made opening a quote faster** — cut the number of separate database trips roughly in half. _(done, live)_
- [x] **Split the app's main download into smaller, cacheable pieces** — normal updates no longer force everyone to fully re-download the app from scratch. _(done, live)_
- [x] **Stopped an analytics library from delaying the very first page load** — it now loads quietly in the background after sign-in instead of holding up the page. _(done, live)_
- [x] **Fixed the home page fetching the same two things twice** (once for the sidebar, once for the page itself) — now fetches once and shares it. Side benefit: also fixed a bug where switching between company accounts could briefly show the previous company's numbers. _(done, live)_
...
- [x] **eq-solves-service: merged the 3 stale Dependabot version bumps** (#456 tailwindcss, #458 tailwind-merge, #459 @vitejs/plugin-react) — 10 days old, real gates (typecheck, `tsc + next build`) all green, only the known pre-existing flaky integration test was red. _(done 2026-07-16)_
...
- [x] **eq-intake: ported the ~210-line MergePanel UI wiring** (canMergeSites prop threading, Preview/Confirm buttons calling the already-exported previewSiteMerge/executeSiteMerge) into IntakeHealthHome.tsx + IntakeModule.tsx. eq-solves-intake **PR #72 MERGED** (squash `937d9dd`). Full workspace build + 161 tests green. _(done 2026-07-16)_
- [x] **eq-shell: re-vendored eq-intake/eq-platform to bring #72 in**, and in the process caught two real eq-shell-only fixes that a blind re-vendor would have silently deleted: the two-entry tsup build + `./calibration-cert` package export (PR #506, fixes a 502 from bundling `@eq/schemas` into a Netlify background function — still live-consumed by the cert-import feature) and `@fontsource/plus-jakarta-sans` as a phantom workspace-root dependency that only resolves in eq-intake's own repo. Both restored before committing. eq-shell **PR #879 MERGED** (squash, rebased onto `37b805a` before merge) — Royce's explicit "deploy it to core.eq.solutions" + "merge PR #879" now live via Netlify auto-deploy. _(done 2026-07-16)_
- [x] **eq-intake: backported the two eq-shell-only fixes into source** (tsup calibration-cert split + `@fontsource` direct dependency) so the next re-vendor doesn't have to rediscover and manually restore them. eq-solves-intake **PR #74 MERGED** (squash `9906530`). `pnpm -r build` + `pnpm -r test` green, 219 tests. _(done 2026-07-16)_
...
- [x] **Follow-up (task_8d6e64f1) SHIPPED by a different concurrent session — eq-shell PR #878 MERGED (squash `37b805a`) → core.eq.solutions auto-deploying.** `fingerprint()`'s per-tenant Management API call in `check-tenant-drift.mjs` is caught individually now instead of throwing uncaught out of the concurrency fan-out; an unreachable tenant is skipped + reported informationally (new `--strict-unreachable-tenant` flag to opt into failing on it). muster review (single lens, codex-rescue unavailable) caught two real gaps before merge — a blanket catch that would've also swallowed a genuine SQL/schema bug on a *live* tenant as "unreachable" (fixed: only known dead-project error signatures get the informational treatment; anything else still fails loud), and an explicit `--reference=<dead-slug>` hard-failing instead of degrading like the default path (fixed: falls back to the first reachable tenant). Deploy verified live against the merge commit (Netlify `ready`, commit_ref matches). **If task_8d6e64f1's own session is still running, it's now redundant — the same fix already landed; don't duplicate.** _(done 2026-07-16)_
...
- [x] **Investigated the 0/5-customers gap — turned out to be dead code, not a real gap.** `canonical_id`/`canonical_synced_at` (and the `customer_local`/`site_local` sidecar tables feeding them) are leftovers from an old push-based sync design, retired when EQ Shell became sole owner of customers/sites/assets — confirmed live, both sidecar tables are 100% empty across every tenant, `canonical_outbox` has zero customer rows ever. Worse, the "Sites: 9/9 (100%)" number was actively wrong, not just idle — a `COALESCE(sl.canonical_id, s.site_id)` fallback meant it could never show anything but 100% regardless of real sync state. Royce's call (via AskUserQuestion): **remove the card and route entirely**, not replace it with a status note. eq-service **PR #545 MERGED + LIVE** (`ad1a03c`). _(done 2026-07-16)_
...
- [x] **eq-solves-service `/terms` page taken down — was an unreviewed legal draft.** The page carried its own internal comment admitting it hadn't been lawyer-reviewed ("review with Webb Financial / SaaS-literate lawyer before relying on in a commercial dispute") and read in a noticeably harder B2B/IP-protective tone than everything else in the suite (ML-training ban, "as is" liability cap, confidentiality clause). Now returns 404 pending real legal review; prior content preserved in git history. eq-service **PR #543 MERGED** (`b587d84`) → service.eq.solutions deployed + verified live (commit ref matches, secret scan clean). Side-finding: `/terms` was never actually in the app's public-paths allowlist, so anonymous visitors were already being redirected to sign-in before reaching it — exposure was smaller than it first looked, but the takedown still matters for anyone signed in. _(done 2026-07-16)_
...
- [x] **DB: closed the one gap that would have blocked permanent-delete on reviewed tenders.** Applied a DELETE policy on `tender_review_decisions` in the SKS-live database, identical in shape to the policies already on the sibling tender-family tables — applied with your explicit separate go-ahead (asked before touching prod RLS). _(done 2026-07-15)_
- [x] **"Load sample data" — Pipeline board, Resource Allocation, Account Explorer.** Fills all three with 12 fake tenders (fabricated, obviously-sample names) so the feature can be demoed to the EQ team. Nothing is fetched or written while it's on — reads are swapped for fake data locally, writes are blocked at the network layer as a backstop across every save point in that part of the app (20+), not just the obvious ones. Persistent banner + one-click exit. eq-field **PR #495 MERGED + LIVE** (v3.5.331). _(done 2026-07-15)_
...
- [x] **NSX/ACB check lists + Test Equipment cert history — fixed for the Shell iframe.** Routed the three affected reads through the server (same pattern as the screens that already worked). eq-service **PR #540 MERGED + LIVE** (`2ed22ef`). _(done 2026-07-15)_
- [x] **Field Run-Sheet now prints recorded breaker nameplate data** (brand, model, serial, poles, rating, trip unit, fixed/withdrawable) instead of always-blank cells, once a tech has entered it via the on-site 3-step workflow. eq-service **PR #541 MERGED + LIVE** (`8fa1ac4`), regression test added. _(done 2026-07-15)_
- [x] **Field Run-Sheet asset order fixed** — breakers now print in name order (NSX 01, 02, 03…) instead of the order they were clicked when the check was created. Same PR #541. _(done 2026-07-15)_
- [x] **Field Run-Sheet now shows recorded tick-box results, comments, and electrical readings too** — previously believed deliberate, Royce confirmed he wants it. A completed step's OK/Fail/N/A now shows as a checked box, any comment prints, insulation-resistance/contact-resistance readings show their real values, and the actual Pass/Fail/Defect result is highlighted (was always a blank hand-tick form regardless of what was recorded). Temperature stays blank — neither workflow has ever recorded it, nothing to show. eq-service **PR #542 MERGED + LIVE** (`7ff3dd6`), regression test added. _(done 2026-07-15)_
...
- [x] **Forecast page's "Project" column header — invisible white-on-white text — fixed.** The header set a light background without resetting the (inherited white) text colour, so the "Project" label rendered invisible at every viewport width. Verified the exact broken rendering side-by-side with the fix before shipping. eq-field **PR #492 MERGED + LIVE** on field.eq.solutions. _(done 2026-07-15)_
- [x] **Leave calendar weekday header — low-contrast text on navy — fixed.** Same opposite-direction contrast problem as below: the header left its own background transparent, so the unstyled navy header background showed through under dark grey text. Matched it to `calendar.js`'s already-correct identical header (background + text colour paired). eq-field **PR #493 MERGED + LIVE** on field.eq.solutions. _(done 2026-07-15)_
...
- [x] **Retired the EQ Service Report-Settings logo pickers (both light + dark) — one source of truth = Shell.** eq-service #539 (`28f68ea`, MERGED + deploying): removed both MediaPickers + dropped `report_logo_url`/`report_logo_url_on_dark` from the update action's schema + DB write (a Report-Settings save had been clobbering the Shell-synced value). Card → a note pointing to Shell → Settings → Document logo. Reports still READ the columns; only the setter moved. _(done 2026-07-14)_
...
- [x] **Verdict capture (migration 0183) — SHIPPED + APPLIED + LIVE-VERIFIED.** Append-only `app_data.site_resolution_verdict` (RLS-on, tenant_isolation) + `public.eq_site_advisory_adjudicate(advisory_id,verdict,note)` write RPC (authenticated-only, refuses advisory rows outside the caller's tenant) + `eq_site_advisory_summary` extended (joins each row's latest verdict, adds pending/decided). Validated whole-migration in BEGIN…ROLLBACK on ehow (4/4 probes: verdict recorded, cross-tenant + bad-verdict rejected, summary joins). eq-shell **PR #848 MERGED** (`654cf6f`); One Pipe apply (Royce-approved) → **1 applied / 188 skipped on eq + sks + favour-perfect**; live-verified on ehow (table RLS-on + 1 policy, both RPCs authenticated-only/anon-revoked). _(done 2026-07-14)_
- [x] **@eq/intake writer + adjudicable console.** `adjudicateSiteAdvisory()` + verdict types + 9 unit tests; `SiteAdvisoryPanel` renders Same/Different/Unsure per flagged row, optimistic pending→decided, soft-fails inline if a tenant isn't on 0183. `check:packages` + all tests green. eq-intake **PR #68 MERGED**, vendored into eq-shell #848 → core.eq.solutions. _(done 2026-07-14)_
- [x] **Claude as the adjudicator — SHIPPED + LIVE-VERIFIED (2026-07-15).** New `adjudicate_duplicate` action on the `eq-ai-assist` Edge Function: Claude reads both flagged site records (abbreviations, address equivalence, the "same address different tenancy" trap) and returns a verdict + confidence + one-sentence plain-English reason, matching the human's Same/Different/Unsure vocabulary. Console gained a "✨ Ask Claude" button on pending rows that pre-highlights the suggested verdict — advisory only, never auto-merges, human still taps to record. `eq-ai-assist` deployed to v4 (Royce's explicit go). eq-intake **PR #69 MERGED**; vendored into eq-shell **PR #867 MERGED** (`2658860`, Royce's explicit "merge without review, go" — CI was fully green, gate wanted an explicit ack since it skipped a review). Blocked most of the session on `ANTHROPIC_API_KEY` not actually being set on ehow (see below) — once Royce set it, live-tested 3/3 correct: SVHN vs St Vincents Health Network → same (acronym); Level 2 vs Loading Dock same address → different (tenancy trap); SY6 vs SY7 → different (distinct DCs). _(done 2026-07-15)_
- [x] **`ANTHROPIC_API_KEY` confirmed live on ehow (sks-canonical) — 2026-07-15.** Was flagged "still not set" as recently as 2026-07-14 by live curl test. Royce set it (dashboard or CLI, no redeploy needed — Edge Functions read `Deno.env` at request time); re-tested live, confirmed working. This unblocks all three AI actions that were silently dead since PR #41 (2026-06-24): `suggest_gaps`, `ask_canonical`, and now `adjudicate_duplicate`. _(done 2026-07-15)_
...
- [x] **Closed the last drift thread — the calibration backfill (#534).** Same story again: the update that fills Test Equipment's calibration dates had been applied straight to the live system (record-book never told), and its file number clashed with the renumber above. Renamed the file clear of the clash (→ 0186), recorded it as applied in the governed record-book, and merged the PR. The pipeline now reads exactly clean — every database-update file has a matching applied-record and vice-versa (checked both directions on the live record-book). No live data changed; the calibration data was already in place. _(done 2026-07-14)_
...
- [x] **Test Equipment register reads canonical plant & equipment — eq-service #525 MERGED (migration 0180).** `service.instruments` rebuilt as a read-through view over `app_data.assets WHERE asset_type='plant_equipment'` (16 SKS meters: Fluke/Megger/Metrel/NORBAR). "Instrument Register" → "Test Equipment"; Add/Import/bulk removed; edit = calibration fields only; identity read-only from canonical + "View certificate" link. ACB/NSX `instrument_id` FK repointed to `app_data.assets`. One version, no copy/sync (reverses the 0158 "Sync from Shell" copy that 0164 killed). _(done 2026-07-14)_
- [x] **Calibration is canonical — eq-service #531 MERGED (migration 0181).** cal date/due/cert/status moved OUT of the Service sidecar into `app_data.asset_calibration` (current state, 1/asset) + `app_data.asset_calibration_events` (append-only cert history / NATA trail). Read by Field/Shell/Service; the Service write-through trigger upgraded to SECURITY DEFINER + `assert_jwt_tenant`. 18 SKS items live-verified (edit writes canonical row + appends a history event). _(done 2026-07-14)_
- [x] **Calibration cert-chain surfaced in the UI — eq-service #532 MERGED + migration 0182 APPLIED to ehow.** Test Equipment detail panel now shows a "Calibration history" section reading `service.instrument_calibration_events` — a READ-ONLY `security_invoker` view over the canonical events table. No write path (INSERT/UPDATE/DELETE revoked from `authenticated`). One source of truth held — a window, not a copy. tsc+build green, 0 ERROR advisors, view verified read-only live. _(done 2026-07-14)_
- [x] **Field + Shell now surface the canonical calibration — FULL CONSOLIDATION done + LIVE (Royce's "full consolidation" call, 2026-07-14).** The register read canonical but the real data still lived on the `app_data.assets` columns (last/next-service + cert) that eq-shell's equipment module writes — so `asset_calibration` was empty and Service showed BLANK dates. Made canonical the one source across all three apps:
  - **eq-service #534 — MERGED (`42c9ae4`, Royce's go) + live-verified.** Backfilled `asset_calibration` from the assets columns → 18 rows + 18 cert-history events; Service's Test Equipment now shows the real dates (was 0/18). Embed the Shell/Field reads depend on confirmed unambiguous (one FK, none reverse). Its migration file was renamed clear of the `0183` clash (→ `0186`) and the out-of-band apply recorded in the governed ledger — see the migration-hygiene section. _(done 2026-07-14)_
...
- [x] **Phase 4 — DONE: nulled the 18 stale plant_equipment values on the assets calibration columns (2026-07-14).** Cleared `last_service_date/next_service_due/cert_url` on the 18 plant_equipment rows via a scoped data UPDATE on ehow (`asset_type='plant_equipment'` only — the maintainable-asset PPM columns for the other 2830 assets are untouched, verified 0 affected). Data-only (execute_sql, not a migration → no ledger/orphan). Reversible: the values live in `app_data.asset_calibration`. Verified after: 0 stale PE rows remain; Service's Test Equipment still shows all 18 dates + certs (now purely from canonical via `COALESCE(cal.*, a.*)`). **This closes the calibration single-source consolidation — canonical is the sole store, no mirror.** Columns kept (shared with the PPM engine — never drop). _(done 2026-07-14)_
...
- [x] **"Balloon years" feature — BUILT, SHIPPED, DEPLOYED + live-verified. eq-service PR #526 MERGED (`a588776`) → service.eq.solutions.** Royce chose per-asset grain, fold the big-year spend into the existing yearly funding figure, and ship the whole loop in one go. You can now open **Major years** on the Assets list (filtered to one site + one plan that has a low-frequency job) and nominate which units get their bigger 2/3/5/8/10-yearly service in which year; the money and the due-list then land in the right year automatically, with no change to how funding is read elsewhere. Includes a one-click **Pre-fill from contract** (seeds the split the contract already implies so you confirm/adjust rather than start blank) and a guard that blocks recompute while nothing is placed so it can't accidentally zero the funding. The database update is live; verified on real SY3/E1.12 data (154 panels). _(done 2026-07-14)_
- [x] **Per-asset maintenance frequency chips added to the Assets list + detail (same PR #526).** Each asset now shows its relevant service cadences as little chips (Monthly / Quarterly / 6-monthly / Annual / 2–10-yearly + dark-site) so you can see at a glance what checks apply to each unit — on the grouped list, a Frequency column in the table view, and the asset detail page. Live-verified (E1.12 shows Annual + 5-yearly). _(done 2026-07-14)_
...
- [x] **Core-side merge/retire action — SHIPPED (0179→0183→0185, done 2026-07-15) — see that day's entry.** The ~30-table dependent-record sweep landed as a 26-table live FK sweep; the "collapse these dupes into one" button is the Health-tab Preview→Confirm flow. _(superseded — see below)_
- [x] **Merge from the Sites Dupes tab (migration 0186) — SHIPPED + MERGED — 2026-07-16.** The Health-tab merge flow (0185) only ever surfaced pairs the write-time resolver caught live — the generic "Sites → Dupes" tab (name-match, read-only) found 43 real duplicate candidates on ehow with no way to act on any of them. New `eq_site_advisory_flag_pair(survivor, loser)` RPC — manager-gated same as merge execute, computes real similarity signals, idempotent, deliberately does NOT set a verdict (the existing Same/Different/Unsure → Preview → Confirm flow still gates the actual merge). Console: "Flag for merge" button on exact 2-row dupe groups (survivor auto-picked: active + has-customer wins); 3+ row groups (the SY9 shape) explicitly left to manual review, not auto-guessed. eq-solves-intake **PR #71 MERGED**; eq-shell **PR #876 MERGED** (`3c4831e`, Royce's "merge both, no review" go) → core.eq.solutions auto-deploying. Every real gate green (typecheck/test/lint, migration ledger hygiene, Plan, Netlify preview) — the one red check (tenant drift) was already broken on `main` itself since the evening before, unrelated (chip task_3c4ee4fa). **Migration 0186 APPLIED to ehow — 2026-07-16, One Pipe run [29487888132](https://github.com/eq-solutions/eq-shell/actions/runs/29487888132), Royce's production-environment approval.** `eq_site_advisory_flag_pair` confirmed live on ehow by direct query. Same run applied cleanly across the whole fleet (`eq`, `sks`, `favour-perfect`) — favour-perfect no longer blocked (drift-gate fix from the concurrent session cleared it). "Flag for merge" on the Sites Dupes tab is now fully callable on production data. Even so, this is a one-pair-at-a-time flow (~3 taps per pair), not a bulk "close all 43" action — Royce chose ship-now-one-by-one over building a bulk-approve screen first. _(done 2026-07-16)_
...
- [x] **Usage-based survivor pick — SHIPPED (migration 0187) — 2026-07-16.** SY9's manual review (above) surfaced a real gap: the survivor auto-pick used a shallow active+customer heuristic that would have gotten SY9 backwards (the "expected wrong" active row actually held all 499 assets, 10 contract scopes, 2 quotes — its twin was empty). Royce asked "can we do this in the app." New `eq_site_dupe_usage(site_ids[])` RPC — read-only, tenant-scoped, counts rows in the volume-heavy tables (assets/quote/contract_scopes/jobs/maintenance_checks), a fast subset of the full 0185 merge-preview sweep. Console: usage is now the primary survivor signal (legacy heuristic is a fallback only for 2-row groups tied at zero); **unlocks 3+-row dupe groups** that were locked out of "Flag for merge" entirely — if exactly one row in a group of any size has real usage, that's the survivor, every other row is a loser (one idempotent flag call each). New "Records" column shows the evidence, not just uses it silently. Validated live via BEGIN…ROLLBACK on ehow. eq-solves-intake **PR #73 MERGED**; eq-shell **PR #880 MERGED** (`71fac54`, Royce's "merge both, no review" go) → core.eq.solutions auto-deploying. **Migration 0187 APPLIED to ehow** — Royce's explicit go, One Pipe run, production-environment approval, `eq_site_dupe_usage` confirmed live by direct query. Usage-first survivor pick is fully live in production. _(done 2026-07-16)_
...
- [x] **Dependabot PR #466 merged** — auto-patches eq-field CI action versions. Immediately opened 4 more version-bump PRs (#501-504), all merged same session. _(done 2026-07-20)_
...
- [x] **Docs + code comments truthed to "Resend is live" — eq-shell PR #850 (admin-squash `ff2f3d0`) MERGED + deployed to core.eq.solutions.** The 3 SKS runbooks + the HTML manual + README env table + inline comments in `invite-user.ts` / `AdminInviteUser.tsx` / `AdminBulkInvite.tsx` all still said email was off / not-wired / paste-the-link-manually; now state Resend sends automatically, with copy-link as the delivery-failure fallback. Verified live before editing (`EQ_EMAIL_PROVIDER=resend`, `EMAIL_FROM` + `RESEND_API_KEY` set). Left the accurate mechanism docs (`_shared/email.ts`, `.env.example`, SMS) alone. Admin-bypass was needed on a red drift gate that was an unrelated calibration-view false positive (since fixed by #849, issue #851 closed). Was chip `task_9323b389`. _(done 2026-07-14)_
...
- [x] **Service-side SameSite gap — CLOSED.** SERVICE-9 `cookie_absent` is now dead code: the `cookie_absent` reason was removed from the auth-handoff canary during the 07-07/08 CHIPS hardening (the current reason set is secret_mismatch|expired|malformed|no_email|slug_unresolved|cookie_unusable, and the replacement only fires when a cookie IS present but fails verify). Last event 07-08 matches that deploy. Confirmed no `cookie_absent` path anywhere in the repo → can't regress → **resolved in Sentry** with the evidence. No code change needed. _(done 2026-07-14)_
...
- [x] **~~Three dead RLS-bypassing twins~~ — premise resolved, opposite direction taken.** At the time this was written (2026-07-10) `field_prestarts`/`field_site_diaries`/`field_toolbox_talks` had `security_invoker` NOT SET, so the "don't grant `authenticated`, cross-tenant leak" warning was correct then. Re-verified live 2026-07-19: all three now carry `security_invoker=on` (set by an untracked-here migration between 07-10 and 07-19) with tenant-scoped RLS confirmed on their `public.*` base tables (`relrowsecurity=true` + `tenant_isolation` policies keyed on JWT `app_metadata.tenant_id`). Given that, eq-field PR #498 (2026-07-19) went the other way from what was proposed here — **granted** `authenticated`/`anon` rather than dropping — to close the matching grant gap on `field_schedule`/`field_timesheets`/`field_leave_requests` (which shared the same "lost their grant" root cause). Verified safe before granting, not just assumed. _(resolved 2026-07-19, see PR #498 above)_
...
- [x] **eq-field client-side gating — PR #496, MERGED, live on field.eq.solutions.** `@eq-solutions/roles` v2.5.3 shipped `field.manage_roster` / `field.manage_licences` / `field.manage_labour_hire`, mirrored into Shell's Access Control page by an earlier session but not enforced anywhere. Verified against live code first: licence-management and labour-hire-company aren't separate eq-field features (no such screens/entities exist) — both are two text fields inside the shared Person modal. Scoped down accordingly: full permission gate on the real roster/teams write surface (`roster.js`/`teams.js`, also closed a real pre-existing gap where 3 team-write functions had no guard of their own, only their modal-opener did) + field-level disable on the two Person-modal inputs. Caught and fixed a real cross-chunk bug before shipping — the standalone `teams` lazy-loader page loads `teams.js` without `roster.js`, so a first draft that put the permission helper in `roster.js` threw `ReferenceError` there; centralized in the eager-loaded `permissions.js` instead. _(done 2026-07-19)_
- [x] **eq-service sibling — PR #551, MERGED, live.** A concurrent session had already built this (`service.reopen` on work-order reopen, `service.record_tests` on instrument calibration) before this session reached it — reviewed rather than duplicated. Confirmed no scope creep (only the 2 exact write paths touched, out of 23 exported actions in the same file), a real bug fixed (reopening a completed check no longer lets an assigned technician bypass the role check), and genuine server-side enforcement since Service's writes run through Next.js Server Actions with a JWT-verified role — architecturally ahead of eq-field, which has no server write path at all for most actions. _(done 2026-07-19)_
- [x] **eq-field server-side enforcement — PR #497, MERGED, live.** RLS policy hardening on `schedule_entries` (roster) + permission checks inside 3 `SECURITY DEFINER` trigger functions (`field_teams_iud`/`field_team_members_iud`/`field_people_iud`, since SECURITY DEFINER bypasses base-table RLS entirely). Applied with explicit go, independently re-validated by a concurrent session which caught that `postgres` bypasses RLS (`rolbypassrls=true`) — a naive test would have false-passed; fixed by testing under `SET LOCAL ROLE authenticated`. _(done 2026-07-19)_
...
- [x] **eq-field hire_company gap — PR #499, MERGED, live, verified with a real write test.** Found reviewing #497 after it merged: `field_people_iud`'s `field.manage_labour_hire` guard only reverted `NEW.agency` on an unauthorized edit, never `NEW.hire_company` — `app_data.staff` has both as separate columns, and #496's client treats them as one unit. Any authenticated Field session, any role, could set an arbitrary `hire_company` via a direct API call, bypassing the gate. Fixed with the same revert pattern already used for `agency`. Applied live and verified for real (not just reading the function definition) — ran a forced-rollback test as a simulated employee session attempting to tamper both `hire_company` and `agency` on a real row (both correctly reverted) and as a supervisor making a legitimate change (went through as expected); nothing persisted from the test itself. _(done 2026-07-19)_
...
- [x] **Cluster 1 — 7 sensitive-read permission keys, full stack, MERGED + LIVE.** `field.view_hours`/`field.view_licences`/`field.view_rates`, `service.view_commercials`, `ops.view_margins`, `entity.view_pii`, `reports.view_financial` — split off the coarse `*.view` keys so opening an app no longer implies seeing pay/margins/PII/financials inside it. Hard cutover to manager+supervisor (Royce's explicit call, `reports.view_financial` manager-only). Shipped **`@eq-solutions/roles` v2.5.2** (PR eq-roles#12, merged+tagged) first — package is the source of truth, `check-perm-sync.mjs` blocks any eq-shell edit that isn't mirrored from it. eq-shell **PR #885**: mirrored the keys, surfaced all 7 on the Access Control page (added a Records column that didn't exist before), server-side redaction (`entity-rows.ts`/`crm-customers.ts`/`staff-org-roster.ts` null PII fields; `gm-reports.ts`/`gm-chat.ts` gate on `reports.view_financial`), UI gating (`StaffPage` PII columns, `gm-reports` financials view, `QuotesModule` margin figures across 5 surfaces). _(done 2026-07-16/17)_
- [x] **Found and fixed a real drift bug while building Access Control surfacing** — `tenant-role-perms.ts`'s overridable-key allowlist was a hand-maintained list, not derived from the package; every new key shipped from here on would have silently 400'd its own toggle until someone remembered to add it by hand. Now derives from `@eq-solutions/roles` directly. _(done 2026-07-16)_
- [x] **Cluster 3 — 6 write-split keys, eq-shell half MERGED + LIVE; originated the cross-repo follow-on program.** Trimmed from 11 candidates to 6 with a distinct owner or irreversible verb each: `service.reopen`, `service.record_tests`, `field.manage_roster`, `field.manage_licences`, `field.manage_labour_hire`, `ops.create_job`. Shipped **`@eq-solutions/roles` v2.5.3** (PR eq-roles#13). `ops.create_job` is the one key eq-shell can enforce end-to-end (EQ Ops is Shell's own code) — gated in `QuotesModule`. Discovered mid-build that the other 5 keys' real enforcement lives in eq-field/eq-solves-service (separate repos, separate write paths) — spawned the 3 background tasks that became PRs #496 (eq-field client gating), #497 (eq-field server enforcement — see below), and #551 (eq-solves-service), which the day's other three sessions carried to completion; see those entries for detail. _(done 2026-07-17, follow-on tasks 2026-07-19)_
- [x] **Migration 0189 built + applied — `eq_set_job_number` had NO permission check at all.** Not a tightening, a genuine gap: any signed-in tenant member could set/clear a quote's job number via the direct browser→Supabase RPC path (no Netlify layer in between). Now raises `42501` for anyone without `ops.create_job`. _(done 2026-07-17)_
- [x] **Migration 0188 built — margin/cost gate on `eq_list_quotes`/`eq_get_quote_detail`.** Same direct-RPC exposure as 0189 — quotes read cost/margin straight from the browser, so this is the *only* durable enforcement point for `ops.view_margins` (the UI gate in QuotesModule is cosmetic without it). _(done 2026-07-17)_
- [x] **PR #885 merged** (Royce's explicit per-PR bypass-review consent — the auto-mode classifier blocks agent self-merge of its own PR without it, a pattern that recurred all session). Auto-deployed to core.eq.solutions. _(done 2026-07-17)_
- [x] **Migrations 0188 + 0189 dispatched via One Pipe, approved, and independently re-verified live on both `ehowgjardagevnrluult` (sks) and `zaapmfdkgedqupfjtchl` (eq)** via direct `pg_proc` source inspection — not just the workflow run's "success" label. (Migration 0190 was a later same-day addition by a concurrent session — see the Phase 4 remainder entry above; dispatched together with these two in the end.) _(done 2026-07-17, verified 2026-07-19)_
- [x] **Originally built + applied + validated eq-field PR #497's migration** (RLS hardening on `schedule_entries` + permission checks inside the 3 `SECURITY DEFINER` trigger functions) — this is the first pass a concurrent session later re-validated and found one follow-up gap in (`hire_company`, fixed as PR #499, see above). Caught a real validation-methodology trap while building it: the Supabase MCP connection runs as `postgres`, which has `rolbypassrls=true` — a naive RLS test via `set_config('request.jwt.claims', ...)` alone silently no-ops and reports a false pass. Fixed by wrapping the test in `SET LOCAL ROLE authenticated` so the policy is genuinely exercised; this is a distinct, reusable lesson from the "DDL commits before rollback" one logged in the cluster-3 entry above — both apply, for different reasons. _(done 2026-07-17)_
- [x] **End-of-program sweep: merged the remaining open PRs at Royce's explicit request and independently re-verified the whole cluster 1+3 program live**, rather than trusting each PR's own claim — #496, #551, #890 merged this session; #497/#499's already-applied state (done by concurrent sessions) confirmed via direct `pg_proc`/`pg_policy` queries and, for #499, a full 3-scenario behavioral test (employee tamper → reverts; manager write → applies; `extra_perms` grant → applies) rather than just reading the function definition back. Also confirmed migrations 0188/0189 were live on both planes (they'd been approved hours earlier via the One Pipe production-environment gate, without a report back at the time). _(done 2026-07-19)_
