...
- [ ] **Still not found: why a device ends up holding a session for a since-deleted `auth.users` row in the first place.** The 2026-08-03 dead-session OCR bug above is a symptom, not the cause. One real lead: the exact same test phone number involved (`61466118646`, signed up live at 20:01 UTC that session) matches a test account (`0466118646`) a *different*, unrelated eq-shell session flagged deleting "again" the same day (see the self-join entry above) — suggests concurrent sessions repeatedly creating/deleting the same shared test number is at least part of what's generating dead sessions, not necessarily a real-worker-facing bug. Not confirmed as the sole cause; hasn't reproduced against a genuine (non-test) worker account. Worth a proper look only if it recurs for a real worker. _(added 2026-08-03)_
