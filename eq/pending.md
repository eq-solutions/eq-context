...
- **Then Phase 2/3 (unchanged):** extend the `sks` Core-only lock in `checkAccess()` to `eq` + drop the `STAFF_CODE=demo`/`MANAGER_CODE=demo1234` backdoor (Phase 2); strip the dead PIN code (Phase 3). Do NOT strip before the handoff is confirmed or `eq` locks out.
