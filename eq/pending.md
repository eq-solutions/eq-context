...
- [ ] **Post-audit finding: 2 SECURITY DEFINER views flagged ERROR on ehow, not eq-service's** — `public.nomination_clashes` + `app_data.field_managers` (read as Field/labour domain). Chip `task_f953bb7a` filed; needs a Field/Shell owner to confirm intent or fix the invoker setting. _(added 2026-07-03, needs your call — not eq-service's to fix)_
