...
## eq-solves-intake + eq-shell: no-explicit-any bucket closed, vendored unused-vars fixed at source (2026-08-03)
*Royce: "continue - sprint this," picking up two of the three "Next" items from the session-close map.*

- [x] **eq-solves-intake: the 13 no-unused-vars instances fixed at the source.** Same errors eq-shell's vendored copy already had fixed (PR #1204) — now fixed here too so they don't come back on the next re-vendor. Mostly dead unused imports/params; one dead `Map` that revealed an unimplemented one-to-one-mapping-preference feature in the CSV column-mapper (flagged, not built — out of scope for a lint pass); two pricing constants kept and underscore-prefixed rather than deleted, pending a future cost-estimate feature. `pnpm run build`/`check` clean across all 7 packages, 254/254 tests pass. eq-solves-intake [PR #107](https://github.com/eq-solutions/eq-solves-intake/pull/107), MERGED (squash `43828f0`, Royce's "merge both" go).
- [x] **eq-shell: all 117 eq-shell-owned `no-explicit-any` instances resolved** (of 157 total; the other 40 live in the vendored eq-intake subtree, PR #107 above). Three treatments: (1) ~37 lines are a systemic, already-documented `SupabaseClient<any,any,any>` generic-inference workaround — added the same `eslint-disable-next-line` convention already used 59+ times elsewhere in this repo rather than fighting a known supabase-js limitation; (2) real type fixes where the shape was genuinely knowable (three dead `role as any` casts removed outright — the value was already typed `EqRole`; an AI-extracted JSON field typed `Record<string, unknown>` instead of `any[]`; a tier-tuple membership check now casts the array, not the input); (3) two dead `eslint-disable` comments removed from a `.mjs` script that was never covered by the TS lint rule in the first place (it was flagging "rule not found," not a real `any`). One real near-miss caught by the full build, not just eslint: an attempt to give two joined-relation queries proper row interfaces passed eslint but failed `tsc` — supabase-js's compile-time select-string parser infers embedded to-one relations as arrays regardless of real FK cardinality on an any-generic client; reverted to the original any-cast-with-disable-comment pattern instead. `pnpm run build` clean, 278/278 tests pass. eq-shell [PR #1207](https://github.com/eq-solutions/eq-shell/pull/1207), MERGED (squash `1d9378a`, Royce's "merge both" go). Netlify auto-deploys eq-shell on merge.

**Deferred:**
- [ ] **eq-shell re-vendor bump — PR opened, CI green, awaiting merge.** Rather than a manual worktree pass, triggered the existing `intake-revendor-check.yml` workflow directly (`workflow_dispatch`) — it's replaced the old by-hand re-vendor routine since 2026-08-01 (PR #1175/#1169), and runs a react/react-dom pin-parity check plus a full build+typecheck+test before ever opening a PR. eq-shell [PR #1210](https://github.com/eq-solutions/eq-shell/pull/1210) — 10 files, +7/-15, all required checks green (`typecheck · test · lint`, `gitleaks`, migration-ledger hygiene, schema-drift+anon-grant+policy-lint), `mergeStateStatus: CLEAN`. Not merged — needs Royce's go, same as every other PR this session. _(added 2026-08-03)_
...
**Deferred:**
- [ ] **#1203's actual fix (the retry after the hotfix) hasn't been confirmed live yet** — Royce was mid-retry when this session closed. Also still unconfirmed: #1195's nav trim and #1199's nudge copy. _(added 2026-08-03)_
...
- [ ] **Dashboard map centering — merged and live, awaiting Royce's real-world confirmation it actually fixed the problem.** Royce, after hard-refreshing to rule out stale cache: "still pinned to the bottom, dig into it" / "I am positive that the map (nominal values) thinks its 1600px tall but the actual window is only 800px - something like that". Real precedent found in this file's own history — v3.5.427 logged the identical complaint verbatim, patched then by raising maxZoom rather than the actual measurement race. `L.map()` measures its container at the exact sync tick it's constructed, which can still race ahead of the Shell-iframe's own layout pass finishing even with `invalidateSize()` immediately after. eq-field [PR #630](https://github.com/eq-solutions/eq-field/pull/630) (v3.5.437, squash `d0290ad`) defers the fit one `requestAnimationFrame` and logs real-vs-Leaflet height once for a hard number if it recurs. Ruled out a `height:1600px` rule in vendored `leaflet.css` — stock Leaflet's Safari-only tile-stretch workaround, inert on Chromium. Merged on Royce's "merge it" go; confirmed live on `field.eq.solutions` via Netlify MCP (deploy `6a705ed57ef8fb0008671c3b`, `commit_ref` matches the merge exactly). **Still not independently reproduced/verified** — no access to Royce's authenticated session to click-test; this is a well-reasoned, precedented fix for a documented failure class, not a confirmed-working claim — leaving this unchecked until he confirms live. **Separate, unrelated finding**: Sentry's EQ-FIELD-10 shows a new `dashboard-map-css-collapsed` event (29 occurrences since 2026-08-01, marked "regressed") — investigated and it's a false positive from stale `3.5.434` code + normal tab-switching, not a real bug; flagged as a follow-up, not fixed here. _(added 2026-08-03)_
