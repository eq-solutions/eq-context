...
- [x] **Root cause of the blank screen investigated — it's already-documented, already-mitigated Netlify cold start, not a new gap.** The middleware round-trip suspected earlier turns out to resolve locally (no network call) when there are no cookies to validate, so it's not the cause on a genuinely fresh Shell embed. `docs/perf/load-time-investigation.md` (eq-solves-service, last updated 2026-07-23) had already root-caused this to Lambda cold start (4.6–5.3s cold vs 0.4–1.4s warm, measured 2026-07-12) and shipped keep-warm + a Sentry-integration trim (#571) as mitigations. Live-checked `service.eq.solutions/api/health` today: 3 hits at 1.57s/0.36s/0.51s — consistent with the documented warm range, so the mitigations are holding in production. Could not force a genuine cold hit (needs a longer idle gap or a fresh deploy recycle) to confirm the cold number itself improved.
...
- [x] **CSRF/iframe-origin enforcement — confirmed already safe**, no code gap. (Netlify env var itself — `ENFORCE_IFRAME_ORIGIN` — still needs a dashboard check, see Deferred.)
- [x] **Audit-log tenant isolation — confirmed safe** by reading the live function body directly (`eq_recent_auth_events` filters by tenant on the JWT).
- [x] **Built the missing piece: what happens to a person's data when they leave.** Before this, deactivating someone in Core cut their access but their personal details (DOB, home address, emergency contact, licence number, login credentials) just sat there forever. Scoped with Royce (7-year Fair Work Act floor, platform-wide from day one), steelmanned before building — which correctly killed a photo-purge idea that would have reached into EQ Cards' worker-owned data (verified no photos are even stored on the employer side; dropped, not needed). **Now fully live everywhere**: security codes/PINs wipe immediately on deactivation; personal details auto-anonymise 7 years later; nothing is ever hard-deleted (so historical timesheets/records stay intact). eq-shell PR #1013, merged; deployed to the control-plane database and both SKS's and EQ's own tenant databases.
- [x] **Cleaned up a stale branch** (`fix-stale-valid-roles`) — checked it against current `main` first and found the identical fix had already shipped via a different, already-merged PR (#674); safe to delete, nothing lost.
...
*Royce reported the same 5 archived SKS staff kept reappearing every day. Root-caused live to `workers-canonical-sync` (a Cards→Field sync, both a nightly cron and a real-time trigger) unconditionally force-setting a synced worker back to active/approved on every run — 87 staff rows affected in one night, not just 5. Asked to steelman the fix and understand what's real before building the fuller solution; that surfaced a much bigger, separate gap.*
- [x] **The actual bug: fixed and live.** `workers-canonical-sync`'s merge branch no longer touches `active`/`field_approved` on an existing row — those are only ever set by the explicit actions that own them (Shell's archive button, Cards' approval flow). eq-shell PR #993, merged, deployed live to jvkn.
- [x] **Governance gap closed**: the DB function driving the nightly sync (`eq_reconcile_worker_sync`) existed only in the live database, no file anywhere — codified into a proper migration and applied. PRs #994/#995, merged + applied.
- [x] **Bigger finding while steelmanning #993 — a live security hole, not just a sync bug**: on the EQ tenant's own database (zaap), any logged-in EQ Shell user could directly edit any colleague's staff record — pay rate, active status, approval status, name, everything — with no permission check at all, by calling the database directly instead of going through the app. Nothing legitimate needed this. **Fixed and applied live** — that direct-edit door is now closed on both EQ's and SKS's databases. eq-shell PR #1009.
- [x] **Added a backstop so this class of bug can't recur**: reactivating an archived staff member now requires a real, attributed human action (who did it, when) — an automated sync silently flipping someone back to active is no longer possible even if a future bug tries it. Applied live on both databases.
- [x] **Retired a second, unrelated dead sync pipeline** (`credentials-canonical-sync`) found along the way — it had never actually processed a single real record (superseded by a different, working licence-sync path months ago) and was hardcoded to the wrong tenant besides. Removed; nothing was using it. Applied live.
...
- [x] **Stray invalid role value in `cards-approve-staff.ts`'s invite path — fixed and merged.** Turned out worse than "dormant": `shell_control.users.role` is a proper `eq_role` enum, so the hardcoded `role: 'worker'` value made that upsert error every single time it ran (silently — the error was never checked), meaning approval-time login provisioning never actually worked for the invite path. Now reuses the same `resolveRole()` helper the application path already uses, and logs if this upsert ever fails again (it previously failed silently, which is how the bug went unnoticed). eq-shell PR #1008, merged (squash `c0dd17f2`), live on core.eq.solutions.
...
*Royce asked for "the same dropdown filtering capabilities as Excel" on the EQ Ops quotes table. Built a reusable multi-value checklist filter (header icon → popover: search, Select All/Reset, checkboxes) into the shared `@eq-solutions/ui` Table component, wired it into EQ Ops' Est./Status/Job No. columns, then — while checking the filtering — Royce reported a labour-hire rate upload wasn't showing up, which turned out to be an unrelated real bug in the AI rate-card parser. Later Royce asked for the same filter on the EQ Service maintenance-check Assets table (a different, hand-built table, not the shared component), then asked whether picking one column's value narrows the others — it didn't yet, so cascading (Excel AutoFilter behaviour) was added everywhere in a follow-up round.*

- [x] **Excel-style multiselect filter, with cascading, is live everywhere it was asked for.** Shared `Table` component (`@eq-solutions/ui` v1.11.1) — eq-ui PRs [#25](https://github.com/eq-solutions/eq-ui/pull/25)/[#27](https://github.com/eq-solutions/eq-ui/pull/27), regression tests added (`Table.test.tsx`). EQ Ops quotes table (Est./Status/Job No.) — eq-shell PRs [#1001](https://github.com/eq-solutions/eq-shell/pull/1001)/[#1003](https://github.com/eq-solutions/eq-shell/pull/1003), live on core.eq.solutions. EQ Service maintenance-check Assets table (Location/Work Order #/Maintenance Plan) — eq-service PRs [#601](https://github.com/eq-solutions/eq-service/pull/601)/[#602](https://github.com/eq-solutions/eq-service/pull/602), live on service.eq.solutions, **Royce confirmed working live**.
- [x] **Real bug found + fixed along the way: labour-hire rate uploads with a date range in the source PDF (e.g. "FY26-27") could silently never show as current.** The AI parser picked the range's END date as the rate's start date; a future start date means the rate never counts as "current" anywhere (dashboard, weekly cost, matching), even though the upload itself succeeded with no error. Fixed the parser's date guidance + added an in-review warning when the extracted date is >60 days out. The one real upload this had already hit (INSELEC Specialised Recruitment Partners, 12 rates) was corrected live on ehow (effective_from 2027-06-30 → 2026-07-01, Royce confirmed the right date). eq-shell PR #1001.
...
*Reopened the accidentally-stopped session flagged above. What started as "tidy up the quote panel" (remove Win/Lose, simplify statuses, fix a sticky-totals regression) grew across the day into a full simplification of the panel plus a ground-up rebuild of the Coupa purchase-order matching tool, once it turned out that feature had never actually been tested against a real Coupa export. Royce then did exactly that — ran a real Coupa file through it — and it failed completely, which led to a same-day bug fix and a full redesign of how automatic matching works.*
- [x] Royce ran a real Coupa export through the new importer — every single row failed with a database error, so nothing had actually been importing despite looking ready. Root cause was two bugs in the matching update, the second one hidden behind the first (nothing ever reached it until the first was fixed): one made every write fail outright, the other would have failed the very next step too. Both fixed and proven against Royce's own real data — reran the exact rows that failed for him, inside a transaction that undoes itself, before it ever touched live data for real.
- [x] Rebuilt how automatic matching works, per Royce's own steer ("I fear automating this will create issues" → his own proposal to show a checkbox per row instead of one bulk button). Every row now shows what it would match to — the quote, the customer, whether the dollar figure lines up — before anything is written. Only genuinely confident matches (exact quote number, value already lines up) are ticked for you; a same-site guess is never ticked automatically, even when its value happens to agree, because Royce's own test data showed several unrelated purchase orders all guessing the same wrong quote that way. One button now imports only what's ticked. Replaces the old single "Import all exact matches" button and separate per-row confirm buttons.
- [x] Both database fixes are live on both company databases (SKS and EQ, dispatched and verified directly against each), and the redesigned import screen is live on the site.
- [x] **The bookkeeping mismatch flagged earlier (two database updates sharing the same tracking number) is fully cleaned up.** Renamed both to free numbers, told the system to re-sync its records, then had it re-check both company databases for real — clean, nothing pending. Along the way the re-check tripped over a real (self-resolving) hiccup: one of the two renamed updates wasn't safe to re-run a second time, and it collided with someone else's unrelated update that landed in the same window, so one sync attempt failed outright on both companies' databases before anything got confirmed. No customer-facing impact — the tool kept working the whole time. A different concurrent session caught it, fixed the update to be safely re-runnable, and a clean sync went through minutes later. Directly checked both company databases afterward: exactly one correct version of the matching logic on each, tracking records match reality on both. _(closed 2026-07-24)_
- [x] Royce ran a real file through the redesigned review screen — it worked (14 purchase orders matched correctly, checked directly against the database) but the screen never actually said so; a successful row looked identical to one that hadn't been actioned yet. Fixed: each row now shows a clear tick or cross once it's actually gone through, with the reason shown if it failed, and a running "N imported, M failed" count as the batch goes. Also added the job title to each row per Royce's ask ("we need to see the job title on this screen") — a quote number and company name aren't always enough to recognise a job at a glance. Both live on the site and in both company databases.
...
- [x] **Correction: "PR #973" in the two bullets above was NOT a wrong number — a same-day note here (2026-07-26) claimed it was, based on the PR title alone, and that note was itself wrong.** "perf(quotes): bound the Ops pipeline fetch, add a real counts RPC" reads unrelated to "database list-loading"/"Overdue follow-up filter" until you read the actual PR body and diff: its migration explicitly ports the 5 client-side smart-filter toggles (`expiringOnly`/`unsentOnly`/`needsJobNoOnly`/`overdueFupOnly`/`staleOnly` — `overdueFupOnly` is the "Overdue follow-up" filter) server-side, verbatim from the existing stat-chip predicates. That's exactly the "will now match the same, stricter rule the on-screen count already uses" change flagged two bullets up. Same PR, same day — opened 2026-07-22 20:53 UTC by a concurrent session, merged 2026-07-23 09:56 UTC — just described in plainer, less precise terms by a session that caught it mid-review before its full shape was clear. Title-only pattern-matching isn't verification; fixed by actually reading the diff. The underlying product question two bullets up (was the filter's stricter behavior ever explicitly signed off on?) is still open. _(corrected 2026-07-26)_
