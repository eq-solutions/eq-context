...
## eq-context + eq-shell/eq-field: CI health sweep traced the suite dashboard's "CI: unknown" status to a 13-day-old blind spot, plus a git-status sweep found one repo pointed at the wrong GitHub project (2026-08-03)
*Widened the eq-solves-service drift investigation above into a full CI health sweep of eq-shell and eq-field, then a git-status check across all 14 local EQ/SKS repos.*

- [x] **eq-shell + eq-field CI health sweep** — both clean on `main`. Two eq-shell incidents this week (a hand-applied database function with no paper trail, an access-token misconfig on the intake auto-vendor workflow) were both self-caught and fixed same-day, no live impact. One chronically flaky check (`smoke.yml`, Netlify cold-start timeouts, different function each time) — not a real bug.
- [x] **Diagnosed why every private EQ repo shows "CI: unknown"** in the suite dashboard — traced to the exact hour: the `EQ_CONTEXT_PAT` GitHub access token was rotated 2026-07-21 with too-narrow permissions, and the status-check script has been silently swallowing every resulting error since — 13 days blind. Fixed the script so a future break shows up loudly in the run log instead of hiding as "unknown". eq-context commit `75f5c11`, pushed to main.
- [x] **Git-status sweep across all 14 local EQ/SKS repos** — found `eq-solves-assets` tracking a personal fork of the Service app, not the real shared-assets repo. Everything else was either clean or normal in-progress work (eq-cards, eq-field, eq-ui each carrying one unpushed feature commit — nothing wrong), except `eq-intake`, which has real uncommitted changes worth a look.
- [x] **Found and fixed a live substrate corruption while closing this session**: a concurrent session's rebase left a literal unresolved `<<<<<<< HEAD` conflict marker committed straight into `sessions/2026-08-03.md` on `main` — caught mid-push when this session's own close hit the same shared-checkout race. Resolved by a different concurrent session before this one finished reconciling; verified clean (no markers anywhere in the file) before pushing this entry.
...
- [ ] **The shared `eq-context` checkout took real damage from concurrent-session git races this session** — a stuck rebase, a live conflict-marker corruption on `main`, and two separate non-fast-forward push rejections, all within about 10 minutes, from multiple sessions writing to the same working directory at once. Worth considering whether concurrent sessions should default to a fresh clone for any `/close`, not just multi-step surgery — the existing guidance undersells how often this is now actually happening. _(added 2026-08-03)_
...
- [ ] **Quotes-vs-jobs Kanban split — steelman discussed, no decision.** `app_data.jobs` already exists as its own canonical table (12 rows) separate from `app_data.quote` (74 rows), and `syncJobToCanonical` already bridges a quote's status across once a job exists — the seam is real, the board just doesn't reflect it. Counter-case: real schema/product change (not a re-bucket), traceability cost, and 12 jobs may not justify the investment yet. Asked Royce what's actually motivating the ask (visual clutter vs. specific people wanting a different view) — no reply yet. _(added 2026-08-03)_
...
- [ ] **Dashboard hero/collapse reorg — built, PR open, awaiting merge.** Royce: "interactive dashboard... basic head count, anniversaries and birthdays and then some collapsed menus including the map." `/decide` run first (six-step pass) — held on building same-night as the still-unconfirmed map centering work above until weighed against it; Royce chose to scope it and build regardless ("Scope it now"). eq-field [PR #636](https://github.com/eq-solutions/eq-field/pull/636) (v3.5.443) moves Headcount tiles + Leave & Absences/Pending Leave (kept hero, not collapsed — Pending Leave is actionable, unlike the map/table) and Birthdays & Anniversaries up to sit right after the filter row; Map card and Site Breakdown table each become collapsed-by-default accordions, same chevron-toggle pattern as the existing Roster Gaps card. Map is lazy-init — `renderDashboard()` only mounts/refreshes it while expanded, so a collapsed card never pays for the JWT/RPC fetch or an `L.map()` mount against a hidden container. `dashboard.js` (dead mirror) crossed its own default 1500-line cap for the first time (1532) — grandfathered to 1700 in `eslint.config.js`. `node --check` clean, `npx eslint@9 scripts tests netlify` (exact CI scope) 0 errors/927 pre-existing-pattern warnings, full existing test suite (24 files) passes. Not merged — holding for Royce's explicit go. **Needs a live click-through once merged**: map/Site Breakdown expand+collapse correctly, map still centers correctly on first expand (not just when already-expanded from a prior session), headcount/leave/birthdays actually read as "hero" now. _(added 2026-08-03)_
