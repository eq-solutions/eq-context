...
- [ ] **Root cause not yet fixed**: the database function behind quote events (tenant-migration 0152) still writes the old app name into the record — only the on-screen label was corrected. Needs a proper database fix and Royce's go to apply it. _(added 2026-07-30)_
- [ ] **Royce to click through live**: open Activity log → Suite activity tab, confirm the sentences read sensibly against real SKS data (quotes, shifts, licence reviews), and check the new search/filter on that tab works as expected. _(added 2026-07-30, PRs #1121/#1123/#1126/#1129 merged, migrations 0225+0226 dispatched, workers-canonical-sync redeployed v12 — full write-up in `sessions/2026-07-30.md` and `changelog/eq-shell.md`)_
- [ ] **Cross-repo attribution — eq-shell, eq-solves-service, and eq-field done; eq-cards deliberately parked, not blocked.** eq-service's admin archive/restore/cascade-archive path attributes to a real actor (eq-service [#652](https://github.com/eq-solutions/eq-service/pull/652)). eq-field's real actor id was found sitting unused in its own login flow (`shell_user_id` off the Shell-minted JWT, both live tenants are Core-only so every real session has it) and threaded through session token → data JWT mint → the write call (eq-field [#574](https://github.com/eq-solutions/eq-field/pull/574)). All three merged + deployed live. eq-cards' `workers-canonical-sync`: cron-vs-live is cheaply distinguishable (`type==='UPDATE' && old_record===null` = cron, zero new plumbing needed) but the naive fix would be wrong, not just incomplete — `eq_cards_admin_upsert_worker` (`0089_admin_upsert_worker_adopt_by_identity.sql`) lets an admin edit *any* worker's row without touching `user_id`, so attributing every live edit to `record.user_id` would misattribute admin-driven edits as the affected worker's own action. Only genuinely self-service edits (`eq_cards_upsert_my_worker`, `auth.uid()`-gated) are safe to attribute as-is; the admin path needs its own identity threaded through first. Royce's call (2026-07-30): stale backlog item, not live pain — park it rather than touch an admin RPC's security model for it now. _(updated 2026-07-30)_
...
- [ ] **Phase 2, second half — drop the `STAFF_CODE=demo`/`MANAGER_CODE=demo1234` backdoor.** The browser-side Core-only lock for `eq` is confirmed shipped and live (v3.5.306, PR #461 — see 2026-07-30 session log) but NOT verified closed here: didn't check whether `verify-pin.js`'s plain PIN-compare action still accepts these codes if called directly (bypassing the browser). CLAUDE.md's env-var table still lists `STAFF_CODE`/`MANAGER_CODE` as live for `eq-solves-field`. Needs an explicit server-side check before this can be ticked. _(carried, added 2026-07-12)_
- [ ] **Phase 3 — strip the dead server PIN code.** Blocked on the line above; not started. _(added 2026-07-12)_
