...
- [x] **eq-shell [PR #1208](https://github.com/eq-solutions/eq-shell/pull/1208) merged** — Register tab: who's signed what, people who haven't signed surface first. Confirmed live on core.eq.solutions.
- [x] **eq-shell [PR #1212](https://github.com/eq-solutions/eq-shell/pull/1212) merged + applied live** — database column to hold a real drawn signature, confirmed on both the SKS and EQ tenant databases.
- [x] **eq-field [PR #635](https://github.com/eq-solutions/eq-field/pull/635) merged** — Sign Documents now opens a real signature pad, the same one Prestart Briefings and Toolbox Talks already use, instead of a bare tap-to-confirm. Confirmed live on field.eq.solutions.
- [x] **eq-shell [PR #1217](https://github.com/eq-solutions/eq-shell/pull/1217) merged** — Register tab gained a "View" button per signer: the drawn signature, exact timestamps, and a link to the actual signed document. Confirmed live on core.eq.solutions.
...
- [ ] **Roll out past the one-person pilot** — widen who can sign in Field beyond you, put a real document through end-to-end beyond the one test document. Your call on timing. _(added 2026-08-03)_
...
- [x] **`EQ_CONTEXT_PAT` re-scoped in GitHub, confirmed live — the dashboard's 13-day blind spot is closed.** Royce created a correctly-scoped token; verified by manually triggering a refresh run rather than waiting on the nightly cron. Real statuses now flow: eq-shell/eq-field/eq-cards green, eq-service showing its one known pre-existing gap (`Integration tests`, a Supabase-local schema-cache issue, already documented as non-blocking) instead of "unknown".
- [x] **eq-intake's uncommitted build-pipeline work extracted, verified, and PR'd** — [eq-solves-intake #108](https://github.com/eq-solutions/eq-solves-intake/pull/108). Pulled off the stale branch it was mixed into onto a clean branch off current `main` (left the unrelated trades-settings feature commit untouched on its original branch). Dropped two things that didn't belong: `.claude/settings.local.json`'s out-of-place `eq-context` git-permission entries (reverted), and 114MB of stale vendored `eq-shell`/`eq-service` app duplicates (added to `.gitignore` instead of committing — the original config notes already called them stale). Re-verified against `main`'s newer vitest (2.1.5 → 3.2.6, bumped independently since this work was drafted): `check:packages` clean, `test` 592/592 passing. Not merged — opened for review since it changes the CI gate shape.
...
- [ ] **eq-solves-intake PR #108 needs a merge decision** — build/test both verified green, but it changes what the CI gate covers (adds a `test` step, narrows `check:packages`), so left for Royce's review rather than auto-merged. _(added 2026-08-03)_
...
*Started as "SKS-17577 has a saved file but nothing shows it." Turned into a full pass on the board's visual hierarchy, plus — while investigating a "drag and drop is broken" report — a genuine root-caused bug: the board's one-click "Sent" checkbox never promoted a draft quote to submitted, unlike the detail panel's equivalent action. 23 of Open's 32 live cards were quotes that had actually been sent weeks ago but were stuck looking untouched, invisible to the "In play $" total. Fixed the code and ran a live one-time backfill against the real SKS quote table.*

- [x] **File-attached paperclip badge added to the board card** (existed only in the list/mcard layout before). eq-shell [PR #1205](https://github.com/eq-solutions/eq-shell/pull/1205), merged.
- [x] **Bolder paperclip pill + estimator-colored initials.** Tried customer-based card coloring first — a static preview against real customer names showed immediate collisions (dozens of customers, ~6 categorical colours), which is worse than no colour since it implies a false grouping. Moved the same colour idea to the estimator initials badge instead (small, fixed roster — a colour reliably means "same person" there). eq-shell [PR #1209](https://github.com/eq-solutions/eq-shell/pull/1209), merged.
- [x] **Unique colour per pipeline stage, re-spread estimator palette, wider columns.** The shared `StatusBadge` component collapsed 4 of 5 board columns onto the same colour; coloured the column header locally instead of touching the shared package. Estimator palette's first pass clustered too tightly (two colours barely distinguishable); regenerated with even hue spacing. Columns went from a fixed 240px to flex-grow, filling dead whitespace instead of adding a 6th column. eq-shell [PR #1211](https://github.com/eq-solutions/eq-shell/pull/1211), merged.
- [x] **Bolder column headers, less-pink estimator colours, bolder customer names.** The estimator palette's hue range ran into magenta/rose, which read as plain pink; capped the range and dropped saturation. eq-shell [PR #1213](https://github.com/eq-solutions/eq-shell/pull/1213), merged.
- [x] **Darkened the customer group header, dropped the duplicate customer name from inside each card** (it was shown twice — once as the group header, once again per card). Frees a full line per card for project detail. eq-shell [PR #1214](https://github.com/eq-solutions/eq-shell/pull/1214), merged.
- [x] **"Drag and drop is broken" root-caused — it wasn't.** Dragging into "Job created" is correctly blocked without a job number (mirrors the detail panel's rule), but the block message rendered as a static banner up by the KPI row — invisible if you're scrolled into a column, so a reverted card just looked broken. Moved it to a fixed bottom-center toast matching the pattern already used on the Staff page. eq-shell [PR #1215](https://github.com/eq-solutions/eq-shell/pull/1215), merged.
- [x] **Real bug found + fixed: the board's "Sent" checkbox never promoted draft → submitted.** The detail panel's "Submit & Mark Sent" already does this (status promotion + sent_at + seeds a 7-day follow-up); the board's one-click checkbox only ever set `sent_at`. Verified against live data: 23 of Open's 32 cards were genuinely-sent quotes permanently stuck showing as `draft`, invisible to the "In play" KPI (which only sums submitted/won-awaiting-job-no). Fixed the checkbox to match the detail panel's logic. eq-shell [PR #1216](https://github.com/eq-solutions/eq-shell/pull/1216), merged.
- [x] **One-time backfill run live against the SKS quote table** (ehow, `app_data.quote`) to correct the 23 already-stuck rows — via the real `eq_update_quote_status`/`eq_set_follow_up_date` RPCs, not a raw `UPDATE`, so status history and job notes were written exactly as if a person had clicked the button. Verified: `draft` status count 23 → 0, `submitted` 7 → 30, 23 status-history rows + 23 job-note rows written.

**Deferred:**
- [ ] **Live click-through not done** — none of the above (badge, colours, toast, checkbox fix) has been confirmed by an actual authenticated click-through yet. Needs a real session, off-limits for me to do myself. _(added 2026-08-03)_
- [ ] **7 quotes already in `submitted` status (unrelated to the bug above) are also missing a follow-up date** — noticed while verifying the backfill, not fixed since it's a separate pre-existing gap outside what was agreed. _(added 2026-08-03)_
- [ ] **Quotes-vs-jobs Kanban split — /decide run, real scope clarified by Royce, one root-caused bug found live. Full detail: [eq/ops/EQ-OPS-ARCHITECTURE.md](../ops/EQ-OPS-ARCHITECTURE.md).** Royce clarified NSW Comms (`app_data.sks_comms_jobs`, 153 rows, a different trade/scale) is deliberately out of scope — its `job_number`/`quote_ref` fields are forward-compatibility wiring, not a live dependency. EQ Field's dependency on `job_number` IS live and real ("it should all be canonical"). Live query found the actual gap: `app_data.jobs` (13 rows) covers only 13 of ~38 quotes that have genuinely reached a job-stage status — `syncJobToCanonical()` only fires from the detail panel (which has `customer_id` on its `QuoteDetail` object), never from the board's own drag-and-drop (the board's plain `Quote` interface has no `customer_id` field at all, confirmed against every affected row at the DB level). This has likely been silently under-populating Field's "canonical" job list since the sync was introduced. Also found: `app_data.jobs.quote_id` has no FK constraint, and one existing row already points at a deleted quote — a real orphan. Royce's ultimate intention: EQ Ops becomes the primary quoting engine, `job_number` is the durable join key across Field/Ops/(eventually) Comms via manual entry or a future Workbench API. Not yet decided: sequencing of the sync-bug fix + backfill vs. the Kanban split itself. _(added 2026-08-03)_
...
- [ ] **Whether the "stuck on profile, no way back to Wallet" experience is actually gone now that the permission fix (PR #204) is live** — architecturally the Wallet/Profile tab bar should never have been affected by that error, so this may have just been the same incident manifesting as a dead end; not independently retested. _(added 2026-08-03)_
...
- [ ] **Dashboard map centering — the fraction-of-canvas-height heuristic (PR #632/#637) is retired; now anchored on Sydney CBD instead — fixed, merged, and deployed live.** Two rounds of "still not centred enough" (30% shift, then 50%) later, Royce sent a live screenshot showing Sydney sitting ~2/3 down the canvas and asked: "Is it possible to move Sydney to be vertically centre of this view?" eq-field [PR #640](https://github.com/eq-solutions/eq-field/pull/640) (v3.5.446, squash `ae659b7`) replaces `_DASH_MAP_CORE_VERTICAL_SHIFT` entirely with an explicit anchor: pans vertically only (horizontal framing from `fitBounds()` untouched) so a fixed Sydney CBD point (-33.8688, 151.2093) always lands dead-centre, regardless of which sites are in the busiest cluster that week — a known point instead of a heuristic fraction that had to be re-guessed every round. Applies to `fitCore()` only; `fitAll()` ("Show all sites") untouched, same scoping as before. `node --check` clean, `npx eslint@9 scripts tests netlify` 0 errors/927 pre-existing-pattern warnings, full test suite passes. Merged on Royce's "merge" go; confirmed live on `field.eq.solutions` via Netlify MCP (deploy `6a70e0ec45c4a300086f070c`, `commit_ref` matches the merge exactly, secret scan clean). **Still needs Royce's live confirmation** Sydney actually sits dead-centre now. The temporary 900px map-height testing bump (PR #634) is still in place — **revert to 600px once this centering work is finally confirmed settled.** _(added 2026-08-03, updated 2026-08-03)_
- [ ] **Stale-version-footer pattern — still unresolved.** Hit repeatedly across this whole session's testing, on both a standalone tab and the Shell-embedded copy; neither an Incognito-adjacent DevTools SW unregister nor a "Clear site data" on the standalone `field.eq.solutions` tab fixed the Core-embedded copy. Working theory going into next session: Chrome partitions a third-party iframe's service worker/cache storage separately from a direct top-level visit to the same origin, so clearing the standalone tab's copy never reaches what Core's iframe actually loads. Gave Royce steps to clear the correct (embedded/partitioned) storage directly from the Core tab's own DevTools, or via `chrome://settings/content/all` filtered to "field" — **not yet confirmed whether this worked.** If it recurs after that, the partitioning theory is wrong and this needs fresh diagnostic eyes, not another cache-clear guess. _(added 2026-08-03)_
- [x] **Dashboard hero/collapse reorg — fixed, merged, and deployed live.** Royce: "interactive dashboard... basic head count, anniversaries and birthdays and then some collapsed menus including the map." `/decide` run first (six-step pass) — held on building same-night as the still-unconfirmed map centering work above until weighed against it; Royce chose to scope it and build regardless ("Scope it now"). eq-field [PR #636](https://github.com/eq-solutions/eq-field/pull/636) (v3.5.443, squash `444fbbc`) moves Headcount tiles + Leave & Absences/Pending Leave (kept hero, not collapsed — Pending Leave is actionable, unlike the map/table) and Birthdays & Anniversaries up to sit right after the filter row; Map card and Site Breakdown table each become collapsed-by-default accordions, same chevron-toggle pattern as the existing Roster Gaps card. Map is lazy-init — `renderDashboard()` only mounts/refreshes it while expanded, so a collapsed card never pays for the JWT/RPC fetch or an `L.map()` mount against a hidden container. `dashboard.js` (dead mirror) crossed its own default 1500-line cap for the first time (1532) — grandfathered to 1700 in `eslint.config.js`. Merged on Royce's "merge" go; confirmed live on `field.eq.solutions` via Netlify MCP (deploy `6a708445fd02010008157ec7`, `commit_ref` matches the merge exactly, secret scan clean). **Still needs a live click-through**: map/Site Breakdown expand+collapse correctly, map still centers correctly on first expand (not just when already-expanded from a prior session), headcount/leave/birthdays actually read as "hero" now. _(added 2026-08-03)_
- [ ] **Map zoom/shift further tuned, plus a real Sentry regression caught — fixed, merged, and deployed live.** Royce, live on v3.5.443: "can zoom in two clicks for the default map view" (`_DASH_MAP_CORE_ZOOM_BACKOFF` 3→1) and "its still on the lower end of the window" (`_DASH_MAP_CORE_VERTICAL_SHIFT` 0.30→0.5 — an empirical bump, not a re-derived fix; the 30% shift's math checks out by hand but evidently wasn't landing far enough visually, and this is flagged as tunable further if 0.5 overshoots). Also reported "when map expands you can't collapse again" — investigated the toggle/mount code thoroughly, found no logic bug, and Royce confirmed on a retry it "seems ok now" (not reproducing, no fix needed for that specific symptom). While digging into it, found and fixed a real, separate regression: `_dashMapWatchForVanish`'s 10s post-mount Sentry watchdog didn't know about PR #636's new collapse toggle, so collapsing the map within 10s of expanding it would misreport as `dashboard-map-css-collapsed` — added `!_dashMapExpanded` to its existing guard. eq-field [PR #637](https://github.com/eq-solutions/eq-field/pull/637) (v3.5.444, squash `3c10074`). `node --check` clean, `npx eslint@9 scripts tests netlify` 0 errors/927 pre-existing-pattern warnings, full test suite passes. Merged on Royce's "merge" go; confirmed live on `field.eq.solutions` via Netlify MCP (deploy `6a708b40b4207f0008497a51`, `commit_ref` matches the merge exactly, secret scan clean). **Still needs Royce's live confirmation** that the new zoom/shift values actually read right now, and that a normal collapse no longer trips Sentry. _(added 2026-08-03)_
- [ ] **Collapse-affordance fix — fixed, merged, and deployed live.** Royce, live on v3.5.444, after confirming the map view sat "slightly higher": "theres no collapse bar once expanded." The collapse toggle (#636) worked correctly the whole time — the chevron just didn't read as an interactive control once attention was on a full expanded map. eq-field [PR #638](https://github.com/eq-solutions/eq-field/pull/638) (v3.5.445, squash `6e8365b`) adds an explicit "Expand"/"Collapse" text label next to the chevron on both the map card and Site Breakdown (proactive fix on both, since they share the identical pattern — not just the one Royce actually hit). `node --check` clean, `npx eslint@9 scripts tests netlify` 0 errors/927 pre-existing-pattern warnings, full test suite passes. Merged on Royce's "merge" go; confirmed live on `field.eq.solutions` via Netlify MCP (deploy `6a70d88c5c2a700008468b4f`, `commit_ref` matches the merge exactly, secret scan clean). **Still needs Royce's live confirmation** the label actually reads right on both cards. _(added 2026-08-03)_
