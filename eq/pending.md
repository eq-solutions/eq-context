...
- [ ] **Dashboard map centering — the fraction-of-canvas-height heuristic (PR #632/#637) is retired; now anchored on Sydney CBD instead — built, PR open, awaiting merge.** Two rounds of "still not centred enough" (30% shift, then 50%) later, Royce sent a live screenshot showing Sydney sitting ~2/3 down the canvas and asked: "Is it possible to move Sydney to be vertically centre of this view?" eq-field [PR #640](https://github.com/eq-solutions/eq-field/pull/640) (v3.5.446) replaces `_DASH_MAP_CORE_VERTICAL_SHIFT` entirely with an explicit anchor: pans vertically only (horizontal framing from `fitBounds()` untouched) so a fixed Sydney CBD point (-33.8688, 151.2093) always lands dead-centre, regardless of which sites are in the busiest cluster that week — a known point instead of a heuristic fraction that had to be re-guessed every round. Applies to `fitCore()` only; `fitAll()` ("Show all sites") untouched, same scoping as before. `node --check` clean, `npx eslint@9 scripts tests netlify` 0 errors/927 pre-existing-pattern warnings, full test suite passes. Not merged — holding for Royce's explicit go. The temporary 900px map-height testing bump (PR #634) is still in place — **revert to 600px once this centering work is finally confirmed settled.** _(added 2026-08-03, updated 2026-08-03)_
