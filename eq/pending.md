...
*Picks up background task `task_42424993`, flagged 2026-08-05 (see that session's log) when the canary had failed 3 consecutive days and the root cause was suspected but not confirmed across all three repos.*

- [x] **Confirmed live, not from stale local trees, across all three repos.** eq-shell's and eq-service's local clones were both stale/wrong-branch — re-verified against actual `origin/main` via `git show` before drawing any conclusion. Root cause: eq-service migrated `ServiceJwtClaims.app_metadata` to `ShellHandoffClaims & { ...local keys }`, importing the base type from a new `@eq-solutions/contracts` package — but eq-shell's mint-side `SupabaseJwtClaims` has NOT migrated, so the shared package doesn't retire this canary yet, it just moved where part of the shape lives. Both repos pin the identical contracts tag (`v0.1.0`), verified == the package's current `main` HEAD.
- [x] **Rewrote `scripts/check-jwt-contract-drift.mjs`** to fetch `@eq-solutions/contracts` as a third source and union its keys with eq-service's remaining local additive keys, instead of expecting one flat consumer interface. Tested against real `origin/main` fixtures — happy path, a synthetic regression (renamed a required key — correctly caught), and a genuinely-missing interface (still fails loud) — before landing.
- [x] **[eq-context PR #129](https://github.com/eq-solutions/eq-context/pull/129) merged** (squash `65e720f`) — the workflow's own `pull_request` self-test passed against live data, not just local fixtures. Built and landed via an isolated worktree throughout; the shared main checkout stayed diverged and mid-write by a concurrent session for the whole task.
...
## eq-context: F10 (core.hooksPath drift) promoted rung 1 -> rung 4 (2026-08-05)
...
## eq-shell: Staff can now have a home address, and the compliance-pack export stops showing "Unknown" for names only ever edited in Shell — two PRs merged, live (2026-08-05)
...
## eq-field: new starters no longer sit on the live roster for weeks before they actually start — merged, live (2026-08-05)
...
## eq-shell + eq-field: EQ Field can now trigger a Shell-owned staff/supervisor write via entity-patch — both halves shipped, live (2026-08-05)
