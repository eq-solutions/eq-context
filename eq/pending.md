...
- [ ] **Substrate correction candidate, not yet made**: `eq/identity/IDENTITY-MODEL.md` states the `__personal__` ghost tenant was "retired — all users homed in their primary employer tenant." Live data contradicts this — 47 active `shell_control.user_tenant_memberships` rows are still on `__personal__`, and `eq_cards_auto_provision` (the function just fixed above) still actively provisions new sign-ups into it today. Spawned as a background task chip (`task_5c57a60e`) rather than fixed inline — needs its own investigation session to determine if this is stale data to clean up or if the retirement never actually finished. _(added 2026-07-31)_
