...
- [x] **All 3 gaps fixed, built and committed locally — not pushed, no PR, nothing deployed (2026-08-04).**
  - **eq-field** (branch `claude/ts-reminder-tenant-scoping`, commit `55e6e96`): `ts_reminders_sent` dedup/cooldown check was keyed only on `person_name`+`week`, no tenant filter — a real cross-tenant coupling bug (one tenant's same-named employee could suppress another's reminder), not a data leak. New migration `20260804_ts_reminders_sent_tenant_id.sql` adds `tenant_id`, backfills every existing row to SKS's id (the only tenant that has ever called this function), both queries in `ts-reminder/index.ts` now filtered. The audit's *other* eq-field finding — the global hourly send-cap having no tenant filter — was investigated directly and is **not** a bug: it's a deliberate anti-abuse control on an endpoint gated only by a display name; scoping it per-tenant would let an attacker exhaust the cap once per tenant instead of once total. Left untouched, commented. **Not run** — no Deno runtime available locally, no existing test coverage for this function; verified by re-reading the diff only.
  - **eq-solves-service** (branch `claude/tenant-defense-in-depth-audit-fixes`, commit `b126b42`): `send-pre-visit-brief.ts` now filters all 9 queries that have a `tenant_id` column (verified live against ehow's schema first, not guessed — `service.profiles` has no such column and was correctly left alone). `archive/helpers.ts`'s `countDependencies()` gained a required `tenantId` parameter + filter on its one service-role branch, threaded through both call sites. `tsc --noEmit` (full non-incremental) and `eslint` both clean on every touched file. **Not run** — no test coverage exists for either function.
  - Push/PR/deploy scope for all three is Royce's call, asked separately — none of this is live yet.
...
- [x] **eq-field [PR #645](https://github.com/eq-solutions/eq-field/pull/645) merged** — Toolbox Talk gained a "Reopen" button for a submitted talk (permission-gated to manager/supervisor, same as Submit), closing the "no way to fix a typo" gap the lock-on-submit fix left open. Deliberately leaves existing attendee signatures untouched — the confirm dialog says so, since detecting exactly what changed to know which signatures to invalidate is a bigger feature than this fix.
- [x] **Decided: leave the `eq` sandbox tenant's Supervision list empty.** Its whole staff table is 1 row — "mark supervisors" there would mean flagging that one disposable record, which Royce confirmed isn't worth doing.

**Deferred:**
- [ ] **3 real SKS people (John Angangan, Scott Hotson, Jack Cluff) are marked as supervisors in Shell but have no category set** — shows as a stray "Direct" group on the live Supervision page. Royce's call: fix it in Core directly (`core.eq.solutions/sks/staff`), not a database patch from Field — "we need to create synergy here." Still not done as of session close.
- [ ] **Shell's Staff form lets you tick "Supervisor" without requiring a category — flagged as a follow-up, now building.** Royce confirmed: flag it. Task spawned against eq-shell (`SplitPanel.tsx:292-307`) to require the category field once Supervisor is checked; Royce started it in its own session same day — in progress, not yet merged. Bundled in: a second, related polish item — the Company field (which labour-hire agency) shows for Direct employees too, where it's meaningless; same session asked to hide it when Employment type = Direct, bundled into the same eq-shell PR rather than opened separately. _(added 2026-08-04)_
