...
## eq-context: agentic-coding rules landed — three follow-ups the rules themselves opened (2026-08-04)
*New `rules/agentic-coding.md` + a hard rule that every service-role query must filter by tenant. Writing the rule is not the same as proving the code already obeys it — these are the gaps that stayed open.*
