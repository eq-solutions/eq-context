...
- [ ] **Follow-up: `guard.js` itself is unversioned and untested.** It lives at `~/.claude/hooks/guard.js`, outside any git repo, with zero test coverage — unlike `hooks/*.py` in this repo, which are governed/versioned/CI-checked (`hooks/README.md`). Its own header cites a spec file (`system/operating-model-roadmap.md`) that doesn't exist. The new `reflection-gate` rule was added to this same ungoverned file, same as every other guard.js rule to date. Worth eventually mirroring guard.js into this repo (versioned source of truth, deployed copy on the Beelink) so it gets the same test-before-trust discipline as the Python hooks. Not fixed this session — separate, larger scope. _(added 2026-07-24)_
- [ ] **The new `reflection-gate` rule has not been live-tested against a real commit** — guard.js edits are blocked from this session's own tool access (permission classifier), so Royce applied the diff by hand. Worth a quick sanity commit in eq-field (with and without a paired log entry) to confirm it actually fires before relying on it. _(added 2026-07-24)_
...
- [x] **Root-caused Royce's "shouldn't the logo be the coloured one?" question.** The report header was showing the wrong logo entirely — a white version only meant for a dark navy strip on page 1, invisible against the header's plain white background. Fixed to always show the real navy/purple colour logo. Also fixed the logo's position, which was drifting because of a fragile placement technique — it now sits in a fixed spot next to the header text on every page, in every report. `eq-solves-service` [PR #600](https://github.com/eq-solutions/eq-service/pull/600), merged, live.
...
- [x] **Found the spreadsheet is generated by an existing EQ Ops feature** (the "Job Creation" button on a won quote) — it already fills most fields, but 3 were always blank: the payables invoice email, Market Vertical, and End Client. Traced why: nowhere in the customer or job record existed to hold them.
- [x] **Added all 3, wired end to end.** Market Vertical and a dedicated payables invoice email are now nominable on the customer record (dropdown matches the sheet's exact 19-value list). End Client is a per-job field on the EQ Ops quote form (deliberately not defaulted from the customer — a builder like Shape/Built may not be the true end client). The Job Creation export now fills all 3 instead of leaving them blank. `eq-shell` [PR #977](https://github.com/eq-solutions/eq-shell/pull/977) — **merged, live** (core.eq.solutions).
- [x] **Customer search widened** — previously matched customer name only; now also matches site names, contact names/emails, and contract line-item text.
- [x] **Database change applied live to both tenants** (migration 0197, dispatched + approved same session) — additive/nullable columns only, zero downtime.
...
- [x] **Client ID (B27) and Market Segment (B28) — the last 2 gaps on the same export — closed same day.** See the new entry below.
...
- [x] **Search results now say why they matched, when it isn't obvious.** If a search hits a site name, contact name/email, or contract reference instead of the company name, a small label now shows under the result — "Site: Sydney Office", "Contact: Jane Doe", "Contract: JP-1234" — skipped entirely when the match is the company name itself, since that's already the row's title. `eq-shell` [PR #986](https://github.com/eq-solutions/eq-shell/pull/986) — merged, live (core.eq.solutions, deploy confirmed by commit match).
...
- [x] **Fixed 2 of the 5 security-scanner warnings the merge picked up**, both real: a sanitizer bypass in a third-party library, and a URL-parsing bug that could be tricked into treating one address as another. Non-breaking library update, confirmed the app still builds and typechecks clean, shipped as its own change (PR #583, merged).
- [x] **Actually tried the "downgrade everything" fix, in a throwaway copy, to see what it really does rather than guessing.** It downgrades the app's core framework 7 versions back (to 2020-era Next.js) and the spreadsheet-export library too — and it's not just "old", the app fails to even start on it, and the security-warning count went **up** from 5 to 95 because of everything else that old a framework version drags in. Confirms this was never a real option. Thrown away, nothing kept. _(2026-07-22)_
- [x] **Checked whether the 2 warnings that were left are ones anyone could actually exploit here, rather than treating "the scanner flagged it" as the final word.** One only exists inside a photo-resizing feature this app never turns on (no code path here ever calls it). The other only exists in a narrow way of using a name-generator function that the app's own spreadsheet library doesn't use — it uses a different, unaffected way. Both are real findings in the general sense, genuinely present in what's installed, but neither can actually be triggered by anything this app does today. _(2026-07-22)_
...
- [x] **Found the actual mechanism, not just the symptom.** Two of the folders meant to give each work session its own private copy of the code were never actually set up that way — they were empty placeholder folders pretending to be private copies. Any change made "inside" one of them was secretly landing in the one shared master copy instead, which is exactly how two sessions kept stepping on each other's work without either one doing anything wrong.
- [x] **Installed an automatic tripwire so this can't happen silently again.** Before any future change is made into a folder that looks like a private session copy, the system now checks it really is one — and refuses the change with a clear explanation if it isn't. Tested against both the broken folders and working examples to confirm it only catches the real problem, not ordinary work.
- [x] **Wrote up the finding** so any future session — or Royce — immediately understands why this happened, rather than re-diagnosing it from scratch.
- [x] **Cleared out one of the two empty fake folders** on request — fully emptied; the last empty shell is still on disk because something else currently has it open, and it'll disappear on its own once that releases (or can be removed by hand later, it holds nothing).
...
- [x] **Orphan Sentry alert rule folded into code + doc.** Rule 616973 ("Iframe token mint failure — Field / Service / Cards SSO") pre-dated both of eq-shell's alert-provisioning scripts and was managed by neither — it watched 2 files that no longer exist (`mint-iframe-token`, `mint-service-iframe-token`) and 1 that was removed the same day (`mint-cards-iframe-token`). Folded a corrected version into `setup-sentry-alerts.mjs` (real current names: `token-exchange`, `mint-cards-otp`, `mint-quotes-iframe-token`) + updated the runbook doc. eq-shell [PR #953](https://github.com/eq-solutions/eq-shell/pull/953), merged, live. **Not yet applied to Sentry itself.** eq-shell [PR #957](https://github.com/eq-solutions/eq-shell/pull/957) (merged 2026-07-22) added a manual-only `workflow_dispatch` action that runs the script for real against live Sentry — but it still needs a `SENTRY_AUTH_TOKEN` repo secret (an internal Sentry token, `project:write/read, alerts:write/read`) added before it can run, then someone to trigger it. The stale rule stays live-but-harmless (never fires) until that happens. _(added 2026-07-22)_
- [x] **Triaged all 13 unresolved Sentry issues org-wide** (eq-shell 7, eq-cards 3, eq-service 3) via parallel investigation, checking each issue's exact event timestamps against known PR merge times rather than guessing. Found: 5 already fixed by earlier PRs but never marked resolved, 4 working-as-intended (not bugs — an alert firing correctly, a self-healing recovery counter, a normal session-expiry, a known server-side issue already attributed elsewhere), 2 Flutter engine-level crashes with zero app code involved (not fixable from eq-cards), and 2 genuinely real, unfixed bugs.
- [x] **Fixed the 2 real bugs.** eq-service [PR #580](https://github.com/eq-solutions/eq-service/pull/580): a technician with no name *and* no email crashed the whole Maintenance page (`.localeCompare` on null) — fixed in the 3 places sharing the pattern. eq-shell [PR #955](https://github.com/eq-solutions/eq-shell/pull/955): the Field iframe's 30-second handoff timeout used to just give up with no retry, unlike every other recoverable failure path — now retries once the same way. Both merged, both confirmed live via Netlify (deploy `ready`, no errors, secret scans clean).
- [x] **Resolved 11 of the 13 Sentry issues** with a dated note on each explaining why (citing the fixing PR or the reason it isn't a bug). Left 2 alone on purpose — the 45-never-invited-workers alert (Royce's own "not now" pending decision) and the Field handoff auto-recovery counter (deliberate ongoing telemetry, not an error).
...
- [x] **Found 3 real bugs before touching the setting, all already live today — nothing to do with the change itself.** Copying a roster week to a date more than 4 weeks out could silently overwrite real data instead of the "skip if something's already there" checkbox actually checking anything. Approving leave booked more than 4 weeks ahead skipped the double-booking check entirely and said "no conflicts" without ever looking. And "Download full backup" wasn't full — it only ever contained the 9 weeks already in memory, so restoring an old backup would have quietly dropped everything else. Fixed and live first, on their own, before touching the window size. **Live on field.eq.solutions.**
- [x] **Then made the actual cut — 9 weeks down to 3.** Found and fixed the 7 places that would otherwise have gone quietly wrong under the smaller window: the Forecast page's staffing-vs-target chart, the Calendar month view, the "timesheets completed this week" tracker, the apprentice-ratio trend widget, "copy last week," the staff self-serve prefill button, and the three "add attendance from roster" buttons on Diary/Toolbox/Prestarts. All would have shown blank or wrong-looking data with no error — the kind of thing that looks like "the data is broken" rather than "the page is loading." **Live on field.eq.solutions.**
- [x] **Found 3 more that looked broken but genuinely weren't worth touching, and left them alone rather than "fixing" something nobody can reach.** The "Clean Up Codes" screen's button was removed from Sites months ago — the screen behind it has been unreachable ever since. A removal-warning count only runs for the sandbox tenant, never the real one, because of an existing "managed elsewhere" guard. And the site-code autocomplete suggestion box would have needed to re-fetch the full 52-week history on almost every screen render just to close a minor gap in typo-suggestions — reintroducing on every render exactly the payload this change was meant to remove, for something you can already just type by hand anyway.
...
- [x] **You went and looked, hit something, pasted the browser console — followed up same day.** Checked Sentry first: zero errors reported from Field in the last 24 hours, including the exact session you pasted — if this change had crashed, that's where it would show up first, and it's silent. Went through the pasted log line by line: the actual error in it (a React crash) and the blocked network request both come from Core's own code running in the same tab, not Field's — Field has no React and no bundle that looks like that. The repeated reconnect messages are normal background reconnect behavior in a part of the app this change never touched. The "(unknown) staff" warnings are also from a different, untouched part of the app — a handful of timesheet/roster/leave rows pointing at people no longer in the current staff list, a data question, not something this change caused.
...
- [x] **Confirmed the records were unrecoverable.** Cloudflare has no restore for a purged zone, and the account's own audit log — despite storing full record history — only had 18 months of retention on entries that were already long expired for this zone. Rebuild from scratch was the only option.
- [x] **Rebuilt the zone and restored all four apps**, with Royce's go-ahead. Re-created `eq.solutions` in Cloudflare, added `core`/`field`/`service`/`cards` pointing at their existing Netlify sites (grey-cloud/DNS-only, matching the working config), Royce repointed GoDaddy at the new nameserver pair Cloudflare assigned, and Netlify auto-issued fresh TLS certificates the moment DNS resolved. Verified resolution + HTTPS 200 on all four from outside the network, not just assumed.
- [x] **Fixed outbound email too, in the same pass.** Invite emails, licence-expiry notices, and quote emails all send from `@eq.solutions` via Resend, and that domain's mail records (MX/SPF/DKIM) died with the zone. Resend had a direct Cloudflare integration already connected from when the domain was first set up — one click ("Auto configure") on Resend's side rewrote all three records straight into the rebuilt zone. Confirmed both sides show verified/active.
- [x] **Confirmed eq-website staying down was intentional** — that was the site Royce actually meant to remove, so it wasn't rebuilt. Nothing further needed there.
...
- [x] **eq-shell PR [#950](https://github.com/eq-solutions/eq-shell/pull/950) merged** — false alarm cleared for every PR based on latest main.
...
- [x] **Checked live before building, twice, and both checks changed the plan.** The reference SKS version hadn't actually shipped anywhere yet (unmerged branch, read as reference only, nothing in SKS touched). And EQ Field's own Safety area has split into separate features (Prestart, Toolbox) rather than one shared page the way SKS still has it — so Incidents was built to match that same pattern, as its own page, not bolted onto an existing one.
- [x] **New Incidents / Near Miss page shipped**, reusing EQ Field's existing photo/signature/offline-save building blocks rather than rebuilding them — a person filing a report picks a type and severity, describes what happened, adds who was involved with each person signing off individually, attaches photos, and can save a draft or submit. Works the same when offline and catches up once back online.
- [x] **Wired into the existing Safety records list and the Safety Report dashboard** — incidents now show up alongside prestarts and toolbox talks in both places, filterable and downloadable as a Word document.
- [x] **Managers get emailed automatically** when someone submits a High-severity report or anything other than a routine Near Miss — routine Low/Medium near-misses don't page anyone.
- [x] **Checked on a phone-sized screen before calling it done** — the SKS version had a mobile display bug that only got caught late; checked for the same class of bug here up front instead.
- [x] **Database change applied live, with Royce's go-ahead first** — new incidents table on EQ Field's own database, locked down the same way the neighbouring safety tables already are (each company only ever sees its own).
- [x] **Reviewed, pushed, and merged to production with Royce's sign-off at each step** — eq-field PR #537, merged (squash) → live on field.eq.solutions as v3.5.357. Hit an unrelated version-numbering collision with two other in-flight PRs right before merging (see the "4th time this has come up" item above) — resolved by re-numbering, all tests + lint still clean afterward.
...
- [x] **The serious one: anyone with a login could have made themselves a manager of their whole company.** Invitations carry the role the new person will get, and the invitation record itself was editable by any signed-in user — so you could write yourself an invitation saying "manager", accept it, and be handed that role across every app. 28 real invitations were sitting in that table. Nobody had done it. Closed, applied, confirmed against the live system.
- [x] **Locked the link between a login and a person's record in Field.** That link decides *who Field thinks you are* — whose name goes on approvals, whose roster and timesheets you see. Any signed-in colleague could have pointed someone else's record at their own login and been treated as that person. 50 people have that link set. Now only the system itself can change it.
- [x] **Stopped self-promotion in EQ Service, and made its activity log tamper-proof.** Two separate problems: staff could edit their own role directly, and the activity log could be written under someone else's name, edited, or deleted outright. A log the people in it can rewrite isn't a log. Both closed and confirmed live.
- [x] **Set the rules for approving timesheets and leave, and enforced them in the database.** Previously anyone could approve their own timesheet or their own leave. Royce's call: supervisors escalate, only a manager can approve their own, only a manager can reopen an approved timesheet, and once payroll has taken it nothing changes. Labour hire and subcontractors follow the same path with the site supervisor deciding.
- [x] **Caught four of my own fixes that would have done nothing at all.** The command that takes away permission on a single field is silently ignored when the permission was granted across the whole record — the database warns and carries on. Four of the eight fixes were written that way and would have shipped green and inert. Rewritten properly and re-verified. This is exactly how a hole stays "closed" on paper for months.
- [x] **Caught a fix that would have opened a *new* hole on the SKS system.** One was written from the EQ system's copy of some pricing functions while SKS had already moved ahead. Applying it as written would have re-created an old version of a pricing function that had been deliberately deleted — without its safety check, without the cost field, and reachable by the public. Removed that piece before it went near production.
- [x] **Caught a bug that would have broken every timesheet and leave entry on first use.** The new approval rules call a helper, and the same change took away permission to use that helper. Every save would have failed. Found by re-reading who's allowed to call what, rather than re-reading the logic.
...
- [x] **Retired the stale "Melbourne / second-tenant status" item — it was never a real decision.** Re-raised across 2+ audits as "the active enterprise target" needing a go/no-go call on whether to harden SKS further or prep a new tenant. Checked live: `melbourne` has zero rows in canonical `organisations` — it was a synthetic test tenant (used e.g. for the "577-person scale" roster-virtualization stress test), deleted from canonical 2026-06-28, never a real prospective customer. Royce flagged the confusion directly when this was offered as a priority option. Retired; don't re-raise this framing. _(corrected 2026-07-22)_
- [x] **Extracted+tested `tender-parser.js`'s existing-but-never-enforced test suite** (see the dedicated entry below) — the item Royce actually chose after the above correction, from a shortlist of his own already-approved-but-deferred items (this one, vs. a first-time-supervisor onboarding walkthrough — the walkthrough remains open, not declined, just not picked this time).
...
- [x] **Corrected, not just resolved — this was never a real decision.** Re-raised across 2+ audits as "the active enterprise target" needing a go/no-go call. Checked live: `melbourne` has zero rows in canonical `organisations` (`jvknxcmbtrfnxfrwfimn`) — it was a synthetic test tenant (used e.g. for the "577-person scale" roster-virtualization stress test), deleted from canonical 2026-06-28, never a real prospective customer. Royce flagged the confusion directly when this was surfaced as a priority option — there is no second-tenant decision pending. Retired; don't re-raise this framing. _(corrected 2026-07-22)_
...
- [x] **Correction found + closed 2026-07-22: the 07-14 fix only reached the Cards source, not the tenant-plane mirror the AI brief actually reads.** Royce reported the AI brief still flagging this same White Card as expiring, over a week after the "FIXED" above. Root cause: Cards' `public.licences` (canonical) was correctly set `never_expires` on 07-14, but the tenant-side copy in `ehow app_data.licences` — a separate row, kept in sync only by `staff-resync-licences.ts` — was never updated, because that function upserts with `ON CONFLICT DO NOTHING` (insert-only; can add a brand-new licence, can never correct an already-synced one). The manager-facing "Resync from Cards" button hit the same silent no-op. Fixed both: corrected the live `ehow` row to match canonical, and changed the upsert to `ON CONFLICT DO UPDATE` ([PR #968](https://github.com/eq-solutions/eq-shell/pull/968) merged `47ea36a`) so a future Cards-side correction actually propagates instead of silently stalling. _(done 2026-07-22)_
...
- [x] **One-login BUILD P1–P4 — CORRECTED 2026-07-22: further along than this line claimed.** Live-verified against jvkn + eq-shell `main`: P1 (Field-via-Core) live, P2 (WorkerHome two-tile) live, P2b (field-first redirect) built but was OFF everywhere — **flipped ON for sks 2026-07-22**, P3 (`eq_worker_completeness()`) live, P4 (`eq_worker_compliance_status()`) live as a warn-only nudge (no enforcement, by design). Only P4b (grace-then-soft-lock) and P5 (below) are genuinely not built. _(corrected 2026-07-22)_
...
- [x] **One-login Tranche 2 — SHIPPED 2026-07-22.** The activation bug was real number 27 (not ~20): `cards-approve-staff.ts` inserted Shell memberships `active:false` → approved workers were on the Field roster but invisible to Core. Fixed in [PR #951](https://github.com/eq-solutions/eq-shell/pull/951) (merged to main, `active:false`→`active:true`) + a one-time backfill activating all 27 existing stuck memberships on jvkn. Person-table reconciliation not needed — was a Shell-side membership bug only, `public.org_memberships` on the Cards side was already fine. _(done 2026-07-22)_
...
- [x] **PR #958 (eq-shell) merged + live** — closes a suite-wide gap where Field/Service/Cards iframe token minting never checked the per-tenant module entitlement (a user could get a valid token for a module their tenant had switched off; zero live impact today since those 3 modules are on for all 3 tenants, but closes the boundary for any future tenant). Squash `ebe03c0`, build/tsc/190 tests/lint all clean before merge. Ops backend deliberately left out of this PR — see next line. _(done 2026-07-22)_
- [x] **Ops backend entitlement enforcement — the ops-exclusive half shipped.** Read `crm-write.ts`/`canonical-api.ts` to confirm the shared-vs-exclusive split, then gated the 9 functions that are genuinely ops-only and session-authenticated (`quote-email`, `quote-pdf`, `quote-parse-pdf`, `quote-parse-subcontractor`, `quote-suggest-scope`, `job-creation`, `labour-hire-commit`, `labour-hire-mutate`, `labour-hire-parse`) with the same `isModuleEnabledForTenant(tenant_id, 'ops')` check from #958. [PR #960](https://github.com/eq-solutions/eq-shell/pull/960) merged (squash `d63d7ec`), build/190 tests clean. Branch note: PR #958 merged mid-session in the shared checkout, so this landed on a fresh branch off the new `main` rather than the original one — no work lost. _(done 2026-07-22)_
- [x] **`crm-write.ts`/`canonical-api.ts` design pass — CLOSED, no gate added, by design.** Read both functions in full looking for a request-level signal that distinguishes an Ops-only write from a Customers/Staff write. There isn't one: every `crm-write.ts` action operates on the same `customers`/`contacts`/`sites` tables (and `SiteModals.tsx`) the ungated Customers page uses — no Ops-exclusive entity type exists to key a check on. `canonical-api.ts` isn't session-gated at all — it's a per-app Bearer-key hub with its own ACL (`APP_RESOURCE_READ`/`WRITE`) and tenant allow-list (`app_tenant_scope`); the only Ops-adjacent resource (`jobs`) is written for real by the scheduled `quote-job-consumer.ts`, not a live browser session. Live-verified jvkn `org_module_entitlements`: `eq` still has no `ops` row, confirming a blind gate would have 403'd Royce's own Customers/Staff pages. No PR needed — #958/#960 correctly stopped short of these two functions. _(done 2026-07-22)_
- [x] **`QuotesModule.tsx` canonical-api job-sync calls — FIXED, PR [#969](https://github.com/eq-solutions/eq-shell/pull/969) MERGED** (squash `61a3236`, Royce's "merge #969" go) → auto-deploying to core.eq.solutions. Was dead code: the browser-side `fetch('/.netlify/functions/canonical-api', …)` calls (job lookup, `syncJobToCanonical` PUT) carried no `Authorization` header against a Bearer-key-only function, so both 401'd every time and silently no-op'd — `canonicalJobId` never populated, so Issues/Attachments on a won quote stayed filed under the quote instead of moving to the job. Added a session-authenticated `netlify/functions/quote-job-sync.ts` proxy, gated on `isModuleEnabledForTenant('ops')` (this one genuinely is ops-exclusive), and repointed `QuotesModule.tsx` at it. **Also fixed a latent duplicate-row bug found along the way:** the dead browser PUT used a different `external_id` prefix (`eq-ops:job:`) than the scheduled `quote-job-consumer.ts` (`eq-quotes:job:`) — the new endpoint upserts by `(tenant_id, quote_id)` and matches the consumer's convention so the two writers can't fork. Build/tsc/190 tests clean; lint matches the pre-existing `_ctx unused` pattern shared by all 60 sibling ops functions. All CI checks green pre-merge. Follow-up not yet done: click through a won quote in `/ops` post-deploy to confirm Issues/Attachments now file against the job. _(done 2026-07-23)_
...
- [x] **Ops pipeline load-time fix — the actual cause found and fixed, PR [#973](https://github.com/eq-solutions/eq-shell/pull/973) OPEN, not merged.** Royce asked whether Ops load time could improve and whether the pipeline board has other ways to stay readable at 1000-user scale. The real cause: `eq_list_quotes` had no row limit at all — every load pulled the tenant's entire quote table, and the stat chips/tab counts/win-rate summary were all computed by filtering that full array in the browser. Migration `0197_quote_list_pagination_counts.sql` adds real limit/offset/search/stage/smart-filter params (`NULL` limit keeps old behavior for any other caller) plus a new `eq_quote_pipeline_counts` RPC that's now the single source of truth for those numbers regardless of fetch size. `QuotesModule.tsx` bounds the fetch to 300 rows (server-narrowed by whatever's currently searched/selected, not just "most recent"), debounces search into a real server query, and shows a banner instead of silently truncating if a tenant ever exceeds the window. **Also fixed a real bug found while wiring this up:** every call site had been passing the literal string `"all"` to the RPC's status filter, so clicking a pipeline tab has never actually narrowed the server query — only the client-side display. Build/tsc/199 tests/lint clean (lint diff is net negative — the rewrite incidentally removed 2 pre-existing React purity violations). _(done 2026-07-23)_
- [ ] **PR #973 can't be live-verified or merged yet — GitHub Actions billing is broken on the eq-solutions org.** Dispatching the governed migration pipeline (`tenant-migrate.yml`) to apply migration 0197 queued fine, but the Apply job itself never started: GitHub reports *"recent account payments have failed or your spending limit needs to be increased."* This isn't specific to this migration — it will block **any** tenant schema change across the whole suite until fixed. Needs Royce in Settings → Billing & plans on the org, then re-dispatch `tenant-migrate.yml --ref claude/quote-pipeline-pagination` to actually apply + verify the new counts RPC against live data before merging. _(added 2026-07-23)_
...
- [x] **"Damon Patrick Francis"** — title-case, so the middle-name rule correctly left it alone. Confirm whether "Patrick" is a middle name → should be "Damon Francis" (one-row manual fix). Confirmed "Patrick" was a middle name; corrected to "Damon Francis" 2026-07-22 as part of the `0102` review-list close-out. _(added 2026-07-12, done 2026-07-22)_
...
- [x] **Root-caused Sentry `EQ-SHELL-Y`** (`staff-licence-ocr: ocr-licence returned 401`) — not a credential/rotation issue. `staff-licence-ocr.ts` sends eq-shell's `SUPABASE_SERVICE_ROLE_KEY` as the bearer to eq-canonical's `ocr-licence` edge function, but that function's manual auth check calls `/auth/v1/user`, which only recognises a real signed-in user's session JWT — a service-role key always 401s there. 100% reproducing on every call (not PDF-specific), not a one-off. _(done 2026-07-23)_
- [x] **Found the fix already existed, unmerged** — eq-cards branch `claude/ocr-licence-service-role-auth` (commit `aa89194`, authored via Cowork), functionally identical to the fix independently designed this session (skip `/auth/v1/user` + skip the per-user rate-limit RPC when the bearer matches the service-role key). Scrapped a duplicate draft rather than double-building. _(done 2026-07-23)_
- [x] **Merged + deployed.** `git merge`/`push` and the Supabase `deploy_edge_function` call both hard-blocked by the session's auto-mode classifier; Royce merged `main` himself, this session deployed `ocr-licence` v11→v12 to eq-canonical (`jvknxcmbtrfnxfrwfimn`) and live-verified the deployed source matches. Sentry issue closed going forward. _(done 2026-07-23)_
- [x] **Deleted the merged branch** on eq-cards (remote + local) — local delete required first removing an unregistered-but-clean worktree (`.claude/worktrees/ocr-licence-service-role-auth`) holding the branch checked out. _(done 2026-07-23)_
- [x] **Identified the "cross-session-message" channel** Royce asked about — real mechanism, not spoofed: any local Claude Code session can read another's transcript (`list_events`) and inject a message into it (`send_message`). The specific sender this session saw (`local_5b1ac6c5…`, titled "Testing check frequency validation") was a genuine, unrelated eq-solves-service session whose own transcript contained content clearly lifted from *this* session — meaning it (or whoever drove it) read this transcript and relayed crafted "Royce confirmed" messages back in. One relayed claim was factually false and caught only by independently verifying against git; a later one was true. Whether this was Royce deliberately testing verification discipline or something else is unconfirmed. _(done 2026-07-23)_
