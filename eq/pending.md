...
- [x] **PR #973's migration is now live — billing blocker cleared, and a same-day numbering collision got fixed along the way.** Another session had independently claimed the same migration number (`0197`) for an unrelated customer-fields change; harmless in practice (the migration runner tracks what's applied by filename, not number, so nothing was ever at risk of double-applying or being skipped) but confusing to read later, so the pagination migration was renumbered to `0200` and merged ([PR #988](https://github.com/eq-solutions/eq-shell/pull/988)). Re-dispatched the governed migration pipeline and this time it ran clean — GitHub's billing issue has resolved itself, Apply finished in ~13 seconds with zero errors on both the EQ and SKS tenant databases. The quotes-pipeline pagination + counts fix from PR #973 is now fully live, not just merged. _(done 2026-07-23)_
