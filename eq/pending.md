...
- [x] **All 5 PRs merged (#416/#417/#418 eq-service, #618/#620 eq-shell); both pending dispatches fired** — eq-service `apply-service-migrations.yml` (lands 0170 through the now-fixed runner) and eq-shell `tenant-migrate.yml` (fleet-wide, lands 0157's field_managers/nomination_clashes fix — safe no-op on every plane except ehow). Both paused at the production-environment approval gate. _(done 2026-07-03, awaiting your approval click on both)_
