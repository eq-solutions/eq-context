...
- [ ] **Confirm `GOTENBERG_URL` is absent from the live `eq-service` Netlify env** (site `6af7bce6…`, service.eq.solutions). Couldn't verify via MCP (Netlify MCP exposes projects/forms/user only, not env vars). Near-certainly already unset — 2+ months of clean DOCX-only reports prove it (`pdf-conversion.ts` would *throw*, not skip, if it were set against the now-dead host). 15-sec manual check: Netlify → eq-service → Environment variables → delete if present. _(added 2026-07-04, needs Royce's call)_
