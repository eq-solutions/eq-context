...
- **D2 — job_plan linkage → `jp_code` TEXT ONLY.** No canonical FK; Service resolves to its local `job_plans` at read time. (job_plan stays Service-local.)
  > **UPDATE 2026-07-02 (live audit):** the *linkage* half of D2 still holds — `contract_scopes.jp_code` is TEXT and the commercial-sheet importer matches on it. But the *storage* half ("job_plan stays Service-local") is **SUPERSEDED**: `job_plans`/`job_plan_items` went canonical with the maintenance-workflow canonicalization (full `service.*` view + INSTEAD OF trigger set over `app_data.*`, 56/667 rows). This is safe and transparent to the app — writes route to `app_data` via the triggers. So: job_plans IS canonical **and** scope→plan linkage is still jp_code text. Line 125 ("job_plans is NOT canonical") is likewise superseded. No code change needed.
