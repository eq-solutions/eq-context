...
| `sprints/2026-08-23-security-outstanding-triage.md` | Triage of the 2026-08-23 §C-G security sweep's leftovers into a 3-item Royce-only runbook: SEC-63 shared-JWT-secret check (do first, changes SEC-61's urgency), SEC-61 per-site `dev`-context secret cleanup (delete-only, don't recreate), SEC-57 `grok-by-xai` org-wide write/admin access — intentional-or-trim decision. Plus a shipped-this-sweep reference table for everything already closed (SEC-44/41/42/50/46/47/45/51) |
| `sprints/2026-08-26-organisations-anon-read-hardening.md` | Sprint hardening after the 3rd `organisations_anon_bootstrap_read` regression (eq-shell) — sequences in-flight work against what's genuinely still unbuilt |
