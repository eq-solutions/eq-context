...
- [eq/pending.md](https://raw.githubusercontent.com/eq-solutions/eq-context/main/eq/pending.md) — EQ-only to-do list index; split 2026-08-17 into `eq/pending/<repo>.md` (one file per repo) — this file just links to them now
...
| `changelog/*.md` | Per-product changelogs. **4 dead-twin pairs physically archived 2026-08-15** — `shell.md`/`service.md`/`cards.md`/`eq-field.md` each already carried a "Superseded"/"RETIRED" banner (dated 2026-07-19 and earlier) but had never actually been moved out of the live folder; now at `archive/changelog-*-dead-twin.md`, `superseded_by:` frontmatter added. Live folder is 10 files: `eq-cards.md`, `field.md`, `eq-shell.md`, `eq-context.md`, `eq-intake.md`, `eq-roles.md`, `eq-ui.md`, `eq-receipts.md`, and the one genuinely unresolved pair below. **⚠ `eq-service.md` and `eq-solves-service.md` are BOTH live** — appended on the same day (2026-08-14) by different sessions, non-overlapping PRs, and until 2026-08-15 neither referenced the other. Read both; neither is complete alone. Consolidating them is a call about which product history survives, so it is flagged for Royce rather than done silently — both files now carry a mutual "UNRECONCILED PAIR" warning (see the head of either file) so the gap can't go silent again. Cost of the silence while it lasted: PR #727 sat recorded in `eq-service.md` as "open, holds for Royce" for a day after it had merged. `scripts/changelog_duplicates.py` now gates every PR on this: any changelog duplicate that isn't self-marked (`superseded_by:` or `UNRECONCILED PAIR`) fails CI, so a sixth silent pair can't happen the same way. |
