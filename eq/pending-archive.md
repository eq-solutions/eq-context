...
- [x] eq-service [branch `fix/site-credentials-service-schema`](https://github.com/eq-solutions/eq-service/tree/fix/site-credentials-service-schema) (`c09d192`) pushed — not yet merged to `main`, since this repo auto-deploys on merge and the code side of this fix (the migration + script) needed no app-code changes to ship the DB fix; merging is a separate call whenever Royce wants the branch closed out.
...
**Deferred:** merging `fix/site-credentials-service-schema` to `main` — no app code changed, so there's no live-behavior reason to merge promptly; whenever Royce wants the branch closed out.
