...
- [x] **doc_type sign-off guard, both write paths — MERGED.** `NO_SIGNOFF_DOC_TYPES` (template/om) skipped Push in the UI and was filtered from the Register, but neither path that actually creates a `document_signoffs` row ever checked it server-side. Built independently by two concurrent sessions from opposite directions — reconciled rather than duplicated: `entity-patch.ts`'s onboarding auto-push guard shipped first as eq-shell [PR #1474](https://github.com/eq-solutions/eq-shell/pull/1474) (`c678f1e1`); a second PR was rebased onto it (its now-redundant `entity-patch.ts` hunk dropped, confirmed byte-identical; its distinct `push-document-audience.ts` `handlePush` guard kept), squash-merged as [PR #1475](https://github.com/eq-solutions/eq-shell/pull/1475) (`8d864327`). Both confirmed live via exact `commit_ref` match.
- [x] **Brian Griffin-Colls's vanished 12 rows — RESOLVED, not a mystery.** A concurrent session's cross-session sibling: Royce reported the *symptom* (duplicate-looking rows on the Documents admin page) independently of this audit thread, tracing to the same root cause from the live-bug-report side — the `DB Schedules`/`Comms` categories' `is_onboarding_bundle` flag fired on every new starter with no `doc_type` check, so Brian, William Hong, and Sonam Gurung all got phantom template-document signoffs (44 rows total, 12 of them Brian's). Royce approved un-flagging both categories and deleting all 44 stray rows — verified `status='outstanding'` + `signed_at IS NULL` on every one first, so nothing signed was touched.
- [x] **Origin-guard on `document-version-upload-init.ts` — MERGED.** Its 3 sibling upload endpoints all imported `checkShellOrigin`; this one shipped later (2026-08-13, after the guard's own rollout) and never got it. Same guard, same call shape, same place in the handler as every sibling. eq-shell [PR #1478](https://github.com/eq-solutions/eq-shell/pull/1478), merged (`2b269ca3`), confirmed live.
- [x] **Reminder-cron reliability pass — MERGED.** Cadence is now `due_at`-based when a row has one (onboarding auto-push sets it; manual push still doesn't, so both cases are handled) — first nudge within 3 days of the deadline instead of a flat week after assignment regardless of urgency, closing the exact "60 days vs tomorrow get the same nudge" case the audit named (now a regression test, 11 new cases). A failed send now backs off on the same 7-day cadence instead of retrying every single day forever. `reminder_count`/`last_reminded_at` (migration `0253`) now surface in the Register tab and the sign-off evidence modal. eq-shell [PR #1481](https://github.com/eq-solutions/eq-shell/pull/1481), merged (`1d8a2332`), confirmed live. **Deliberately did NOT swap to `signed_content_hash`** despite the audit naming it — the live frontend already has a comment correctly explaining why it's redundant under this feature's `is_current_version=true` scoping (document versions are immutable, so the two hashes are guaranteed equal for any row either handler can return); the audit's finding on this specific point didn't hold up against the live design, confirmed before writing any migration.
...
## eq-shell: 21 CRM/staff database functions only checked which tenant you were in, not who you were — closed, merged, live (2026-08-15) (rotated 2026-08-18 — open items remain in eq-shell.md)
...
## eq-shell: switching someone off didn't actually stop them — closed at both ends, live (2026-08-15) (rotated 2026-08-18 — open items remain in eq-shell.md)
...
## eq-shell: two Sentry monitors kept re-firing after being "fixed" — the earlier fix had never actually landed (2026-08-15) (rotated 2026-08-18)
...
## eq-shell: load speed — Suppliers and Staff were slow; database wasn't the cause (2026-08-15) (rotated 2026-08-18 — open items remain in eq-shell.md)
...
## eq-shell: account-enumeration hole closed on the phone+PIN sign-in door (2026-08-15) (rotated 2026-08-18)

---

## eq-shell: Mobile Home redesign — compliance card collapsed, Suppliers + Compliance report quick links added (2026-08-14) (rotated 2026-08-18 — open items remain in eq-shell.md)
...
## eq-shell: Staff list — apprentice year badge + Trade multi-select shipped, text[] conversion blocked on eq-field coordination (2026-08-14) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-shell: Tom's licence-upload timeout root-caused for real — Shell's admin path was sending full-res photos, unlike Cards (2026-08-14) (rotated 2026-08-18 — open items remain in eq-shell.md)
...
## eq-shell: hard-delete for archived user accounts — built, merged, live (2026-08-14) (rotated 2026-08-18 — open items remain in eq-shell.md)
...
## eq-shell: Shell Conversations built end-to-end — logging, permission-locked, resourcing dashboard, draft org chart, team assignment (2026-08-11 → 2026-08-13) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-shell: self-join bulk-approve + gap-analysis-driven onboarding fixes (2026-08-06) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-shell: EQ-SHELL-R closed (false alarm) + EQ-SHELL-1B fixed — Outlook email attachments on quotes, merged + live (2026-08-06) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-shell: EQ-SHELL-1A "eq-ops rpc ... failed: TypeError: Failed to fetch (ehow)" — durable fix live, all known consumers migrated (2026-08-06) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-shell: self-join's "double sign-in" for Cards root-caused and fixed — worker-add nav trimmed further too (2026-08-03) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-shell: fixed 8 pre-existing react-hooks/refs eslint errors in the iframe pre-warm keeper (2026-08-03) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-shell: Richard Brown's mobile crash fixed, then a simplified mobile nav for supervisors driven by real usage data (2026-07-31) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-shell: EQ Ops quote status → job status sync fixed for all 5 stages, plus a new "Target period" badge for future-dated quotes (2026-07-31) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-shell: Compliance-roster-only workers — Field access can now be switched off per worker (2026-07-30) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-shell: Staff page edits silently reverting overnight — root-caused and fixed, deployed (2026-07-28) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-shell: EQ Ops quote-detail panel simplified for real-world use, then the Coupa PO import tool rebuilt from scratch against the real export (2026-07-23 → 2026-07-24) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## Core dashboard rebuilt — replaced the passive AI-brief-only home with three permission-gated live signal bands (2026-07-17, MERGED + LIVE) (rotated 2026-08-18 — open items remain in eq-shell.md)


---

## eq-cards: CI silently never deployed edge functions — found after a live fix didn't ship, closed for good (2026-08-14) (rotated 2026-08-18)
*Discovered when PR #238's ocr-licence timeout fix merged, `Build & Deploy` reported success, but the live function on jvkn kept 504ing anyway — the workflow only ever built and deployed the Flutter web app; nothing under `supabase/functions/` was ever touched by CI. Had been silently true the whole time; today's fix was deployed by hand as a stopgap while this was built.*
...
## eq-cards: invite-claim IDOR fixed, 3-day claim outage found + fixed, stale-invite cleanup + guard shipped (2026-08-14) (rotated 2026-08-18)
*A live code audit found `eq_cards_claim_invite` only checked that the caller was signed in — never that their verified phone matched the invite's target phone, letting one worker claim a colleague's invite by looking up their phone number. Verified against the live database before writing the fix. While preparing it, found something bigger already broken in production.*
...
## eq-cards: Shell tenant auto-login bug root-caused and fixed — deployed live, needs your click-through (2026-08-04) (rotated 2026-08-18 — open items remain in eq-cards.md)


---

## eq-field: Dashboard map → own page, Map hover shows names, cache-buster hotfix (2026-08-14) (rotated 2026-08-18 — open items remain in eq-field.md)


---

## eq-field: Leave notification gaps closed, digest widened to 4 weeks, Email Templates pilot shipped (2026-08-14) (rotated 2026-08-18 — open items remain in eq-field.md)


---

## eq-field: staff resource management (skills/reviews) — built, deployed, migration applied live (2026-08-11) (rotated 2026-08-18 — open items remain in eq-field.md)


---

## EQ Field: real Incidents / Near Miss reporting, shipped and live (2026-07-22) (rotated 2026-08-18 — open items remain in eq-field.md)
...
## eq-solves-service: NSX Test Report fixed — dead ACB-only fields left it always printing blank sections (2026-08-14) (rotated 2026-08-18)
*Simon Bramall reported an ACB check's report came out empty in some sections. ACB's report wiring traced clean end-to-end (every field has a real collection path). The NSX Test Report, though, still carried template rows copy-pasted from ACB that the NSX workflow never actually collects.*
...
## eq-solves-service: /admin/* pages closed to non-managers (2026-08-14) (rotated 2026-08-18)
*Royce: gate the remaining Admin pages that were reachable by any signed-in technician who typed the URL directly — the sidebar hid the link, but that's not access control. A prior sweep (PRs #707-#727) had already closed 3 of 9 admin pages; this closed the rest.*
...
## eq-service: migrations dispatched live; mobile check-detail header overflow found+fixed+deployed; eq-context accidental-checkout scare investigated (2026-08-13) (rotated 2026-08-18 — open items remain in eq-solves-service.md)


---

## eq-solves-service: fixed a broken safety check that was silently skipping every code review, then found the "176,000 findings" it surfaced was almost entirely noise, cleaned up what was real (2026-08-01) (rotated 2026-08-18 — open items remain in eq-solves-service.md)


---

## eq-solves-service: Found why photo uploads were failing everywhere, then added a link/create/skip option to the paste-import flow (2026-07-31) (rotated 2026-08-18 — open items remain in eq-solves-service.md)


---

## eq-solves-service: Field Run-Sheet asset headers now show the maintenance plan's Job Code (2026-07-29) (rotated 2026-08-18 — open items remain in eq-solves-service.md)


---

## Substrate: a wrong note said Shell changes don't go live when you merge — corrected everywhere (2026-08-15) (rotated 2026-08-18 — open items remain in eq-context.md)
...
## eq-context: proper re-score against the 2026-07-20 audit found real gaps outside the campaign's scope (2026-08-15) (rotated 2026-08-18)
...
## eq-cards + eq-shell: changing your mobile number used to split you into two accounts — fixed, and a second way in shipped (2026-08-15) (rotated 2026-08-18 — open items remain in cross-repo.md)
...
## eq-cards + eq-shell: audited every sign-in door, retired the two that weren't real — both live (2026-08-15) (rotated 2026-08-18 — open items remain in cross-repo.md)
...
## eq-shell/eq-field: deactivating someone didn't actually cut their EQ Field access — fixed + 2 follow-ups (2026-08-14) (rotated 2026-08-18 — open items remain in cross-repo.md)


---

## eq-shell / eq-cards: suite-wide Sentry sweep, identity-collision root cause fixed, 2 bugs shipped (2026-08-14) (rotated 2026-08-18 — open items remain in cross-repo.md)
...
## eq-cards + eq-shell: labour-hire licence intake — multi-document OCR extraction + PDF review + flag notifications, all merged + live (2026-08-13) (rotated 2026-08-18 — open items remain in cross-repo.md)


---

## eq-service + eq-solves-intake: RCD in-app entry (manual + photo) shipped, ACB mobile nav bug fixed, RCD threshold corrected (2026-08-11) (rotated 2026-08-18 — open items remain in cross-repo.md)


---

## eq-solves-intake + eq-shell: duplicate-site console's two dead ends fixed, then a live permission bug found and fixed mid-testing (2026-08-01) (rotated 2026-08-18 — open items remain in cross-repo.md)


---

## eq-shell + eq-solves-intake + eq-receipts: closed every open security alert across the EQ suite, found 5 repos where the alert system was switched off entirely (2026-08-01) (rotated 2026-08-18 — open items remain in cross-repo.md)


---

## eq-intake + eq-shell: 4-part fix from Royce's live screenshot review of the Intake console (2026-07-31) (rotated 2026-08-18 — open items remain in cross-repo.md)
...
## eq-shell: Documents module had no permission gate at all — found, fixed, merged, live (2026-08-16) (rotated 2026-08-19)
...
## eq-shell: two staff pages could be reached from any linked company site, not just the main one — found, fixed, merged, live (2026-08-16) (rotated 2026-08-19 — open items remain in eq-shell.md)
...
## eq-shell: a way to hand yourself full admin power through the "custom access groups" screen — found, fixed, and live (2026-08-16) (rotated 2026-08-19)
...
## eq-shell: checked whether a worker-protection feature exists on both company databases or just one — confirmed it's a deliberate difference, not something missed (2026-08-16) (rotated 2026-08-19 — open items remain in eq-shell.md)
...
## eq-shell: 4 places were showing worker or contact details to people who shouldn't see them — fixed, PR open, waiting on your go to ship (2026-08-16) (rotated 2026-08-19 — open items remain in eq-shell.md)
...
## eq-shell: an AI tool anyone signed in could use to run up costs on the company's AI account — closed, merged, live (2026-08-16) (rotated 2026-08-19)

---

## eq-shell: workers can now be asked for different required licences depending on their role, not one shared list for everyone — closed, merged, live (2026-08-16) (rotated 2026-08-19)
...
## eq-shell: 21 CRM/staff database functions only checked which tenant you were in, not who you were — closed, merged, live (2026-08-15) (rotated 2026-08-19 — open items remain in eq-shell.md)
...
## eq-shell: switching someone off didn't actually stop them — closed at both ends, live (2026-08-15) (rotated 2026-08-19 — open items remain in eq-shell.md)
...
## eq-shell: two Sentry monitors kept re-firing after being "fixed" — the earlier fix had never actually landed (2026-08-15) (rotated 2026-08-19)
*Royce asked what's next after the load-speed work below; a `/decide` pass first wrongly suggested two P0 security findings that turn out to be standing "not now" decisions already on record — corrected before acting. The real next thing was two Sentry alerts for Richard Brown's duplicate-account incident, supposedly closed the day before, firing again.*
...
## eq-shell: load speed — Suppliers and Staff were slow; database wasn't the cause (2026-08-15) (rotated 2026-08-19 — open items remain in eq-shell.md)
...
## eq-shell: account-enumeration hole closed on the phone+PIN sign-in door (2026-08-15) (rotated 2026-08-19)

---

## eq-shell: Mobile Home redesign — compliance card collapsed, Suppliers + Compliance report quick links added (2026-08-14) (rotated 2026-08-19 — open items remain in eq-shell.md)
...
## eq-shell: Staff list — apprentice year badge + Trade multi-select shipped, text[] conversion blocked on eq-field coordination (2026-08-14) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: Tom's licence-upload timeout root-caused for real — Shell's admin path was sending full-res photos, unlike Cards (2026-08-14) (rotated 2026-08-19 — open items remain in eq-shell.md)
...
## eq-shell: hard-delete for archived user accounts — built, merged, live (2026-08-14) (rotated 2026-08-19 — open items remain in eq-shell.md)
...
## eq-shell: Shell Conversations built end-to-end — logging, permission-locked, resourcing dashboard, draft org chart, team assignment (2026-08-11 → 2026-08-13) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: self-join bulk-approve + gap-analysis-driven onboarding fixes (2026-08-06) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: EQ-SHELL-R closed (false alarm) + EQ-SHELL-1B fixed — Outlook email attachments on quotes, merged + live (2026-08-06) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: EQ-SHELL-1A "eq-ops rpc ... failed: TypeError: Failed to fetch (ehow)" — durable fix live, all known consumers migrated (2026-08-06) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: self-join's "double sign-in" for Cards root-caused and fixed — worker-add nav trimmed further too (2026-08-03) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: fixed 8 pre-existing react-hooks/refs eslint errors in the iframe pre-warm keeper (2026-08-03) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: Richard Brown's mobile crash fixed, then a simplified mobile nav for supervisors driven by real usage data (2026-07-31) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: EQ Ops quote status → job status sync fixed for all 5 stages, plus a new "Target period" badge for future-dated quotes (2026-07-31) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: Compliance-roster-only workers — Field access can now be switched off per worker (2026-07-30) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: Staff page edits silently reverting overnight — root-caused and fixed, deployed (2026-07-28) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: EQ Ops quote-detail panel simplified for real-world use, then the Coupa PO import tool rebuilt from scratch against the real export (2026-07-23 → 2026-07-24) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## Core dashboard rebuilt — replaced the passive AI-brief-only home with three permission-gated live signal bands (2026-07-17, MERGED + LIVE) (rotated 2026-08-19 — open items remain in eq-shell.md)


---

## eq-shell: the daily duplicate-identity check is stale, and it surfaced a second misleading-error gap (2026-08-16) (rotated 2026-08-19 — open items remain in eq-shell.md)
...
## eq-shell: fixed a live production crash on SKS admin settings, cleared the merge blocker, shipped (2026-08-16) (rotated 2026-08-19 — open items remain in eq-shell.md)
...
## eq-cards: jvkn Supabase branch-replay diagnosed and documented (2026-08-16) (rotated 2026-08-19 — open items remain in eq-cards.md)
...
## eq-cards: role-assignment could hand someone suite-wide manager power with no audit trail — found, fixed, merged, live (2026-08-16) (rotated 2026-08-19 — open items remain in eq-cards.md)
...
## eq-cards: 3 permission-audit gaps closed — dead JWT minter retired, empty employer credential list fixed, unreachable admin policy fixed (2026-08-16) (rotated 2026-08-19)

---

## eq-cards: CI silently never deployed edge functions — found after a live fix didn't ship, closed for good (2026-08-14) (rotated 2026-08-19)
*Discovered when PR #238's ocr-licence timeout fix merged, `Build & Deploy` reported success, but the live function on jvkn kept 504ing anyway — the workflow only ever built and deployed the Flutter web app; nothing under `supabase/functions/` was ever touched by CI. Had been silently true the whole time; today's fix was deployed by hand as a stopgap while this was built.*
...
## eq-cards: invite-claim IDOR fixed, 3-day claim outage found + fixed, stale-invite cleanup + guard shipped (2026-08-14) (rotated 2026-08-19)
*A live code audit found `eq_cards_claim_invite` only checked that the caller was signed in — never that their verified phone matched the invite's target phone, letting one worker claim a colleague's invite by looking up their phone number. Verified against the live database before writing the fix. While preparing it, found something bigger already broken in production.*
...
## eq-cards: Shell tenant auto-login bug root-caused and fixed — deployed live, needs your click-through (2026-08-04) (rotated 2026-08-19 — open items remain in eq-cards.md)


---

## eq-field: dozens of pages had no access check at all — a direct link could open any of them regardless of role (2026-08-16) (rotated 2026-08-19 — open items remain in eq-field.md)
...
## eq-field: Dashboard map → own page, Map hover shows names, cache-buster hotfix (2026-08-14) (rotated 2026-08-19 — open items remain in eq-field.md)


---

## eq-field: Leave notification gaps closed, digest widened to 4 weeks, Email Templates pilot shipped (2026-08-14) (rotated 2026-08-19 — open items remain in eq-field.md)


---

## eq-field: staff resource management (skills/reviews) — built, deployed, migration applied live (2026-08-11) (rotated 2026-08-19 — open items remain in eq-field.md)


---

## EQ Field: real Incidents / Near Miss reporting, shipped and live (2026-07-22) (rotated 2026-08-19 — open items remain in eq-field.md)
...
## eq-solves-service: any signed-in worker — apprentice, labour hire, subcontractor — could write maintenance checks, defects, test results and assets straight to the database, skipping every in-app permission check. Fixed, shipped, and confirmed live (2026-08-16) (rotated 2026-08-19)

**Deferred:**

---

## eq-solves-service: Settings page showed broken account controls to Shell-embedded users — fixed, merged, live (2026-08-16) (rotated 2026-08-19 — open items remain in eq-solves-service.md)
...
## eq-solves-service: NSX Test Report fixed — dead ACB-only fields left it always printing blank sections (2026-08-14) (rotated 2026-08-19)
*Simon Bramall reported an ACB check's report came out empty in some sections. ACB's report wiring traced clean end-to-end (every field has a real collection path). The NSX Test Report, though, still carried template rows copy-pasted from ACB that the NSX workflow never actually collects.*
...
## eq-solves-service: /admin/* pages closed to non-managers (2026-08-14) (rotated 2026-08-19)
*Royce: gate the remaining Admin pages that were reachable by any signed-in technician who typed the URL directly — the sidebar hid the link, but that's not access control. A prior sweep (PRs #707-#727) had already closed 3 of 9 admin pages; this closed the rest.*
...
## eq-service: migrations dispatched live; mobile check-detail header overflow found+fixed+deployed; eq-context accidental-checkout scare investigated (2026-08-13) (rotated 2026-08-19 — open items remain in eq-solves-service.md)


---

## eq-solves-service: fixed a broken safety check that was silently skipping every code review, then found the "176,000 findings" it surfaced was almost entirely noise, cleaned up what was real (2026-08-01) (rotated 2026-08-19 — open items remain in eq-solves-service.md)


---

## eq-solves-service: Found why photo uploads were failing everywhere, then added a link/create/skip option to the paste-import flow (2026-07-31) (rotated 2026-08-19 — open items remain in eq-solves-service.md)


---

## eq-solves-service: Field Run-Sheet asset headers now show the maintenance plan's Job Code (2026-07-29) (rotated 2026-08-19 — open items remain in eq-solves-service.md)


---

## ⏩ Session close — 2026-06-29 — SKS data reset + maintenance check page perf (rotated 2026-08-19 — open items remain in eq-solves-service.md)
...
## eq-solves-intake: the data-cleaning queue actually shrinks as you work it now, plus a bad-merge error fixed (2026-08-16) (rotated 2026-08-19 — open items remain in eq-solves-intake.md)
...
## Substrate: a wrong note said Shell changes don't go live when you merge — corrected everywhere (2026-08-15) (rotated 2026-08-19 — open items remain in eq-context.md)
...
## eq-context: proper re-score against the 2026-07-20 audit found real gaps outside the campaign's scope (2026-08-15) (rotated 2026-08-19)
*Royce asked for a fresh rating "properly," not a guess. Re-ran the original 7 findings against live state and actually ran every guard script rather than reading about them. Landed at 87/100 — below the 90 baseline, because the rigor of running the tools surfaced real problems a desk review wouldn't have.*
...
## eq-field + eq-shell: access-control cleanup — Pipeline/Teams/Apprentices/Email Templates get their own permission switches, then a real gap in Shell's Access Control page found and closed (2026-08-16) (rotated 2026-08-19 — open items remain in cross-repo.md)
...
## eq-cards + eq-shell: changing your mobile number used to split you into two accounts — fixed, and a second way in shipped (2026-08-15) (rotated 2026-08-19 — open items remain in cross-repo.md)

- [x] **Global CLAUDE.md corrected + F13 reached rung 4, same day.** Global `CLAUDE.md`'s eq-shell deploy note now reads "corrected 2026-08-15 — the previous version of this note was WRONG and had been re-verified twice while staying wrong... There is no gap between 'merge it' and 'ship it' on this repo," with the 13-merge evidence table this item was asking for. `system/failures.md` F13 independently confirms the guard side: scanner (`substrate_honesty.py`) plus a `pre_tool_use.py` write-time block, both live. Verified 2026-08-15 by reading both files directly, not assumed from this note's own memory of them. _(added 2026-08-15, closed 2026-08-15)_

---

## eq-cards + eq-shell: audited every sign-in door, retired the two that weren't real — both live (2026-08-15) (rotated 2026-08-19 — open items remain in cross-repo.md)
...
## eq-shell/eq-field: deactivating someone didn't actually cut their EQ Field access — fixed + 2 follow-ups (2026-08-14) (rotated 2026-08-19 — open items remain in cross-repo.md)


---

## eq-shell / eq-cards: suite-wide Sentry sweep, identity-collision root cause fixed, 2 bugs shipped (2026-08-14) (rotated 2026-08-19 — open items remain in cross-repo.md)
...
## eq-cards + eq-shell: labour-hire licence intake — multi-document OCR extraction + PDF review + flag notifications, all merged + live (2026-08-13) (rotated 2026-08-19 — open items remain in cross-repo.md)


---

## eq-service + eq-solves-intake: RCD in-app entry (manual + photo) shipped, ACB mobile nav bug fixed, RCD threshold corrected (2026-08-11) (rotated 2026-08-19 — open items remain in cross-repo.md)


---

## eq-solves-intake + eq-shell: duplicate-site console's two dead ends fixed, then a live permission bug found and fixed mid-testing (2026-08-01) (rotated 2026-08-19 — open items remain in cross-repo.md)


---

## eq-shell + eq-solves-intake + eq-receipts: closed every open security alert across the EQ suite, found 5 repos where the alert system was switched off entirely (2026-08-01) (rotated 2026-08-19 — open items remain in cross-repo.md)


---

## eq-intake + eq-shell: 4-part fix from Royce's live screenshot review of the Intake console (2026-07-31) (rotated 2026-08-19 — open items remain in cross-repo.md)
...
## eq-solves-service: offline safety net's last gap closed, two real bugs found + fixed, Tier B of the offline-first proposal scoped — 6 PRs merged and confirmed live (2026-08-20) (fully closed, no open items remain)
*Opened investigating whether EQ Service works offline — it doesn't, but Royce had already shipped the core draft-persistence safety net the day before (2026-08-19, commit `119eba4`); this session closed the one gap it left (NSX Step 1), found two unrelated real bugs while checking related pages (both fixed same-day), and later the same session scoped the still-gated Tier B into a task-level breakdown.*

- [x] NSX Step 1 (breaker identification) was still an uncontrolled one-shot form with no draft-autosave, unlike every other ACB/NSX/RCD step. Converted to controlled state and wired into the existing autosave hook. eq-service [PR #776](https://github.com/eq-solutions/eq-service/pull/776), merged, confirmed live.
- [x] **Real bug found + fixed:** the Maintenance Plans list page was silently hiding the shared Global-plan library whenever a customer or site filter was applied — a customer with no plans of its own got dropped to an empty "add starter plans" state instead of the full library it should fall back to. Fixed to match the 3-tier scope logic the check-creation form already used correctly. eq-service [PR #778](https://github.com/eq-solutions/eq-service/pull/778), merged, confirmed live — verified twice: once against raw DB counts (0 → 47 matches for Equinix Australia Pty Ltd), once live in Royce's own authenticated Shell session.
- [x] **Real bug found + fixed:** `/whats-new` and `/terms` both claimed to be public (no-auth) pages in their own source comments but were missing from the proxy's `PUBLIC_PATHS` allow-list — an unauthenticated visitor clicking either link from the sign-in page was bounced straight back to sign-in. Auth-flow file, so flagged in chat before touching; fixed on Royce's explicit go-ahead. eq-service [PR #780](https://github.com/eq-solutions/eq-service/pull/780), merged, confirmed live.
- [x] A stale code comment in `NsxWorkflow.tsx` claimed the NSX checklist still needed a "field-tech pass" to trim ACB-only items (arc chutes, spring-charging, etc.). Checked the actual checklist array and live reading data instead of asking a field tech: the trim had already happened, nothing left to do. Comment corrected so this doesn't get re-asked. eq-service [PR #779](https://github.com/eq-solutions/eq-service/pull/779), merged, confirmed live.
- [x] `docs/proposals/offline-first.md` updated to mark Tier A (draft persistence) shipped instead of proposed; Tiers B (PWA/read cache) and C (write queue/sync) remain correctly unstarted. eq-service [PR #777](https://github.com/eq-solutions/eq-service/pull/777), merged, confirmed live.
- [x] **Found a working live-session path for click-testing this app**, closing a limitation noted throughout this file's history: `claude-in-chrome` MCP against Royce's own already-authenticated Shell browser session reaches the real embedded app with a real session. First used this session to visually confirm the job-plans fix, not just via DB query — reuse for other "not click-tested live" items in this file.
- [x] **Tier B (installable PWA + read cache) scoped into a task-level breakdown** — app-icon blocker (no EQ Service app icon exists anywhere in the repo yet), manifest, service worker, read-cache scope, an auth-grace step explicitly gated on Royce's sign-off (`verifyServiceJwt()` hard-fails on `exp` today, no grace concept), a smoke test, acceptance criteria, and an explicit fence against Tier C scope creep. Scoping only — not started; both tier-gating decisions (connectivity bad enough, auth grace approved) remain open. eq-service [PR #781](https://github.com/eq-solutions/eq-service/pull/781), merged, confirmed live.

**Deferred:** none — Tiers B/C of the offline-first proposal remain deliberately unstarted (not a this-session deferral; revisit only if connectivity volume or a near-miss argues for it).

## eq-shell: Excel cost-breakdown import + Labour/Materials quote view for EQ Ops — built, merged, live, click-tested with Royce's own file (2026-08-20) (fully closed, no open items remain)
*Royce shared his own Excel cost breakdown for a real job plus the reference PDF he'd hand-typed it into EQ Ops to produce, and asked two things: can this be imported, and — separately, since customers like seeing labour and materials per line item — what are the options for showing that in the quote document. Ran AskUserQuestion on scope; Royce chose "Both."*

- [x] **New `quote-parse-excel.ts`** — a 3rd quote-import path alongside the 2 existing PDF importers, and the first deterministic (no AI) one: the source is already structured cells, so it finds the header row by scanning for a "Labour" + "Materials" cell pair (case-insensitive, any column order, any wording), reads every worksheet in the workbook, and skips totals/note rows on its own.
- [x] **New "Category Split" quote-document mode** (`quoteDocGenerator.ts`) — EQ Ops previously only totalled line items by category across the whole quote; this mode pivots by line-item name instead, so each named line carries its own Labour $ and Materials $ side by side, matching how estimators already build these breakdowns by hand. Reuses the existing template's table grid via `gridSpan` merging — no new template file.
- [x] **Caught and fixed a real pricing bug before shipping** — the shared PDF-import pricing path applied the standard materials markup uniformly, which would have marked up labour figures that should be a direct 1:1 charge-out rate. Fixed to short-circuit `cost === sell` for labour before the cost/sell-basis branch runs.
- [x] eq-shell [PR #1485](https://github.com/eq-solutions/eq-shell/pull/1485), `tsc -b`/eslint clean, full CI green, squash-merged on Royce's explicit "merge" — confirmed live via exact Netlify `commit_ref` match against the newest ready production deploy.
- [x] **Click-tested live, not just verified in code** — unlike most same-day entries in this file, this one got a real browser pass: Royce asked for it explicitly, so the import ran against production with his own source spreadsheet, confirming both the parser and the labour-markup fix end-to-end.
- [x] **Format flexibility verified with 8 real layout variations run against the shipped parser**, not described from memory — 5 pass (header wording, column order, extra columns, multiple worksheets, casing), 2 fail cleanly (renamed columns, one combined cost column), 1 flagged directly to Royce as a silent risk: the item name must be the leftmost column, or the row still imports but with the wrong description text.

**Deferred:** none.

---

## Suppliers/site-credentials feature — found broken via a live grant sweep, fixed same day on Royce's schema call (2026-08-20) (fully closed, no open items remain)

*Prompted by a real question earlier that day about a different app (does eq-field have login PINs — it does). Answering it surfaced a known, never-fully-checked risk: a migration can write a `GRANT` right after creating a function and that grant can still end up missing live. Ran the actual check — every function in the guarded schemas on all three databases (jvkn, zaap, ehow) with no `anon`/`authenticated` grant, cross-referenced against every real call site in eq-cards, eq-shell, eq-field and eq-solves-service. Almost everything came back correctly locked to service-role only, exactly as designed. One genuine hit.*

- [x] **The Suppliers directory (`/api/site-credentials`, built 2026-07-21) was provably broken on all three of its moving parts, and had likely never worked.** Three independent bugs stacked: (1) the functions wrote to `app_data.site_credentials` while the app's GET route queried `public.site_credentials`, which didn't exist — a 2026-06-13 rework in the migration file's own history had moved the intended home to `public`, but that rework was only ever applied to the file, never to the live database (confirmed by diffing the file against the statements actually recorded in `supabase_migrations.schema_migrations`); (2) `decrypt_site_credential`/`upsert_site_credential` never carried an `authenticated` EXECUTE grant live; (3) both called `pgp_sym_encrypt`/`pgp_sym_decrypt` unqualified while pinning `search_path TO 'public'`, but `pgcrypto` is installed in the `extensions` schema on this project — even fully granted and correctly targeted, every call would still have failed with "function does not exist." That third bug wasn't visible from the original grant-sweep finding; only surfaced while writing the actual fix.
- [x] **Royce's call: `service` schema** — matching how the rest of this repo's operational tables are organised, and `service.site_credentials` turned out to already exist, fully provisioned (RLS policies `scc_iso`/`scc_svc`, correct table grants) — built for this and never wired up.
- [x] **Migration `0223_site_credentials_service_schema.sql`** moves all three functions into `service`, fully qualifies the pgcrypto calls, grants `authenticated` EXECUTE on the two app-facing functions, drops the old `public.*` versions. No route.ts changes needed — every Supabase client handed to a route handler already defaults `db.schema` to `service` (`lib/supabase/server.ts`), so the existing `.rpc()` calls resolve directly once the function lives there. `scripts/rekey-site-credentials.ts` used its own unscoped client and needed the schema added explicitly (its header comment carried the same stale "lives in public" belief — corrected too).
- [x] **Applied live to ehow and verified with a real round trip**, not just checked structurally: inserted a throwaway credential through `upsert_site_credential`, read it back through `decrypt_site_credential` under a simulated tenant JWT, confirmed the decrypted username/password matched exactly what was written, then deleted the test row. First `apply_migration` attempt hit a connection timeout on the migration-history bookkeeping step before any DDL ran (confirmed nothing partially landed before retrying) — second attempt succeeded cleanly and is recorded as the newest entry in the live migration ledger.
- [x] eq-service [PR #784](https://github.com/eq-solutions/eq-service/pull/784) opened, merge-readiness audited, squash-merged (`5d040af`) on Royce's explicit "merge it", branch deleted.
- [x] **Found and fixed a second, unrelated-to-the-original-bug gotcha while merging**: eq-solves-service has its own governed migration ledger (`service._eq_migrations`, written only by `scripts/migrate-service.mjs` / the `apply-service-migrations.yml` workflow) — separate from the raw Supabase CLI ledger (`supabase_migrations.schema_migrations`) that `mcp__supabase__apply_migration` writes to. Applying 0223 directly via MCP (this session's normal pattern for every other repo) updated the CLI ledger but not this repo's own governed one, so `scripts/migrate-service.mjs --plan` correctly reported 0223 as still "pending" — confirmed via the PR's own automated plan comment, not assumed. Left as-is, the next `apply-service-migrations` dispatch would have tried to re-run 0223's plain `CREATE FUNCTION` statements against objects that already exist, failed on "already exists," and (since the runner can't tell that specific failure apart from a genuinely indeterminate one) halted the whole apply run with a scary but harmless-in-this-case error. Reconciled with one manual ledger row (`applied_by: manual-reconcile:claude-out-of-band-2026-08-20`, checksum matched to the exact committed file) before merging, confirmed via a fresh read of the ledger row.
- [x] `app_data.site_credentials` — dropped, on Royce's explicit "drop app_data.site_credentials". Re-verified live immediately before dropping (0 rows, no FK references, no dependent views, no function anywhere still mentioned it — checked `pg_proc.prosrc` directly), then eq-service [PR #785](https://github.com/eq-solutions/eq-service/pull/785) (migration `0224_drop_app_data_site_credentials.sql`, ledger reconciled same as 0223), merged (`4a16482f`) once `tsc + next build` passed (the one CI failure was the same pre-existing, unrelated integration-test bootstrap issue traced on the prior PR).

**Deferred:** none.

**Worth remembering for next time:** eq-solves-service is the only repo touched this session with its own governed migration ledger distinct from the raw Supabase one — applying a migration to ehow for THIS repo via the raw `apply_migration` MCP tool needs a matching manual row in `service._eq_migrations`, or the next `apply-service-migrations` dispatch breaks on it. No other repo touched this session has this extra layer.

---

## eq-shell + eq-cards: Ali Alsalman's employment type kept flipping back to Labour Hire — root-caused and fixed, confirmed live (2026-08-20) (fully closed, no open items remain)
...
- [x] **Confirmed genuinely fixed, not just theoretically**: Royce re-set Ali to Direct one more time (his 4th attempt) — landed with `employment_type_locked_by_shell: true` stamped on the same write, row unchanged since. Next nightly reconcile read the lock and left it alone.
- [x] **Royce: "Delete the eq-shell copy of workers-canonical-sync."** eq-shell [PR #1495](https://github.com/eq-solutions/eq-shell/pull/1495) removed the dead duplicate file entirely, all required checks green, merged (`18dabcdb`) on Royce's "merge it when CI's green." Only eq-cards' copy exists anywhere now — nothing left to silently diverge from it a second time.
...
## eq-shell: the Shell licence-scanner page had never once saved a licence — found, fixed, merged, live (2026-08-17) (rotated 2026-08-20)
...
## eq-shell: auth-stall Sentry regression — blocking-spinner watchdog fired before session-verify's own retry finished, fixed, merged, live (2026-08-17) (rotated 2026-08-20)
...
## eq-shell: Access Control page redesigned — searchable diffed drawer for Base permissions, unified Field permissions view — both shipped, live (2026-08-17) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-shell: Staff table gets Excel-style filtering — built, merged, live (2026-08-17) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-shell: the Shell licence-scanner page has never once saved a licence — found, fixed, merged, live (2026-08-17) (rotated 2026-08-20)

---

## eq-shell: Documents module had no permission gate at all — found, fixed, merged, live (2026-08-16) (rotated 2026-08-20)
*Any signed-in tenant member of any role — including labour_hire/subcontractor — could upload documents, push a sign-off obligation onto other people, archive documents, manage categories, and pull completion-evidence/certificate PDFs. Zero permission check anywhere, client or server, by the module's own code comments.*
...
## eq-shell: two staff pages could be reached from any linked company site, not just the main one — found, fixed, merged, live (2026-08-16) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-shell: a way to hand yourself full admin power through the "custom access groups" screen — found, fixed, and live (2026-08-16) (rotated 2026-08-20)
*The Access Control page has two ways to change what someone can do: editing a role's default permissions (already blocked handing out admin/audit-trail powers that way), and building a custom group of extra permissions to hand to specific people (had no such block). Someone who could build custom groups could build one with full admin power — including the power to build custom groups itself — and add themselves to it. Today only managers can build custom groups, so this was dormant, but that's exactly the power this screen exists to hand to other people, so it stops being dormant the moment it is.*
...
## eq-shell: checked whether a worker-protection feature exists on both company databases or just one — confirmed it's a deliberate difference, not something missed (2026-08-16) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-shell: 4 places were showing worker or contact details to people who shouldn't see them — fixed, PR open, waiting on your go to ship (2026-08-16) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-shell: an AI tool anyone signed in could use to run up costs on the company's AI account — closed, merged, live (2026-08-16) (rotated 2026-08-20)

---

## eq-shell: workers can now be asked for different required licences depending on their role, not one shared list for everyone — closed, merged, live (2026-08-16) (rotated 2026-08-20)
*Started from two of Royce's brain-dump emails: audited every idea in them against the live systems (not docs) first, ranked them into a task list, then built the top-priority one (#4 — per-role minimum requirements) end to end, including multi-org support from the start since it turned out to be nearly free once the underlying design was right. Closes the "minimum-requirements model" question left undecided since 2026-07-07 (see that entry further down).*
...
## eq-shell: 21 CRM/staff database functions only checked which tenant you were in, not who you were — closed, merged, live (2026-08-15) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-shell: switching someone off didn't actually stop them — closed at both ends, live (2026-08-15) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-shell: two Sentry monitors kept re-firing after being "fixed" — the earlier fix had never actually landed (2026-08-15) (rotated 2026-08-20)
*Royce asked what's next after the load-speed work below; a `/decide` pass first wrongly suggested two P0 security findings that turn out to be standing "not now" decisions already on record — corrected before acting. The real next thing was two Sentry alerts for Richard Brown's duplicate-account incident, supposedly closed the day before, firing again.*
...
## eq-shell: load speed — Suppliers and Staff were slow; database wasn't the cause (2026-08-15) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-shell: account-enumeration hole closed on the phone+PIN sign-in door (2026-08-15) (rotated 2026-08-20)

---

## eq-shell: Mobile Home redesign — compliance card collapsed, Suppliers + Compliance report quick links added (2026-08-14) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-shell: Staff list — apprentice year badge + Trade multi-select shipped, text[] conversion blocked on eq-field coordination (2026-08-14) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: Tom's licence-upload timeout root-caused for real — Shell's admin path was sending full-res photos, unlike Cards (2026-08-14) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-shell: hard-delete for archived user accounts — built, merged, live (2026-08-14) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-shell: Shell Conversations built end-to-end — logging, permission-locked, resourcing dashboard, draft org chart, team assignment (2026-08-11 → 2026-08-13) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: self-join bulk-approve + gap-analysis-driven onboarding fixes (2026-08-06) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: EQ-SHELL-R closed (false alarm) + EQ-SHELL-1B fixed — Outlook email attachments on quotes, merged + live (2026-08-06) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: EQ-SHELL-1A "eq-ops rpc ... failed: TypeError: Failed to fetch (ehow)" — durable fix live, all known consumers migrated (2026-08-06) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: self-join's "double sign-in" for Cards root-caused and fixed — worker-add nav trimmed further too (2026-08-03) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: fixed 8 pre-existing react-hooks/refs eslint errors in the iframe pre-warm keeper (2026-08-03) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: Richard Brown's mobile crash fixed, then a simplified mobile nav for supervisors driven by real usage data (2026-07-31) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: EQ Ops quote status → job status sync fixed for all 5 stages, plus a new "Target period" badge for future-dated quotes (2026-07-31) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: Compliance-roster-only workers — Field access can now be switched off per worker (2026-07-30) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: Staff page edits silently reverting overnight — root-caused and fixed, deployed (2026-07-28) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: EQ Ops quote-detail panel simplified for real-world use, then the Coupa PO import tool rebuilt from scratch against the real export (2026-07-23 → 2026-07-24) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## Core dashboard rebuilt — replaced the passive AI-brief-only home with three permission-gated live signal bands (2026-07-17, MERGED + LIVE) (rotated 2026-08-20 — open items remain in eq-shell.md)


---

## eq-shell: the daily duplicate-identity check is stale, and it surfaced a second misleading-error gap (2026-08-16) (rotated 2026-08-20 — open items remain in eq-shell.md)

- [x] **Resolved [EQ-SHELL-1M](https://eq-solutions.sentry.io/issues/EQ-SHELL-1M) in Sentry** 2026-08-16 — data was already clean; blocked from the initial attempt by the safety classifier as a monitoring-state change, unblocked on your explicit go-ahead.

---

## eq-shell: fixed a live production crash on SKS admin settings, cleared the merge blocker, shipped (2026-08-16) (rotated 2026-08-20 — open items remain in eq-shell.md)
...
## eq-cards: jvkn Supabase branch-replay diagnosed and documented (2026-08-16) (rotated 2026-08-20 — open items remain in eq-cards.md)
...
## eq-cards: role-assignment could hand someone suite-wide manager power with no audit trail — found, fixed, merged, live (2026-08-16) (rotated 2026-08-20 — open items remain in eq-cards.md)
...
## eq-cards: 3 permission-audit gaps closed — dead JWT minter retired, empty employer credential list fixed, unreachable admin policy fixed (2026-08-16) (rotated 2026-08-20)

---

## eq-cards: CI silently never deployed edge functions — found after a live fix didn't ship, closed for good (2026-08-14) (rotated 2026-08-20)
*Discovered when PR #238's ocr-licence timeout fix merged, `Build & Deploy` reported success, but the live function on jvkn kept 504ing anyway — the workflow only ever built and deployed the Flutter web app; nothing under `supabase/functions/` was ever touched by CI. Had been silently true the whole time; today's fix was deployed by hand as a stopgap while this was built.*
...
## eq-cards: invite-claim IDOR fixed, 3-day claim outage found + fixed, stale-invite cleanup + guard shipped (2026-08-14) (rotated 2026-08-20)
*A live code audit found `eq_cards_claim_invite` only checked that the caller was signed in — never that their verified phone matched the invite's target phone, letting one worker claim a colleague's invite by looking up their phone number. Verified against the live database before writing the fix. While preparing it, found something bigger already broken in production.*
...
## eq-cards: Shell tenant auto-login bug root-caused and fixed — deployed live, needs your click-through (2026-08-04) (rotated 2026-08-20 — open items remain in eq-cards.md)


---

## eq-field: roster site-map query scoped to field_enabled, not just active (2026-08-17) (rotated 2026-08-20)
...
## eq-field: dozens of pages had no access check at all — a direct link could open any of them regardless of role (2026-08-16) (rotated 2026-08-20 — open items remain in eq-field.md)
...
## eq-field: Dashboard map → own page, Map hover shows names, cache-buster hotfix (2026-08-14) (rotated 2026-08-20 — open items remain in eq-field.md)


---

## eq-field: Leave notification gaps closed, digest widened to 4 weeks, Email Templates pilot shipped (2026-08-14) (rotated 2026-08-20 — open items remain in eq-field.md)


---

## eq-field: mobile-centering sprint — 12 of 16 audited items already shipped, real gaps closed same day (2026-08-12) (rotated 2026-08-20 — open items remain in eq-field.md)
...
## eq-field: staff resource management (skills/reviews) — built, deployed, migration applied live (2026-08-11) (rotated 2026-08-20 — open items remain in eq-field.md)


---

## EQ Field: real Incidents / Near Miss reporting, shipped and live (2026-07-22) (rotated 2026-08-20 — open items remain in eq-field.md)
...
## eq-solves-service: Calendar + every people-list in Service made canonical, 3 database updates shipped to live (2026-08-17) (rotated 2026-08-20)
...
## eq-solves-service: notification bell was silently broken for anyone signed in through Shell — found, fixed, reviewed, merged, live (2026-08-17) (rotated 2026-08-20 — open items remain in eq-solves-service.md)
...
## eq-service: ACB/NSX cover masthead + blank page 2 fixed; live Secondary Injection load bug found and fixed (2026-08-17) (rotated 2026-08-20 — open items remain in eq-solves-service.md)
...
## eq-solves-service: any signed-in worker — apprentice, labour hire, subcontractor — could write maintenance checks, defects, test results and assets straight to the database, skipping every in-app permission check. Fixed, shipped, and confirmed live (2026-08-16) (rotated 2026-08-20)

**Deferred:**

---

## eq-solves-service: Settings page showed broken account controls to Shell-embedded users — fixed, merged, live (2026-08-16) (rotated 2026-08-20 — open items remain in eq-solves-service.md)
...
## eq-solves-service: NSX Test Report fixed — dead ACB-only fields left it always printing blank sections (2026-08-14) (rotated 2026-08-20)
*Simon Bramall reported an ACB check's report came out empty in some sections. ACB's report wiring traced clean end-to-end (every field has a real collection path). The NSX Test Report, though, still carried template rows copy-pasted from ACB that the NSX workflow never actually collects.*
...
## eq-solves-service: /admin/* pages closed to non-managers (2026-08-14) (rotated 2026-08-20)
*Royce: gate the remaining Admin pages that were reachable by any signed-in technician who typed the URL directly — the sidebar hid the link, but that's not access control. A prior sweep (PRs #707-#727) had already closed 3 of 9 admin pages; this closed the rest.*
...
## eq-service: migrations dispatched live; mobile check-detail header overflow found+fixed+deployed; eq-context accidental-checkout scare investigated (2026-08-13) (rotated 2026-08-20 — open items remain in eq-solves-service.md)


---

## eq-solves-service: fixed a broken safety check that was silently skipping every code review, then found the "176,000 findings" it surfaced was almost entirely noise, cleaned up what was real (2026-08-01) (rotated 2026-08-20 — open items remain in eq-solves-service.md)


---

## eq-solves-service: Found why photo uploads were failing everywhere, then added a link/create/skip option to the paste-import flow (2026-07-31) (rotated 2026-08-20 — open items remain in eq-solves-service.md)


---

## eq-solves-service: Field Run-Sheet asset headers now show the maintenance plan's Job Code (2026-07-29) (rotated 2026-08-20 — open items remain in eq-solves-service.md)


---

## ⏩ Session close — 2026-06-29 — SKS data reset + maintenance check page perf (rotated 2026-08-20 — open items remain in eq-solves-service.md)

- [x] **CONFIRMED STALE, do not build — checked live 2026-08-18.** `/assets` now has a working "Show Archived" toggle (`app/(app)/assets/page.tsx`) that depends on `service.assets` including inactive rows so it can filter them in/out on request. Adding `WHERE a.active = true` to the view itself — as this note originally proposed — would silently break that live feature by making archived assets permanently invisible even with the toggle on. This note predates the toggle; the toggle is correct, the note was not.

---

## eq-solves-intake: the data-cleaning queue actually shrinks as you work it now, plus a bad-merge error fixed (2026-08-16) (rotated 2026-08-20 — open items remain in eq-solves-intake.md)
...
## Substrate: F9's general copy-back guard (F12) — ratchet promoted rung 2 → 4, built, merged (2026-08-17) (rotated 2026-08-20)
...
## Substrate: eq/pending.md split by repo, plus three follow-on queue-management fixes (2026-08-17) (rotated 2026-08-20)
...
## Substrate: a wrong note said Shell changes don't go live when you merge — corrected everywhere (2026-08-15) (rotated 2026-08-20 — open items remain in eq-context.md)
...
## eq-context: proper re-score against the 2026-07-20 audit found real gaps outside the campaign's scope (2026-08-15) (rotated 2026-08-20)
*Royce asked for a fresh rating "properly," not a guess. Re-ran the original 7 findings against live state and actually ran every guard script rather than reading about them. Landed at 87/100 — below the 90 baseline, because the rigor of running the tools surfaced real problems a desk review wouldn't have.*
...
## eq-field + eq-shell: access-control cleanup — Pipeline/Teams/Apprentices/Email Templates get their own permission switches, then a real gap in Shell's Access Control page found and closed (2026-08-16) (rotated 2026-08-20 — open items remain in cross-repo.md)

- [x] **3 database functions on Shell's control-plane database exist live with no matching file anywhere in the repo** — not a mystery after all: all 3 traced directly to a same-day eq-cards change that landed minutes before this check ran. Confirmed by reading eq-cards' own files and comparing every definition to what's actually live, line for line — genuinely accounted for, just not written down in this repo too. Marked as known/accepted so the check stops flagging it; no database change needed. eq-shell [PR #1389](https://github.com/eq-solutions/eq-shell/pull/1389), merged — this is also what had been blocking PR #1380/#1381 above from merging cleanly.

---

## eq-cards + eq-shell: changing your mobile number used to split you into two accounts — fixed, and a second way in shipped (2026-08-15) (rotated 2026-08-20 — open items remain in cross-repo.md)

- [x] **Global CLAUDE.md corrected + F13 reached rung 4, same day.** Global `CLAUDE.md`'s eq-shell deploy note now reads "corrected 2026-08-15 — the previous version of this note was WRONG and had been re-verified twice while staying wrong... There is no gap between 'merge it' and 'ship it' on this repo," with the 13-merge evidence table this item was asking for. `system/failures.md` F13 independently confirms the guard side: scanner (`substrate_honesty.py`) plus a `pre_tool_use.py` write-time block, both live. Verified 2026-08-15 by reading both files directly, not assumed from this note's own memory of them. _(added 2026-08-15, closed 2026-08-15)_

---

## eq-cards + eq-shell: audited every sign-in door, retired the two that weren't real — both live (2026-08-15) (rotated 2026-08-20 — open items remain in cross-repo.md)
...
## eq-shell/eq-field: deactivating someone didn't actually cut their EQ Field access — fixed + 2 follow-ups (2026-08-14) (rotated 2026-08-20 — open items remain in cross-repo.md)


---

## eq-shell / eq-cards: suite-wide Sentry sweep, identity-collision root cause fixed, 2 bugs shipped (2026-08-14) (rotated 2026-08-20 — open items remain in cross-repo.md)
...
## eq-cards + eq-shell: labour-hire licence intake — multi-document OCR extraction + PDF review + flag notifications, all merged + live (2026-08-13) (rotated 2026-08-20 — open items remain in cross-repo.md)


---

## eq-service + eq-solves-intake: RCD in-app entry (manual + photo) shipped, ACB mobile nav bug fixed, RCD threshold corrected (2026-08-11) (rotated 2026-08-20 — open items remain in cross-repo.md)


---

## eq-solves-intake + eq-shell: duplicate-site console's two dead ends fixed, then a live permission bug found and fixed mid-testing (2026-08-01) (rotated 2026-08-20 — open items remain in cross-repo.md)


---

## eq-shell + eq-solves-intake + eq-receipts: closed every open security alert across the EQ suite, found 5 repos where the alert system was switched off entirely (2026-08-01) (rotated 2026-08-20 — open items remain in cross-repo.md)


---

## eq-intake + eq-shell: 4-part fix from Royce's live screenshot review of the Intake console (2026-07-31) (rotated 2026-08-20 — open items remain in cross-repo.md)
...
## eq-shell: the Shell licence-scanner page had never once saved a licence — found, fixed, merged, live (2026-08-17) (rotated 2026-08-21)
*Started as a request to make a scanned licence's date of birth flow through to a worker's profile. Turned out the save button on that page (`/:tenantSlug/onboarding/licence`, the fallback for anyone who can't use the Cards app) has been silently broken since a backend rewrite — every save has failed with an error since then, for every field, not just date of birth. Checked live: 118 licences exist in the backup copy that page was supposed to write to, and every single one came from the Cards app's own sync, zero from this page, ever. Also found that even a working save button would have written to the wrong place — a backup table nothing else reads anymore. Full detail in `eq/changelog/eq-shell.md` (PR #1423). Note: this page and its two backend functions were subsequently removed outright on 2026-08-20 (Royce's call — licence capture goes through Cards only), see the entry near the top of this file.*
...
## eq-shell: Cards iframe was blocking the Web Share API, breaking iOS exports through Shell — fixed, live (2026-08-18) (rotated 2026-08-21)
...
## eq-shell: "horrendous" screen-to-screen loading — root cause was full page reloads on every sidebar click, fixed, merged, live (2026-08-18) (rotated 2026-08-21)
...
## eq-shell: Staff-page corrections now propagate to the canonical worker copy — found via Cameron Tregoning's stale email, fixed, merged, live (2026-08-18) (rotated 2026-08-21)
...
## eq-shell: QR/join-code Cards signups notified nobody — admins now get the same email + roster badge the in-app connect flow already had (2026-08-18) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: Access Control gets a real ring visual + tab strip; roster now exposes real permissions instead of raw groups (2026-08-18) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: compliance-pack caption misread as an unwanted org-wide PDF — reverted same day (2026-08-18) (rotated 2026-08-21)
...
## eq-shell: compliance-pack export blanking the whole SPA on desktop — found, fixed, merged, live (2026-08-18) (rotated 2026-08-21)
...
## eq-shell: Cards-linked worker's DOB guard was blocking unrelated edits (start date, phone, ...) — found, fixed, merged, live (2026-08-18) (rotated 2026-08-21)
...
## eq-shell: auth-stall Sentry regression — blocking-spinner watchdog fired before session-verify's own retry finished, fixed, merged, live (2026-08-17) (rotated 2026-08-21)
*Sentry "auth-stall: session-spinner-timeout" (7 occurrences since 2026-08-07) — root-caused in an earlier sprint-scoping pass and held for explicit go before building.*
...
## eq-shell: Access Control page redesigned — searchable diffed drawer for Base permissions, unified Field permissions view — both shipped, live (2026-08-17) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: Staff table gets Excel-style filtering — built, merged, live (2026-08-17) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: the Shell licence-scanner page has never once saved a licence — found, fixed, merged, live (2026-08-17) (rotated 2026-08-21)

---

## eq-shell: Documents module had no permission gate at all — found, fixed, merged, live (2026-08-16) (rotated 2026-08-21)
*Any signed-in tenant member of any role — including labour_hire/subcontractor — could upload documents, push a sign-off obligation onto other people, archive documents, manage categories, and pull completion-evidence/certificate PDFs. Zero permission check anywhere, client or server, by the module's own code comments.*
...
## eq-shell: two staff pages could be reached from any linked company site, not just the main one — found, fixed, merged, live (2026-08-16) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: a way to hand yourself full admin power through the "custom access groups" screen — found, fixed, and live (2026-08-16) (rotated 2026-08-21)
*The Access Control page has two ways to change what someone can do: editing a role's default permissions (already blocked handing out admin/audit-trail powers that way), and building a custom group of extra permissions to hand to specific people (had no such block). Someone who could build custom groups could build one with full admin power — including the power to build custom groups itself — and add themselves to it. Today only managers can build custom groups, so this was dormant, but that's exactly the power this screen exists to hand to other people, so it stops being dormant the moment it is.*
...
## eq-shell: checked whether a worker-protection feature exists on both company databases or just one — confirmed it's a deliberate difference, not something missed (2026-08-16) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: 4 places were showing worker or contact details to people who shouldn't see them — fixed, PR open, waiting on your go to ship (2026-08-16) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: an AI tool anyone signed in could use to run up costs on the company's AI account — closed, merged, live (2026-08-16) (rotated 2026-08-21)

---

## eq-shell: workers can now be asked for different required licences depending on their role, not one shared list for everyone — closed, merged, live (2026-08-16) (rotated 2026-08-21)
*Started from two of Royce's brain-dump emails: audited every idea in them against the live systems (not docs) first, ranked them into a task list, then built the top-priority one (#4 — per-role minimum requirements) end to end, including multi-org support from the start since it turned out to be nearly free once the underlying design was right. Closes the "minimum-requirements model" question left undecided since 2026-07-07 (see that entry further down).*
...
## eq-shell: 21 CRM/staff database functions only checked which tenant you were in, not who you were — closed, merged, live (2026-08-15) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: switching someone off didn't actually stop them — closed at both ends, live (2026-08-15) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: two Sentry monitors kept re-firing after being "fixed" — the earlier fix had never actually landed (2026-08-15) (rotated 2026-08-21)
*Royce asked what's next after the load-speed work below; a `/decide` pass first wrongly suggested two P0 security findings that turn out to be standing "not now" decisions already on record — corrected before acting. The real next thing was two Sentry alerts for Richard Brown's duplicate-account incident, supposedly closed the day before, firing again.*
...
## eq-shell: load speed — Suppliers and Staff were slow; database wasn't the cause (2026-08-15) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: account-enumeration hole closed on the phone+PIN sign-in door (2026-08-15) (rotated 2026-08-21)

---

## eq-shell: Mobile Home redesign — compliance card collapsed, Suppliers + Compliance report quick links added (2026-08-14) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: Staff list — apprentice year badge + Trade multi-select shipped, text[] conversion blocked on eq-field coordination (2026-08-14) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: Tom's licence-upload timeout root-caused for real — Shell's admin path was sending full-res photos, unlike Cards (2026-08-14) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-shell: hard-delete for archived user accounts — built, merged, live (2026-08-14) (rotated 2026-08-21)

---

## eq-shell: Shell Conversations built end-to-end — logging, permission-locked, resourcing dashboard, draft org chart, team assignment (2026-08-11 → 2026-08-13) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: self-join bulk-approve + gap-analysis-driven onboarding fixes (2026-08-06) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: EQ-SHELL-R closed (false alarm) + EQ-SHELL-1B fixed — Outlook email attachments on quotes, merged + live (2026-08-06) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: EQ-SHELL-1A "eq-ops rpc ... failed: TypeError: Failed to fetch (ehow)" — durable fix live, all known consumers migrated (2026-08-06) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: self-join's "double sign-in" for Cards root-caused and fixed — worker-add nav trimmed further too (2026-08-03) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: fixed 8 pre-existing react-hooks/refs eslint errors in the iframe pre-warm keeper (2026-08-03) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: Richard Brown's mobile crash fixed, then a simplified mobile nav for supervisors driven by real usage data (2026-07-31) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: EQ Ops quote status → job status sync fixed for all 5 stages, plus a new "Target period" badge for future-dated quotes (2026-07-31) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: Compliance-roster-only workers — Field access can now be switched off per worker (2026-07-30) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: Staff page edits silently reverting overnight — root-caused and fixed, deployed (2026-07-28) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: EQ Ops quote-detail panel simplified for real-world use, then the Coupa PO import tool rebuilt from scratch against the real export (2026-07-23 → 2026-07-24) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## Core dashboard rebuilt — replaced the passive AI-brief-only home with three permission-gated live signal bands (2026-07-17, MERGED + LIVE) (rotated 2026-08-21 — open items remain in eq-shell.md)


---

## eq-shell: the daily duplicate-identity check is stale, and it surfaced a second misleading-error gap (2026-08-16) (rotated 2026-08-21 — open items remain in eq-shell.md)

- [x] **Resolved [EQ-SHELL-1M](https://eq-solutions.sentry.io/issues/EQ-SHELL-1M) in Sentry** 2026-08-16 — data was already clean; blocked from the initial attempt by the safety classifier as a monitoring-state change, unblocked on your explicit go-ahead.

---

## eq-shell: fixed a live production crash on SKS admin settings, cleared the merge blocker, shipped (2026-08-16) (rotated 2026-08-21 — open items remain in eq-shell.md)
...
## eq-cards: replacing an existing licence wasn't obvious — root-caused and fixed (2026-08-18) (rotated 2026-08-21)
...
## eq-cards: export flow finished end-to-end — Export/Download merged into one sheet, auto-provision session race fixed, export-all PDF/Excel added, PDF photo embed shipped then fixed twice, iOS downloads fixed, two nagging reminders removed (2026-08-18) (rotated 2026-08-21)
...
## eq-cards: per-licence PDF export shipped, real bugs found in the xlsx register (2026-08-18) (rotated 2026-08-21)
...
## eq-cards: jvkn Supabase branch-replay diagnosed and documented (2026-08-16) (rotated 2026-08-21 — open items remain in eq-cards.md)
...
## eq-cards: role-assignment could hand someone suite-wide manager power with no audit trail — found, fixed, merged, live (2026-08-16) (rotated 2026-08-21 — open items remain in eq-cards.md)
...
## eq-cards: 3 permission-audit gaps closed — dead JWT minter retired, empty employer credential list fixed, unreachable admin policy fixed (2026-08-16) (rotated 2026-08-21)

---

## eq-cards: CI silently never deployed edge functions — found after a live fix didn't ship, closed for good (2026-08-14) (rotated 2026-08-21)
*Discovered when PR #238's ocr-licence timeout fix merged, `Build & Deploy` reported success, but the live function on jvkn kept 504ing anyway — the workflow only ever built and deployed the Flutter web app; nothing under `supabase/functions/` was ever touched by CI. Had been silently true the whole time; today's fix was deployed by hand as a stopgap while this was built.*
...
## eq-cards: invite-claim IDOR fixed, 3-day claim outage found + fixed, stale-invite cleanup + guard shipped (2026-08-14) (rotated 2026-08-21)
*A live code audit found `eq_cards_claim_invite` only checked that the caller was signed in — never that their verified phone matched the invite's target phone, letting one worker claim a colleague's invite by looking up their phone number. Verified against the live database before writing the fix. While preparing it, found something bigger already broken in production.*
...
## eq-cards: Shell tenant auto-login bug root-caused and fixed — deployed live, needs your click-through (2026-08-04) (rotated 2026-08-21 — open items remain in eq-cards.md)


---

## eq-field: apprentices can self-create their initial profile (2026-08-18) (rotated 2026-08-21)
...
## eq-field: boot-perf — 3 of the 4 flagged scripts moved off the critical path, closes the 2026-07-28 audit item (2026-08-18) (rotated 2026-08-21 — open items remain in eq-field.md)
...
## eq-field: apprentice list fail-open bug + full onboarding/login audit across ehow, jvkn, Sentry (2026-08-18) (rotated 2026-08-21)

---

## eq-field: apprentice data readable by any authenticated SKS session — RLS gated by tenant only, never person (2026-08-18) (rotated 2026-08-21 — open items remain in eq-field.md)
...
## eq-field: Apprentices list showed the full company roster to any signed-in user, not just managers (2026-08-18) (rotated 2026-08-21 — open items remain in eq-field.md)
...
## eq-field: My Schedule cold-boot cache fallback, built from SKS NSW Labour usage data (2026-08-18) (rotated 2026-08-21 — open items remain in eq-field.md)


---

## eq-field: weekly digest — per-section on/off + custom intro (2026-08-18) (rotated 2026-08-21 — open items remain in eq-field.md)
...
## eq-field: sprint prep — desktop polish slice 1, Access-Model Phase 3 keys (2026-08-18) (rotated 2026-08-21 — open items remain in eq-field.md)
...
## eq-field: Contacts screen skipped the rehire-rating prompt when archiving Labour Hire (2026-08-18) (rotated 2026-08-21 — open items remain in eq-field.md)
...
## eq-field: roster site-map query scoped to field_enabled, not just active (2026-08-17) (rotated 2026-08-21)
*Found during a live-recon pass answering "what's outstanding" at session start, not from a prior report — the roster's site-map query filtered sites on `active=eq.true` only, missing the `field_enabled` flag that gates whether a site should actually appear in Field.*
...
## eq-field: dozens of pages had no access check at all — a direct link could open any of them regardless of role (2026-08-16) (rotated 2026-08-21 — open items remain in eq-field.md)
...
## eq-field: Dashboard map → own page, Map hover shows names, cache-buster hotfix (2026-08-14) (rotated 2026-08-21 — open items remain in eq-field.md)


---

## eq-field: Leave notification gaps closed, digest widened to 4 weeks, Email Templates pilot shipped (2026-08-14) (rotated 2026-08-21 — open items remain in eq-field.md)


---

## eq-field: mobile-centering sprint — 12 of 16 audited items already shipped, real gaps closed same day (2026-08-12) (rotated 2026-08-21 — open items remain in eq-field.md)

- [x] **Timesheets mobile-entry — the deferred condition was met.** Royce reported 3 real mobile bugs directly (screenshots) on 2026-08-20: a non-manager worker (Phillip Krikellis) could see every other worker's hours, mobile card rows rendered ~2x oversized, and the top nav appeared to overlap content. Root-caused and shipped 2 of 3: `_getTsFilteredPeople()` had zero identity scoping (isManager only ever gated editing, never row visibility) — now filters non-managers to their own row, mirroring `leave.js`'s existing worker-view pattern; the oversized rows traced to a bare `.empty` class collision with `base.css`'s generic full-page empty-state placeholder rule, not a media-query bug. Both fixed, tested (31/31 suite, eslint clean, cache-busters verified), shipped as eq-field [PR #738](https://github.com/eq-solutions/eq-field/pull/738), squash-merged, confirmed live (`sw.js` shows v3.5.529). Third symptom (nav overlap) could not be reproduced — no `#page-timesheets`-specific override found, forcing `.shell-mode` synthetically showed no overlap — best guess is it's the same `.empty` collision misread, not confirmed. _(updated 2026-08-20)_

---

## eq-field: staff resource management (skills/reviews) — built, deployed, migration applied live (2026-08-11) (rotated 2026-08-21 — open items remain in eq-field.md)


---

## EQ Field: real Incidents / Near Miss reporting, shipped and live (2026-07-22) (rotated 2026-08-21 — open items remain in eq-field.md)
...
## eq-solves-service: calendar wired into maintenance checks — caused and fixed a same-session P0 regression along the way (2026-08-18) (rotated 2026-08-21)
...
## eq-solves-service: click-to-create on the calendar, a working "reconnect" button on session timeouts, faster warm-up after a deploy, and a safety net for lost in-progress readings (2026-08-18) (rotated 2026-08-21)
...
## eq-solves-service: Calendar + every people-list in Service made canonical, 3 database updates shipped to live (2026-08-17) (rotated 2026-08-21)
*Royce asked whether the maintenance calendar could be made canonical (shared with the rest of the EQ suite, not just Service's own copy) and asked that every place Service shows a list of people pull from the one shared roster Shell uses — the same duplicate-person risk already fixed for the Users list. While sweeping the app for "list of people" spots, found the separate notification-bell bug written up in the entry directly below — spun that off as its own piece of work rather than mixing it into this one.*
...
## eq-solves-service: notification bell was silently broken for anyone signed in through Shell — found, fixed, reviewed, merged, live (2026-08-17) (rotated 2026-08-21 — open items remain in eq-solves-service.md)
...
## eq-service: ACB/NSX cover masthead + blank page 2 fixed; live Secondary Injection load bug found and fixed (2026-08-17) (rotated 2026-08-21 — open items remain in eq-solves-service.md)
...
## eq-solves-service: ACB/NSX check saves could wipe a technician's readings on a dropped connection — fixed and shipped live (2026-08-18) (rotated 2026-08-21 — open items remain in eq-solves-service.md)
...
## eq-solves-service: any signed-in worker — apprentice, labour hire, subcontractor — could write maintenance checks, defects, test results and assets straight to the database, skipping every in-app permission check. Fixed, shipped, and confirmed live (2026-08-16) (rotated 2026-08-21)

**Deferred:**

---

## eq-solves-service: Settings page showed broken account controls to Shell-embedded users — fixed, merged, live (2026-08-16) (rotated 2026-08-21 — open items remain in eq-solves-service.md)
...
## eq-solves-service: NSX Test Report fixed — dead ACB-only fields left it always printing blank sections (2026-08-14) (rotated 2026-08-21)
*Simon Bramall reported an ACB check's report came out empty in some sections. ACB's report wiring traced clean end-to-end (every field has a real collection path). The NSX Test Report, though, still carried template rows copy-pasted from ACB that the NSX workflow never actually collects.*
...
## eq-solves-service: /admin/* pages closed to non-managers (2026-08-14) (rotated 2026-08-21)
*Royce: gate the remaining Admin pages that were reachable by any signed-in technician who typed the URL directly — the sidebar hid the link, but that's not access control. A prior sweep (PRs #707-#727) had already closed 3 of 9 admin pages; this closed the rest.*
...
## eq-service: migrations dispatched live; mobile check-detail header overflow found+fixed+deployed; eq-context accidental-checkout scare investigated (2026-08-13) (rotated 2026-08-21 — open items remain in eq-solves-service.md)


---

## eq-solves-service: fixed a broken safety check that was silently skipping every code review, then found the "176,000 findings" it surfaced was almost entirely noise, cleaned up what was real (2026-08-01) (rotated 2026-08-21 — open items remain in eq-solves-service.md)


---

## eq-solves-service: Found why photo uploads were failing everywhere, then added a link/create/skip option to the paste-import flow (2026-07-31) (rotated 2026-08-21 — open items remain in eq-solves-service.md)


---

## eq-solves-service: Field Run-Sheet asset headers now show the maintenance plan's Job Code (2026-07-29) (rotated 2026-08-21 — open items remain in eq-solves-service.md)


---

## ⏩ Session close — 2026-06-29 — SKS data reset + maintenance check page perf (rotated 2026-08-21 — open items remain in eq-solves-service.md)

- [x] **CONFIRMED STALE, do not build — checked live 2026-08-18.** `/assets` now has a working "Show Archived" toggle (`app/(app)/assets/page.tsx`) that depends on `service.assets` including inactive rows so it can filter them in/out on request. Adding `WHERE a.active = true` to the view itself — as this note originally proposed — would silently break that live feature by making archived assets permanently invisible even with the toggle on. This note predates the toggle; the toggle is correct, the note was not.

---

## eq-solves-intake + eq-shell: Overview "Fix these" gap cards get a real bulk-fill grid instead of one-row-at-a-time (2026-08-18) (rotated 2026-08-21)

---

## eq-solves-intake: the data-cleaning queue actually shrinks as you work it now, plus a bad-merge error fixed (2026-08-16) (rotated 2026-08-21 — open items remain in eq-solves-intake.md)
...
## Substrate: F9's general copy-back guard (F12) — ratchet promoted rung 2 → 4, built, merged (2026-08-17) (rotated 2026-08-21)
*F12 (a raw file copy/move whose destination lands inside the shared eq-context checkout, silently clobbering a concurrent session's already-pushed work) had a narrow partial fix from the day before (a dirty-checkout guard in `substrate_sync.py`) but the ledger's own `target_rung: 4` was still unmet — recurred twice (2026-08-05, 2026-08-17). Ran via `/decide`; Royce: "Promote to rung 4."*
...
## Substrate: eq/pending.md split by repo, plus three follow-on queue-management fixes (2026-08-17) (rotated 2026-08-21)
*Royce's reaction to seeing the raw scale of the EQ backlog ("insane to manage") drove this — the 3,741-line, 356-section monolith was hard to work from and getting harder. Picked all four fixes offered: split by repo, fix the Royce-queue label, add duplicate detection, surface aging items.*
...
## Substrate: a wrong note said Shell changes don't go live when you merge — corrected everywhere (2026-08-15) (rotated 2026-08-21 — open items remain in eq-context.md)
...
## eq-context: proper re-score against the 2026-07-20 audit found real gaps outside the campaign's scope (2026-08-15) (rotated 2026-08-21)
*Royce asked for a fresh rating "properly," not a guess. Re-ran the original 7 findings against live state and actually ran every guard script rather than reading about them. Landed at 87/100 — below the 90 baseline, because the rigor of running the tools surfaced real problems a desk review wouldn't have.*
...
## eq-roles + eq-shell + eq-solves-service: PM Calendar digest switched from a hardcoded group ID to a real, grantable permission — built, released, live (2026-08-18) (rotated 2026-08-21 — open items remain in cross-repo.md)
...
## eq-field + eq-shell: access-control cleanup — Pipeline/Teams/Apprentices/Email Templates get their own permission switches, then a real gap in Shell's Access Control page found and closed (2026-08-16) (rotated 2026-08-21 — open items remain in cross-repo.md)

- [x] **3 database functions on Shell's control-plane database exist live with no matching file anywhere in the repo** — not a mystery after all: all 3 traced directly to a same-day eq-cards change that landed minutes before this check ran. Confirmed by reading eq-cards' own files and comparing every definition to what's actually live, line for line — genuinely accounted for, just not written down in this repo too. Marked as known/accepted so the check stops flagging it; no database change needed. eq-shell [PR #1389](https://github.com/eq-solutions/eq-shell/pull/1389), merged — this is also what had been blocking PR #1380/#1381 above from merging cleanly.

---

## eq-cards + eq-shell: changing your mobile number used to split you into two accounts — fixed, and a second way in shipped (2026-08-15) (rotated 2026-08-21 — open items remain in cross-repo.md)

- [x] **Global CLAUDE.md corrected + F13 reached rung 4, same day.** Global `CLAUDE.md`'s eq-shell deploy note now reads "corrected 2026-08-15 — the previous version of this note was WRONG and had been re-verified twice while staying wrong... There is no gap between 'merge it' and 'ship it' on this repo," with the 13-merge evidence table this item was asking for. `system/failures.md` F13 independently confirms the guard side: scanner (`substrate_honesty.py`) plus a `pre_tool_use.py` write-time block, both live. Verified 2026-08-15 by reading both files directly, not assumed from this note's own memory of them. _(added 2026-08-15, closed 2026-08-15)_

---

## eq-cards + eq-shell: audited every sign-in door, retired the two that weren't real — both live (2026-08-15) (rotated 2026-08-21 — open items remain in cross-repo.md)
...
## eq-shell/eq-field: deactivating someone didn't actually cut their EQ Field access — fixed + 2 follow-ups (2026-08-14) (rotated 2026-08-21 — open items remain in cross-repo.md)


---

## eq-shell / eq-cards: suite-wide Sentry sweep, identity-collision root cause fixed, 2 bugs shipped (2026-08-14) (rotated 2026-08-21 — open items remain in cross-repo.md)
...
## eq-cards + eq-shell: labour-hire licence intake — multi-document OCR extraction + PDF review + flag notifications, all merged + live (2026-08-13) (rotated 2026-08-21 — open items remain in cross-repo.md)


---

## eq-service + eq-solves-intake: RCD in-app entry (manual + photo) shipped, ACB mobile nav bug fixed, RCD threshold corrected (2026-08-11) (rotated 2026-08-21 — open items remain in cross-repo.md)


---

## eq-solves-intake + eq-shell: duplicate-site console's two dead ends fixed, then a live permission bug found and fixed mid-testing (2026-08-01) (rotated 2026-08-21 — open items remain in cross-repo.md)


---

## eq-shell + eq-solves-intake + eq-receipts: closed every open security alert across the EQ suite, found 5 repos where the alert system was switched off entirely (2026-08-01) (rotated 2026-08-21 — open items remain in cross-repo.md)


---

## eq-intake + eq-shell: 4-part fix from Royce's live screenshot review of the Intake console (2026-07-31) (rotated 2026-08-21 — open items remain in cross-repo.md)


---

## eq-cards/eq-shell: onboarding minimum-requirements switch, bulk connect-worker, and a live anon-EXECUTE fix (2026-07-26) (rotated 2026-08-21 — open items remain in cross-repo.md)
