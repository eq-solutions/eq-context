...
- **Role default = `employee`.** The claim bridge defaults every bridged worker to `employee`. Correct for SKS's own team (pilot); **over-permissioned for actual labour-hire tradies** (`labour_hire` tier). Fix before any outside-SKS / labour-hire rollout: the invite should carry the intended role and the bridge should honour it (default to the *safest* t