...
**One `employment_type_locked_by_shell` lock remains, deliberately — and it marks a real inconsistency with §3.3, recorded rather than papered over.** `employment_type` is an **employer-owned** field by the table above, so canonical should never move it. But `workers-canonical-sync` derives it from `jvkn.workers.role` and *will* overwrite it unless locked (`employment_type: employmentTypeLocked ? c.employment_type : (knownEmploymentType ?? c.employment_type ?? "Direct")`). The lock is therefore compensating for a sync that reaches into a field it does not own. The structural fix is to stop the sync writing `employment_type` at all, not to lock it per-row — **not attempted here**, since it changes behaviour for every tenant and deserves its own pass.
