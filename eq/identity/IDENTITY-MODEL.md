...
- The 0055 migration's role-hardcoding on the personal membership row (always `'employee'`, regardless of the user's real role) is a real inconsistency worth a follow-up fix in eq-cards, now that this is the sanctioned model rather than a side effect — flagged, not fixed here (doc-only pass, no eq-cards code touched).
