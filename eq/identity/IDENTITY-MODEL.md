...
#### 3.3.3 Shared DB objects — objects two pipelines can both fully replace

**Registry, verified live 2026-08-24 — since then: `field_job_numbers_src` (added 2026-08-26), `field_people`/`field_people_removed` (added 2026-09-01), `field_managers`/`field_managers_digest_iu` (added 2026-09-01) — see below for each.** A handful of `app_data` objects on ehow are created and edited by BOTH eq-field's own hand-applied `supabase/migrations/*.sql` AND eq-shell's governed One Pipe (`tenant-migrate.yml` → `supabase/tenant-migrations/*.sql`). This table is the source of truth for that list — eq-shell's CLAUDE.md and eq-field's CLAUDE.md both reference it rather than restating it; keep any copy there in sync with this one, not the reverse. **Machine-readable copy: `eq/identity/shared-db-objects.json`, read by `scripts/check_shared_object_drift.py` (see "Automated check" below) — that file and this table must list the same objects; whichever notices a gap first adds to both in the same PR.**

| Object | Kind | Plane | Why both repos touch it |
|---|---|---|---|
| `field_people` | view | ehow only | The read shape behind every Field roster/timesheets/leave screen — eq-field's client code depends on its exact column set; eq-shell edits it to keep zaap and ehow at parity. Not in this table until 2026-09-01 despite three prior `security_invoker` incidents (see below) — it was tracked only as prose in eq-field's own CLAUDE.md |
| `field_people_removed` | view | ehow only | Twin of `field_people` for the archived/removed roster — same history, same gap |
| `field_people_iud` | function | ehow only | INSTEAD OF trigger behind Field's Add Person/Edit Roster — eq-field owns the tenant-side permission gates (licence/agency/hire_company/rating/active/field_approved/user_id), eq-shell owns the upward identity push (§3.3 above) |
| `field_people_removed_iud` | function | ehow only | Twin trigger for the archived/removed roster (Restore, hard-delete) |
| `field_teams_iud`, `field_team_members_iud`, `field_team_supervisors_iud` | function | ehow only | Crew-scoping write path (eq-field feature); zaap has no `app_data.teams` at all |
| `eq__guard_timesheet_status`, `eq__guard_leave_status` | function | ehow only | Status-transition gates on timesheets/leave |
| `field_job_numbers_src` | function | ehow only | eq-field owns the read-view shape and security posture (`security_invoker` wrapper, the fixed safe-column projection that keeps `app_data.quote`'s financial columns away from `authenticated`); eq-shell's Ops/Quotes pipeline owns the underlying `app_data.quote` schema and status vocabulary the function reads from |
| `field_managers` | view | ehow only | Field's Supervision-list / leave-approver / crew-lead directory view over `app_data.staff` — eq-shell created it (`0055`) as a shell-owned canonical view; eq-field has since rebuilt it 3x standalone (`20260630` digest opt-in write-back + trigger, `20260804` zaap parity, `20260827` a SEC-33 collision fix) with zero eq-shell re-involvement since creation. `20260827`'s fix made `security_invoker=false` + an inline `tenant_id` filter a *deliberate* exception (see the view's own `COMMENT ON VIEW`) — a future eq-shell touch to `app_data.staff` RLS has no visibility that reverting this view's invoker mode reopens the bug eq-field just closed |
| `field_managers_digest_iu` | function | ehow only | INSTEAD OF UPDATE trigger behind `field_managers`'s one writable column (`digest_opt_in`) — forwards the Friday-digest toggle to `app_data.staff`. eq-field-authored (`20260630`), `SECURITY DEFINER`; unchanged live since creation, but replicated by hand to zaap (`20260804`) and shares `field_managers`'s risk profile |

**Not yet on this table, flagged not promoted:** `field_people_worker_id_iu` (writes `cards_worker_id` back onto `app_data.staff`) surfaced during the 2026-09-01 audit below — same object family, shared-edit status not yet confirmed either way. Tracked in `shared-db-objects.json`'s `candidates_not_yet_decided` list; not enforced by the automated check until someone decides it belongs.
...
**A second, independent confirmation, 2026-08-26.** `field_job_numbers_src()` was missing from this table despite matching the pattern exactly. eq-shell's Ops/Quotes pipeline rewrote it four times since eq-field created it on 2026-07-04 — `0161` (reassert security_invoker, same day), `0163` (added the retire/restore override table `public.field_job_number_overrides`, which the function's final `SELECT` reads), `0173` (2026-07-12, renamed the source column `app_data.quote.workbench_job_no` → `job_number` and added the `status <> 'invoiced'` auto-retire exclusion), `0275` (2026-08-24, flipped the `site_name` fallback from `COALESCE(code, name)` to `COALESCE(name, code)`) — with zero visibility into this repo throughout. eq-field's own on-disk copy (`20260704b_field_job_numbers_invoker_over_secdef.sql`) sat frozen at the pre-`0173` baseline for those 7 weeks, still referencing the dropped `workbench_job_no` column. That stale reference is exactly what kept this instance from silently reverting anything live — hand-applying it would hit a hard error, not a quiet rollback — but the on-disk record was still wrong the whole time, undetected until this table was audited for gaps rather than by anyone hitting the error. Reconciled in eq-field [PR #796](https://github.com/eq-solutions/eq-field/pull/796): a fresh migration matching the verified-live body exactly, no behaviour change. Same root cause, same fix shape as `field_people_iud` above — also flagged, separately, that `field_job_number_overrides` itself has no migration file anywhere in eq-field, despite existing live and being read/written by `jobnumbers.js`; out of scope for that PR, not yet actioned.

**A third confirmation, 2026-08-19 → not discovered until 2026-09-01.** eq-shell's `0249_field_people_view_parity.sql` (paired with `0249_add_staff_field_people_parity_columns.sql`) rewrote `field_people`/`field_people_removed` on both tenant planes to bring zaap's column set up to ehow's — aliasing `employment_type AS "group"` and adding a stubbed `NULL::text AS employment_type` output column. The migration's own header is explicit that this was "a content no-op" on ehow specifically (the shape was apparently already live there before this migration ran; its real target was zaap) — and it carefully reasserts `security_invoker` because "this exact view has drifted on that setting before," the one pattern the guards below already covered. There was no equivalent way for its author to know eq-field's *client* code depended on the `employment_type` column's values — that dependency lives in JavaScript in a different repo, not in Postgres's catalog, and `field_people` wasn't on this table at all at the time. Undetected for 12 days until this session queried the live view directly; fixed reactively in eq-field PR #855 (v3.5.625) and PR #858 (v3.5.626). Different shape from both prior incidents (column list, not `security_invoker` or a function-body collision) and neither existing guard below was built to see it — this is what prompted both the table addition above and the automated check just below.

**A fourth object found during routine registry hygiene, 2026-09-01 — not yet a confirmed collision like the three above, but the same structural risk.** `field_managers`/`field_managers_digest_iu` were never added to this table despite matching the pattern: eq-shell created `field_managers` (`0055`); eq-field alone has rebuilt it three times since (`20260630`, `20260804`, `20260827`), most recently to fix a real live bug — SEC-33's `staff_own_or_manager_read` policy (migration `0256`, PR #1510, both planes, 2026-08-21) collapsed this view to 0 rows for every non-manager caller for ~6 days, because the view was still `security_invoker=on` and silently inherited `app_data.staff`'s new RESTRICTIVE row policy. eq-field's fix flips `security_invoker=false` and inlines an explicit `tenant_id` filter, documented in the view's own `COMMENT ON VIEW` as a deliberate, permanent exception. No eq-shell pipeline has touched `field_managers` since `0055`, so this hasn't collided yet — but nothing stops a future eq-shell migration touching `app_data.staff`'s RLS from reasserting `security_invoker=on` on this view (the default posture, and the one every other guard on this page pushes toward) and silently reopening the exact bug just closed. Added to the registry and automated check below so that touch would be caught before it ships, not six days after.

**Mitigation shipped 2026-08-24, functions only (surfaces the collision, doesn't block it — neither pipeline can tell a real eq-field addition apart from its own intended change):**
...
- Neither closes the gap completely: eq-field has no live-DB CI at all (migrations are hand-applied, by design — see its own CLAUDE.md), so its side is a forcing-function at commit time, not a live check. Dispatch to ehow stays the human gate. And neither one is built to notice a *column-shape* change at all — only a missing `security_invoker` clause or a missing citation comment — which is exactly what let the third incident through both.

**Automated check, added 2026-09-01 — object-shape-agnostic, runs independently of either pipeline.** `scripts/check_shared_object_drift.py` (in this repo) diffs the live `pg_get_viewdef`/`pg_get_functiondef` of every object in `eq/identity/shared-db-objects.json` against a checked-in snapshot (`shared-db-objects.snapshot.json`) and fails on ANY difference — column list, `security_invoker`, grants, function body, or the object disappearing — regardless of which repo's pipeline caused it, or whether the change was hand-applied outside both. Runs nightly (`.github/workflows/shared-object-drift.yml`, `SUPABASE_ACCESS_TOKEN` — same secret `security-audit.yml` already has) and on any push touching the script/registry/snapshot. A failure surfaces automatically in `digest.md`'s Needs You via the existing `scheduled_workflow_health()` cron-health scan (system/failures.md → F11) — no separate alerting was built. This is deliberately the *only* one of the three mechanisms on this page that doesn't care what kind of object it's checking or what shape the drift takes; the other two stay in place as fast, PR-time signals for the two specific patterns they already know how to catch.
