...
**Current version:** v3.5.707 · **Deployed:** `field.eq.solutions` (Netlify site `eq-field`, which also answers on `eq-field.netlify.app`) + the Shell embed at `core.eq.solutions/sks/field`
...
>
> **Version bump only, 2026-09-09** — v3.5.685 → v3.5.707 (22 versions, multiple sessions this week) — not itemized here; see `eq/changelog/eq-field.md` and `sessions/2026-09-0[5-9].md` for the full list. This pass reports only the latest, **v3.5.707** ([PR #960](https://github.com/eq-solutions/eq-field/pull/960)): `leave.js`'s balance/business-day math (`_leaveGetBalances`/`_leaveBizDays`) extracted into a new pure, unit-tested `scripts/leave-rules.js` — closes multi-lens review decision #12, the last of the five extracted-domain modules (timesheets/roster/apprentices/sks-pipeline-resource/leave) that had zero test coverage. No schema, entitlement, or tenant changes. Everything else in this file is unchanged from the 2026-09-04/05 passes above and has **not** been re-verified this time.
