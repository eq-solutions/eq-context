...
*Direct continuation of the PM-calendar work below — asked to build the classification gate that work had deliberately left out. Covers both halves of Royce's "1 and 2" answer now: the admin-screen half (part 2, PR #766) merged 2026-08-19; the at-import half (part 1) built 2026-08-20 — see below, PR open but not yet merged.*
...
- [x] **At-import half built too:** the same timing picker now also appears inside the Excel commercial-sheet import wizard, set once per batch rather than per row (matches how these imports are actually used — one customer/contract at a time). Shared the recurrence-builder code between both screens instead of duplicating it. eq-service [PR #775](https://github.com/eq-solutions/eq-service/pull/775) — open, CI not yet checked, not yet merged. _(2026-08-20)_

**Deferred:**
- [ ] **PR #775 needs merge + live verification** — CI checked 2026-08-20: green on the real gate (`tsc + next build`, typecheck+audit, Netlify preview), only the pre-existing "Integration tests (Supabase local)" flake failed (same non-blocker every PR today has hit). Mergeable, no conflicts. Awaiting Royce's merge instruction — same per-step process #766 went through. _(added 2026-08-20)_
- [ ] **Not click-tested live** — no working local sign-in for this app in this environment, same limitation noted throughout this file's history. Verified via full type-check + production build instead (now covers both the admin screen and the importer). Worth a few minutes once #775 is live: open a scope item, try all three timing options, confirm the label looks right, press Generate Calendar once; separately, run an import and confirm the batch timing picker sets the right dates on the created rows. _(added 2026-08-19, updated 2026-08-20)_
