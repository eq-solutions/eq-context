...
- [ ] **Still paused on purpose (`SUPERVISOR_DIGEST_PAUSED=true`), and there's nothing to send yet anyway.** Re-verified live 2026-08-19, unchanged since 2026-08-18, all contexts. New finding same session: `service.pm_calendar` has **zero active rows** on live ehow right now — the generator that populates it from contract-scope dates (migration 0221, merged 2026-08-19) is written and dry-run verified but deliberately not yet applied to production (governed migration pipeline). So unpausing today would send correctly-targeted but empty digests, not real ones — two separate decisions needed, in order: (1) apply the pending PM-calendar migrations, (2) flip the pause flag. Recipient count re-checked live: 21 people currently eligible (18 explicit members of the "Calendar Digest Recipients" group + the rest via the `manager` role default, which includes **you** and, per the 2026-08-18 note, `dev@eq.solutions` — still worth deciding if that system/test account should hold `manager` at all) — close to the 20 recorded 2026-08-18, difference not investigated. To add/remove a specific person: `core.eq.solutions/sks/admin/access-control` → Groups tab → "Calendar Digest Recipients". To change who gets it by role instead: same page, Base tab (role matrix) — `service.receive_calendar_digest` is manager-only by default today. _(re-verified 2026-08-19)_
