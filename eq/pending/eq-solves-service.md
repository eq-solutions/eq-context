...
**Deferred:**
- [ ] **The `baited.has('assets')`/`baited.has('defects')` sanity assertions were never actually added — corrected 2026-09-14.** An earlier same-day entry in `sessions/2026-09-14.md` claimed these were "pushed as a second commit on the same PR #846 branch." Verified directly, not assumed: `gh pr view 846 --json commits` shows PR #846 had exactly one commit (`c9eb54c6`, the `raised_date` removal only, additions:0/deletions:1 across the whole PR), and `main`'s `tests/integration/rls/cross-tenant-sweep.test.ts:186-189` today only asserts `customers`/`sites`/`maintenance_checks`/`audit_logs`. Whatever that other session did locally never reached the remote branch. So the coverage gap this whole thread exists to close is still open **in code**, not just blocked on CI. Two steps remain, in order: (1) someone actually adds the two assertions, (2) once the concurrent app_data/service-schema grant fixes land (`fix/app-data-fixture-grants-rls`, [PR #845](https://github.com/eq-solutions/eq-service/pull/845) `fix/integration-test-tenant-claim-and-app-data-grants`, and `fix/service-schema-grants-and-contacts-sweep` above, all in flight as of this writing) and the test rebases onto them, a green re-run is the real end-to-end confirmation.
