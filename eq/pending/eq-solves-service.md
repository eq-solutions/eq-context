...
*Direct continuation of the PM-calendar work below — asked to build the classification gate that work had deliberately left out. Covers both halves of Royce's "1 and 2" answer now: the admin-screen half (part 2, PR #766) merged 2026-08-19; the at-import half (part 1) merged 2026-08-20 — see below, live verification still outstanding.*
...
- [x] **At-import half built too:** the same timing picker now also appears inside the Excel commercial-sheet import wizard, set once per batch rather than per row (matches how these imports are actually used — one customer/contract at a time). Shared the recurrence-builder code between both screens instead of duplicating it. eq-service [PR #775](https://github.com/eq-solutions/eq-service/pull/775), squash-merged (`c34696f`) on Royce's explicit "merge". _(2026-08-20)_

**Deferred:**
- [ ] **PR #775 merged, not yet confirmed live** — merge commit `c34696f78bfcb979be6189f7ada2c3db26dfdb21`. Not yet checked against Netlify's own deploy record the way #766 was — don't assume live from the merge alone. _(added 2026-08-20)_
