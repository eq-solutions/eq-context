...
## eq-solves-service: offline safety net's last gap closed, two real bugs found + fixed (Global-scope filter, public-page auth gate) — 5 PRs merged and confirmed live (2026-08-20)
*Opened investigating whether EQ Service works offline — it doesn't, but Royce had already shipped the core draft-persistence safety net the day before (2026-08-19, commit `119eba4`); this session closed the one gap it left (NSX Step 1) and found two unrelated real bugs while checking related pages, both fixed same-day.*
...
**Deferred:** none new — Tiers B/C of the offline-first proposal remain deliberately unstarted (pre-existing, not a this-session deferral; revisit only if connectivity volume or a near-miss argues for it).
