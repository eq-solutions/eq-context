...
- [x] **Swept the other file-storage buckets for the same problem — none found.** Checked all 5 live buckets on the shared database directly. The "logos" bucket (the only other one eq-service actually uses — 20 real files) is correctly set up and matches what the app does with it. The other three ("compliance-packs", "licence-photos", "safety-photos") hold zero files and have no code anywhere in eq-service that touches them — they belong to a different app sharing the same database, not this one, and aren't at risk since there's nothing in them. One of those three uses an inconsistent rule (keys off a user-editable field instead of the tenant field everything else uses) — moot in practice since it's empty and not this app's feature to fix; noted in case whoever owns it ever revisits it. _(added 2026-09-05, closed same day)_
