...
- [~] **Applying PR #818's migration surfaced that the shared database-update pipeline itself was stuck** — a much older, unrelated update (0234) had never been recorded as applied, and the pipeline refuses to skip past a stuck one. Un-stuck it directly (a bookkeeping fix only; 0234's real effect had already happened weeks ago). The bigger pattern behind it — several sessions today applying database changes by hand instead of through the tracked pipeline, which is what caused the jam — is spun off as its own piece of work; Royce has already started it running in a separate session as of this close. _(added 2026-08-31)_
