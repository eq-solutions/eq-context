...
**Notes:**
- **Correction, found and fixed same day:** the 2026-09-08 entry below describes `backup-code.yml` as "built (nightly git-bundle of 6 EQ repos → R2...) but not armed yet." That was wrong — the file had never actually been committed anywhere (checked `git log --all`, every branch tip, and local disk — zero trace beyond a scratch worktree that got discarded before a commit landed). Rebuilt for real and pushed this session; confirmed live on `origin/main`. Live-verified against Supabase (`list_projects`) at the same time: only one dedicated per-tenant project exists (`eq-tenant-madagins`, created today) — the second-real-tenant test above genuinely can't happen yet, not a Royce-owned delay.
...
## eq-context: `safe_commit.py` silently overwrote a concurrent session's register entry — caught, restored, not yet hardened (2026-09-09)
*While investigating and logging a separate finding (SEC-77, see `eq/pending/eq-solves-service.md`), this session's push of a new `ops/security-register.md` row (as SEC-76) landed cleanly, then was silently overwritten minutes later by a concurrent session's own, unrelated SEC-76 push (`canonical-members.ts`, the other task spawned from the same scoping doc). `safe_commit.py` copies a file's bytes wholesale from the caller's local working tree into a fresh scratch worktree off `origin/main` — it does not diff or merge against what changed on `origin/main` since the caller's copy was last read. The other session's local copy predated this session's push, so its push replaced the whole file, silently dropping this session's row. No error, no conflict, no signal to either session. Caught only because this session's own `/close` re-read `origin/main` fresh before writing the session log and noticed SEC-76 was now a different finding than the one just pushed.*

- Content restored as SEC-77 (both sessions' entries now coexist correctly) — `ops/security-register.md`, commit `a4d7ee33`.
- [ ] **`safe_commit.py` has no defense against this class of race** — its own docstring already names the risk ("this does NOT resolve content conflicts... that's still the caller's call to make") but nothing forces a fresh re-read of a fast-moving, frequently-appended file immediately before push, and two sessions minutes apart have no way to see each other coming. A cheap partial mitigation: for known append-heavy files (this register, `digest.md`, `pending.md` files), diff the caller's copy against fresh `origin/main` before staging and abort/warn on unexpected non-append changes, rather than trusting the caller's copy is still current. Not designed or built this session — flagging the gap, not the fix. _(added 2026-09-09)_
...
**Shipped, merged, and verified live:** eq-context [PR #202](https://github.com/eq-solutions/eq-context/pull/202) (`12a530f3`, merged 2026-09-04 22:41 UTC). `refresh_digest.py` records every 401/403 in `GH_AUTH_FAILURES`, warns to stderr, retries once with the runner's own `GITHUB_TOKEN` (`GH_FALLBACK_TOKEN`), adds a 🔴 "GitHub token rejected" Needs-you item, labels CI cells "token error" instead of "unknown", and says "unavailable — token rejected" instead of "No merges in the last 7 days". `refresh_suite_state.py` gains `fetch_active_users_7d()` — a read-only count of jvkn `shell_control.users.last_login_at >= now() − 7 days` via the Supabase Management API (`SUPABASE_ACCESS_TOKEN`, already a repo secret) — replacing the hard-coded "Active users | blocked" row that pointed at a column Shell SSO never writes. **Verified same night** via manual `workflow_dispatch` of both workflows (not waiting for the nightly cron): `suite-state.md` now reads "Active users (Shell sign-ins, jvkn) | 48"; `digest.md` shows the 🔴 token-rejected banner and "? token error" per repo — exactly as designed. The fix works; the token is still dead.

- [ ] **Royce to regenerate `EQ_CONTEXT_PAT`** — fine-grained PAT with read access to Actions, Pull requests and Contents on eq-shell, eq-service, eq-field, eq-cards and eq-solves-intake; store as the `EQ_CONTEXT_PAT` repo secret. Pick a longer expiry or calendar the renewal — this is the second time a dead token has silently blinded the nightly digest (first: 2026-07-21 → 08-03, 13 days). _(added 2026-09-04)_
...
- [ ] **Confirmed a second casualty of the same dead `EQ_CONTEXT_PAT`: `jwt-contract-drift.yml`** — failing since 2026-09-02 (same day the token died), identical signature (fetch to the *public* `eq-contracts` repo 404s because the script sends the dead token as a bearer header regardless; reproduced the exact 404 by sending a deliberately invalid token at the same public URL). Ran the check locally with a valid token for ground truth: **contract is intact, zero real drift** — the `tenant_role_overrides` fail-closed auth PRs that landed on eq-shell 2026-09-04/05 (#1762/#1764/#1767/#1768/#1770) did not break the Shell→Service JWT handoff. No separate fix needed — clears once `EQ_CONTEXT_PAT` above is rotated. Incidental, non-blocking: eq-shell now mints a new `denied_perms` claim eq-service doesn't consume yet (surfaced as an informational warning, not a failure, by the check's own design) — worth an eq-service adoption or a `KNOWN_MINT_ONLY` allowlist entry at some point, not urgent. _(added 2026-09-05)_
