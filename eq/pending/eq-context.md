...
- [x] **F9 carve-out for a strictly-safe fast-forward — BUILT, 2026-09-15** (Royce's explicit instruction, same day it was offered). `hooks/pre_tool_use.py` F9(b) now allows `pull`/`merge` carrying an explicit `--ff-only` when, checked live at hook time, the working tree is clean AND HEAD is 0 commits ahead of the ref being fast-forwarded onto. `rebase` is never eligible (no such flag), `--no-ff`/`--rebase` disqualify, a `--ff-only` inside a quoted string doesn't count, and anything unverifiable falls through to the block — fail closed. Adversarial suite 173/173 with 12 new cases both directions; verified against the real shared checkout in its live state (clean, 0 ahead, 24 behind): the safe sync is allowed, plain `pull`/`rebase`/`--ff-only --rebase` still blocked. F9's ledger `guard:`/`note:` updated; `recurrences`/`rung` unchanged — a guard refinement, not a new incident.
