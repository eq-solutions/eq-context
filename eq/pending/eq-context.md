...
- [ ] **Sweep run 2026-09-04: 112 of 119 deleted, 6 left for individual review.** Re-verified fresh immediately before deleting (not reused from the count-only check above) — each was `git status --porcelain` clean AND had zero commits ahead of its upstream at delete time. This session's own then-active clone was excluded regardless of status. Local disk cleanup only: every deleted clone was already fully pushed, so nothing exists only in a deleted copy — restorable by re-cloning `origin/main` any time. **6 left untouched, real remaining work:** `eq-field/…/scratchpad/eq-context-fresh` (dirty), `eq-field—wt-documents-to-sign-feature-3035a3/…/eq-context-close-wt` (dirty), `eq-shell/…/eq-context-reg2` (dirty), `eq-shell/…/eq-context-sprint-update` (dirty), `eq-shell—wt-wonderful-brahmagupta-fa0d32/…/eq-context-sprint` (dirty), `eq-shell/…/eq-context-close` (1 commit ahead of its upstream — the one genuinely at risk if deleted blind). Whoever owns those sessions should commit/push or explicitly abandon before this list is revisited. _(added 2026-09-04)_
