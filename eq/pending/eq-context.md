...
- [x] **Real gap found while checking this work, not caused by it: 6 changelog entries the session log confidently narrates as written (`eq/changelog/eq-shell.md`, PRs #1465/#1467/#1468/#1469/#1470/#1472) never actually landed on `origin/main`.** Confirmed via direct grep of the live file — the tail still ended at 2026-08-18 before this session, nothing from 2026-08-19/20 existed there despite multiple session-log chapters today describing the write as done. **Resolved same session, via `/decide`:** all 6 now written — 3 (#1468/#1469/#1470) cross-checked against direct firsthand evidence already gathered earlier this same session (live jvkn queries, `gh pr view`), the other 3 (#1465/#1467/#1472) scanned against eq-shell's own git log first (`git log --all | grep <hash>`) to confirm every cited commit hash and title is real before writing from the session log's account of it — not a blanket secondhand transcription. _(added 2026-08-20, resolved 2026-08-20)_
