...
## eq-context: `safe_commit.py` silently overwrote a concurrent session's register entry — caught, restored, and independently hardened by another session (2026-09-09)
...
- [ ] **Regex-tightening for F10's false-positive shape** — F10's `signal` regex still matches an incidental mention (a guard's own diagnostic firing as designed) as readily as a real incident. F1's half of this item is now DONE (2026-09-09, commit [`4503f57`](https://github.com/eq-solutions/eq-context/commit/4503f5798174d7ce638c0b9d03d83249589cd975) — gated all three shape alternatives behind a same-line substrate-specific anchor, verified against the full 152-file `sessions/*.md` corpus with zero new matches, 10 new unit tests in `.github/scripts/test_failure_recurrence_signals.py`; see `system/failures.md`'s F1 entry for full detail). F10 remains open — same shape of fix, not attempted this pass. _(added 2026-08-30, F1 half closed 2026-09-09)_
