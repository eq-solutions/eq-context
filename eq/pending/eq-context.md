...
- [ ] **Real gap found while checking this work, not caused by it: 6 changelog entries the session log confidently narrates as written (`eq/changelog/eq-shell.md`, PRs #1465/#1467/#1468/#1469/#1470/#1472) never actually landed on `origin/main`.** Confirmed via direct grep of the live file — the tail still ended at 2026-08-18 before this session, nothing from 2026-08-19/20 existed there despite multiple session-log chapters today describing the write as done. Added this session's own #1468 entry directly (verified firsthand, see changelog); the other 5 need whoever has firsthand knowledge of each PR to backfill for real rather than reconstructed secondhand from the session log. _(added 2026-08-20)_
