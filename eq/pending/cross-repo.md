...
## eq-shell + eq-field: Document Sign-off Audit follow-through — doc_type guard shipped (PR open), and Brian's 12 incident rows have vanished from the live table with no record of who/why (2026-08-20)
*Continuation of the 2026-08-19 audit (this file's own T4 entry, closed out and trimmed by a concurrent session earlier today). Built the two remaining "tidy the edges" items, then found something bigger while starting the third (reminder-cron reliability): the incident's actual 12 signoff rows are gone.*

- [x] **doc_type sign-off guard, both write paths** — `NO_SIGNOFF_DOC_TYPES` (template/om) skipped Push in the UI and was filtered from the Register, but neither path that actually creates a `document_signoffs` row ever checked it server-side. `entity-patch.ts`'s onboarding auto-push now excludes these types from its category query; `push-document-audience.ts`'s `handlePush` now rejects (400) a direct push against one, reusing the constant already in that file. `tsc -b` clean, 276/276 tests, eslint clean. eq-shell [PR #1475](https://github.com/eq-solutions/eq-shell/pull/1475) — **open, not merged.**
- [ ] **Brian Griffin-Colls's 12 `document_signoffs` rows are gone.** Live query on ehow right now: the table holds exactly 1 row total (Royce's own original 2026-08-03 pilot signature, `status: signed`). Not resolved, not signed, not archived-with-a-flag — just not there. Separately, `app_data.document_categories."DB Schedules".is_onboarding_bundle` is now `false` (it was `true` when the audit ran — that flag is literally what caused the incident). Neither change was made by this session, and no session logged anywhere in today's log explains either one — checked. Two live possibilities: (a) Royce manually cleaned this up directly in the DB/admin UI, in which case this line is just confirming it for the record, or (b) something else happened that nobody's accounted for yet. **Asked Royce directly in-chat; no answer yet as of this close.** Whoever picks this up next: confirm with Royce before assuming either explanation. _(added 2026-08-20)_
