...
## eq-cards + eq-shell: Madagins tenant onboarding sprint — most gaps closed same day, 2 follow-ups remain (2026-09-09)
...
- **Shipped and deployed, same day**: two real eq-cards client bugs from the same sprint (items 8/9) — `org_admin_provider.dart` no longer truncates a multi-org admin to `rows.first`, and `required_by_org_strip.dart` now groups by `orgId` instead of the display-string `orgName` — [eq-cards#350](https://github.com/eq-solutions/eq-cards/pull/350), merged, deployed, verified live three independent ways. Turned out more load-bearing than originally scoped: `is_org_admin()`'s widened predicate (below) means an admin can already span 2+ orgs via Shell manager roles alone, no `org_memberships` row required.
- **Correction, now reflected in the sprint doc**: decision #1 ("shared database by default, dedicated project as a rare opt-in") was wrong — Royce corrected it directly, explicit and emphatic: tenants must never share a database, full stop. Madagins keeps its own dedicated project; the sprint doc's decision body and summary table now both show the correction, not the original call. Separately, Madagins' own dedicated database turned out to already be properly provisioned (`shell_control.tenant_routing`, status `active`) — the "orphaned project, nobody wants it" premise behind sprint item 5 was wrong, caught live before archiving anything.
- **Closed as unnecessary, not built**: the planned Cards-side lazy-seed fix (sprint item 6) turned out to be moot — `is_org_admin()`'s actual live definition already has a third clause checking `shell_control.user_tenant_memberships.role = 'manager'` directly, no `org_memberships` row required. Every admin RPC in the app gates through this function, so Madagins' Shell managers can already administer Cards today with no code change.
- **Backfill done, structural fix deferred**: sprint item 7 (`organisations.tier` split-brain) — Madagins' `organisations.tier` backfilled to `Advanced` to match `shell_control.tenants.tier`, so EQ Field now reads the right value. The permanent fix (trigger, or a read-through join) is bigger than a quick fix: eq-cards' own migration-apply pipeline for this database (`jvkn-control-plane-apply.yml`) has never been dispatched once, and its own header warns that bootstrapping it over unreconciled migrations "stamps them applied without ever running them."
...
- [ ] **Structural fix for `organisations.tier` sync (sprint item 7)** — needs your call on scheduling; requires first-time-bootstrapping the never-dispatched `jvkn-control-plane-apply.yml` pipeline, not something to fold into a quick fix. _(added 2026-09-09)_
...
- [ ] **A second look on auth/security PRs before merge — needs your call** — 42 of the week's 89 eq-shell PRs merged within 10 minutes of opening (median 11.9 min), 0 recorded reviews, 16 security/auth PRs with no test change, ~1,600 lines of rewritten SQL function bodies with no automated test path. Even an asynchronous review of anything touching `netlify/functions/_shared/token.ts`, `totp.ts`, `permissions.ts` or `tenant-migrations/` would have caught the fail-open deadline and the four-slice denial rollout. **Partially exercised 2026-09-04/05:** every eq-shell PR merged that evening (#1760, #1761, #1762, #1763, #1765, #1766) got an independent merge-readiness audit first, and the 4 further fail-closed follow-on PRs that landed after (#1764, #1767, #1768, #1770 — closing every gap the #1762 audit had flagged as out of scope, including the ~124-call-site `resolvePrincipal()` conversion) got a post-merge audit the next morning: **clean with caveats** — all correct, all tested against slow/failed reads, zero remaining fail-open `tenant_role_overrides` callers bar one deliberately-scoped, documented exception; the only caveat is process (same author/merger, no formal GitHub review on any of the 10, consistent with how this repo ran all night). Not yet a standing practice, still your call whether it becomes one. _(added 2026-09-04)_
