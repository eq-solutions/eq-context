...
## eq-shell + eq-field: Document Sign-off Audit follow-through — doc_type guard shipped on both write paths, Brian's-vanished-rows mystery resolved (2026-08-20)
*Continuation of the 2026-08-19 audit (this file's own T4 entry, closed out and trimmed by a concurrent session earlier today). Two "tidy the edges" items remain open below — unrelated to either resolved item, not touched by the session that resolved them.*

- [x] **doc_type sign-off guard, both write paths — MERGED.** `NO_SIGNOFF_DOC_TYPES` (template/om) skipped Push in the UI and was filtered from the Register, but neither path that actually creates a `document_signoffs` row ever checked it server-side. Built independently by two concurrent sessions from opposite directions (this file's own entry below vs. `eq/pending/eq-shell.md`'s "Documents duplicate rows" entry, same date) — reconciled rather than duplicated: `entity-patch.ts`'s onboarding auto-push guard shipped first as eq-shell [PR #1474](https://github.com/eq-solutions/eq-shell/pull/1474) (`c678f1e1`); this file's own [PR #1475](https://github.com/eq-solutions/eq-shell/pull/1475) was then rebased onto it (its now-redundant `entity-patch.ts` hunk dropped, confirmed byte-identical to #1474; its distinct `push-document-audience.ts` `handlePush` guard kept and retitled to match), squash-merged (`8d864327`). Both confirmed live via exact `commit_ref` match.
- [x] **Brian Griffin-Colls's vanished 12 rows — RESOLVED, not a mystery.** It was this session's cross-session sibling: Royce reported the *symptom* (duplicate-looking rows on the Documents admin page) independently of this audit thread, which traced to the same root cause from the live-bug-report side rather than the audit side — the `DB Schedules`/`Comms` categories' `is_onboarding_bundle` flag fired on every new starter with no `doc_type` check, so Brian, William Hong, and Sonam Gurung all got phantom template-document signoffs (44 rows total, 12 of them Brian's). Royce approved (via that session's own AskUserQuestion, "Fix + clean up") un-flagging both categories and deleting all 44 stray rows — verified `status='outstanding'` + `signed_at IS NULL` on every one first, so nothing signed was touched. Full write-up: `eq/pending/eq-shell.md`'s "Documents duplicate rows" entry, same date. Neither explanation guessed in the line this replaces was quite right — not Royce acting directly, and not an unexplained third party; a concurrent Claude session, with Royce's explicit in-chat approval, a few PRs away in the same repo the same afternoon.
- [ ] **Reminder-cron reliability pass — investigated, not built, paused by the finding above.** `scheduled-document-signoff-reminders.ts` deliberately does NOT use `due_at` today — its own header comment explains why (nothing used to set it) — but that comment is now stale: `entity-patch.ts`'s onboarding auto-push *does* compute and pass a real `dueAt` (confirmed live — Brian's now-vanished rows had `due_at: 2025-09-01` set). The manual admin push path (`handlePush`) genuinely still never sets one. Any cadence fix needs to handle both cases (`due_at`-based when present, current `assigned_at`+7d fallback when null), not just flip the whole cron over. Bundled with 3 smaller items from the same audit finding: backoff for signers whose email never delivers (currently retried daily forever, no ceiling), surfacing `last_reminded_at`/`reminder_count` in the Register tab (columns exist, nothing reads them), and swapping the certificate/register to `signed_content_hash` instead of `content_hash`. _(added 2026-08-20)_
- [ ] **Origin-guard on `document-version-upload-init.ts`** — audit's last "tidy the edges" item. Its 3 sibling upload endpoints (`upload-document-version.ts`, `document-version-upload-commit.ts`, one more) all import `checkShellOrigin`; this one doesn't. Confirmed structurally (import lists), not yet fixed. _(added 2026-08-20)_
