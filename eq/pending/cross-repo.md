...
- [x] **P1 CONFIRMED FIXED, verified live 2026-09-05.** The Field panel had rendered blank 4/4 times in Royce's own signed-in session (18:02–18:06 AEST, 4 Sep): iframe mounted, correct src, white after 8/10/10/20 s, no spinner, tab unresponsive after the 20-s wait. Code-side fix landed the same evening — eq-shell #1758 (10s stall notice + Retry), #1765 (gates the notice on Field's own `rendered` signal instead of the unreliable iframe `load` event), eq-field #917/v3.5.674 (Field posts `rendered`/`render-failed` once its own boot settles). **Hand-check done 2026-09-05 09:14–09:18 AEST** (fresh tab, full reload, twice, no cache): the handoff completes in ~4 s with the full telemetry chain firing clean every time (`mint-start → mint-ok → src-set → boot → iframe-load → accepted → rendered`, zero errors) and real content renders (`?tab=dashboard`, `?tab=schedule` both confirmed with live data). **One separate, much smaller bug found in the process** (spun off as `task_5c33a025`, not this P1): navigating to `/sks/field` with no `tab=` param defaults to `?tab=home`, which EQ Field's router has no case for — the handoff still completes and "rendered" still fires, but the content pane stays blank until the user clicks a real nav item. First-time/no-deep-link visitors only; anyone with a saved link to a specific tab is unaffected. _(added 2026-09-04, closed 2026-09-05)_
