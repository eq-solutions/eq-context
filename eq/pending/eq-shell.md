...
- [x] **(2026-08-17) Base-permissions drawer + search** — confirmed live: typing "licence" correctly dims every column except Field; clicking a role×app cell (Manager/Field) opens the diffed drawer, "93 of 94 granted — matches the Manager default exactly," with its own filter box and collapsible per-area sections (Roster, Timesheets, etc.), each showing a live count.
- [x] **(2026-08-18) Ring visual + tab strip + real permissions in Custom groups** — confirmed live: every role×app cell renders a proportional ring (not a placeholder), the amber "Custom override" ring/legend matches real `tenant_role_overrides` rows, and the tab strip (Base permissions / Custom groups / Preview a person / Activity) all switch correctly. Custom groups tab lists 4 real groups (Calendar Digest Recipients, Field Managers, Service tech, Staff Conversations) with real descriptions, not raw group names.
- [x] **(2026-08-25/26) "Preview a person" — grants AND denials, both checked** — confirmed live on two real people: Anthony Hartley (supervisor) shows `Role overrides: +Approve quotes, +Show records in Field & Service, +Add records`; Aiden Crowley (apprentice) shows `Role overrides: -View imports, -View records, -View field information` — the exact 3 live denial rows already known to exist for apprentice/employee. The "what they actually see" block is genuinely honest, not just for managers: Aiden's preview correctly says "Never sees this dashboard on sign-in — lands on a two/three-tile WorkerHome page instead, with no sidebar at all." Grouped/plain-English sections (Effective permissions + a separate Field permissions block) render with correct live counts for both people.
- [x] **Non-platform-admin can't see the grant/revoke control — confirmed, but by code, not by a second login**: no second non-admin session was available to click-test the negative case directly. Read both gates instead — client (`src/pages/AdminEditUser.tsx:960`, the whole "Platform admin" section is wrapped in `{isPlatformAdmin && target.id !== session?.user.id && (...)}`) and server (`netlify/functions/set-platform-admin.ts:64`, `if (!session.is_platform_admin) return 403 'platform-admin-only'`, plus a `self-action-forbidden` guard and an audit-log write). Both are real, independent checks, not just a hidden button.
- [ ] **The one piece not done: actually clicking Grant/Revoke platform admin end-to-end.** Deliberately not tested against a real employee — granting or revoking "every permission, in every tenant," even briefly and reversibly, is real enough that it needs either Royce's own hands or a disposable test account named for the purpose. Nobody's pointed at one yet.

Minor, likely unrelated observation: the Access Control tab went briefly unresponsive to the browser automation mid-session (script injection timed out repeatedly) and recovered cleanly on a plain reload, no data loss or console error seen. Could easily be the automation browser's own resource contention rather than the app — not treated as a finding, noted only in case it recurs. _(added 2026-08-17, 2026-08-18, 2026-08-25; consolidated 2026-09-05; click-tested 2026-09-05)_
...
*Four findings from a 2026-09-04 code review of `origin/main`, each verified against live code before touching it and fixed in its own PR: `crm-write.ts` validated a chosen contact only after already writing the site/role changes (now validates every contact up front in both `update_site` and `add_site`, [PR #1757](https://github.com/eq-solutions/eq-shell/pull/1757)); `staff-employment-purge.ts` deleted across dependent tables with no tenant/org predicate and swallowed delete errors (now every delete carries an explicit tenant or org scope, and a `workers` row owned by a different login is refused rather than deleted, [PR #1759](https://github.com/eq-solutions/eq-shell/pull/1759)); `shell-join-tenant.ts` rate-limited by phone number only, checked *after* token validation — an attacker could burn a real phone's rate-limit window without ever presenting a valid token (now an IP-keyed bucket gates before validation, the phone-keyed bucket stays after, [PR #1760](https://github.com/eq-solutions/eq-shell/pull/1760)); and duplicate numeric migration prefixes had no CI guard, closing the deferred item from 2026-08-30 below ([PR #1761](https://github.com/eq-solutions/eq-shell/pull/1761)). PR #1760 touches an auth door and was held for Royce's explicit go-ahead before merge; the other three merged under a general "merge if safe" instruction. All four confirmed live via commit-ancestry check against the production deploy, not just "merged."*
...
- [ ] **`chore(intake): auto re-vendor eq-intake/eq-platform` opened as two separate PRs on consecutive days (#1753, #1756) for the same upstream change** instead of updating one — the workflow doesn't appear to check for an already-open PR before opening a new one. #1753 closed as a duplicate of #1756 (confirmed byte-identical patch, just cut from an older base commit); #1756 itself is still open and unreviewed. _(added 2026-09-05)_
...
- [x] **2 dangling `app_data.staff.cards_worker_id` pointers on ehow** (Sentry EQ-SHELL-14) — re-investigated 2026-09-05 (a full suite-wide Sentry sweep, not just re-checking this line): both rows are self-labeled test fixtures ("TEST VERIFICATION"/"DO-NOT-CONTACT", "Alpha Tester"), not the labour-hire duplicates this line originally assumed — zero FK references across all 20 tables that reference `app_data.staff`, confirmed before deleting. Royce ran the DELETE directly (same classifier block as any live-prod DML); re-verified 0 rows remain, marked resolved in Sentry.
