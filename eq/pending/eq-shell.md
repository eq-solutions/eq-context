...
*Investigation started from a data question: why is `app_data.staff.start_date` NULL on 52 of 74 active SKS staff (100% of rows created since 2026-08-05)? Traced across eq-shell, eq-field, and eq-cards to find every write path, built a fix, then used `/decide` to pressure-test it — which caught that the main fix mostly doesn't fire for SKS, and led to a second, more effective build.*

- [x] **Root cause**: 4-5 independent code paths write `app_data.staff` on ehow, and none of them reliably captured `start_date` for SKS specifically — eq-field's own Add Person form has the field but its entry points are deliberately CSS-hidden on the `sks` tenant (people are meant to flow from Cards, a prior deliberate decision); eq-shell's `staff-create.ts` (Shell manual roster-add) never asked for it at all; eq-cards' `workers-canonical-sync` and `labour-hire-candidate-intake` have zero start_date concept anywhere in their data model (confirmed via a zero-match repo-wide grep). Proven live, not just inferred: 9 `app_data.staff` rows were created during the 2026-08-18→23 window when the Field UI's only write path (the `field_people_iud` trigger) was provably detached — meaning a second, independent write path was active the whole time.
- [x] **Built**: optional `start_date` on `cards-approve-staff.ts`'s two branches (Cards application approval — the point a manager reviews one specific new person) and `staff-create.ts`'s two branches (Shell manual roster-add), plus matching date-input UI in `StaffPage.tsx` and `AddToRosterModal.tsx`. Fill-if-missing only, never overwrites an already-set value.
- [x] **`/decide` pass caught a real gap before it shipped as "the fix"**: `workers-canonical-sync` (eq-cards) auto-sets `field_approved: true` and inserts `org_access_requests` as already `status: 'approved'` for SKS specifically — its own code comment says why: "SKS's accepts-applications config means no admin ever reviews or approves this." The one live UI caller of the `cards-approve-staff.ts` application-path branch (`StaffPage.tsx`'s pending-review queue) is very likely always empty for SKS, so that capture point mostly doesn't fire for the tenant this was actually about. Only `staff-create.ts`/`AddToRosterModal.tsx` (Shell manual add) is a confirmed-live SKS path — and it's the lowest-volume one historically (2 of 74 rows).
- [x] **Built a second fix that works regardless of tenant config**: a "missing a start date" count + filter tab on `StaffResourcingPage.tsx`, using data the page already fetches (no new query), so the gap is visible and fixable via Field's already-working Edit Person page no matter which door someone came through. `tenureLabel(null)` now reads "Not set" instead of a bare em dash.
- [x] eq-shell [PR #1674](https://github.com/eq-solutions/eq-shell/pull/1674) — independent merge-readiness audit (tests 453/0, full prod-build reproduction, clean dry-run merge, no auth-adjacent risk), all 5 required CI checks passed, squash-merged (`7b3eaccd`), confirmed live.
- [x] **Two follow-up investigations spawned mid-session, both resolved**: a CSV people-import bug (unconditional full-row PATCH nulled `start_date` + 6 other optional fields on every re-import, not just when the StartDate column was missing) — fixed in eq-field [PR #831](https://github.com/eq-solutions/eq-field/pull/831), not yet merged, no confirmed real damage found in `audit_log`. And a possible duplicate-staff check (Conor Horgan / Nelson Sareto each got 2 rows during the trigger-outage window) — resolved, zero live double-counting (only one row per person is active) — but surfaced a real, still-open bug: `cards-approve-staff.ts`'s `findExistingStaff()` only matches against `active=true` rows, so a stub deactivated between two Cards submissions from the same person mints a duplicate instead of reactivating. Not fixed — a real behaviour-change call, not silently patched.
...
- [ ] **`findExistingStaff()`'s active-only matching** (`cards-approve-staff.ts`) — real recurring bug, not fixed. Changing it to also consider a recently-deactivated row risks auto-reviving something someone deliberately archived — Royce's call. Possibly already in progress: a branch named `claude/cards-approve-dedup-inactive-match` was seen checked out mid-session by what looked like a concurrent session. _(added 2026-08-30)_
