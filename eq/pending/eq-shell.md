...
- [ ] **Same gap, tenant-plane version — NOT covered by #1638, not decided.** The `field_managers`/`field_people_directory` CHECK 7 incident (see the dedicated entry above, now archived — fully resolved via #1642) is the identical shape one plane over: eq-field changed something eq-shell's CI treats as absolute, zero pre-merge signal either way. #1638 explicitly does not cover this — confirmed by reading its own description, it omits the tenant-plane secrets on purpose, jvkn-only. eq-field is also a structurally different "other repo" than eq-cards for this purpose: confirmed live, none of its 9 `.github/workflows/*.yml` touch Supabase/migrations at all — it hand-applies `supabase/migrations/*.sql` via the Supabase MCP with no CI-driven apply step, so the #1638 reusable-workflow shape has no equivalent hook point to attach to. Sprinted per Royce's ask: [eq/sprints/2026-08-27-tenant-plane-cross-repo-consumer-check.md](../sprints/2026-08-27-tenant-plane-cross-repo-consumer-check.md) — 3 options laid out (PR-time check on eq-field, strengthen the existing 3-hourly schedule + auto-filed issue with an auto-attributed likely-source link, or documentation-only), leans option 2, genuinely Royce's call. _(added 2026-08-27)_
