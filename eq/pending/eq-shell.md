...
---
## eq-shell: security register reconciled — SEC-71 + SEC-74 open; SEC-72 + SEC-73 closed (2026-09-04/05)
...
- [x] **SEC-74 — `resolvePrincipal()` fails open on `tenant_role_overrides` across ~124 functions (P2). CLOSED 2026-09-05 via eq-shell [#1770](https://github.com/eq-solutions/eq-shell/pull/1770) (`4dc8e8e9`), merged by Royce and confirmed live.** Corrected from this row's own original text minutes earlier, which recorded the opposite call (leave as-is, accepted risk) from a different concurrent session — two of Royce's sessions gave different instructions on the same question within about 12 minutes, both were acted on, and #1770 is what's actually live now. **Open question for Royce, not resolved here: should #1770 stand, or should it be reverted back to the accepted-risk framing this row originally recorded?** Full detail in `ops/security-register.md` SEC-74 and `sessions/2026-09-05.md`. _(added 2026-09-05, corrected same day)_
