...
- [x] **Root cause: EQ Field tells Shell it signed in successfully BEFORE it has drawn anything on screen** (`auth-shell-handoff.js:239`, fired the instant sign-in verifies, before Field's own page render runs) — Shell removes its loading spinner on that message, so a Field frame that hangs afterward is a blank white pane with nothing on top and no way out. The one safety timer that existed only ran for a Field tab the user had already opened, not for the pre-loaded copy Shell keeps warming in the background (the normal case since the 2026-09-02 pre-warm work) — so a background hang could sit invisible until the user actually clicked into Field.
- [x] **PR [#1758](https://github.com/eq-solutions/eq-shell/pull/1758)** — after 10 seconds with no sign-in response, shows "Still loading EQ Field… / Retry" instead of spinning forever; if sign-in succeeded but the page still hasn't drawn 10 seconds later, a small "Reload EQ Field" strip appears without hiding whatever Field is doing underneath; the sign-in request itself now times out after 20 seconds instead of never; and a pre-loaded copy that's gone stale by the time the user opens Field now retries immediately instead of making them sit through the old 30-second wait. Every step of the sign-in handoff now reports to Sentry and PostHog with timing, so the next stall names exactly where it got stuck. No auth/permission logic changed. `tsc -b --force`, eslint, and the full test suite (491/491) all clean. Squash-merged `b3c687f1` by Royce; confirmed live same day.
- [x] **Click-tested live 2026-09-05, on Royce's own signed-in session (SKS Technologies, real Chrome via Claude in Chrome) — works as designed.** Deliberately blocked the sign-in request, navigated straight into EQ Field: the "Still loading EQ Field…" card appeared at exactly 10 seconds (confirmed by the handoff's own timestamped log, `mint-start` → `stall-notice` 10.0s apart), Retry genuinely fired a second sign-in attempt (not just a UI reset — confirmed the request count actually incremented) and returned cleanly to the loading state. Unblocked and reloaded: EQ Field signed in and rendered normally end to end, leaving a real working session behind, not a broken one.
- [x] **Checked whether EQ-SHELL-T/V (the other Sentry auth-stall pair, "verify-timeout" / "session-spinner-timeout") explained the same symptom — they don't, and PR #1736 (merged 2026-09-02) did not close them.** Both are Shell's own sign-in check taking too long, not the Field handoff above; both kept firing on `/sks/field` after #1736 merged — EQ-SHELL-T 3 more times (last 2026-09-04 02:17 UTC), EQ-SHELL-V's last event lands at the identical timestamp (same user session: Chrome/Mac, Sydney). #1736 only put a time limit on 4 of the 11 background checks that sign-in step runs; the other 7 had none.
- [x] **The durable EQ Field fix shipped the same day, not just spawned as a task.** [eq-shell PR #1765](https://github.com/eq-solutions/eq-shell/pull/1765) (`d5ee79a5`, live, published 2026-09-04T20:50:44Z) added a real `rendered` signal from EQ Field's own boot completion and gates Shell's "did it actually draw" check on that instead of just the iframe's `load` event — closing the one remaining case this session's own PR #1758 couldn't cover (a hang after both sign-in success and `load` but before Field had actually drawn anything). Confirmed genuinely live, not just deployed: the click-test above independently caught a real `rendered` event firing in the console during a normal (non-stalled) EQ Field open — proof this is doing its job on real traffic, not just passing a synthetic check.
- [x] **The EQ-SHELL-T/V root cause also has a fix live, same day.** [eq-shell PR #1764](https://github.com/eq-solutions/eq-shell/pull/1764) (`d434a407`) bounds the remaining 7 of 11 background checks in Shell's sign-in verification that #1736 left untouched. Confirmed live: newest ready production deploy for `d434a407`, published 2026-09-04T21:01:17Z, and `verify-shell-session` still answers correctly post-deploy. **Not yet confirmed to have actually stopped EQ-SHELL-T/V recurring** — that needs a few days of real traffic against Sentry, not a deploy record. _(added 2026-09-05)_
- [ ] **Watch EQ-SHELL-T/V in Sentry for a few more days before calling them closed.** Both PRs (#1758, #1764) that plausibly explain every occurrence so far are live as of 2026-09-04 evening; check back ~2026-09-08 for any new occurrence with a timestamp after both deploys. If clean, close both issues in Sentry and tick this row. _(added 2026-09-05)_
...
- [x] **SEC-72 — session / Field-JWT mint fails open on denials when the `tenant_role_overrides` read exceeds 6 s (P2). CLOSED 2026-09-05.** Fix: eq-shell [PR #1762](https://github.com/eq-solutions/eq-shell/pull/1762) (`6c6ec81b95375`), merged 2026-09-04, confirmed live via commit-ancestry against production deploy `6a9b2b399c4c650008b181ab` (the PR's own deploy was superseded/skipped by a concurrent merge — ancestry against the live deploy is the correct check, not the PR's own deploy record). `verify-shell-session` issues the session with every overridable key hard-denied when the read can't be resolved; `token-exchange` instead refuses the Field mint with a 503 rather than risk a 7-day crippled Field session (eq-field re-signs the handoff). Same day, the three other JWT minters this row originally flagged as a residual were also closed — eq-shell [PR #1767](https://github.com/eq-solutions/eq-shell/pull/1767) (`726ba3ef`) and a sixth call site found in that work, eq-shell [PR #1768](https://github.com/eq-solutions/eq-shell/pull/1768) (`364993e3`) — both merged and confirmed live directly (no supersede). One residual remains (cache over-restriction on a removed denial for up to two polls), plus one deliberate, undecided hold (`_shared/permissions.ts`'s `resolvePrincipal()`, ~124 call sites, needs Royce's explicit call before anyone widens the fix that far). Full detail in `ops/security-register.md` SEC-72. _(added 2026-09-04, closed 2026-09-05, updated 2026-09-05)_
- [x] **SEC-73 — eq-field's definer-rights `field_people_directory` / `field_managers` views on both tenant planes (P2 latent).** Decided 2026-09-04 (`/decide` pass): accept as a governed exception, review 2026-12-04. Two PRs, merged 2026-09-04: eq-context [#203](https://github.com/eq-solutions/eq-context/pull/203) `bedc7e9` (baselines the 4 advisor errors + a test that fails once a baselined review date is past) and eq-shell [#1763](https://github.com/eq-solutions/eq-shell/pull/1763) `0b37676` (CHECK 7 comment refresh + `allowedColumns` guard). **Shipped the same day, #1763 commit 2:** `allowedColumns` on both CHECK 7 exception entries (live 2026-09-04 lists, managers 11 / directory 24); `checkViewInvoker` fails on any column outside them, fail-closed on an unreadable list; the PR's drift-gate run passed live on both planes with the allow-list verified. #203 also gained the review_by date guard (`overdue_reviews()` + tests, 13/13). Ticked 2026-09-04 at the go-live-review session's request: the call is made and both guards are up; residual risk tracked in the register row (OPEN, accepted, review_by 2026-12-04), and the weekly audit's next run on `main` confirmed green: [run 33915167442](https://github.com/eq-solutions/eq-context/actions/runs/33915167442) — all four findings ACCEPTED under SEC-73. _(added 2026-09-04)_
...
- [x] **PR #1738** — Register's Active view gets a checkbox per document group, a select-all, and a "N selected → Push selected" bar (same `runPooled` bulk pattern `TemplatesTable` already uses for bulk category assignment). `PushMoreModal` generalized from one `RegisterGroup` to an array: a single entry behaves exactly as before (same inherited default site/customer scope), 2+ entries starts the picker empty and fans each submit out across every selected document, reporting a combined resolved/new/failed count instead of erroring the whole batch on one failure. No backend change — reuses the existing push endpoint per document. Merged `9b618cb0`, live 2026-09-02 00:03 UTC (confirmed via ancestry check against the newest ready production deploy, not just this commit's own deploy row, which had already been superseded by a later concurrent merge by the time of checking).
- [x] **Click-tested live by a person, not just deployed** — selected 2 real documents (the same SWMS-005/SWMS-008 from earlier today), submitted a real push through the actual UI on Royce's own authenticated session (driven via Claude in Chrome). Deliberately pushed to one named person (Royce himself) rather than a role/crew, specifically to avoid dropping a real outstanding task on other real staff members' lists as a side effect of testing. Confirmed in the Register itself afterward: both documents show the new signer, "Pushed to 2 people... across 2 documents" summary rendered correctly, selection cleared on modal close.
...
- [x] **PR #1724** — `labour_hire` role mislabeled "Contractor" fixed; Gotenberg conversion timeout 15s→22s (Sentry `EQ-SHELL-1Y` — a cold-start tax on the first hits right after a fresh deploy, not a steady-state problem; confirmed 1.6–3.8s once warm); "Convert missing PDFs" manual backfill button added (Reference library tab at ship time). Merged `3c8a3d35`, live 2026-09-01 10:54 UTC.
- [x] **PR #1725** — Register's per-document signer list redesigned: completion bar, outstanding-only default view, already-signed rows collapsed behind a disclosure. Pure rendering change over existing `RegisterGroup.signers` data, no schema/endpoint change. Merged `1338e932`, live 2026-09-01 11:15 UTC.
- [x] **PR #1726** — Unarchive action added (mirrors Archive exactly: same confirm-dialog pattern, same `documents.manage` gate, same `handleSetDocumentStatus` handler underneath with a `targetStatus` param). "Convert missing PDFs" moved from Reference library to Register, next to Manage categories — Royce's own framing: the PDF-view payoff is on Register (`SignoffEvidenceModal`'s inline viewer), Reference library "doesn't need the PDF feature." Merged `caca597f`, live 2026-09-01 18:24 UTC.
- [x] **Click-tested live by a person, not just deployed** — archived and unarchived all 3 originally-stuck SWMS documents through the real admin UI (Royce's own authenticated session, driven via Claude in Chrome), confirmed every signer (32/32/1) survives the round-trip with status intact. Fired "Convert missing PDFs" live mid-test — all 3 are now `pdf_status: 'ready'` with a stored PDF twin (had been stuck `failed` since before this session's timeout fix).
