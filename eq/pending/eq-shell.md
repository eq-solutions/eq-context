...
- [x] **Field staff records created for all 5 people** — Royce, Aditi, Michelle (Manager), Nelson Sareto, Conor Horgan (Labour Hire), each linked to their real Shell identity via `user_id`, `field_approved=true`, confirmed showing in the `app_data.field_people` view Field's own UI reads. Root cause was architectural, not a bug: `field_people` is a filtered read view over `staff`, not a separate input — nothing bridges Shell's tenant membership into Field's own roster automatically, by design. A legacy `field_people_iud()` trigger function exists in the schema but isn't wired to anything on this tenant.
