...
- [~] **Writer audit completed; one real gap found and fixed, not yet merged.** All remaining eq-shell `from('staff')` functions traced to source: `staff-licence-backfill`, `delete-user`, `backfill-worker-links`, `cards-approve-staff` (fully now, not just partially), `staff-teams`, `staff-resync-licences`, `staff-record-licence-review`, `shell-login-phone-otp`, `licence-push`, `licence-ocr-commit` — all CONFORM to §3.3 (full per-file table in `IDENTITY-MODEL.md` §3.3's condition-1 audit). `push-document-audience`/`comms-jobs` were false positives from the original grep — no staff-identity writes at all. One real gap found: `entity-patch.ts` pushed email/phone/name upward but left `date_of_birth`/`address_*` tenant-local forever (its own code comment already flagged this) — extended the existing upward-push block to cover them too. eq-shell [PR #1555](https://github.com/eq-solutions/eq-shell/pull/1555), CI green. _(2026-08-23)_
- [ ] **DB-side writer catalog queried live (ehow + zaap) — the writer audit's biggest finding, fix built, not yet merged.** `app_data.field_people_iud()` — the INSTEAD OF trigger behind Field's own Add Person/Edit Roster screens — exists **only on ehow, not on zaap** (real cross-tenant drift in Field's own write path) and writes identity content straight to `app_data.staff` with **zero path back to jvkn**. Same class of gap as the two items above, in code a `from('staff')` grep can't see at all (it's DB-side SQL, not a Netlify function). Fix mirrors jvkn's own `sync_worker_to_canonical()` pattern reversed — vault secret → `net.http_post` → new eq-shell endpoint `field-identity-push.ts` — since Field has no Netlify function of its own in the write path to piggyback on. eq-field [PR #761](https://github.com/eq-solutions/eq-field/pull/761) (trigger) + eq-shell PR #1555's second commit (receiver). **Needs two out-of-band secrets before it's live** (`field_identity_push_secret` on ehow's vault, `FIELD_IDENTITY_PUSH_SECRET` on eq-shell's Netlify env) — deliberately not created, explicit go needed. Also found and deliberately left alone: `eq_cards_upsert_my_profile`/`eq_cards_upsert_my_licence` + 3 siblings exist on both ehow and zaap and would bypass jvkn entirely if called, but confirmed `service_role`-only on both planes with no caller found in eq-shell/eq-cards source — almost certainly a pre-canonical-Cards fossil, a cleanup candidate, not a live risk. _(added 2026-08-23)_
