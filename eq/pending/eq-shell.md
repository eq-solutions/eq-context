...
- [ ] **CHECK 7's new reviewed-exception logic not yet exercised against a real, live violation in CI** — only verified via a standalone Node simulation and the two legitimate entries. Small, mechanical, no design call needed (unlike the item above) — scoped in the same sprint doc §2: create a disposable scratch view on a non-production project that intentionally violates the guarded property, confirm CHECK 7 fails with the expected reason, drop it. ~15 minutes whenever someone has a spare Supabase project. _(added 2026-08-27)_
- [x] **CHECK 11 (jvkn migration-identity) — blocker found and fixed; CHECK 11 itself still not built.** Live comparison of eq-shell's own 144 `supabase/migrations/*.sql` against jvkn's native ledger (`supabase_migrations.schema_migrations`) found 86-92 with no matching entry under any name pattern tried — including migrations confirmed applied and live this same session (`2026_08_26_restore_organisations_anon_bootstrap_read_again.sql`, PR #1618 — zero matching entry). That table isn't a reliable record of what's applied to jvkn; a check built on it would misreport eq-shell's own legitimate history permanently, not just need a cutoff. Root cause: jvkn had no governed apply path. Royce: build eq-shell's own apply-path runner first, hold eq-cards. Shipped and **merged**: [eq-shell#1641](https://github.com/eq-solutions/eq-shell/pull/1641) (`c3bf2c8f`, 10:10 UTC) — `scripts/migrate-control-plane.mjs` + `control-plane-migrate.yml`, modeled on the proven `migrate-tenants.mjs`/`tenant-migrate.yml` pattern, writing to a new dedicated ledger (`shell_control._eq_control_plane_migrations`). First merge attempt was blocked by the unrelated `field_managers`/CHECK 7 repo-wide failure above; a plain CI re-run stayed stale (replays the original merge snapshot, doesn't recompute against new `main`) — fixed by rebasing the branch onto current `main` and force-pushing, which forced a genuinely fresh run. **`--bootstrap` dispatched and verified live 2026-08-27 10:21 UTC**: `shell_control._eq_control_plane_migrations` has exactly 144 rows, all stamped in the run's own execution window — confirmed via direct query, not just the job log. The apply path is live for real going forward. eq-cards' own migrations against the same database still have no governed path — explicitly on hold, not this PR's scope. **Still open: CHECK 11 itself** — the check code hasn't been written yet; needs a cutover-date design call now that there's finally a trustworthy ledger to build it against. _(deferred by #1628, investigated + blocker found 2026-08-27, runner PR opened + merged + bootstrapped 2026-08-27)_
