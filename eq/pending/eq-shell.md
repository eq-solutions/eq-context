...
- [x] ~~`CHECK 14` (tenant/self/org isolation invariant for jvkn, analog of CHECK 5) — scoped, not started~~ — **built and merged**: investigated live (mapped every browser-reachable jvkn table's isolation key before writing code — same discipline as CHECK 12/13), found and fixed a real design bug along the way (a "reachable" test based on policy roles alone produced 13 false positives — tables with a broad-role policy but no actual anon/authenticated table grant, so the policy is inert; corrected to require both). Dry-run + real CI both confirmed clean. eq-shell [PR #1633](https://github.com/eq-solutions/eq-shell/pull/1633), squash-merged (`ef075d5f`). **Live-deploy commit-ancestry not independently confirmed** — the usual `npx netlify api listSiteDeploys` check was blocked by this environment's own sandbox classifier (npx execution pattern) and no substitute was found (no list-deploys operation in the available Netlify MCP tools, no NETLIFY_*_TOKEN env var for a direct curl, no per-commit Netlify status posted for main-branch pushes — only PR deploy-previews get one). Lower stakes than usual since this is a CI-script-only change with no app/UI surface, but flagging the gap itself: a future session hitting this same tooling wall should know it's a known dead end, not worth re-exploring, and should escalate to a Chrome/browser walkthrough of the Netlify dashboard if a deploy genuinely needs confirming. _(added 2026-08-27, closed 2026-08-27)_
