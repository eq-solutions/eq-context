...
## eq-shell: madagins's own database now fully caught up (governed pipeline: 38 applied, 0 failures); a "tenant_routing missing" finding logged here earlier today was WRONG — corrected below, do not act on it (2026-09-09)
...
- **CORRECTION (later the same day, same session): the "`tenant_routing` row missing" claim originally in this bullet was wrong — nothing needs restoring.** It was found by joining `shell_control.tenant_routing.tenant_id` against `public.organisations.id` — the wrong key. `tenant_routing.tenant_id` has a foreign key to `shell_control.tenants(id)`, a separate id space entirely (surfaced by literally hitting that FK constraint on a since-abandoned insert attempt — caught before it landed a row under the wrong key). Under the real id (`shell_control.tenants.id = fc06cd56-ec63-4507-a03e-c3c552ea09a9` for madagins, matching what an eq-shell memory file already had on record), `tenant_routing` has a genuine, correct row: `status='active'`, `supabase_project_ref='ornndtbdkxfsewspbrwk'`, `supabase_url` correct, `provisioned_at` 2026-09-09 00:56:11 UTC. Routing was never actually broken — don't restore anything, don't click re-provision. (Original wrong claim: "`jvkn.shell_control.tenant_routing` has no row for madagins right now" — this was a bad-join false alarm, not a real gap. Whatever's caused this row to look like it flipped present/missing across sessions today per other entries in this file may be a similar join-key mixup rather than a real oscillation — worth re-checking with the correct key before trusting any of those either.)
...
- [x] ~~Restore `shell_control.tenant_routing` for madagins~~ — **not actually needed.** The "missing row" finding was a bad-join false alarm (joined against `organisations.id` instead of `shell_control.tenants.id`) — the real row was `status='active'` and correct the whole time. See the corrected write-up above. _(added 2026-09-09, closed 2026-09-09 as a false alarm)_
