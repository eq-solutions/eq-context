...
*Royce asked for a critique of the Resourcing feature against its actual goal: "ensure employees are being spoken to, happy and engaged + identify who wants to grow." Live data (74 active SKS staff, 60 never had a conversation logged, 27 historical reviews backfilled from Royce's HR folder but 0 with a happy_engaged answer, only 2 members of the gating security group and one of those inactive) grounded the critique before any code was written. Royce then asked for the highest-value fixes plus two complaints from actually using the page: "goes back to staff back" (full navigation away on row click) and "you cant really see previous conversations... Ive uploaded proper reviews but you cant see them / compare."*

- [x] **In-place side panel replaces full-page navigation.** Row click on the Resourcing dashboard now opens conversation history in a slide-in panel without leaving the roster (mirrors `?open=<id>`, survives refresh, shareable URL) instead of navigating to `/staff?open=...&focus=conversations`. "Open full profile" still exists inside the panel for actual editing.
- [x] **Past conversations are readable again.** A saved formal entry's `answers` (20+ fields per Development Review) was written to the DB but never rendered anywhere after the save — only an auto-generated one-line heading showed. Formal entries are now clickable and open a full read view of every field, with a `(was N, +delta)` rating comparison against the person's previous Development Review where one exists.
- [x] **"Happy and engaged?" made required and asked first**, in both templates — was optional and the last question after 20+ others; live data showed a 0% real answer rate across all 27 backfilled reviews.
- [x] **Dashboard/RLS visibility fix.** `app_data.staff_conversations` has DB-enforced creator-only RLS (content visible to no one but whoever logged it, Royce's explicit instruction behind migration 0250). The Resourcing dashboard read via a service-role client that bypassed it entirely — invisible with 1 real user, but the moment a second supervisor is granted `staff.manage_conversations` they'd have seen every other supervisor's summaries and engagement answers. Fixed by redacting content once, immediately after fetch (`redactForViewer` in `staff-resourcing.ts`), rather than per-field, so a field added to this response later can't reintroduce the leak. Coverage metadata (last spoken to, counts) stays roster-wide — Royce's explicit choice between 3 options offered.
- [x] **Default A-Z sort + an "overdue" flag** (180+ days since any conversation, new starters excluded) — closes the "proactive overdue nudges" item held open since 2026-08-12 pending real conversation dates to be stale against (unblocked when the 27 historical reviews were backfilled 2026-08-30).
- [x] **8-angle review before merge** (correctness x3, reuse, simplification, efficiency, altitude, CLAUDE.md conventions) — 10 real findings, all fixed, most notably: the new panel didn't remount `ConversationsSection` on person switch, so an in-progress "Log a conversation" entry could have silently saved under the wrong person's id if you clicked a different row mid-entry; `ENGAGEMENT_STYLE`'s `{bg,text}` shape was being spread directly as CSS keys instead of mapped, so engagement tags rendered with no color.
- [x] **eq-shell [PR #1683](https://github.com/eq-solutions/eq-shell/pull/1683), merged `ae90be65`, confirmed live** — production deploy `ready`, no error, and `core.eq.solutions/.netlify/functions/staff-resourcing` verified serving (401 unauthenticated, not 500) post-merge.
- [ ] **Not click-tested live by a person** — verified via `tsc -b --force`, `eslint`, an 8-angle automated review, and a Netlify deploy-preview smoke test; no Shell session/credentials in this environment. Worth a real pass: panel opens in place with a shareable `?open=` URL; a formal entry opens with full detail and rating deltas; saving without answering "happy and engaged" is blocked and scrolls to the field; engagement tags render with color; a person with no `start_date` is flagged "missing a start date" but NOT also "overdue". _(added 2026-08-30)_
