...
## eq-shell: fixed a cross-tenant data-invisibility bug in the Field admin picker (PR #1884, merged + live); closed the stale `0311` dispatch question; reviewed all 18 org-wide Sentry issues (2026-09-10 to 2026-09-13)
*Started from a report claiming a platform admin's Field iframe silently showed empty data on Madagins after a token-exchange 401. The report's own root-cause theory didn't survive a direct code read — the real bug was one layer over.*

- **Real bug found and fixed: platform admins using Field's own cross-tenant workspace picker got a session scoped to the wrong tenant.** `token-exchange.ts` minted the Field JWT's `tenant_id` claim from the admin's own Core session tenant (`activeTenantId`), not the tenant they picked in the dropdown. Field's RLS scopes every read by that claim, so the iframe pointed at tenant X while every query inside it evaluated against tenant Y — silent zero rows everywhere, no error anywhere in the chain. Confirmed live 2026-09-10 on Madagins (`dev@eq.solutions`, home tenant `eq`, no membership on madagins, picked `madagins` in the picker, saw an empty Contacts list despite `app_data.staff` on madagins' own project correctly holding all 5 people). Verified structurally too: `select-tenant.ts`/`verify-shell-session.ts` both require a real membership to switch a session's own active tenant — so the original "401 degrades to empty state" theory was impossible as described; `FieldIframe.tsx` already shows an explicit error card on a real token-exchange failure.
- **[PR #1884](https://github.com/eq-solutions/eq-shell/pull/1884)** — `token-exchange.ts`'s platform-admin branch now resolves the picked tenant's own `shell_control.tenants` row and requires a real membership in it (same bar as everyone else — confirmed via the file's own comment that Field has no platform-admin RLS escalation, unlike Service's `is_super_admin()` bridge), then scopes the JWT + its enrichment reads (group perms, role overrides, worker id) to that tenant and the admin's real role in it. A picked tenant with no real membership now returns `403 not-a-member-of-tenant` instead of a token that silently shows nothing; `FieldIframe.tsx` surfaces that as its own message. `tsc -b`/`eslint`/full test suite (634 tests, incl. 2 new regression tests for this fix) all clean. Merged (`7816faa`), confirmed live via commit ancestry against the deployed `404580f4`.
- **`0311` dispatch question closed — nothing to run, on either tenant.** Asked to dispatch the rescued CMMS-baseline migration to madagins; live investigation found the file is deliberately `-- Plane: ehow ONLY` scoped (added after a real fleet-wide attempt already crashed on madagins with `42P16` — madagins' `app_data.field_*` views exist there already, in a different, eq-field-owned shape) and separately confirmed already checksum-applied on ehow (`applied_at` 2026-09-09T10:11 UTC; all 61 objects live-verified present). The "dispatch to madagins" bullet carried in this file since 2026-09-09 was stale by the time it was actually actioned — see the archived write-up in `eq/pending-archive.md` for the original finding.
- **Full org-wide Sentry pass, 18 unresolved issues reviewed.** [EQ-SHELL-27](https://eq-solutions.sentry.io/issues/EQ-SHELL-27) (connect-worker status-badge crash on an unmapped `org_access_requests.status` value) confirmed already fixed live by #1870 + #1887 (both co-authored with Royce, 2026-09-09) — marked resolved in Sentry. `EQ-SHELL-22` and the newer `EQ-SHELL-1P` occurrence (stale JS chunk after a deploy, `/sks/data/staff`) confirmed self-healing by design — `src/lib/chunkReload.ts` (landed 2026-09-08/09, its own comment cites these exact issue IDs) auto-reloads the page and reports to Sentry on every occurrence *on purpose*, so it will never reach zero — not a bug. The Field-handoff-stall family (`EQ-SHELL-20/21/26/29/2A`) is the same known, monitored, self-healing telemetry from the `FieldIframe.tsx` handoff system — a separate, more thorough same-week pass on this exact cluster ([PR #1896](https://github.com/eq-solutions/eq-shell/pull/1896), merged 2026-09-13) already found and fixed one genuine false-alarm inside it (EQ-SHELL-29's stall-notice timer undercounting a slow-but-normal token mint); the rest of the cluster checked out as designed-in telemetry, matching this session's own read. `EQ-FIELD-1R` (deliberate `canon-read` tenant-slug-fallback telemetry) and `EQ-FIELD-1Q` (Madagins dashboard RPC gap, ties to the still-open `0311`-adjacent service-schema gap) reviewed, unchanged. Remainder (`EQ-FIELD-19/1N/1S`, `EQ-CARDS-1N`, `EQ-SOLVES-SERVICE-B`, `EQ-SHELL-28/1T`, `EQ-CARDS-1P`) — low-volume network noise, deliberate PIN/OTP-on-inactive-account logging, and a Flutter/CanvasKit engine-level WebGL bug in Cards not fixable in app code. None need code changes right now.

- [ ] **`EQ-SHELL-23` — Royce's own 6 licences on the madagins tenant have been invisible since 2026-09-08.** `check-missing-org-memberships` (deliberate alert-only daily monitor, `netlify/functions/check-missing-org-memberships.ts`) flags a missing `public.org_memberships` (jvkn) row for `85e30693-b467-407a-88e8-539e345b88cd` on madagins — classified `invisible_licences`, explicitly documented as fresh drift, not backlog. The checker never inserts/revokes by design ("both are operator decisions"). Needs someone to grant the membership through whatever the normal admin flow is. _(added 2026-09-13)_
- [ ] **`EQ-SHELL-1V` — 1 stale `org_membership` remaining on SKS**, down from a 12-row backlog at the check's 2026-08-23 launch (11 already cleaned up since). Held by an identity no longer a live member of the tenant. Needs an operator decision to revoke. _(added 2026-09-13)_
