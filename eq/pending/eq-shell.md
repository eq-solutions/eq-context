...
## eq-shell: provisioning-completeness audit script built, verified live, merged — surfaced 2 new untracked extensions + a live RLS gap beyond the two known incident gaps (PR #1832, 2026-09-09)
*Follow-up to the 2026-09-09 madagins tenant-provisioning incident (pg_cron never enabled by any tracked step; ~37 public-schema tables predating the tracked migration system) — this session built the ongoing CHECK for that gap class, not a fix for those two (still uncommitted elsewhere, see below).*

- **Built `scripts/check-provisioning-completeness.mjs`**: read-only, diffs everything live on ehow (extensions/schemas/tables/functions/standalone sequences) against what `supabase/tenant-migrations/*.sql` + `provision-tenant-background.ts` can actually reproduce, respecting each migration's `-- Plane:` header. Same Management-API convention as `check-tenant-drift.mjs`/`check-control-plane-drift.mjs`. Wired into `tenant-drift.yml`'s existing job, informational-only (no `--strict` yet — no clean baseline to enforce against).
- **Found and fixed 3 real bugs during verification, before shipping**: a comment in `provision-tenant-background.ts` ("Both CREATE SCHEMA calls are...") was regex-matching as a fake schema named `calls`; the platform-schema exclusion list (copied from `check-tenant-drift.mjs`) only named `pg_temp_1` literally and would have flagged ehow's 20+ live temp schemas as a different false gap every run; the `service` schema's ~30 real tables are governed entirely by `eq-solves-service`'s own separate migration pipeline, not this repo's — would have flooded every run with false gaps if not excluded.
- **First live run (via the Supabase MCP, then confirmed for real in this PR's own CI) found real, previously-unflagged gaps**: two more untracked extensions beyond pg_cron — `vector` and `pg_net` — confirmed live on ehow, absent from a same-day fresh tenant project, and absent from every tracked source. Also surfaced `shell_control` (5 tables) and `wipe_backup` (4 tables) as live-only schemas, and ~71 live-only tables / 65 live-only functions total, a meaningful share of which is very likely eq-field's own separate hand-applied pipeline writing to this same ehow database (same cross-repo-attribution shape `check-control-plane-drift.mjs`'s `KNOWN_UNSOURCED` already handles for jvkn) — not yet triaged item-by-item.
- **Discovered along the way, not fixed here**: the pg_cron fix + a new 37-table baseline migration (the two known incident gaps) are still uncommitted on the pre-existing `eq-shell-wt-pgcron` worktree (`fix/tenant-provisioning-pg-cron`, itself 3 commits behind main) — its own untracked migration file also collides on number `0308` with an already-merged file. The task brief assumed these were already fixed; verified live they are not yet on `main`.
- **Merged**: [PR #1832](https://github.com/eq-solutions/eq-shell/pull/1832), squash, on Royce's explicit instruction (this repo's merge-is-the-deploy rule applies, but the change is CI/workflow-only — zero app-bundle surface, nothing to verify live).
- **Spun off separately**: 4 `wipe_backup.*` tables have RLS disabled (Supabase's advisor flags it critical — anon/authenticated-writable, 0 rows today) — `task_e9a26fb7`, which Royce has already started in a separate session (running independently as of this write-up).

- [ ] **Triage the ~71 live-only tables / 65 live-only functions / 3 extensions / 2 schemas this check's first run found** — `KNOWN_LIVE_ONLY` in the new script ships empty on purpose. Each needs the same per-item process `check-control-plane-drift.mjs`'s `KNOWN_UNSOURCED` already uses: confirm live against the owning repo (eq-shell vs. eq-field vs. genuinely orphaned), then backfill a real migration or accept into the allowlist with a citation. Not started. _(added 2026-09-09)_
- [ ] **`fix/tenant-provisioning-pg-cron` (uncommitted, `eq-shell-wt-pgcron` worktree) still needs to land** — pg_cron fix + the 37-table public-schema baseline migration, plus a numeric-prefix collision (`0308`) to resolve first. Not this session's task; flagging so it isn't lost. _(added 2026-09-09)_
- [ ] **Once `fix/tenant-provisioning-pg-cron` lands, re-run the new check** — expect the `pg_cron` extension finding to clear; `vector`/`pg_net` will not (separate, still-open gaps this session found). _(added 2026-09-09)_
