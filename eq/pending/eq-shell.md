...
- [ ] **Get `0309` (rescued, path below) reviewed, renumbered, and dispatched to madagins.** This is the actual fix for the eq-service-side schema gap logged below as "not yet handed off to anyone." Saved at `C:\Users\EQ\AppData\Local\Temp\claude\C--Projects-eq-shell\2612f78d-8fab-4144-b792-dd402f11d9de\scratchpad\rescued-pgcron-migrations\0309_app_data_legacy_baseline_and_tenant_members.sql` — local to this machine/session only, not committed anywhere, so it needs pulling into the repo properly (not just copied in) before anyone else can use it. Whoever picks up eq-service's madagins provisioning gap should start here instead of rebuilding from scratch. _(added 2026-09-09)_
