...
- [x] **CHECK 7's reviewed-exception logic exercised against a real, live violation in CI — passed, fully closed loop.** Royce's explicit go to actually run the test scoped above. Created `public.zz_scratch_check7_livetest` on `zaap` (`SELECT 1`, no `security_invoker`, `GRANT SELECT` to `authenticated`) via Supabase MCP `execute_sql` — not a tracked migration, deliberately disposable. Manually dispatched `tenant-drift.yml` (it supports `workflow_dispatch`): **`Schema drift + anon-grant + policy-lint` correctly failed** — CI log shows `public.zz_scratch_check7_livetest reloptions=NONE` flagged on the EQ tenant, while the same run's SKS-tenant output confirms both real entries (`app_data.field_managers`, `app_data.field_people_directory`) still pass as "content-verified this run" — proves the exception logic discriminates correctly, not just that CHECK 7 still fires in general. Bonus: the issue-filer (PR #1639) fired for real too, opening [#1649](https://github.com/eq-solutions/eq-shell/issues/1649) — a live exercise of that fix's own still-open verification item below. Dropped the scratch view immediately after, re-dispatched to confirm clean: check passed, #1649 auto-closed itself ("Drift check passed — all violations resolved") with no manual intervention. Nothing left behind. _(added 2026-08-27, scoped 2026-08-27, executed + closed 2026-08-27)_

**Deferred:**
- [~] **Partially exercised against a real live violation** — the CHECK 7 live test above incidentally proved this too: the issue-filer correctly opened [#1649](https://github.com/eq-solutions/eq-shell/issues/1649) for a real CHECK 7 violation and auto-closed it once resolved. Only CHECK 7 of the 7 newly-wired checks has been proven this way — CHECK 6/8/9/12/13/14 still untested against a real violation, lower priority now that the mechanism is proven to work for at least one. _(added 2026-08-27, partially exercised 2026-08-27)_
- [ ] **eq-cards' own jvkn migrations still have no governed apply path** — eq-shell's now does (`migrate-control-plane.mjs`, #1641), deliberately scoped eq-shell-only per Royce's explicit "hold eq-cards" instruction. eq-cards' migrations still reach jvkn however someone happens to run them, same unreliable-ledger problem #1641 just fixed for eq-shell. Not building without Royce picking a direction (own independent runner writing to the same ledger, vs. some other shape) — real, not urgent, since eq-cards' migration volume against jvkn is low. _(added 2026-08-27)_
