...
- [~] **Cross-repo consumer-check gap — decided 2026-08-27, #1638 now merged, #328 still open.** Royce picked the reusable-workflow option (of 3 scoped) after a cost investigation found eq-cards' CI already had the credential kind it needed. [eq-shell#1638](https://github.com/eq-solutions/eq-shell/pull/1638) (new `workflow_call` running CHECK 2/6/7/8/10/12/13/14 against jvkn) — **confirmed MERGED** (verified directly via `gh pr view` this session, `eq-shell-d4`, superseding the earlier "not yet merged" note here). [eq-cards#328](https://github.com/eq-solutions/eq-cards/pull/328) (advisory consumer job, not required-checks yet) — re-checked same session, still OPEN. `access_level: organization` already applied live on eq-shell. Still needed: mint a new fine-grained PAT (`EQ_SHELL_CHECKOUT_TOKEN`, Contents: Read-only, eq-shell only — no API for this, manual) + add `CONTROL_PROJECT_REF` as secrets on eq-cards, then merge #328 — neither checked this session, don't assume done. Full reasoning trail in [eq/sprints/2026-08-26-organisations-anon-read-hardening.md](../sprints/2026-08-26-organisations-anon-read-hardening.md) §3. _(corrected 2026-08-26, re-scoped 2026-08-27, decided + PRs opened 2026-08-27, #1638 merge confirmed 2026-08-27)_
- [x] **Same gap, tenant-plane version — decided, built, merged live.** The `field_managers`/`field_people_directory` CHECK 7 incident (see the dedicated entry above, now archived — fully resolved via #1642) is the identical shape one plane over: eq-field changed something eq-shell's CI treats as absolute, zero pre-merge signal either way. #1638 explicitly does not cover this (jvkn-only) and eq-field is a structurally different "other repo" than eq-cards here (no CI-driven migration-apply step at all, hand-applied via Supabase MCP) — full reasoning in [eq/sprints/2026-08-27-tenant-plane-cross-repo-consumer-check.md](../sprints/2026-08-27-tenant-plane-cross-repo-consumer-check.md). `/decide` pass picked option 2 (strengthen the existing 3-hourly schedule + issue-filer with likely-source attribution) over a pre-merge gate — checked, not assumed, that eq-field's CI has zero Supabase credentials today, confirming a pre-merge gate would hit the same manual-PAT bottleneck already stalling #1638/eq-cards#328, while option 2 ships on the already-provisioned `FIELD_PERMS_DRIFT_PAT`. **Merged**: [eq-shell#1648](https://github.com/eq-solutions/eq-shell/pull/1648) (`3eff84c2`, 10:53:29Z, Royce's explicit "merge it") — `scripts/extract-tenant-violation-names.mjs` + `scripts/attribute-violations-to-eq-field.mjs`, 3 new best-effort `continue-on-error` steps in `tenant-drift.yml`. CI green (incl. the drift-check job itself) both before and after merge; 10-case fixture test verifies both scripts as real child processes against data shaped like this exact incident. Not yet exercised against a real live violation in production (can't force one from a PR, and deliberately not staging one on a live tenant plane just to test tooling). _(added 2026-08-27, decided + built + merged 2026-08-27)_
- [ ] **CHECK 7's new reviewed-exception logic not yet exercised against a real, live violation in CI** — only verified via a standalone Node simulation and the two legitimate entries. Small, mechanical, no design call needed — scoped in the sprint doc "What's left" §2: create a disposable scratch view (not the two real exempted views — those are live and load-bearing) with `security_invoker=false` and no matching `VIEW_INVOKER_REVIEWED_DEFINER` entry, confirm CHECK 7 fails with the expected reason, drop it. `zaap` (EQ tenant) is the natural target — already documented elsewhere in this repo as disposable demo data, unlike `ehow`/SKS. ~15 minutes. Not attempted unprompted — a live Supabase write, even a scratch one. _(added 2026-08-27, scoped more precisely 2026-08-27)_
- [x] **CHECK 11 (jvkn migration-identity) — built, merged, live. Fully closed.** Blocker (jvkn's native ledger `supabase_migrations.schema_migrations` under-recording what's actually applied — 86-92 of 144 files with no matching entry, confirmed live) fixed first via [eq-shell#1641](https://github.com/eq-solutions/eq-shell/pull/1641) (`c3bf2c8f`, merged, bootstrapped). CHECK 11 itself shipped and merged: [eq-shell#1646](https://github.com/eq-solutions/eq-shell/pull/1646) (`fbc7b85e`, 10:53 UTC) — three dimensions (gaps/out-of-band informational via the shared `--strict-identity` flag, checksum drift ABSOLUTE, no grandfather cutoff needed since this ledger's INSERT always stamps a real checksum). No design call was actually needed once the real ledger existed — the "cutoff date" question that deferred this dissolved on its own. Re-verified live this session (fetched current `origin/main`'s `check-tenant-drift.mjs` directly) — the check code is really there, not just claimed. **Caught a real bug in #1641's own work in the process**: this PR's first CI run failed — not on CHECK 11, on CHECK 13 — because `shell_control._eq_control_plane_migrations` (the new ledger table #1641 created) had RLS disabled, missed on first creation. No actual exposure (grants were postgres/service_role only, checked live before fixing — anon/authenticated were never granted, the default-privilege lockdown already covers that), but a real norm violation, fixed both live (one-off `ALTER TABLE ... ENABLE ROW LEVEL SECURITY`, idempotent) and in `migrate-control-plane.mjs`'s own `CREATE TABLE` block, same PR. Final CI: `144 repo migration(s), all applied, no out-of-band entries, no drift`. eq-cards' own migrations against the same database still have no governed path — explicitly on hold, not this PR's scope. _(deferred by #1628, investigated + blocker found + runner shipped 2026-08-27, CHECK 11 built + merged 2026-08-27)_
