...
- [~] **Scope corrected 2026-08-27 — this is 7 checks, not 2.** CHECK 6 (`function_exec`), 7 (`view_invoker`), 8 (`column_grants`), 9 (`stacked_policy`), 12 (`control_plane_stacked_policy`), 13 (`control_plane_rls`), and 14 (`control_plane_isolation`) all have the identical issue-filer wiring gap #1623 just closed for CHECK 10 — none of their `_report` keys are read by `tenant-drift.yml`'s issue-filing step, so a real violation on any of them would still only go red in CI, never file a GitHub issue on a schedule-only catch. Full key list and the exact `jq` pattern to extend: [eq/sprints/2026-08-26-organisations-anon-read-hardening.md](../sprints/2026-08-26-organisations-anon-read-hardening.md). Flagged as `task_f5c61eb9` — **picked up 2026-08-27** as `task_e2eed444` (session `work-wiring-priorities-0150f3-c8`), in progress, no PR yet, outcome not confirmed as of this entry. Two independent peer sessions (this repo's own spawned task, plus `eq-shell-2a` arriving at the identical scope in parallel) both collision-checked — by messaging peer sessions directly, since git/PR state alone showed nothing yet — and stood down rather than duplicate. Whoever checks this next: verify `task_e2eed444`'s actual outcome (merged PR, or abandoned) via `git log origin/main -- .github/workflows/tenant-drift.yml` before picking it back up. Full detail: `sessions/2026-08-27.md`. _(added 2026-08-26, unblocked 2026-08-26, scope corrected 2026-08-27, picked up 2026-08-27)_
