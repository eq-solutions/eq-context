...
## eq-shell + eq-context: Madagins tenant provisioning — full review, Field-access fix shipped, deeper pattern scoped for follow-up (2026-09-09)
*Royce: "Ive created my second tenant today which is an actual customer - it feels VERY clunky... do a full review of the existing process." Investigated live across the day's full session log, both sibling sprint docs, live Supabase (jvkn + Madagins' own project), GitHub, and eq-shell's actual provisioning code — not from any doc. Full write-up: `eq-context/eq/sprints/2026-09-09-tenant-provisioning-review.md`.*

- [x] **Four live gaps found and confirmed resolved same day**: `shell_control.tenant_routing` row, RLS disabled on the migration ledger ([#1835](https://github.com/eq-solutions/eq-shell/pull/1835), fleet-wide, not just madagins), the Field JWT-mint allowlist, and a tier split-brain between `organisations.tier`/`shell_control.tenants.tier`.
  - **Independent corroboration (later, eq-field session, same day):** re-checked `tenant_routing` directly against `shell_control.tenants.id` (`fc06cd56-...`, not `organisations.id` `dd5d8622-...` — the same confusion PR #1839 fixed structurally) and confirmed `status: active`, service-role key present, no error — the fix is holding. That session nearly wrote a bad row re-diagnosing this from scratch (caught by a live FK-constraint violation) without having found this entry or the review doc first; noted in `eq/pending/eq-field.md`'s madagins section as a pointer for next time.
- [x] **Field access allowlist fixed** — [PR #1838](https://github.com/eq-solutions/eq-shell/pull/1838): `token-exchange.ts`'s `ALLOWED_FIELD_TENANT_SLUGS` *and* a second, separate copy in `src/lib/fieldTenants.ts`'s `TENANT_OPTIONS`/`FIELD_TENANT_URLS` — the second one is easy to miss even knowing about the first. Merged via admin override past one confirmed-unrelated failing check; confirmed live via commit-ancestry, not the merge alone.
- [x] **`public.tenants`/`public.organisations` drift found and closed structurally** while investigating #1838 — [PR #1839](https://github.com/eq-solutions/eq-shell/pull/1839) backfills the missing madagins row and adds a `trg_sync_tenants` trigger so this can't recur for any future tenant.
- [ ] **The deeper pattern — tenant identity duplicated across ~7 places with nothing keeping them in sync**: schema vs. migration files, the ledger's claims vs. reality, tier, three separate hardcoded tenant-slug lists, and the tenants/organisations tables all showed the identical shape today. Rated the overall process 3/10 as of this close. Two scoping tasks spawned to address it structurally, both still running as of this entry:
  - Extend the drift-check pattern from schema objects to every hardcoded tenant list (background task, not yet reported back)
  - Scope a canary tenant provisioned + torn down on a schedule, to catch the next gap in CI instead of on a real customer (background task, not yet reported back)
- [ ] **Stale runbook found**: `docs/runbooks/add-field-trial-tenant.md` points at `netlify/functions/mint-iframe-token.ts`, deleted months ago. Spawned as its own task, not yet reported back.
- [ ] **Not a code gap, still open**: Madagins' EQ Field staff records (`app_data.staff`, on its own dedicated project) are still 0 rows for all 5 people who already have Shell-level tenant access — Shell membership doesn't create a Field roster record on its own.
- [ ] **The single highest-value remaining item needs Royce's own calls, not more scoping**: the 0308/0309 legacy-baseline migration (the real, general fix for the ~100-missing-schema-object gap, not just a Madagins patch) is drafted but blocked on 2 self-documented bugs — hardcoded ehow tenant UUIDs in its RLS policies, and a migration-ordering conflict with `0257`/`0260`. Both need a decision before anyone builds further, per the migration's own header.

**Notes:** Full session detail: `sessions/2026-09-09.md` (this close's entry). Review doc, kept current: `eq/sprints/2026-09-09-tenant-provisioning-review.md`.
