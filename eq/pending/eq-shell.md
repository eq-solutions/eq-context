...
- [ ] **Backend permission-CHECKING logic itself still not consolidated** — `entity-rows.ts`'s 5 Sets, `crm-write.ts`'s `PERM_BY_ACTION`, `EntityBrowserPage.tsx`'s 7 Sets all still independently decide their own required permission per entity. This PR only consolidated table/pk/field *metadata* (zero permission-logic change) — the permission-checking consolidation is a separate, higher-stakes PR, deliberately not attempted here. _(added 2026-09-15)_
- [ ] **labour_hire's `entity.view`/`equipment.view` read-permission split — Royce's explicit hold, needs a real decision.** `equipment.view` already grants labour_hire access (live role matrix), `entity.view` doesn't. Raised as a quick "grant it too" parity fix; before implementing, found labour_hire's own role description reads "Agency worker. Sees field information and the equipment list, **nothing else**" — contradicting the quick-fix framing. Also not an eq-shell change at all: role grants live in the separate, versioned `@eq-solutions/roles` package shared across Field/Service/Cards/Quotes. Royce: hold off, revisit with full context. _(added 2026-09-15)_
- [ ] **`src/lib/navScope.ts` — an adjacent, already self-documented duplication** (its own header comment admits it "mirrors RECORD_ICONS... because the permission-gating logic already has [drifted] across four independent copies") — not touched, a near-zero-risk fast-follow if picked up later. _(added 2026-09-15)_

Built two canonical registries: `src/lib/entityRegistry.ts` (frontend nav/discovery — sidebar, ⌘K palette, mobile Records drawer — absorbs the old `entityCapabilities.ts`) and `netlify/functions/_shared/entity-registry.ts` (backend table/pk/field-allowlist metadata for `entity-patch.ts`/`entity-insert.ts`/`entity-actions.ts` — metadata only, zero permission-logic change, verified byte-identical against the three files' old separately-maintained copies). Also fixed a real, disclosed gap Royce explicitly signed off on: `tenant-dashboard.ts` had excluded `asset` from its permission-gated entity-count set since that check first landed, passing dashboard counts through unfiltered while every sibling CRM endpoint gated `asset` under `entity.view` — now consistent.

**Notable finding, not a code change**: the asset/equipment *write* permission split (`entity.edit` vs `equipment.edit`) turned out to be a real, already-reviewed, documented decision (`docs/access-control-sweep-followup-sprint.md` §S2, cited in `entity-patch.ts`'s and `entity-actions.ts`'s own header comments) — correctly left alone. The *read* split (above) is a live, unreviewed divergence found while checking whether the write-side precedent extended to reads — it doesn't, cleanly.

**In-flight-work rescue, worth remembering the pattern**: this session's investigation found the shared eq-shell root checkout sitting on a branch (`feat/records-find-anything-search`) with real, uncommitted work — Royce's own "find anything" cross-record search, built via a separate window/session. Confirmed with him live, then relocated it into an isolated worktree before building on top of it rather than risk it to the same "shared checkout branch drift" hazard already logged twice elsewhere today. That branch's own PR ([#1932](https://github.com/eq-solutions/eq-shell/pull/1932)) was separately merged by Royce mid-session while this session was still working in its isolated worktree — a clean example of the isolation actually doing its job, not a collision. One real gotcha from it: a squash-merge severs git ancestry, so continuing to build on the now-merged branch and opening a fresh PR from it would have re-presented the already-shipped search feature as new, unreviewed changes in the diff. Fixed by cherry-picking this session's 3 commits onto a fresh branch off current `main` instead of reusing the stale one. Full detail: `sessions/2026-09-15.md`.
