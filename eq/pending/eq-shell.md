...
*Royce trialled the Excel-import feature (#1485) live and asked to make the import drag-and-drop, then asked whether the Excel/PDF click-buttons could be one. Confirmed the direction — one button, either file type, route each row wherever you like — once the per-row section picker turned out cheap to add (pricing already keys off each row's category dynamically, so reassigning a row's section needs no separate recalculation logic). Separately found two OTHER "PDF import" buttons elsewhere in the app that read as the same thing but aren't (client-RFQ header autofill vs. starting a new quote from a supplier's priced document) — ran `/decide` on whether to extend the client one to Excel too; Royce confirmed client RFQs are PDF/email only, so relabeled instead of building.*

- [x] **Line Items card: drag-and-drop + one "Import file" button** (Excel or supplier PDF/photo, auto-detected by file type, same pattern the Jobs page's drop zone already used) **+ a per-row Section dropdown** in the review screen, so any parsed line — from either source — can be routed to any of the 4 sections before it's confirmed. eq-shell [PR #1492](https://github.com/eq-solutions/eq-shell/pull/1492), squash-merged (`0cc002d7`), confirmed live via exact Netlify `commit_ref` match.
- [x] **Relabeled the two other PDF-import buttons** so the difference reads at a glance — "Fill from client PDF" (New Quote form header autofill) vs. "From supplier PDF" (Jobs page, starts a new quote) — plus matching tooltips, drop-zone copy, and the code comments that quoted the old labels. eq-shell [PR #1493](https://github.com/eq-solutions/eq-shell/pull/1493), squash-merged (`8cffe0bb`), confirmed live via exact Netlify `commit_ref` match.
- [x] **Sent Royce 2 trial `.xlsx` files** (built with openpyxl, verified against the real shipped parser before sending, not just synthetic test cases) plus click-through instructions, so he could test the Excel importer directly rather than reusing his one real job file.

**Deferred:**
- [ ] **Extend the Jobs-page "From supplier PDF" button to also accept Excel** — same auto-detect pattern as #1492, offered but not yet confirmed or built. _(added 2026-08-20)_
...
- [ ] **Not click-tested live** — the final consolidated shape (single button, drag-and-drop, section picker) is verified via typecheck/lint/CI/deploy-preview, and the *previous* Excel-only version was click-tested live, but the new shape specifically hasn't been. _(added 2026-08-20)_
