...
## eq-shell: EQ-SHELL-22/1P fix built, merged, live (manual deploy — Netlify auto-deploy didn't fire); second Sentry sweep; EQ-SHELL-23 traced to a test-data account, partially cleaned up (2026-09-09)
*Continuation of the entry below — Royce confirmed the matcher fix, then "merge it," then approved a data cleanup found during a follow-up sweep.*

- **eq-shell [PR #1826](https://github.com/eq-solutions/eq-shell/pull/1826)** — `isChunkLoadErrorMessage()` (`src/lib/chunkReload.ts`) now recognizes Firefox's MIME-type wording, same shape as the EQ-SHELL-10/1S precedents in that file. `tsc -b`, eslint, and the test suite (6/6 incl. 2 new cases) all clean. Merged (`61b06a02`) on explicit go-ahead.
- **Netlify's auto-deploy didn't trigger on the merge** — confirmed, not assumed: no new GitHub Deployment record, no status check, `currentDeploy` unchanged for 2.5+ minutes, despite CI/`notify`/release-tag jobs all firing normally on the same commit. Breaks the documented "13/13 merges" auto-deploy pattern. Worked around via the Netlify MCP's signed one-time deploy command (`npx @netlify/mcp@latest --proxy-path ...`, run from a `git archive`'d clean copy of the merge commit — the same mechanism global CLAUDE.md's manual-deploy note describes) — confirmed live, `currentDeploy` now `6aa07bf0`, `core.eq.solutions` serving the fix. The webhook/git-integration itself is still unexplained.
- Second Sentry sweep: EQ-SHELL-22 and EQ-SHELL-1P marked resolved in Sentry (comments point at the PR/commit). EQ-SHELL-21 found new — folded into the EQ-SHELL-1R item below rather than duplicated here.
- **EQ-SHELL-23 investigated — not a code bug.** `check-missing-org-memberships.ts` is a deliberately alert-only daily monitor; it correctly reported a worker with licences but no active org grant. Traced to "Jordan A. Sample," an SKS Apprentice test record (address "12 Example St, Sampleville," emergency contact "jon smoth") created 2026-09-01. Its `org_membership` had already been revoked and its original 2 test licences soft-deleted, both at the same instant (2026-09-08 08:16 UTC) — a clean prior cleanup — but 2 NEW test licences were uploaded to the same account 2 hours later (10:13/10:15 UTC) with no membership re-grant, which is what actually fired the alert. On Royce's explicit go-ahead: `app_data.staff` deactivated (ehow) and both new licences soft-deleted (jvkn `public.licences`).

- [ ] **EQ-SHELL-23 will very likely still fire again** — downgraded from `invisible_licences` to the milder `at_risk` (0 licences now, so nothing's actually hidden), but the check's real trigger is jvkn-side (`shell_control.users.active` + an active tenant membership + presence in `public.workers`) — none of which were touched, since Royce's ask was scoped to the staff row + licences specifically. Fully silencing it needs the shell account/tenant-membership deactivated too — his call whether that's worth doing. _(added 2026-09-09)_
- [ ] **Netlify auto-deploy webhook/git-integration — root cause not found from this session's tools.** Worth Royce checking the site's GitHub integration status directly in the Netlify dashboard; the manual-deploy path worked but shouldn't need to become routine. _(added 2026-09-09)_
...
- [ ] **EQ-SHELL-1R + EQ-SHELL-21 need an eq-field-rooted session — handoff prompt sharpened 2026-09-09, still not run.** Shell-side handoff (token mint, iframe boot, handoff-accept) completes in ~4.2s in both issues; the failure is specifically post-accept, pre-render — EQ Field knows who the user is but takes 12+s to actually paint content for at least one supervisor (EQ-SHELL-21: Chrome Mobile/Android). That's the concrete thing to profile in eq-field, not eq-shell. _(added 2026-09-09, updated 2026-09-09)_
- [ ] **EQ-SHELL-T/V — try replay `ee5ceacc486f425fb42efb3115a3e219` next**, not just the Sep-4 one already named in the 09-08 entry above — its 3-fetches-in-4s pattern on a role_code login URL is a tighter, more specific repro than "5 in ~10s," and might be easier for a person to watch directly and spot what's issuing them. _(added 2026-09-09)_
