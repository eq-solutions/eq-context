...
- [ ] **Casual-note attachment friction** — can't attach a document on a Casual note's first save, has to be saved once and reopened first. **Decided 2026-09-09** (via pending-items triage): Royce — restructure to allow attachment on first save. Spawned as a background task (same task as the backdate-indicator item, above). _(added 2026-09-09, decided + spawned 2026-09-09)_
- [ ] **"Overall score per person"** — held, not approved (2026-09-09, via pending-items triage). See the full write-up below for what it'd take to build — needs a real decision on who's allowed to see a cross-manager score first, not a quick call. _(added 2026-09-09)_
...
- [ ] **No signal anywhere that an entry was backdated** — once `occurred_at` differs from `created_at`, the UI shows the chosen date as if it were contemporaneous, with nothing like "logged 4d later." Named during the critique as a real product gap, not fixed. **Decided 2026-09-09** (via pending-items triage): Royce — build the label. Spawned as a background task. _(added 2026-09-08, decided + spawned 2026-09-09)_
...
- [ ] **"Overall score per person" — sketched, explicitly parked, no code. Held 2026-09-09** (via pending-items triage) — Royce declined to approve building it now; needs a real decision on who's allowed to see a cross-manager score before it's buildable, not a quick call. Raw material already exists unused (`happy_engaged` trend, the 7 tech-skill + 4 values ratings from Dev Reviews). The real blocker is access, not UI: `staff_conversations` RLS is creator-only, so a genuine cross-manager score needs a narrow derived-data path (compute server-side, expose only the number/trend) — the same boundary `staff-resourcing.ts` already draws for "was this person spoken to," not a broader RLS loosening. Also worth weighing against `staffLib.ts`'s own precedent of rejecting a single score for mobilisation readiness ("every blocker has a different fix and a different owner") — not quite the same shape (independent gates vs. a sentiment trend) so it doesn't veto this, but it's this codebase's stated bias. _(added 2026-09-08, held 2026-09-09)_
