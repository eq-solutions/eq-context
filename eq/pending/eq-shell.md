...
## eq-shell: Madagins licence-sync chain fixed end-to-end (manager_id scope explained, Cards licence link repaired, missing unique constraint found+fixed, control-plane drift blocking all PRs fixed) — one migration still can't reach madagins, handed to the session that owns it (2026-09-09)

*Continuation of the Madagins visibility/entitlements session above/below — Royce asked three follow-up questions in sequence, each led somewhere real.*

- **"Why can't I make Madagins people managers?"** — wasn't about the `manager` role (already answered: full 62-key ceiling). Royce meant `manager_id`, the reporting-line field. Confirmed live: the column doesn't exist on Madagins' plane at all — migration `0303_staff_manager_id.sql` is deliberately `Plane: ehow (SKS) ONLY` by its own header, written 2026-09-07, two days before Madagins existed. Separately, even on SKS, editing/viewing it is locked to one specific custom-group permission (`staff.manage_reporting_line`) — granted to nobody by role, pending Royce's real org-chart backfill. Two independent, unrelated reasons; neither is a bug.
- **"Conor and Nelson are EQ Cards users, we should have turned on the link for their licenses."** Royce was right, I'd missed it. Both have real Cards worker records on jvkn with 8 real licences between them (White Cards, driver licences, EWP, electrical licence) — invisible in Madagins because `app_data.staff.cards_worker_id` (their link back to that Cards identity) was never set; they joined via the labour-hire intake link, which skips the normal Cards-approval flow that sets it automatically. Fixed the link directly (pure metadata, matched by email against `public.workers`, mirrors what `backfill-worker-links.ts` phase 3 does). Deliberately did NOT hand-copy the licence rows themselves — that's a real photo/document storage copy between two Supabase projects, and there's already a working "Re-sync from Cards" button (`staff-resync-licences.ts`) that does it correctly.
- **"check Aditi and michelle were invited as managers"** — confirmed via `shell_control.user_invites`: both invited as `manager` by Royce directly, both accepted within minutes, no discrepancy. Bonus: the invite records also carried `entitlements: [field, cards, intake]` — Royce's original "core" meant **Cards**, not nothing. My earlier entitlements fix (this session, above) had wrongly left `cards` off — corrected: Madagins is now `field=true, cards=true, intake=true`, everything else off, matching what he actually invited them with.
- **Conor's real "Re-sync from Cards" click failed live**: `there is no unique or exclusion constraint matching the ON CONFLICT specification`. Root-caused: `app_data.licences.cards_credential_id` has a `UNIQUE` constraint on ehow only — hand-applied at some unknown point, never captured in any migration anywhere in the repo. zaap and madagins both genuinely missing it, confirmed no duplicate data on either (safe to add). Wrote `0312_licences_cards_credential_id_unique.sql`, opened [PR #1845](https://github.com/eq-solutions/eq-shell/pull/1845).
- **#1845's CI failed on something unrelated**: the required "Schema drift + anon-grant + policy-lint" check found `shell_control.sync_organisations_tier` live on jvkn with no matching migration file (hand-applied at some point, same failure class as the control-plane drift blocking PR #1842 earlier today) — blocking every open PR on the repo, not just this one. Ran `/brief eq-shell` (gate fired correctly, first Edit/Write of the day in this repo this session). Captured the function+trigger's exact live definition, wrote `2026_09_09c_sync_organisations_tier_trigger.sql`, applied it live via `apply_migration` (confirmed true no-op against already-live state), recorded in `CONTROL-PLANE-LEDGER.md`, pushed as a second commit onto #1845. CI went green, merged (squash, `2cbc173d`) on Royce's explicit "check CI and merge it."
- **Dispatched `tenant-migrate.yml` fleet-wide** (Royce: "Dispatch it") — sks applied clean (2 migrations, including 0312). **zaap and madagins both failed outright, 0312 never got attempted on either** — not this fix's fault. The failure was a *different*, just-merged migration next in the queue, `0311_app_data_legacy_baseline_and_tenant_members.sql` ([PR #1842](https://github.com/eq-solutions/eq-shell/pull/1842), merged ~90s before this dispatch). Runner stops per-tenant on first failure, so 0312 was blocked as pure collateral. No partial damage on either plane — confirmed live via `pg_constraint`, clean Postgres errors, nothing half-applied.
- **Identified and messaged the session that owns 0311** (`local_98743c84`, "Build a tenant-provisioning completeness check") rather than digging into someone else's 2901-line migration blind — matched via `search_session_transcripts` on the exact file content, not guessed from title. **Their reply, real findings**: zaap genuinely doesn't have `service.tenants` at all (architectural, not a bug — 0311 shouldn't target zaap). Madagins is a real shape collision: eq-field's own PR #959 provisioning already created several `app_data.field_*` views there in *eq-field's own convention* (`org_id`, `schema_version`, `leave_request_id`-style PKs) — only 3 of 0311's 10 expected views match; the rest collide, which is what threw `42P16: cannot drop columns from view`. Their read (not yet acted on, correctly left for Royce): 0311 should carry a `Plane: ehow ONLY` header, same pattern as its own siblings (0258-0261, 0266, 0270, 0273, 0290, 0303) — but the deeper question, **which `field_*` shape is actually meant to be canonical on a non-ehow tenant, eq-shell's legacy one or eq-field's own**, is a real architecture call neither session should make unilaterally.

- [ ] **0312 still hasn't reached madagins or zaap.** Blocked on 0311 getting properly scoped (or the shape-collision question being resolved) and a clean re-dispatch. Owned by `local_98743c84`'s thread now, not a fresh task — check there before re-investigating. _(added 2026-09-09)_
- [ ] **The "which `field_*` shape is canonical for Madagins" question is unresolved and bigger than 0311** — eq-shell and eq-field have independently written two different shapes to the same tenant's tables today. Needs Royce's call before anyone extends either pipeline further on Madagins. _(added 2026-09-09)_
- [ ] **Once 0312 lands on madagins: re-run Conor Horgan's "Re-sync from Cards" to confirm the original ask (see him and Nelson's real licences) actually completes end-to-end** — the fix chain is built, just not yet provable live. _(added 2026-09-09)_
- [ ] **The entitlements write-path trace from earlier this session (`comms`/`service`/`ops` silently enabled on Madagins) is in progress elsewhere** — `local_c776b53b`, PR [#1847](https://github.com/eq-solutions/eq-shell/pull/1847) OPEN as of this close, not yet confirmed merged/resolved. Check that PR before re-doing the legwork. _(added 2026-09-09, updated 2026-09-09)_
...
- [ ] **PR #1842 needs the control-plane drift resolved before it (or anything else) can pass CI and merge** — whoever owns the `eq_cards_worker_claimed_by_phone` / phone-claim-check work needs to either commit the missing migration file (`CREATE OR REPLACE`, idempotent) + record it in `supabase/CONTROL-PLANE-LEDGER.md`, or triage it as accepted debt into the check's own `KNOWN_UNSOURCED` list. Blocking suite-wide, not just this PR. _(added 2026-09-09)_
- [ ] **Once CI is unblocked: review + merge PR #1842, then dispatch `0311` to madagins via `tenant-migrate.yml`** — closes the CMMS-tables gap for real. Royce's call on timing. _(added 2026-09-09)_
...
## eq-shell: labour-hire batch-intake portal copy simplified and shipped; madagins-tenant readiness audit — re-landed after a same-day clobber (F17 recurrence); RLS gap and CSP dev-blocker both fixed (2026-09-09)
...
- [ ] **"Overall score per person" — sketched, explicitly parked, no code.** Raw material already exists unused (`happy_engaged` trend, the 7 tech-skill + 4 values ratings from Dev Reviews). The real blocker is access, not UI: `staff_conversations` RLS is creator-only, so a genuine cross-manager score needs a narrow derived-data path (compute server-side, expose only the number/trend) — the same boundary `staff-resourcing.ts` already draws for "was this person spoken to," not a broader RLS loosening. Also worth weighing against `staffLib.ts`'s own precedent of rejecting a single score for mobilisation readiness ("every blocker has a different fix and a different owner") — not quite the same shape (independent gates vs. a sentiment trend) so it doesn't veto this, but it's this codebase's stated bias. _(added 2026-09-08)_
...
- [ ] **`PdfBackfillButton` relocated, not re-verified live** — moved from the Reference-library tab (where it landed by accident, per that component's own dated comment) onto the new `admin/documents` List page, matching Royce's original 2026-09-02 placement call. Not click-tested.
