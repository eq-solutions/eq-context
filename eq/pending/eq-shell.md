...
## eq-shell: Field-handoff stall cluster — EQ-SHELL-29 (mint-timing) and EQ-SHELL-21 (draw-notice) threshold bugs fixed; EQ-SHELL-1P/22 (stale chunk) still open (2026-09-13 → 09-14)
...
- [ ] **EQ-SHELL-1P / EQ-SHELL-22 (stale JS chunk after a deploy crashes instead of recovering) — diagnosed, not built.** Classic SPA stale-chunk failure: a tab open across a deploy requests an old chunk hash, gets `index.html` back from the SPA fallback route, and throws trying to execute it as a module. Dormant since 2026-09-11 (4 occurrences/2 users) but expected to recur on a future deploy — dormancy isn't evidence it's fixed. Fix shape already scoped: wrap the router's lazy-loaded imports in a handler that detects this specific failure and triggers a full page reload (needs care to avoid a reload loop on an unrelated failure). The plausible-looking `chunk-prefetch-catch` branch is confirmed stale/unrelated, not existing coverage. Lower priority — sequenced second in the sprint doc, not started. _(added 2026-09-14)_
