...
- [ ] **Real remaining gap, now correctly scoped**: nothing makes a jvkn-control-plane change in another repo check eq-shell's CHECK 10 / `INTENTIONAL_ANON_READS` before merging. The 3rd occurrence's cause is now fully known — it was eq-cards PR #324 (migrations `0163`/`0164`), Royce-approved, investigated thoroughly but only within eq-cards' own scope (no visibility into EQ Field's dependency on this table, which lives in a different repo). Not an unidentified actor, not a pipeline bypass — a cross-repo consumer-check gap. Full chain in eq-shell memory `organisations-anon-bootstrap-read-recurring-incident.md`. No guard yet against a 4th occurrence. _(corrected 2026-08-26 — supersedes this item's original "WHO/WHAT ran 0163/0164 is unknown" framing)_
...
- [ ] **Found while merging #1622/#1623**: a required check ("Schema drift + anon-grant + policy-lint") was blocking every PR repo-wide on a pre-existing, unrelated gap — `public.eq_cards_admin_sync_tenant_access` is live on jvkn with no matching migration file. Confirmed unrelated to either PR's own diff (same failure on both, from a completely different function-drift step); worked around via admin-merge with Royce's explicit go, not fixed. Spawned as background task `task_393ba31c`, not yet started. _(added 2026-08-26)_
- [ ] **Found while cleaning up worktrees post-merge**: an independent, now-redundant duplicate fix for CHECK 10's printer-crash bug sits unmerged on branch `claude/check10-drift-printer-fix` (worktree `eq-shell-check10-fix`) — main already has the fix via #1623, merged first. Couldn't identify which of 14 concurrent peer sessions owns that branch to flag directly; worth a nudge or a plain branch cleanup next time someone's in there. _(added 2026-08-26)_
- [ ] `eq-shell` [PR #1627](https://github.com/eq-solutions/eq-shell/pull/1627) (another session, in progress, not this one) — paper-trail migrations for `0162`/`0163` plus a further ledger correction — OPEN, not merged as of this session's close. _(added 2026-08-26)_
