...
- [~] **Cross-repo consumer-check gap — decided 2026-08-27, #1638 now merged, #328 still open.** Royce picked the reusable-workflow option (of 3 scoped) after a cost investigation found eq-cards' CI already had the credential kind it needed. [eq-shell#1638](https://github.com/eq-solutions/eq-shell/pull/1638) (new `workflow_call` running CHECK 2/6/7/8/10/12/13/14 against jvkn) — merged. [eq-cards#328](https://github.com/eq-solutions/eq-cards/pull/328) (advisory consumer job, not required-checks yet) — still open. `access_level: organization` already applied live on eq-shell. Still needed: mint a new fine-grained PAT (`EQ_SHELL_CHECKOUT_TOKEN`, Contents: Read-only, eq-shell only — no API for this, manual) + add `CONTROL_PROJECT_REF` as secrets on eq-cards, then merge #328. Full reasoning trail in [eq/sprints/2026-08-26-organisations-anon-read-hardening.md](../sprints/2026-08-26-organisations-anon-read-hardening.md) §1. _(corrected 2026-08-26, re-scoped 2026-08-27, decided + PRs opened 2026-08-27, #1638 merged 2026-08-27)_

**Deferred:**
- [~] **Only 1 of 7 newly-wired checks (CHECK 7) has been proven against a real live violation** — the mechanism is confirmed working (issue opened + auto-closed cleanly, full detail in the sprint doc), but CHECK 6/8/9/12/13/14 remain unexercised. Per-check scratch-test approach scoped in the sprint doc "What's left" §2 — **CHECK 8 flagged as not safely testable this way** (hardcoded to 4 specific columns on the real `organisations` table, no substitute scratch object exists to test against instead). Lower priority now that the pattern's proven for one. _(added 2026-08-27, scoped per-check 2026-08-28)_
- [ ] **eq-cards' own jvkn migrations still have no governed apply path** — eq-shell's now does (`migrate-control-plane.mjs`, #1641), deliberately scoped eq-shell-only per Royce's explicit "hold eq-cards" instruction. Quantified live rather than left as a category statement: of eq-cards' 161 tracked `supabase/migrations/*.sql` files, 132 matched a jvkn ledger entry under one of eq-cards' own documented naming conventions, 29 did not (not proof they're missing — a third naming variant would look identical — but a real number, not just a suspicion). Not building without Royce picking a direction — 3 shapes scoped in the sprint doc "What's left" §4, leaning toward extending the same reusable-workflow pattern already picked for the check side of this incident. Real, not urgent — eq-cards' migration volume against jvkn is low, and nothing found indicates any of the 29 represent an actual unrecorded security-relevant change. _(added 2026-08-27, quantified 2026-08-28)_
