...
## eq-shell: madagins's own database fully caught up; PR #1842 (0311 CMMS baseline) admin-merged but its dispatch failed on stale views that conflict with eq-field's own live work (2026-09-09)
...
- **RE-CORRECTING, again: `shell_control.tenant_routing` was NEVER missing.** An earlier write-up in this exact section claimed it was — that was already corrected once this same session (found by literally hitting the FK constraint on an abandoned insert attempt: `tenant_routing.tenant_id` references `shell_control.tenants(id)`, not `public.organisations.id`, a completely different id space; under the real id, `status='active'`, correct the whole time). **That correction was then silently reverted** by a later concurrent session's `safe_commit.py` push built from a stale pre-correction copy of this file (the exact F17 whole-file-overwrite pattern this file's own substrate already documents elsewhere) — this section briefly went back to claiming it was "CRITICAL... not fixed" again with no fault of anyone actively re-breaking anything, just a stale write clobbering a fresher one. Re-applying the correction now. **Nothing needs restoring here. Do not act on any earlier version of this bullet.**
- **madagins's own database: fully caught up** on eq-shell's tracked pipeline. Unblocked `0257`'s crash (created `public.app_config`/`organisations` directly), applied `service.tenant_members` + the full 37-object `public`-schema legacy baseline (stays uncommitted on disk by design, per the sprint doc below), re-dispatched `tenant-migrate.yml --slug=madagins`: 38 applied, 299 skipped (legitimate Plane-scoped skips), zero failures, current through `0308_sites_deleted_at.sql`.
- **`_eq_migrations` RLS gap found, fixed, then deduped** against [PR #1835](https://github.com/eq-solutions/eq-shell/pull/1835) (merged 2.5 min before this session's own #1836 went up) — closed #1836 as a duplicate.
- **PR #1842 (`0311`, the rescued ~28-table CMMS/app_data baseline) admin-merged, then its dispatch failed.** CI's required "Schema drift" check was blocked by an unrelated repo-wide control-plane drift issue (a different session's uncommitted worker-invite migration, since merged as [#1844](https://github.com/eq-solutions/eq-shell/pull/1844)) — merged fresh `main` into the PR branch to refresh the check, which surfaced a *second*, genuine drift finding also unrelated to this PR (`shell_control.sync_organisations_tier`, hand-applied by yet another session, no migration file). With Royce's explicit go, admin-merged past it (`gh pr merge --admin`, same precedent as the 5 Dependabot PRs earlier today) since neither drift issue was this PR's own fault. **Dispatching `0311` to madagins then failed for a real, different reason**: `ERROR 42P16: cannot drop columns from view`. Traced to all 10 of `0311`'s `app_data.field_*` view definitions (field_prestarts, field_site_diaries, field_leave_requests, field_timesheets, field_toolbox_talks, field_team_members, field_teams, field_people_directory, field_people_removed, field_audit_log) being **stale relative to what's already live on madagins** — e.g. `0311`'s `field_prestarts` has 21 columns, the live view already has 30 (`org_id`, `project_number`, `affects_trades`, `controls`, `other_hazards`, `permits_selected`, `measures`, `crew_count`, `crew_signed_count`). Matches what a concurrent eq-field session described doing (their own idempotency-guarded script, org_id backfills). Postgres's own safety check is what caught this — the dispatch failed transactionally (0 applied, madagins's database untouched), so nothing was actually regressed, but forcing it through would have silently stripped those columns back out from views eq-field's own code likely depends on right now. **Asked the eq-field session to confirm before anyone edits `0311`** — unanswered as of this close.
- **New finding, not chased further this session**: madagins's `app_data` schema is also short ~20-25 CMMS/Service-flavored *tables* (not the views above — separate objects: `maintenance_checks`, `defects`, `check_assets`, `rcd_tests`/`nsx_tests`/`acb_tests`, `instruments`, `pm_schedule`, etc.) vs. ehow. `0311` covers these too, once its view conflict is resolved.
- **eq-field's own side confirmed and handed off cleanly** — their migration-replay script (trimmed to skip the prerequisite block this session already covers) was their one remaining step, before this session's own `0311` dispatch surfaced the view conflict above.

- [ ] **Fix `0311`'s 10 stale `field_*` view definitions before any re-dispatch** — needs eq-field's confirmation on the current/final live column shape first (asked, unanswered as of this close). Do NOT force-apply `0311` as merged; it would regress live views. Top-priority item in this thread now. _(added 2026-09-09)_
- [ ] **`shell_control.sync_organisations_tier`** — live on jvkn with no matching migration file (found while unblocking #1842's CI), same "hand-applied, ledger doesn't show it" pattern as everything else in this thread. Will re-block every future PR's required drift check until someone writes the migration or triages it into `KNOWN_UNSOURCED`. Not this thread's fault, not fixed here. _(added 2026-09-09)_
- [ ] **eq-service-side schema-table gap on madagins** (~20-25 tables, listed above) — covered by `0311` once its view conflict is resolved; no separate eq-solves-service session needed after all (correction to this section's own earlier framing, which called it an "eq-service" gap — it's an eq-shell migration, always was). _(added 2026-09-09)_
- [ ] **314 falsely-stamped `_eq_migrations` ledger rows** (see "tenant creation doesn't actually apply the real schema" below) — still not cleared. Hasn't blocked anything so far; ledger itself remains inaccurate. _(added 2026-09-09)_
...
**Deferred:**
- [ ] **`madagins`'s ledger needs correcting before any real apply can succeed on it** — the 314 falsely-stamped rows have to be cleared/reset first, or every future apply attempt will keep trusting them and skipping real work. Not done here — Royce's call on timing/ownership, and who ran the original bootstrap (and why) is still unknown. Spawned as a background task 2026-09-09 (via pending-items triage) — task briefed to re-verify current state first, since the ledger count and eq-field's own schema layer have both moved since this finding. _(added 2026-09-09, spawned 2026-09-09)_
...
- [ ] **"Overall score per person"** — held, not approved (2026-09-09, via pending-items triage). Needs a real decision on who's allowed to see a cross-manager score before it's buildable at all — not a quick call, revisit deliberately when it matters. Full detail in the sprint doc. The other 2 items from this same follow-up sprint ("logged after the fact" indicator, Casual attachment friction) were decided the same pass — both approved, spawned as a background build task. _(added 2026-09-09)_
