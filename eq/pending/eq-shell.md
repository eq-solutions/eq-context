...
- [ ] **The durable fix is on EQ Field's side, not Shell's** — Field needs to tell Shell once the page has actually finished drawing, not just that sign-in succeeded (which happens before anything renders). Spawned as its own task (`task_043e28eb`); Royce started it in a separate session on 2026-09-04. Not this session's to track further. _(added 2026-09-04)_
- [ ] **EQ-SHELL-T/V auth-stall pair still open — 12+ occurrences in 14 days, unaffected by this fix.** 7 of the 11 background checks in Shell's sign-in verification remain unbounded (the other 4 were fixed by #1736). Root cause not yet fixed. _(added 2026-09-05)_
