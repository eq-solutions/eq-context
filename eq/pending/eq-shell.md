...
- [ ] **New gap this surfaced: CHECK 7 (`view_invoker`) has no allow-list mechanism**, and PR #813's `security_invoker=false` is a genuine, reviewed exception — CHECK 7 will now fail on `app_data.field_managers` every run until the check itself gains an allow-list (mirroring CHECK 9's `STACKED_POLICY_ALLOW` pattern). Spawned as `task_a3b16883`, running as of this entry. _(added 2026-08-27)_
