...
- [ ] `eq-shell` [PR #1627](https://github.com/eq-solutions/eq-shell/pull/1627) (paper-trail for `0162`/`0163` + ledger correction) and its stacked follow-up [PR #1628](https://github.com/eq-solutions/eq-shell/pull/1628) (`CHECK 12`/`13`) — both OPEN, not merged, another session's work. **Do not edit `scripts/check-tenant-drift.mjs` until these land** — #1628 is actively changing it. Sequencing + what's left after they merge: see the sprint doc above. _(added 2026-08-26, updated 2026-08-26)_
- [ ] CHECK 7 (`view_invoker`) and CHECK 8 (`column_grants`) have the identical issue-filer wiring gap #1623 just closed for CHECK 10 — neither is wired into the security-issue-filing step, so a real violation on either would still only go red in CI, never file a GitHub issue. Flagged as `task_f5c61eb9`, not built. Blocked on the same `scripts/check-tenant-drift.mjs` edit-freeze as the item above until PR #1628 lands. _(added 2026-08-26)_
