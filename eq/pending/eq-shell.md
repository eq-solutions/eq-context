...
- [ ] **Real remaining gap, now precisely scoped — full plan in [eq/sprints/2026-08-26-organisations-anon-read-hardening.md](../sprints/2026-08-26-organisations-anon-read-hardening.md)**: nothing makes a jvkn-control-plane change in another repo check eq-shell's CHECK 10 / `INTENTIONAL_ANON_READS` before merging. Not an unidentified actor, not a pipeline bypass — a cross-repo consumer-check gap. CHECK 10/12/13/14 all harden jvkn's own same-repo invariants but do NOT close this specific gap — it still needs its own design call, 3 options scoped in the sprint doc, none built. _(corrected 2026-08-26, re-scoped 2026-08-27)_
