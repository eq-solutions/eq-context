...
- [x] **SEC-73 — eq-field's definer-rights `field_people_directory` / `field_managers` views on both tenant planes (P2 latent).** Decided 2026-09-04 (`/decide` pass): accept as a governed exception, review 2026-12-04. Two PRs open: eq-context [#203](https://github.com/eq-solutions/eq-context/pull/203) (baselines the 4 advisor errors + a test that fails once a baselined review date is past) and eq-shell [#1763](https://github.com/eq-solutions/eq-shell/pull/1763) (CHECK 7 comment refresh + `allowedColumns` guard). **Shipped the same day, #1763 commit 2:** `allowedColumns` on both CHECK 7 exception entries (live 2026-09-04 lists, managers 11 / directory 24); `checkViewInvoker` fails on any column outside them, fail-closed on an unreadable list; the PR's drift-gate run passed live on both planes with the allow-list verified. #203 also gained the review_by date guard (`overdue_reviews()` + tests, 13/13). Ticked 2026-09-04 at the go-live-review session's request: the call is made and both guards are up; residual risk tracked in the register row (OPEN, accepted, review_by 2026-12-04), and the weekly audit goes green on the first `main` push after #203 merges. _(added 2026-09-04)_
