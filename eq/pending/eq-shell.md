...
- [ ] **Proactive "overdue for review" nudges** — deliberately held per `/decide`: there's no conversation data yet for staleness to mean anything. **Precondition now met 2026-08-30** — 27 historical reviews (2021-2026) backfilled into `staff_conversations` for 14 people, sourced from real signed review documents in Royce's HR folder. Worth building now that real dates exist to be stale against. _(added 2026-08-12, unblocked 2026-08-30)_
...
- [ ] **Simon Bramall's Tenure shows "42 years 9 months" on Resourcing — found 2026-08-30, not yet root-caused.** Either a bad `start_date` value or a date-math bug in `tenureLabel()`. Spawned as its own background task (`task_56ef42fd`), running independently as of session close.
