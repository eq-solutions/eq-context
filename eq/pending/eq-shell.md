...
## eq-shell: Schema drift gap — closed, but a bigger in-progress feature found underneath it (2026-08-25)
*Royce: "fix the schema drift gap" — the `public.eq_cards_get_my_licence` block noted in the Permissions/nav audit entry below. Turned out to be 3 functions, not 1, discovered one CI run at a time as each backfill revealed the next live dependency.*

- [x] **`eq_cards_get_my_licence`** — not eq-shell's to source; fully defined in eq-cards `supabase/migrations/0142_get_my_licence_rpc.sql` (PR #298). This checker only scans eq-shell's own migrations tree, so a cross-repo function always reads as unsourced here even when tracked elsewhere. Allowlisted in `KNOWN_UNSOURCED`. eq-shell [PR #1593](https://github.com/eq-solutions/eq-shell/pull/1593) (opened by an earlier session/spawned task, verified and merged by this one), admin-override merged, confirmed on `main`.
- [x] **`_grant_fix_scratch_test` / `_grant_fix_scratch_test_2`** — already dropped live by the time this was investigated; re-queried and confirmed absent. Naming + disappearance both point to scratch/test artifacts from an unrelated session tonight. Nothing to source.
- [x] **`eq_sweep_orphaned_licence_photos`, `_eq_storage_dispatch`, `eq_confirm_retention_purge_dispatch`** — real, deliberately-built retention/cleanup utilities for the `licence-photos` storage bucket, just never committed. All 3 verified live (body via `pg_get_functiondef`, grants via `information_schema.role_routine_grants` — `postgres`/`service_role` only on every one, re-asserted explicitly since a bare `CREATE OR REPLACE` would otherwise get silently stripped by `eq_enforce_function_privacy`) and backfilled. eq-shell [PR #1596](https://github.com/eq-solutions/eq-shell/pull/1596), admin-override merged, confirmed on `main`.
- [ ] **The retention/purge feature itself is incomplete** — `eq_confirm_retention_purge_dispatch`'s own branch logic already anticipates a third job type, `eq_finalize_deleted_accounts`, which does not exist live yet. Someone is actively building this feature on jvkn without committing migrations as they go — the same recurring pattern this repo's `CONTROL-PLANE-LEDGER.md` has already documented multiple times. Backfilled what exists now; stopped there rather than chase a moving target. Worth a word to whoever owns it. _(added 2026-08-25)_
- [x] **4 admin-override merges tonight total across this session's own PRs** (#1595, #1590, #1593, #1596) — every one confirmed the failing check was `Schema drift`, confirmed pre-existing and unrelated to that PR's own diff, before overriding; each override was Royce's explicit go, asked fresh via `AskUserQuestion` the first time this session hit it.
...
*Royce asked for a nav-bar mockup (Manager/Supervisor/Employee/Apprentice desktop views), then asked who the second platform admin was and how that flag gets set — surfaced that eq-shell's permission matrix and its nav-rendering logic periodically disagree, with no audit trail behind the most powerful flag in the system. Confirmed with Royce via `AskUserQuestion` which of 3 follow-ups to build (all three), then which side of one specific mismatch was wrong (the permission, not the nav).*

- [x] **Supervisor no longer holds `audit.view`** — eq-shell's own "Audit log" nav link was always gated on `admin.list_users` (manager-only) regardless, so the grant never reached anything a Supervisor could act on. Fixed at the source: [@eq-solutions/roles PR #31](https://github.com/eq-solutions/eq-roles/pull/31), released as `v2.7.5`, eq-shell pinned to it in [PR #1594](https://github.com/eq-solutions/eq-shell/pull/1594). Merged, confirmed live (commit `d1968b57`, deploy state ready).
- [x] **Fixed a real, independently-diverged bug found while building the above**: `TenantHome.tsx`'s dashboard app-tile filter and `HubLayout.tsx`'s sidebar app-tile filter had silently drifted apart — the sidebar let a platform admin bypass Ops's tenant-entitlement check, the dashboard tile didn't. Unified into one shared `isAppTileVisible()` in `session.ts`/`hubApps.ts`. [PR #1591](https://github.com/eq-solutions/eq-shell/pull/1591), merged, confirmed live (commit `0e3d00ad`).
- [x] **"Preview a person" on Access Control now shows what actually renders**, not raw permission-key pills — reuses the real `isFieldFirstWorker`/`isAppTileVisible` gating functions against a synthesized session for the target user, so it can't drift from the real nav the way a fourth reimplementation would have. Same PR #1591.
- [x] **`is_platform_admin` grant/revoke is now governed**: new `set-platform-admin` function (platform-admin-only, self-action blocked, writes `shell_control.audit_log`, force-revokes the target's live sessions on revoke) and an interactive control on the Edit User page (two-step confirm, same pattern as Archive). [PR #1592](https://github.com/eq-solutions/eq-shell/pull/1592), merged, confirmed live (commit `51311354`). Two accounts hold the flag today (`dev@eq.solutions`, `royce.milmlow@sks.com.au`) — both grants predate this and still have no audit-log row; not backfilled.
- [x] **Root-caused a required-check failure blocking every open eq-shell PR right now**, not just this session's: `Schema drift + anon-grant + policy-lint`'s `check-control-plane-drift.mjs --strict` step fails on `public.eq_cards_get_my_licence`, a live jvkn function with no matching migration file anywhere eq-shell's scanner can see. Confirmed pre-existing/environmental — it failed identically on 3 completely unrelated diffs (#1591, #1592, #1594), same root cause as the block noted in the Staff-page-edit entry below. Spawned a background task to triage it (write the missing migration, likely owned by eq-cards given the prefix, or triage into `KNOWN_UNSOURCED`) — running independently, not yet resolved as of this write-up. **Update, later the same day: fully resolved — see the "Schema drift gap" entry below for the actual fix (this was 3 functions, not 1, and the spawned task's `eq_cards_get_my_licence` half landed as [PR #1593](https://github.com/eq-solutions/eq-shell/pull/1593)).**
- [x] **All 4 PRs needed an admin-override merge** (`gh pr merge --admin`) because of the above — each confirmed live via Netlify deploy-state polling plus `git merge-base --is-ancestor` against whatever commit was actually serving at the time, not just each PR's own reported deploy state (two other concurrent sessions' merges landed on `main` in between, normal for this repo under concurrent load).

**Deferred:**
- [ ] **Not click-tested live by a person** — every fix this round verified via `pnpm run build` + `tsc -b --force` + production commit-ancestry, not a real signed-in session. Worth a few minutes each: open Access Control → Preview a person for a real Supervisor and confirm the new "what they actually see" block matches the sidebar; confirm a non-platform-admin can't see the new grant/revoke control at all; confirm the control actually works end-to-end against a disposable test user, not a real account. _(added 2026-08-25)_
- [ ] **eq-solves-service also pins `@eq-solutions/roles#v2.7.4`** — not bumped as part of this session; Royce to decide separately whether/when to move it to `v2.7.5`. _(added 2026-08-25)_
