...
- [~] **Cross-repo timing gap on new-starter onboarding push — scope call made, build in flight.** `app_data.staff.user_id` is written only by eq-cards' `workers-canonical-sync`; `cards-approve-staff.ts`'s 3 worker-link blocks now backfill it too ([PR #1774](https://github.com/eq-solutions/eq-shell/pull/1774), merged+live), closing the same-repo half. For the remaining piece (`staff-create.ts`'s brand-new-roster insert, and any future `user_id`-linking path) — ran a `/decide` pass on the two candidate shapes this line used to leave open (a cross-repo webhook vs. a periodic eq-shell-side sweep). Called it for the sweep: a webhook/DB-trigger approach still needs an app-layer consumer regardless (the push logic lives in TypeScript, not SQL) and carries real blast-radius risk on `app_data.staff`, one of the hottest-written tables in the product, for no stronger a guarantee than a nightly sweep already gives using patterns (`licence-expiry-scheduler.ts`, today's own concurrency-cap fix to `scheduled-document-signoff-reminders.ts`) this repo already trusts. Spawned as `task_752f9a65`; Royce started it in a separate session, build in progress as of this close. _(added 2026-09-05, decided 2026-09-05)_
