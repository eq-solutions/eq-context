...
*Started as three small UI asks on the Staff table (email regression, table clutter, a missing filter search box) and ended up surfacing and fixing a live data-exposure gap on a different feature (Conversations) that Royce raised mid-session.*

- [x] **Ben Ritchie's email "reverting" root-caused, not a mystery** — his July correction predates the `email_locked_by_shell`/`phone_locked_by_shell` columns (migration 0224/PR #1118); the lock was never backfilled onto pre-existing corrections, so his row sat unlocked and vulnerable to the next Cards resync. Royce re-saved it himself through the Staff page to set the lock; no code change needed.
- [x] **Staff table decluttered** — [PR #1454](https://github.com/eq-solutions/eq-shell/pull/1454): dropped the duplicate email under the person's name (CONTACT column already owns it). [PR #1455](https://github.com/eq-solutions/eq-shell/pull/1455): Contact/Trade were the only `'text'`-filter columns on a table otherwise built from `'multiselect'` popovers (the visual "why does only Contact have a filter box" bug) — Contact drops its per-column filter (global search still covers phone/email), Trade moves to `multiselect` with its existing options. Status column: checkbox+caption → clickable pill badges (Supervisor / On roster / Off roster), and the now-redundant inline "Off roster" tag next to the name was dropped since Status owns that fact. Both merged, live.
- [x] **Root-caused and fixed a live RLS gap on `app_data.staff_conversations`** (personal HR/performance notes) — RLS was tenant-only, so any signed-in SKS user (or the shared `dev@eq.solutions` platform-admin account) could bypass the UI's `staff.manage_conversations` permission entirely and read every conversation directly via the browser's own Supabase client. [PR #1457](https://github.com/eq-solutions/eq-shell/pull/1457) (migration 0250) tightened RLS to `created_by = auth.uid()`. Dispatching it to `sks` also applied an older, previously-undispatched migration (0242, merged via #1304 months ago, sat "written not applied") that added a *different*, looser policy on the same table — Postgres ORs multiple permissive policies together, so the live gate briefly became "creator OR permission-holder," not creator-only as intended. Caught immediately via a live `pg_policies` check; [PR #1458](https://github.com/eq-solutions/eq-shell/pull/1458) (migration 0251) consolidated to exactly one creator-only policy. Both merged, both dispatched, verified live: exactly one policy on the table now.
- [x] **`/decide`-triggered follow-on sweep of 6 more ehow tables** (`feedback_entries`, `quarterly_reviews`, `skills_ratings`, `buddy_checkins`, `checkins`, `engagement_logs`) for the same RLS shape — concluded no fix needed (see that session's own log entry, 2026-08-19). Surfaced a real table-name collision worth remembering: three of those names exist twice on ehow (`app_data.*`, dead/unused, vs `public.*`, the tables eq-field's apprentices module actually uses) — grep by table name alone will find the wrong one.
- [ ] **eq-ui `Table` multiSlicer prop (Excel-style click-to-toggle, AND-combined slicer chips)** — [eq-ui PR #40](https://github.com/eq-solutions/eq-ui/pull/40) open, needs Royce's merge sign-off before the Changesets release pipeline can publish it. Once published, still needs the eq-ui pin bumped in eq-shell's `package.json`/lockfile and `multiSlicer` wired onto the Staff table's slicer chips — not done yet. _(added 2026-08-19)_
- [ ] **eq-ui `Skeleton` shimmer-sweep animation** (replaces the opacity pulse) — [eq-ui PR #41](https://github.com/eq-solutions/eq-ui/pull/41) open, same release-pipeline dependency as above. Applies automatically everywhere `Skeleton` is used once released and the pin is bumped — no eq-shell-side wiring needed beyond the version bump. _(added 2026-08-19)_
...
- [ ] **No CI check catches two permissive RLS policies silently stacking on the same table** — the actual root cause of the #1458 follow-up fix above. A migration author has to manually check `pg_policies` before adding a new policy to an existing table; nothing enforces it. Worth a `check-tenant-drift.mjs` addition that flags >1 permissive policy per table/command as needing explicit acknowledgment. _(added 2026-08-19)_
