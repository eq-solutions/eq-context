...
## eq-shell: security-hardening sprint — SEC-35 shipped + live, SEC-34/SEC-59 shipped (2026-08-30)

- [x] **Correction to the 2026-08-28 entry: there was no `guard.js` bug.** The Write blocked twice on 2026-08-28 with the flag file provably present, which read at the time like `fs.existsSync` disagreeing with reality. Landed clean on the first retry 2026-08-30 with a freshly-dated flag — root cause was simpler and less interesting: this session ran long enough that the real local date had quietly rolled past what the conversation still assumed, so every "freshly recreated" flag that day was being recreated under the *wrong* date. `Test-Path` correctly found that (stale-dated) file; `guard.js` correctly looked for the (correctly-dated) one that didn't exist yet. Both sides of the check were right — the session's own clock was wrong. No fix needed in `guard.js` itself; flagged here so nobody spends time re-debugging a hook that isn't broken.
- [x] **SEC-34 + SEC-59 shipped**: `shell_control.user_invites` reads scoped to `eq_role='manager'` (matching the existing `admin.invite_user` write-side gate); stray `TRUNCATE` revoked from `authenticated` on the 9 named `shell_control` tables. Re-verified live unchanged before landing. eq-shell [PR #1662](https://github.com/eq-solutions/eq-shell/pull/1662). **Open, not merged.**
- [x] **SEC-53 + SEC-67 (eq-shell half) shipped**: dead `ktmjmdzqrogauaevbktn.supabase.co` (connect-src) and `quotes.eq.solutions` (frame-src) dropped from core.eq.solutions's CSP; `field-supabase.ts`'s `getFieldServiceClient()` deleted outright — zero callers anywhere in the repo, existed only to read the same dead project's URL/key. eq-shell [PR #1663](https://github.com/eq-solutions/eq-shell/pull/1663). **Open, not merged.**
- [ ] **SEC-67's other half still needs Royce** — the now-orphaned `FIELD_SUPABASE_URL`/`FIELD_SUPABASE_SERVICE_ROLE_KEY`/`NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY` env vars (confirmed zero code references for all 4) still need deleting from the eq-shell Netlify site itself — blocked by Claude Code's own classifier on unattended env-var writes. Commands ready, see `sessions/2026-08-30.md`. _(added 2026-08-30)_
- [x] **Same-origin-check gap closed**: 15 more cookie-authenticated POST handlers gated with `checkShellOrigin()` (account-security, GM Reports, Intake, Labour Hire ×9, uploads ×3, job-creation) — mechanical fix, same pattern as the 76 files that already had it. This is enforcing, not logging (`ENFORCE_IFRAME_ORIGIN=true` in prod already). Invites turned out already fully covered — the original finding's "spans... invites" was stale. eq-shell [PR #1665](https://github.com/eq-solutions/eq-shell/pull/1665). **Open, not merged.**
- [x] **SEC-26 residual — investigated, no code change needed.** The register's own text already correctly says "92 of 94 *mutate*" (this file's earlier note mis-transcribed that as "reads" from a truncated read — corrected here, register itself was right). The write-side chokepoint (`requirePerm` → `denyIfDeactivated`) is fully shipped for every endpoint that calls `requirePerm` with a write permission. What's left: ~30 write endpoints that bypass `requirePerm` entirely (explicitly scoped out of PR #1371 as its own "individual triage" backlog — not a quick fix, real follow-up work) and a handful of read-only endpoints the register's own SEC-26 row explicitly says are **"OPEN AND DELIBERATELY ACCEPTED — not an oversight, a decision. Do not 'discover' this again and patch it in passing."** Correctly left alone.
- [x] **SEC-6 — confirmed not eq-shell's.** `context_proposals` doesn't exist anywhere in this repo (zero grep hits). The register itself already correctly attributes it to `eq-substrate`, not eq-shell — this file's earlier "ownership unconfirmed" note was wrong, not the register. Nothing to do here.
- [x] **SEC-58 corrected** (`ops/security-register.md`, eq-context) — its "84/131, 48 untracked" figures predated the ledger's own 2026-08-24 refresh. Live-checked: 147 files, 135 tracked, 12 untracked (all recent, not a discovered gap). Downgraded P2→P3. Misfiled-migration lead noted (`2026_07_11_tender_tables_anon_lockdown.sql`, same family as SEC-36) but not yet confirmed as the specific file the original finding meant.
- [x] **SEC-36 shipped**: zaap's 4 tender-pipeline tables get real `authenticated` policies. Turned out ehow already had the fix (`tender_tenant_read`/`_write` and siblings) — zaap-only drift — but ehow's version is hardcoded to one literal org_id/tenant_id (its single real org), which would have silently locked out zaap's other 2 real orgs if copied verbatim. Shipped the generic per-request form instead, functionally proved live: the real `eq` org sees all 323 live tenders, a different real org (`melbourne`) sees 0. eq-shell [PR #1667](https://github.com/eq-solutions/eq-shell/pull/1667). **Open, not merged.**
...
## eq-shell: multi-select document push + jvkn control-plane RLS gap closed (2026-08-27)
*Royce, looking at the Documents admin page: the "Person" push picker was a single-select dropdown — pushing to several specific people meant repeating the whole pick-and-submit cycle once per person.*

- [x] **Built**: both person pickers (the fresh-upload push step in `AdminDocumentUpload.tsx`, and the Register tab's "Push to more people" modal) now use a checkbox list. Fires one push per selected person in parallel via a new shared `pushAudienceOnce()` helper, aggregates into a single "Pushed to N people" result, and leaves only failed names checked on a partial failure so they're easy to retry. No backend/schema change — `push-document-audience.ts` already resolved a person-push one signer at a time, same as role/crew already fanning out server-side. eq-shell [PR #1645](https://github.com/eq-solutions/eq-shell/pull/1645), merged (squash `368dd3db`), confirmed live via Netlify deploy state matching the merge commit exactly + a production smoke check.
- [x] **Merge blocked by an unrelated, freshly-live security gap, found and closed en route**: the required "Schema drift + anon-grant + policy-lint" check failed — `shell_control._eq_control_plane_migrations` had RLS disabled on jvkn. Root cause: the brand-new jvkn control-plane migration pipeline (`scripts/migrate-control-plane.mjs` + `control-plane-migrate.yml`, PR #1641, merged ~30min earlier) self-creates that ledger table with no RLS in its inline DDL — same bare-table idiom `migrate-tenants.mjs` already uses for `app_data._eq_migrations`, which needed the identical retroactive-migration fix (0060) when it was first built. Auto-filed as [eq-shell#1647](https://github.com/eq-solutions/eq-shell/issues/1647); closed itself automatically once the fix went live.
- [x] **Fixed**: `supabase/migrations/2026_08_27b_control_plane_migrations_ledger_rls.sql` — RLS enabled, zero policies, service_role-only grants, mirroring `app_data.migration_baseline`'s already-proven posture (`0039_migration_baseline_rls.sql`). Deliberately NOT a `deny_all`-shaped policy — that exact pattern was scoped `roles={public}` on 13 other jvkn tables until this same day's PR #1634 narrowed them to `service_role` as a footgun fix; a fresh policy here risked re-introducing that shape. Dispatched `control-plane-migrate.yml` against the PR branch (Royce's explicit go, both on the migration content and the dispatch itself), applied cleanly to live jvkn, re-ran the drift-check job clean, merged normally.
- [x] **Concurrent-session collision, reconciled — not a duplicate fix, a genuine two-session race**: a second, independent session hit the identical live gap ~4 minutes after this one (on its own unrelated PR, #1646/CHECK 11) and separately (a) claimed to apply a live one-off `ALTER TABLE` directly (no migration file — an out-of-band deviation from this repo's own "no DDL outside the One Pipe" rule, unverified independently) and (b) fixed `migrate-control-plane.mjs`'s self-creating `CREATE TABLE` block so a *future* bootstrap is safe by construction — that script fix landed on `main` via #1646, confirmed present in the current file. This session's own migration is unaffected either way: fully idempotent, converges to the same end state, and — unlike the other session's claimed one-off — is a durable, replayable migration file, which the script fix alone doesn't provide for the table that already exists live. No conflict between the two; flagging so neither gets mistaken for the sole fix later.
...
**Deferred:**
- [ ] **eq-cards' own jvkn migrations still have no governed apply path — direction DECIDED 2026-08-29, not yet built.** eq-shell's now does (`migrate-control-plane.mjs`, #1641), deliberately scoped eq-shell-only per Royce's explicit "hold eq-cards" instruction. Quantified live rather than left as a category statement: of eq-cards' 161 tracked `supabase/migrations/*.sql` files, 132 matched a jvkn ledger entry under one of eq-cards' own documented naming conventions, 29 did not (not proof they're missing — a third naming variant would look identical — but a real number, not just a suspicion). Royce picked, of the 3 shapes scoped in the sprint doc "What's left" §4: **route eq-cards' own jvkn migrations through eq-shell's existing governed pipeline**, rather than eq-cards standing up a separate one or leaving it manual — same reuse-over-duplication call already made for the check side of this incident (eq-cards#328). Not built yet — needs its own scoping pass (eq-shell's pipeline currently only applies eq-shell's own migration files; letting it also apply eq-cards' is a real design/security question, not just wiring). Real, not urgent — eq-cards' migration volume against jvkn is low, and nothing found indicates any of the 29 represent an actual unrecorded security-relevant change. _(added 2026-08-27, quantified 2026-08-28, direction decided 2026-08-29)_
