...
**Session Gate caught real overlap before any code was written.** Of ~29 active local worktrees on eq-shell, 3 (`feat/customer-detail-route`, `feat/staff-detail-route`, `feat/equipment-detail-route`) looked like they'd collide with the planned build — confirmed live in `App.tsx` that `/customers/:id`, `/staff/:id`, `/equipment/:id` already exist and work (squash-merged 2026-09-08 per digest.md; those worktrees are stale leftovers, not active work — worth a cleanup nudge, not urgent). That meant the new search could route straight into already-good destinations for 3 of 5 entity types instead of building a new detail-page component from scratch. A 4th worktree (`worktree-entity-browser-fk-display`) turned out to be Royce independently running the raw-UUID fix spawned as `task_91ff074d` earlier the same session — confirmed no file overlap.

**Built** (one engine, not two): `useRecordSearch.ts` (debounced, fans out to the existing `entity-rows` search across customer/contact/site/staff/asset, permission-gated same as the pages it reuses, grouped People/Places/Equipment + best-match promotion — zero new backend/schema surface) and `recordHistory.ts` (tenant-scoped localStorage recent/pinned). Both surfaces below consume these directly, not parallel copies.

- **Mobile** — the actual bug Royce flagged, after an initial mis-prioritization toward desktop that he corrected mid-session. Every row in the Records drawer linked to the same unfiltered list page regardless of which record was tapped (tap "Priya Nadan" → land on the full Staff list, not her). Fixed (`MobileRecordsDrawer.tsx`'s `rowHref`) and added a live cross-category search box on top of the existing tab-browse view.
- **Desktop** — `CommandPalette.tsx` (⌘K/`/`) gained the same grouped live search, a Recent/Pinned idle state, a best-match row, status pills, and a star-to-pin. Added the first-ever *visible* entry point into the palette (`HubLayout.tsx`'s new sticky search trigger, desktop-only, skipped on `fullWidth` pages) — previously keyboard-shortcut-only with no on-screen affordance anywhere in the app.

**Verified without a live browser** (see Deferred below): clean `pnpm run build` (re-run after every review pass), clean `eslint`, 648/650 `pnpm test` (2 pre-existing skips, unchanged), and `node scripts/check-css-coverage.mjs` (553 `eq-*` classes resolve). Two full self-review passes caught and fixed 6 real issues before Royce ever saw the diff: a licence-link regression, duplicated routing logic between the drawer and the search hook, an invalid nested-interactive-elements structure (a `<button>` containing another clickable control for the star-pin), and a non-conformant listbox wrapper (`<div>` → `<Fragment>`).

**Substrate correction, worth recording so a future session doesn't get misled by the artifact itself:** the Claude Design handoff doc (zipped, Royce's Downloads, not committed anywhere) states the target stack is "Next.js + Tailwind + Supabase (EQ Solves Service)" — wrong. eq-shell is Vite + React Router + Netlify Functions + TanStack Query; EQ Solves Service is a different repo. Built against the real stack, not the doc's claim.

**Deferred:**
- [ ] **Live browser verification never obtained.** 6 attempts across the session via 2 different spawn methods (Browser-pane `preview_start`, plain Bash background) all hung identically on `netlify dev`'s proxy (port 8888) at the first HTTP request — isolated, not assumed: a plain `vite` dev server on a different port connected instantly through the identical browser tooling in the same session, and `pnpm run build`/tests are unaffected. Reads as this session's automated browser flow hitting something specific to netlify dev's proxy on this machine, not a code problem and not general resource exhaustion (though one attempt separately hit the documented Windows EMFILE issue too). Royce asked to try `netlify dev --filter eq-shell` from his own terminal rather than more retries here. _(added 2026-09-15)_
- [ ] **Branch `feat/records-find-anything-search` is fully uncommitted** — 7 modified + 4 new files, sitting in the shared `C:\Projects\eq-shell` checkout, never committed or pushed (not asked to). Same physical risk this substrate's own worktree-isolation rules exist to prevent for eq-context — worth committing once Royce has looked at it, not left indefinitely uncommitted in a checkout other concurrent sessions also use. _(added 2026-09-15)_
- [ ] **Site/contact search results still land on the old generic `EntityBrowserPage` table** (no dedicated detail route exists for those two, unlike customer/staff/asset) — the same raw-ID-display page `task_91ff074d`/`worktree-entity-browser-fk-display` is independently fixing. Check that work's status before building a dedicated site/contact page — may already be moot, may not be. _(added 2026-09-15)_
- [ ] **Licences aren't in the unified search at all** — different data source (jvkn canonical `public.licences` via `staff-canonical-licences`, not the tenant `entity-rows` every other entity searches through). Still reachable only via the mobile drawer's own Licences tab; not in ⌘K at all. Folding it in means a genuinely different fetch path, not just another entity in the existing list. _(added 2026-09-15)_
- [ ] **`useRecordSearch`'s entity list is a 5th hardcoded "what's a record" definition**, alongside the 4 already diagnosed this session (`ENTITY_VIEW` in `EntityBrowserPage.tsx`, the old `RECORD_TABS` in `MobileRecordsDrawer.tsx`, `SIDEBAR_RECORDS` in `sidebarConfig.ts`, `RECORD_DEFS` in `useCommandIndex.tsx` — the last of which hand-copies `SIDEBAR_RECORDS` instead of importing it). Not reconciled this session — flagged in `/decide`'s own pre-mortem as real, self-inflicted debt if left to drift further. _(added 2026-09-15)_
- [ ] **Phase 2 relational richness (on-site-now, equipment-on-site on a record's own page) not started** — needs a small RPC/migration extension, meaning the One Pipe (`tenant-migrate.yml`) and Royce's explicit go — deliberately not touched this session. _(added 2026-09-15)_
- [ ] **3 stale worktrees found and left alone**: `feat/customer-detail-route`, `feat/staff-detail-route`, `feat/equipment-detail-route` — squash-merged already (see above), just never cleaned up. Not this session's task, worth a nudge. _(added 2026-09-15)_

**Notes:**
- Reused, didn't duplicate: pulled the entity→icon map that would otherwise have been copy-pasted a third time into a shared `recordIcons.ts`, consumed by both the mobile drawer and the desktop palette.
- No schema, migration, or backend changes anywhere in this session's work — everything composes off the already-shipped `entity-rows` function and already-live detail routes. Nothing merged, nothing deployed, nothing pushed.
...
- **EQ-SHELL-29 (mint-timing false alarm) — fixed, live.** The in-flight stall-notice timer counted purely from attempt-start, so a slow-but-normal token mint (within token-exchange's own stated p95) could eat most of the notice budget before the iframe even started loading. Fixed via `inFlightStallDelayMs()` (`fieldHandoffTelemetry.ts`, unit-tested) — floors the wait separately for the mint and boot legs so a slow earlier leg can't rob a later one of a fair chance. [eq-shell#1896](https://github.com/eq-solutions/eq-shell/pull/1896), merged + live 2026-09-13.
- **EQ-SHELL-21 (Field accepts the handoff but never reports 'rendered') — fixed, live, not yet confirmed by silence.** A different mechanism from the above (draw phase, not mint/boot). Root-caused against real evidence: pulled all 9 live event traces (disproved an initial iOS-only theory — 6 of 9 are Windows desktop) plus a 684-sample PostHog percentile query on accepted→rendered timing. The existing 10s notice threshold was calibrated for the mint/boot leg, not the separate draw leg — real draw times on production traffic run genuinely longer. Fixed via `DRAW_NOTICE_MS = 15_000`, its own constant (deliberately not merged with the 10s `STALL_NOTICE_MS` — different legs, different real-world timing). [eq-shell#1899](https://github.com/eq-solutions/eq-shell/pull/1899), merged + live 2026-09-14 (commit `1a81981f`, confirmed via Netlify `commit_ref` ancestry, published 08:32:32Z).
- [ ] **EQ-SHELL-21 recurrence check** — left `unresolved`/`regressed` in Sentry deliberately: the fix has only been live a few hours as of this close, too soon to credit silence as confirmation given the issue's own ~1-3 day historical recurrence rate. Check again in a few days; mark resolved only once it's gone quiet across that window. _(added 2026-09-14)_
- **Interesting confirmation, not a concern:** a fresh EQ-SHELL-29 event fired 2026-09-14T07:44Z, *after* #1896's fix deployed — checked the trace directly rather than assuming regression: mint was fast (2.34s), the notice fired ~8s after mint completion with zero iframe-load progress by then. That's `inFlightStallDelayMs`'s `minIframeLoadWindowMs` floor working exactly as designed on a genuinely slow load, not the old mint-timing artifact recurring. Worth knowing going forward: this Sentry issue ID will keep collecting real slow-iframe-load events under the same signature now that the false-alarm cause is gone — a future occurrence here isn't automatically "the old bug is back."

- **EQ-SHELL-1P / EQ-SHELL-22 (stale JS chunk after a deploy crashes instead of recovering) — already fixed, not a gap.** **Correction to the 2026-09-13/14 write-up above and the sprint doc: "no existing coverage" was wrong** — that check only ruled out one plausibly-named branch (`chunk-prefetch-catch`) and never found the real mechanism, which lives under a differently-named issue family. `ChunkErrorBoundary` + `installChunkLoadRejectionGuard` (`fieldHandoffTelemetry`'s sibling, `lib/chunkReload.ts`) already self-heal this exact failure via a silent reload, built out incrementally for EQ-SHELL-1S/10 (PRs #1523, #1528) — and [eq-shell#1826](https://github.com/eq-solutions/eq-shell/pull/1826) (merged 2026-09-08, three days before the "no coverage" claim) added the specific Firefox MIME-type wording this issue needed. Confirmed fixed, not just merged: both Sentry issues already show `resolved`, zero occurrences since 2026-09-11 across several deploys since, and eq-shell's entire unresolved-issue list (checked live 2026-09-14) has nothing chunk/MIME-related — the handful of occurrences between the fix merging and 2026-09-11 read as tabs that already had the pre-fix bundle loaded before the deploy, the one population this class of fix can't reach until that tab reloads on its own. No code needed. Royce confirmed closing it out rather than building anything further. _(corrected 2026-09-14)_

- **EQ-SHELL-20 ("stalled at 'booted'") — already closed, resolved in Sentry with reasons attached.** Royce asked to check this and EQ-SHELL-2A alongside 1P/22. All 15 historical occurrences trace cleanly to 3 separate merged fixes, each landed after the occurrences it explains: [#1785](https://github.com/eq-solutions/eq-shell/pull/1785) (2026-09-07 — backgrounded tabs throttle `setTimeout`, so the notice can fire long after the fact reporting huge real elapsed time; explains the 18s/38s/162s/293s outliers), [#1853](https://github.com/eq-solutions/eq-shell/pull/1853) (2026-09-09 — a memory-saver iframe reboot after an already-successful render re-arms the notice against the stale original mint-start, firing near-instantly), and today's own #1896 (2026-09-13 — the boot-to-accept window floor; the last occurrence, 2026-09-13T20:45:42, is the exact event `inFlightStallDelayMs`'s own code comment cites as its evidence). Zero occurrences since, spanning ~22h and several deploys. Marked resolved in Sentry with the full trace attached as a comment.
  - **Correction (2026-09-14, a parallel same-day Sentry-sprint session):** the boot→accepted window floor is [eq-shell#1900](https://github.com/eq-solutions/eq-shell/pull/1900), not #1896 — #1896 is the earlier mint→iframe-load floor (`MIN_IFRAME_LOAD_WINDOW_MS`, for EQ-SHELL-29). #1900 added a third anchor (`bootedAtRef`/`MIN_BOOT_TO_ACCEPT_WINDOW_MS`) specifically for the boot→accepted leg, merged after resolving a real conflict with #1899 (both touched `FieldIframe.tsx`'s notice-timing block, landed within the same hour) — reconciled by hand, both changes kept, full suite reran clean post-merge (643/645, 2 pre-existing skips). Also worth recording: this PR was first committed directly in eq-field's shared root by mistake mid-sprint (unrelated repo, caught by that repo's own pre-push cache-buster guard before anything pushed) — relocated to an isolated worktree and redone properly; no lasting effect, just a reminder that the shared-root anti-pattern isn't only an eq-context problem.
...
- **EQ-SHELL-26 ("network error: Fetch is aborted") — genuine real-world flakiness, not a code defect. Ignored (forever), matching precedent.** Single occurrence ever (2026-09-09). Traced the code path (`FieldIframe.tsx`'s `network-error` case): Field's own iframe relays a failed internal fetch via `postMessage`; Shell reports it verbatim to Sentry with no extra context, which is why this one has no `steps` timeline like its siblings. Same `trace_id` as an **EQ-SHELL-A** ("Load failed") event 2 seconds later — one real handoff attempt threw two differently-worded network errors, not two incidents. EQ-SHELL-A was already `ignored`/`archived_forever` from a prior review (real connectivity flakiness, not a bug) — matched that same treatment here rather than inventing a new call. The user-facing message is already benign ("We couldn't reach EQ Field. Check your connection."). No code needed. **EQ-SHELL-1X, checked — correction to the guess above: not a stale-chunk variant.** The `/assets/index-*.js` path in its title was misleading — the actual `handled: yes` exception is a `fetch()` failure (`TypeError: Load failed`, same generic network-error class as EQ-SHELL-A/26) inside Cards' iframe-token-exchange listener (`surface: cards-iframe-token`), not the chunk-loading path at all; it just happened to fire while on `/sks/field`. Single occurrence (2026-09-01, Royce's own account), already caught gracefully, already `resolved` in Sentry, zero recurrence in 2 weeks. Nothing to do.
...
**Landed, not merged:** the first draft (2026-09-10) was found gone at session close — the shared `C:\Projects\eq-shell` checkout had moved on (another session had it mid-flight on an unrelated feature branch by the time this was picked back up), and the draft was never committed. Rebuilt from scratch in a dedicated worktree this time, verified again (`pnpm run build` clean, all 27 existing tests across the three touched files pass unchanged), pushed, and opened as **[eq-shell#1895](https://github.com/eq-solutions/eq-shell/pull/1895)**. Same fix: `resolveTenantJwtSecret()` in `_shared/supabase-jwt.ts` (looks up `<SLUG>_SUPABASE_JWT_SECRET` per tenant, falls back to the shared secret), replacing both files' hardcoded sks-only branch with one call.

- [x] **PR #1895 merged** — merged to main 2026-09-14T08:03:21Z (`db002681`), Royce's explicit review satisfied. Still incomplete without the next item (madagins' own env var). _(added 2026-09-13, closed 2026-09-14)_
- [ ] **madagins itself still needs `MADAGINS_SUPABASE_JWT_SECRET` set** — a manual step (copy the value from madagins' own Supabase project dashboard, Settings → API → JWT Secret, into a new Netlify env var on eq-shell) that only someone with dashboard access can do. Without it, PR #1895 changes nothing observable for madagins. _(added 2026-09-13)_
- [ ] **Whether a tenant project's own JWT secret can be fetched via the Management API at provisioning time is still unconfirmed** — `SUPABASE_ACCESS_TOKEN` does exist as a Netlify env var on this site (confirmed via the Netlify MCP, value never read), so the credential is available in principle, but testing the call means writing and running server-side code against live infrastructure, which needs its own review rather than being folded into this fix. _(added 2026-09-13)_
- [ ] **zaap likely has the same underlying gap, now better-evidenced but still not proven** — checked eq-shell's Netlify env vars directly: no `EQ_SUPABASE_JWT_SECRET` or `ZAAP_SUPABASE_JWT_SECRET` exists, only `SKS_SUPABASE_JWT_SECRET`. So zaap is riding the shared secret today, same as madagins was. Best-guess reason it hasn't surfaced: zaap's project (created 2026-05-19, one of the earliest) may have had its own native secret copied into the "shared" `SUPABASE_JWT_SECRET` var early on, before more tenants existed — i.e. the "shared" secret may only ever have really been zaap's own. Unverified — nobody has compared the two values directly, and this session didn't either (would require reading zaap's actual project secret, not just checking env var existence). _(added 2026-09-13)_
...
- [x] **A third stray, uncommitted copy of this same fix turned up in the shared checkout again (2026-09-14) — confirmed byte-for-byte identical to PR #1895, not new work.** A different session found `_shared/supabase-jwt.ts`/`mint-tenant-jwt.ts`/`tenant-data-proxy.ts` modified and uncommitted while branching for an unrelated fix, didn't recognize it as already-tracked, and parked it for safety on `wip/tenant-jwt-secret-resolver-recovered-dd4b9417` rather than discard or silently fold it in. Diffed directly against PR #1895 at close time: identical `resolveTenantJwtSecret()` implementation, same env-var convention, same 3 files — #1895 is the newer/complete version. **The parked branch is redundant and safe to delete once you confirm** — nothing in it isn't already in #1895. These same 3 files have now been independently found uncommitted in this one shared checkout by at least 3 sessions across 2026-09-10 through 2026-09-14 — worth treating as a pattern (this checkout genuinely can't hold WIP safely across a gap) rather than three unrelated incidents.

- [x] **Branch cleanup confirmed and closed.** PR #1895 merged to main 2026-09-14T08:03:21Z (`db002681`). Re-verified live before touching anything: `wip/tenant-jwt-secret-resolver-recovered-dd4b9417` was already absent from origin by the time this check ran — confirmed three ways (`git ls-remote`, GitHub API 404, full remote-branch listing) — most likely removed by another concurrent session in this same checkout, consistent with the "can't hold WIP safely" pattern noted above. Nothing was left to delete. Also found and removed, with Royce's explicit go since it wasn't the original target, the adjacent leftover PR-source branch `fix/tenant-jwt-secret-resolver` (#1895's own head branch, not auto-deleted post-merge) — that delete raced a concurrent removal too (`git push --delete` failed with "remote ref does not exist"; re-checked and confirmed already gone rather than assumed). Both branches are now confirmed absent from origin. _(added 2026-09-14)_
