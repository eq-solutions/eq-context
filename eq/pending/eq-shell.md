...
## eq-shell: security register reconciled with the 2026-08-30 → 09-03 security/auth PR run — 4 closed, 3 partial, 3 new, 1 false status fixed (2026-09-04)
*`ops/security-register.md` was last edited 2026-08-30 (`d71b6d2`); eq-shell merged ~20 security/auth PRs after that. Every claim below was re-verified live 2026-09-04 — eq-shell `origin/main` source at the cited lines, `app_data._eq_migrations` + `pg_policies` + view columns/reloptions on both ehow and zaap, Supabase `get_advisors` on both planes, the failed `security-audit.yml` run log, core.eq.solutions response headers — not taken from the changelog. eq-context [PR #201](https://github.com/eq-solutions/eq-context/pull/201) opened, not merged (Royce's click).*

- [x] **SEC-34 / SEC-35 / SEC-36 / SEC-59 closed in the register** — all four went live 2026-08-30 (#1662 / #1657 / #1667, dispatched + DB-verified per `sessions/2026-08-30.md`) but their rows read OPEN for 5 days. Closed with the evidence cited.
- [x] **SEC-43 / SEC-53 / SEC-67 partial-closure notes** — SEC-43: `eq_update_quote_status`/`eq_add_quote_note` gated by #1681, ownership-scoped by #1692, deny-checked by #1694; `eq_bulk_update_quote_status`, `eq_set_`/`eq_reset_field_importance_override`, `eq_archive_estimator`, `eq_archive_duplicate_record` still unverified. SEC-53: core CSP clean (#1663, live headers); eq-field `_headers` line 22 still lists deleted `ktmjmdzqrogauaevbktn`. SEC-67: #1663 removed the code; 4 dead Netlify vars still Royce's hands (tracked in the 2026-08-30 section below).
- [x] **False "merged, NOT yet dispatched" on #1711 / migration `0298` corrected** (here, `eq/changelog/eq-shell.md`, `sessions/2026-09-01.md`) — applied on both planes 2026-09-01 09:03Z, RESTRICTIVE policy present. The 2026-09-01 conversations section below is now ticked.
- [x] **SEC-9 / SEC-28 misread guard** — one-line clarifications added at the top of each Status cell (SEC-9 still OPEN despite the struck Severity cell; SEC-28 CLOSED despite the leading "OPEN"), after `sessions/2026-08-30.md` caught a doc reading the wrong column. Logged so nobody "fixes" the strikethroughs.
...
- [ ] **SEC-72 — session / Field-JWT mint fails open on denials when the `tenant_role_overrides` read exceeds 6 s (P2).** `verify-shell-session.ts:168` (#1736) and `token-exchange.ts:330` (#1747) fall back to `{ grants: [], denials: [] }` on timeout, silently — a tenant's per-role denial doesn't apply for that request and nothing logs it. Reasoned from code, not observed live. Fix shape: fail closed on the denials half (retryable 503, or baseline-minus-cached-denials) + a Sentry event when the fallback fires. _(added 2026-09-04)_
- [x] **SEC-73 — eq-field's definer-rights `field_people_directory` / `field_managers` views on both tenant planes (P2 latent) — and eq-context's weekly `security-audit.yml` has been red since 2026-08-30 because of them.** Advisor ERROR ×4 (2 views × ehow/zaap); isolation holds by the view's own `tenant_id = auth.jwt()…` predicate, not RLS; rows carry name/phone/email (widened by eq-field #817 on Royce's call). eq-shell's CHECK 7 exception (#1642) already content-verifies the predicate + no-anon-grant every run, so the "predicate dropped" failure is caught — but it doesn't assert the column list and its comment still says "no email/phone". Needs a call: (a) baseline the four in `scripts/security_audit.py` `ACCEPTED_ERRORS` as `"SEC-73 — review_by <date>"` (the register's intended mechanism; un-reds CI) and refresh the CHECK 7 comment, or (b) switch back to `security_invoker=on`, which first means reconciling SEC-33's RESTRICTIVE `app_data.staff` policy with what non-managers must read. Lean: (a). **Call made 2026-09-04: (a), review_by 2026-12-04** — [eq-context #203](https://github.com/eq-solutions/eq-context/pull/203) (baseline + tests + register note) and [eq-shell #1763](https://github.com/eq-solutions/eq-shell/pull/1763) (CHECK 7 comment + `allowedColumns` guard) open, not merged. Ticked 2026-09-04 at the go-live-review session's request: the call is made; the residual risk lives in the register row (OPEN, accepted, review_by 2026-12-04 — now date-enforced by `test_security.py`), and the weekly audit goes green on the first `main` push after #203 merges. _(added 2026-09-04)_
