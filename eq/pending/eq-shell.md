...
- [ ] **Real remaining gap, now precisely scoped — full plan in [eq/sprints/2026-08-26-organisations-anon-read-hardening.md](../sprints/2026-08-26-organisations-anon-read-hardening.md)**: nothing makes a jvkn-control-plane change in another repo check eq-shell's CHECK 10 / `INTENTIONAL_ANON_READS` before merging. The 3rd occurrence's cause is fully known — eq-cards PR #324 (migrations `0163`/`0164`), Royce-approved, investigated thoroughly but only within eq-cards' own scope (no visibility into EQ Field's dependency on this table, a different repo). Not an unidentified actor, not a pipeline bypass — a cross-repo consumer-check gap. `CHECK 12`/`13`/`14` (PRs #1628, #1633, both **merged**) harden jvkn's own same-repo invariants but do NOT close this specific gap — it still needs its own design call, 3 options scoped in the sprint doc, none built. _(corrected 2026-08-26, re-scoped 2026-08-26, #1628 merged 2026-08-26, #1633 merged 2026-08-27)_
- [x] ~~`CHECK 14` (tenant/self/org isolation invariant for jvkn, analog of CHECK 5) — scoped, not started~~ — **built and merged**: investigated live (mapped every browser-reachable jvkn table's isolation key before writing code — same discipline as CHECK 12/13), found and fixed a real design bug along the way (a "reachable" test based on policy roles alone produced 13 false positives — tables with a broad-role policy but no actual anon/authenticated table grant, so the policy is inert; corrected to require both). Dry-run + real CI both confirmed clean. eq-shell [PR #1633](https://github.com/eq-solutions/eq-shell/pull/1633), squash-merged (`ef075d5f`). ~~**Live-deploy commit-ancestry not independently confirmed**~~ — **confirmed live, same day, as a side effect of a different check**: a later session closing out [PR #1632](https://github.com/eq-solutions/eq-shell/pull/1632) (see below) needed the newest `ready`/`context:production` deploy for its own unrelated ancestry check, and that deploy's `commit_ref` was `ef075d5f` — this PR's own merge commit — confirming it live for free. Fetched via the bundled Netlify MCP's `netlify-deploy-services-reader` (`get-deploy`; `get-deploy-for-site` hit a transient connector error first try, plain `get-deploy` worked but returns the full ~100KB deploy object — save-to-file-then-grep rather than reading inline). Separately, `npx netlify api listSiteDeploys` was *not* a hard block that session — just slow (~2min cold), completed fine once backgrounded and its output recovered — worth retrying rather than assuming it's a dead end; the two sessions may not have hit the same failure mode. _(added 2026-08-27, closed 2026-08-27, live-deploy gap closed 2026-08-27)_
- [ ] **The 13 inert-policy tables CHECK 14 surfaced** (a real `TO public`/broad-role RLS policy exists, but the underlying anon/authenticated table grant was never given, so the policy is currently dead — not a live hole, but a footgun if the grant is ever added later without re-checking the policy text): `shell_control.tenant_routing`, `.platform_config`, `.security_groups`, `.provision_tokens`, `.rate_limit_buckets`, `.tenant_config`, `.tenant_role_overrides`, `.pin_reset_tokens`, `.cards_field_approvals`, `.audit_log`, `.security_group_perms`, `.user_security_groups`, and `public.revoked_agent_tokens`. Spawned as `task_7a07fe60`; fix built as eq-shell [PR #1634](https://github.com/eq-solutions/eq-shell/pull/1634) — **open, CI green, mergeable**. Merging is mechanical (migration file only, no CI/script change); the live `apply_migration` against jvkn is deliberately left for Royce's explicit go per non-negotiable #6 (RLS on tables adjacent to credential reset/tenant-role overrides) — not urgent, nothing is exposed today. _(added 2026-08-27, PR opened 2026-08-27)_
- [ ] CHECK 10's own bugs are fixed (eq-shell PR #1623, merged — issue-filer wasn't wired to this check's violations + a printer crash) and the ledger doc corrected (PR #1622, merged) — but #1623 hasn't been click/CI-tested against a real live violation yet (no live credentials in this environment). _(added 2026-08-26)_
- [ ] Spawned a background investigation (`task_873144b9`) into today's unusually heavy concurrent-merge/deploy-skip churn while this was happening — separate session, ended, outcome not yet reviewed. May or may not be related. _(added 2026-08-26)_
- [x] ~~A required check ("Schema drift + anon-grant + policy-lint") was blocking every PR repo-wide on a pre-existing, unrelated gap — `public.eq_cards_admin_sync_tenant_access` live on jvkn with no matching migration file~~ — **fixed**, eq-shell [PR #1629](https://github.com/eq-solutions/eq-shell/pull/1629) (merged 19:24 UTC), allowlisted as sourced in eq-cards. Also closes a second, independently-spawned background task (`task_d75fa538`) that hit this identical blocker from the worker-invite session above — same shape as the CHECK 10 printer-crash duplicate-fix pattern (`check10-anon-read-printer-crash-fixed.md`); worth checking `eq/pending/<repo>.md` for an existing spawned-task reference before creating a new one next time. _(added 2026-08-26, closed 2026-08-26)_
- [x] **PR #1629's own open caveat closed, live-verified, merged same day**: that entry's `KNOWN_UNSOURCED` comment for `eq_cards_admin_sync_tenant_access` ended with an explicit unresolved caveat (no Supabase MCP access at the time to confirm the live function body/grants matched the file byte-for-byte). A concurrent session did that verification (jvkn `pg_get_functiondef` + `information_schema.role_routine_grants`: body byte-for-byte identical bar cosmetic normalisation, grants exactly `postgres`+`service_role`, no anon/authenticated) and opened [PR #1632](https://github.com/eq-solutions/eq-shell/pull/1632) with the corrected comment. A second, independent session had picked the identical branch name for the same task and only found out via Edit's own "file modified since read" error after already running `/brief` — verified #1632's content was equivalent (plus one extra check, a companion `org_access_requests_status_check` constraint change) and stood down rather than duplicate it. Merged on Royce's explicit "merge #1632" (squash `3514c264`), live-deploy confirmed via commit-ancestry against the newest production deploy — see the CHECK 14 correction just above for how that same check also closed #1633's live-deploy gap. _(closed 2026-08-27)_
- [x] ~~An independent, now-redundant duplicate fix for CHECK 10's printer-crash bug sat unmerged on branch `claude/check10-drift-printer-fix`~~ — **resolved**, branch no longer exists on origin (deleted by its owning session, presumed superseded by #1623 landing first). _(added 2026-08-26, closed 2026-08-26)_
- [x] ~~`eq-shell` PR #1627 (paper-trail for `0162`/`0163` + ledger correction) and its stacked follow-up PR #1628 (`CHECK 12`/`13`) — both OPEN~~ — **both merged**: [#1627](https://github.com/eq-solutions/eq-shell/pull/1627) (`02a9b405`) corrected the ledger further (0162/0163/0164 turned out to be eq-cards' own tracked migrations, not out-of-band drift — its earlier paper-trail files were removed as redundant), [#1628](https://github.com/eq-solutions/eq-shell/pull/1628) (`ea3d649a`) shipped CHECK 12/13 cleanly once #1627 landed. `scripts/check-tenant-drift.mjs`'s edit-freeze is lifted. Full narrative of how the wrong premise was found and corrected: `sessions/2026-08-27.md`. _(added 2026-08-26, closed 2026-08-26)_
- [~] **Scope corrected 2026-08-27 — this is 7 checks, not 2.** Re-verified live against the current `origin/main`: CHECK 6 (`function_exec`), 7 (`view_invoker`), 8 (`column_grants`), 9 (`stacked_policy`), 12 (`control_plane_stacked_policy`), 13 (`control_plane_rls`), and 14 (`control_plane_isolation`) all have the identical issue-filer wiring gap #1623 just closed for CHECK 10 — none of the seven's `_report` keys are read by `tenant-drift.yml`'s issue-filing step (only `anon_grants`/`spine_rls`/`policy_lint`/`intentional_anon_reads` are), so a real violation on any of them would still only go red in CI, never file a GitHub issue on a schedule-only catch. Full key list and the exact `jq` pattern to extend: [eq/sprints/2026-08-26-organisations-anon-read-hardening.md](../sprints/2026-08-26-organisations-anon-read-hardening.md). Flagged as `task_f5c61eb9` (scope widened, not re-spawned) — **picked up 2026-08-27** as `task_e2eed444` (session `work-wiring-priorities-0150f3-c8`), in progress, no PR yet, outcome not confirmed as of this entry. A second, independent session (`eq-shell-2a`) arrived at the identical scope in parallel and collision-checked by messaging peer sessions directly (not just git/PR state, which showed nothing since no PR existed yet) — confirmed the collision within minutes and stood down without duplicating. Whoever checks this next: verify `task_e2eed444`'s actual outcome (merged PR, or abandoned) via `git log origin/main -- .github/workflows/tenant-drift.yml` before picking it back up. Full detail: `sessions/2026-08-27.md` (`eq-shell-2a` entry). _(added 2026-08-26, unblocked 2026-08-26, scope corrected 2026-08-27, picked up 2026-08-27)_
