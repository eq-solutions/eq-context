...
## eq-shell: security register reconciled — 3 items still open, all with PRs up (2026-09-04)
...
- [ ] **SEC-73 — eq-field's definer-rights `field_people_directory` / `field_managers` views on both tenant planes (P2 latent).** Decided 2026-09-04 (`/decide` pass): accept as a governed exception, review 2026-12-04. Two PRs open: eq-context [#203](https://github.com/eq-solutions/eq-context/pull/203) (baselines the 4 advisor errors + a test that fails once a baselined review date is past) and eq-shell [#1763](https://github.com/eq-solutions/eq-shell/pull/1763) (refreshes the stale CHECK 7 comment, comment-only). **Not yet shipped:** an allowed-column guard on CHECK 7 (would catch a future widen to DOB/emergency-contact) — #1763 writes up the exact ~20-line shape but deliberately didn't add it (changes gate behaviour, the forbidden-column list is eq-field's to own) — "say the word and it becomes its own PR." _(added 2026-09-04)_
