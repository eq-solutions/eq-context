...
- [ ] **This machine's Flutter SDK isn't on PATH, and there are two dead ends before the real one.** `PATH` points at `C:\Projects\flutter\bin`, which doesn't exist; a second, separately-installed copy at `C:\FlutterSDK` exists but is version 3.41.9 — too old for this pubspec's current Dart floor. The real SDK is at `C:\src\flutter`. Recurred as a fresh discovery across at least 3 sessions today (the round-2/3 polish sprint, PR #336's own commit message noting it skipped local analyze/test because of it, and a licence-edit-screen test session) — genuinely costing real time, not a one-off. Fix is a PATH edit (System Environment Variables), outside what Claude can do itself. _(added 2026-08-31)_
