...
- [x] `licence_edit_screen.dart` filled 4 fields with `EqColours.surface` while the rest of the app (incl. its own sibling fields) uses `EqColours.white` — the actual outlier, found while investigating a different suspected drift that turned out not to be one. Royce started this in a separate concurrent session (chip `task_824a07b7`); closed same-day — all 4 sites fixed, [PR #335](https://github.com/eq-solutions/eq-cards/pull/335) merged, deployed live on cards.eq.solutions (Netlify deploy timestamp cross-checked against the triggering GitHub Actions run, not just a green CI check). Follow-up: this screen has zero test coverage (verified via grep — nothing under `test/` references `LicenceEditScreen`, unlike its siblings); spawned as its own chip, separately started by Royce. _(added 2026-08-31, closed 2026-08-31)_
