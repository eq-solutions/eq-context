...
- [~] **Found the config.toml `verify_jwt` gap PR #319 (below, now archived) didn't cover**: `promote-labour-hire-photo` (added by #318, which PR #319 predates) has no explicit `[functions.promote-labour-hire-photo]` block either — same fix, same rationale, just a third function PR #319 couldn't have known about yet. Spawned as a background task; Royce has started it in a separate session. _(added 2026-08-26)_
