...
*Direct follow-up to the licence-photos-org-drift sprint above: after confirming Conor Horgan's and Nelson Sareto's data looked clean, Royce pushed back with a live screenshot of "We couldn't load your licences / Something unexpected happened" and made clear this has been blocking labour-hire workers' cards-on-phone for weeks. Root cause is distinct from the 2026-09-02 stale-session bug archived this same close (see `eq/pending-archive.md`) — that one only explains a "Sign in again" symptom; this one explains the actual "Something unexpected happened" `UnknownFailure` symptom screenshotted.*

**Shipped:**
1. `eq_cards_claim_invite` inserted a new licence with OCR's `expiry_date` verbatim and never set `never_expires`, so it took the table's `false` default. Credential types with no printed expiry (`working_at_heights`, confirmed live) legitimately OCR to a null `expiry_date` at high confidence — producing a row (`expiry_date` NULL + `never_expires` false) the app's own save form can never create and `Licence.fromJson` can't parse. Migration `0175` (written as `0173`, renumbered twice pre-merge — collided with PR #361's `0173` then PR #363's `0174`, both merged first) defaults to `never_expires = true` / `expiry_date = 9999-12-31` when OCR found nothing, mirroring the sentinel already used everywhere else a licence is marked non-expiring. Built on the function's current *live* body (confirmed via a fresh `pg_get_functiondef`, not the migration file on disk) — which turned out to already include migration `0172`'s `origin_org_id` stamp despite that PR's own commit message saying "not applied to jvkn." That note was stale by the time this session read it; basing the fix on live state instead of the file avoided a silent-overwrite collision. Applied to jvkn, confirmed live.
2. `LicenceRepository.getAllForCurrentUser()` (`lib/features/licences/data/licence_repository.dart`) parsed the whole licence list in one `.map().toList()`, so one malformed row crashed a worker's entire wallet, not just the one card. Now parses each row independently and reports a bad one to Sentry instead of throwing for the whole list — defence in depth against whatever writes a bad row next, not just this specific bug. [PR #364](https://github.com/eq-solutions/eq-cards/pull/364), squash-merged (`f9c02cb7`), not yet deployed.
3. Data repair for the two already-broken rows (Conor Horgan `ea6a3b6c-c3b7-4f9b-8ae2-4de61e615a82`, Nelson Sareto `facceba0-0dd0-40ef-861a-c947b881e4b6`, both `working_at_heights`) handed to Royce as a plain UPDATE (data mutation against real customer rows is classifier-blocked for direct MCP execution) — he ran it himself in the Supabase Dashboard SQL editor; confirmed live independently afterward, both rows now `expiry_date: 9999-12-31, never_expires: true`. This is the part that actually unblocks them — doesn't need PR #364's deploy, since the row itself changed, not the client code.

**Deferred:**
- [ ] **Not a live click-test as either worker** — data + root cause verified live, not eyes-on-phone confirmed. Same open question as the archived 2026-09-02 entry, now on firmer footing (the specific mechanism in the actual screenshot is fixed) but still genuinely unconfirmed. _(added 2026-09-15)_
- [ ] **A third, similarly-shaped broken row (Fernando Alba, `driver_licence`, no `source_worker_cred_id`) has a different, unidentified writer** — not `eq_cards_claim_invite`, so not fixed by migration `0175`. Spawned as background task `task_f6592919`; Royce started it running in a separate session, in progress as of this update. _(added 2026-09-15)_
- [ ] **PR #364 merged, not yet deployed** — merging to `main` does not deploy eq-cards (`workflow_dispatch` only); the Dart resilience layer needs an explicit deploy to actually reach cards.eq.solutions. _(added 2026-09-15)_
