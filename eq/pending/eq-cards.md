...
- [ ] **A third, similarly-shaped broken row (Fernando Alba, `driver_licence`, no `source_worker_cred_id`) has a different, unidentified writer** — not `eq_cards_claim_invite`, so not fixed by migration `0173`. Spawned as background task `task_f6592919`; Royce started it running in a separate session, not yet reported back. _(added 2026-09-15)_
- [ ] **PR #364 not yet merged or deployed** — merging to `main` does not deploy eq-cards (`workflow_dispatch` only); the Dart resilience layer needs an explicit deploy once merged. _(added 2026-09-15)_
