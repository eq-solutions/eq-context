...
- [x] **Fix built**: new `netlify/functions/apprentice-data.js` resolves the caller's real identity server-side (reusing `verify-pin.js`'s existing `resolveFieldPerson()` — Shell-verified identity → `field_person_by_user_id` RPC) and scopes reads to that person unless manager/supervisor; no resolvable identity → empty, fail closed. `scripts/apprentices.js` calls this instead of 7 direct `sbFetchAll()`s. Migration `20260818_apprentice_authenticated_select_lockdown.sql` revokes `authenticated` SELECT on all 7 tables so the endpoint is the only path in. eq-field [PR #717](https://github.com/eq-solutions/eq-field/pull/717) (v3.5.514) — open, CI green (17-case scoping test suite + full existing suite), deploy preview smoke-tested (endpoint reachable, correctly 401s a bad token, no console errors). **Migration not yet applied to live ehow** — waiting on PR review before touching live grants.
...
**Deferred:**
- [ ] **Write paths are a separate, still-open gap** — INSERT/UPDATE/DELETE grants on all 7 apprentice tables are untouched by PR #717; any authenticated tenant member can currently write feedback/ratings/journal/rotations/reviews under any `apprentice_id`, not just their own. Same root cause, roughly doubles the fix surface (needs the same identity-resolution as a write-side proxy or a `WITH CHECK` that can verify the target profile belongs to the caller) — deliberately not folded into the read-side fix. _(added 2026-08-18)_
...
- [ ] **`eq` tenant not covered** — apprentice reads stay unfiltered there (disposable demo data, no `field_person_by_user_id` equivalent on zaap). Lower urgency per this repo's own CLAUDE.md; revisit only if `eq` ever carries real data. _(added 2026-08-18)_
- [ ] **Not click-tested live by a human as a real apprentice/manager pair** — SKS is Core-only, so the deploy preview can't exercise a real signed-in session from this sandbox. The 17-case mocked test suite covers the scoping logic; a real click-through (one manager session, one apprentice session, confirm each sees what they should) is still worth doing once the migration is live. _(added 2026-08-18)_
