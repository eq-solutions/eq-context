...
## eq-field: Map / Roster Overview tiles broken — CARTO started requiring an API key we don't have, swapped to OpenStreetMap (2026-08-30)
...
- [ ] **Not click-tested live through the actual rendered map** — no live SKS/Core credentials in this environment (same standing limitation as most entries in this file). Verified instead via direct tile fetches against both the broken and replacement providers, which is arguably the stronger check for this specific bug class (a tile-server response failure, not app logic) — but a real click-through would still catch a Leaflet-level integration issue this can't. _(added 2026-08-30)_
