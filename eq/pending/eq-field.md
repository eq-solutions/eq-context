...
- [ ] **Labour Hire coverage is mixed today, not zero — corrected 2026-08-21, an earlier note here overstated the gap.** Both the roster-gate badge and the My Schedule card key off individual Core-authenticated identity (`worker_id` on the Field person record is not enough by itself — the live-checked signal is `user_id`, meaning the person has actually claimed a login), not employment group, so any Labour Hire worker who's already signed up for their own account gets full coverage right now, same as Direct/Apprentice. Live-checked against SKS: of 19 active Labour Hire people, **6 already have their own login and full coverage today**; of the other 13, only 2 have an `agency` tag making the shared per-agency fallback (`auth-agency-gate.js`) available to them at all — the remaining 11 have neither an individual login nor an agency fallback, so they can't be reached under their own identity by anything in Field right now, a real access gap that predates and is broader than this feature. Coverage extends automatically as more people individually sign up (already an active suite-wide identity rollout) — no further Field-side work needed for that. An agency-level aggregate nudge ("3 of your people are missing something," no individual names) for the 2 with an agency fallback was scoped as an option and explicitly deferred to a later round, Royce's call. _(added 2026-08-21, corrected same day)_
