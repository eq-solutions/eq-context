...
- [ ] **Labour Hire workers get nothing from either feature today** — both the roster-gate badge and the My Schedule card key off individual Core-authenticated identity, which Labour Hire's shared per-agency login (`auth-agency-gate.js`) doesn't have. Coverage extends automatically to any Labour Hire worker once they're individually linked (already an active suite-wide identity rollout), but agency-mode-only workers are invisible to both features until then. An agency-level aggregate nudge ("3 of your people are missing something," no individual names) was scoped as an option and explicitly deferred to a later round, Royce's call. _(added 2026-08-21)_
