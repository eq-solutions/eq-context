...
- [ ] **Shell↔Field worker-sync gap — a real self-join apprentice landed with `employment_type='Direct'` and no `apprentice_profiles` row.** Confirmed live: `shell_control.user_tenant_memberships` on jvkn correctly has the reporter's role as `apprentice` (genuine self-join, `self_join_code_id` populated) — the invite link itself worked. But whatever creates the `app_data.staff` row on ehow when a Shell self-join lands never mapped that role onto Field's `employment_type` (canonical vocabulary: `Direct`/`Apprentice`/`Labour Hire`/`Subcontractor` — `Direct` looks like just the unmapped default, not a deliberate choice) or created an apprentice profile. **Royce's call, not fixed here**: the specific record gets corrected via Shell's Staff UI (same precedent as the Angangan/Cluff case, 2026-08-04) — not a database patch. The underlying sync gap (does every self-join apprentice land misclassified, or was this one-off?) is unscoped and worth a look if it recurs. _(added 2026-08-18)_
