...
- [ ] **Dispatch the timesheets/leave write-side migration once the unlinked-staff population clears.** Re-checked live 2026-08-20: 34 of 101 SKS staff still have no linked login (28 of them plain workers, not supervisors) — applying today would lock those 28 out of saving their own timesheet or leave request on day one. Real, steady progress since 2026-08-19 (38→34 unlinked, 31→28 at-risk). Same call Royce made on this exact blocker on 2026-08-16, reconfirmed 2026-08-19, reconfirmed again 2026-08-20 via `/decide` (chose "wait, re-check periodically" over scoping a fallback identity path now). Branch is ready (`claude/timesheets-leave-write-actor-identity`, single commit `01af1496`, no PR opened yet); needs a fresh explicit go at the moment the population actually clears, not assumed from a lower count alone. _(added 2026-08-19, updated 2026-08-20)_
- [ ] **Housekeeping, found while re-checking the above**: a stale local branch, `claude/p1-timesheet-leave-own-crew-rls` (remote already deleted — `[gone]`), carries the ORIGINAL pre-actor-identity versions of both the read and write migrations. Fully redundant now — the read-side content shipped via PR #705's squash-merge on 2026-08-16 and was then superseded by PR #719's actor-identity fix on 2026-08-18; the write-side content was likewise superseded by this same actor-identity branch. Nothing unique left on it. Candidate for local deletion; not deleted here (destructive git ops need explicit go, not assumed). _(added 2026-08-20)_
