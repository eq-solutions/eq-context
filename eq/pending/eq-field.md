...
- **Addendum from a parallel session, same thread:** this whole investigation started from a *separate* bug this entry doesn't mention — [eq-field#981](https://github.com/eq-solutions/eq-field/pull/981), CSP allowlist for madagins missing from `netlify.toml` (only `_headers` had it from #976). That's what actually caused "Cannot reach server" + empty Contacts; the JWT-mint 500 above was the *second*, separate layer underneath it, only visible once the CSP block was cleared. Also confirmed live via Sentry as of 09:13:29Z (after the code fix deployed, after a manual redeploy) that EQ-FIELD-1T was *still* firing — couldn't personally verify the 09:32Z all-clear this entry claims before moving to other work; leaving both data points on record rather than reconciling them. See [[feedback_netlify_function_env_vars_need_redeploy]] and [[feedback_csp_dual_file_recurs_despite_doc]] in eq-field's own memory. _(added 2026-09-14)_

---

## eq-field: madagins person-record cluster — group value, dropped phones, duplicate identity (2026-09-14)
*Royce: "why can't I see staff from Core in Field for Madagins" → CSP fix (#981, above) → then a chain of smaller, real data gaps surfaced one at a time as he clicked around: his own record's group value, then his and two others' missing mobiles, then a duplicate login for Aditi. All trace to the same rushed 2026-09-09 onboarding. Full writeup: `eq-field` repo memory [[project_madagins_onboarding_integrity_gaps]].*

- **Fixed live, hand-corrected on madagins's own Supabase project (`ornndtbdkxfsewspbrwk`)**: Royce's own `app_data.staff.group` was the literal string `'Manager'` instead of a valid `ROSTER_KNOWN_GROUPS` value — corrected to `'Direct'`. Backfilled real mobile numbers for Nelson Sareto, Royce, and Conor Horgan (all three had correct numbers sitting in canonical — `public.profiles.mobile` / `shell_control.users.phone` / `auth.users.phone` — that never made it into madagins's own staff table). Corrected Aditi's and Michelle's numbers too, using the real numbers Royce supplied directly (Aditi's canonical-side number was wrong; Michelle had none anywhere in the system).
- [ ] **Root cause not yet found** — something in the tenant-provisioning/promotion pipeline drops `phone` (and possibly other fields) when creating a new tenant's `app_data.staff` rows from canonical. Spawned as a background task (`task_88357119`); Royce started it in a separate session, running independently, not reported back as of this close. _(added 2026-09-14)_
- **Separate discovery, reconciled the same session**: Aditi had two live Shell identities on madagins — the original admin-invited one (created 2026-09-09, wrong phone, real email) and a second one from a phone-OTP sign-in (created 2026-09-13, her real mobile, no name/email saved). `shell_control.identity_recycle_review` — the table built specifically to auto-stage this exact "new phone signup matches an existing account" scenario — had zero rows for it despite the collision being live for about a day. Reconciled by hand via the official `eq_resolve_recycle_review()` RPC (staged a review row, approved it — cleared the old identity's email/phone) plus a manual name/email backfill onto the surviving identity, since the RPC's own insert no-ops when the new-side row already exists (which it did here). Review row `567b769b-65c6-4377-8140-ac10f5fd22f4`, marked `approved`.
- [ ] **Why the auto-detector missed a real, live collision is still open** — spawned as a background task (`task_58cb7bcf`), not started as of this close. _(added 2026-09-14)_
- [ ] **Darren O'Connor's account status is still unclear.** He showed as "invite not claimed" on Core's Staff list at the start of this session; a mystery `shell_control.users` row initially suspected to be him turned out to be Aditi's duplicate instead (see above). His actual status was never re-checked after that correction. _(added 2026-09-14)_

**Notes:** Full session detail: `sessions/2026-09-14.md`.

---

## eq-field: CSP CI guard proposed — netlify.toml vs _headers drift (2026-09-14)
...
- [ ] **Build a CI check that fails when the two files' CSP directives disagree** — modeled on this repo's existing drift-guard pattern (e.g. `cache-buster-drift.yml`), not a new shape. Spawned as a background task (`task_bb3d4098`), not started as of this close. _(added 2026-09-14)_
...
- [ ] **Anthony Hartley hit a related-shaped identity-resolution miss twice in ~1 week** — this session's mechanism (JWT-mint timeout → unresolved actor identity → RESTRICTIVE policy blocks) is a plausible but unproven match for the 2026-08-30 "client-side load-order race" theory logged further down this file. Two hits for the same person is enough to wonder if something about his device/connection makes him more likely to hit this than most — not investigated. _(added 2026-09-07)_
- [ ] **The underlying identity-linking gap is unchanged** — 13 of 77 active SKS staff still have no linked login as of today (was 14/74 on 2026-08-27), the same population the write-side hold and the Documents-to-Sign audience-reach gap both depend on. Still an ops task (people completing Cards/Core signup), not a code fix; still Royce's call whether to accept residual risk and dispatch the write-side migration anyway. _(added 2026-09-07)_

**Notes:**
- Full technical trace: `sessions/2026-09-07.md`.
- Cross-referenced (not rewritten) in the 2026-08-30 Anthony Hartley item and the 2026-08-19 write-side-hold item further down this file, so both stay findable from either direction.
