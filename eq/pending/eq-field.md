...
- [x] **Root cause: no page — Edit Roster, Weekly Roster, or My Schedule — checked whether a week's data had actually finished loading before rendering it.** `STATE.schedule` only holds a ±1-week sliding window; switching weeks re-rendered immediately against whatever was already in memory. A week still loading read as "genuinely empty," which Edit Roster painted as a loud amber "needs filling in" cell with the day name as placeholder text — indistinguishable at a glance from the roster having been wiped — until the real values landed and overwrote it. `_preloadAdjacentWeeks()`, the function that looked like it should already prevent this, was a literal no-op stub. eq-field [PR #893](https://github.com/eq-solutions/eq-field/pull/893) (v3.5.654 — collided on its version number with 3 separate concurrent same-day PRs in a row, #892/#894/#896, all touching roster-adjacent code; rebased/renumbered/re-verified each time), merged and confirmed live (`field.eq.solutions/sw.js` shows v3.5.654).
- [x] **Fix mirrors an existing pattern rather than inventing one** — `_ensureSurfaceWeekLoaded()` (new, `roster-week-nav.js`) uses the same render-now/background-load/redraw-on-arrival shape `_ensureApprenticeWidgetWeeksLoaded` already uses, wired into all three affected render functions. Uses a 15s retry cooldown rather than a permanent give-up-after-one-failure latch, specifically because Edit Roster uniquely disables the app's 30s background poll (`if (currentPage === 'editor') return;`) — a permanent latch would leave a supervisor stuck on a genuinely-failed week with no automatic recovery.
- [x] **4 new functional tests** added to `tests/week-window-narrowing.test.js` exercising the real dedupe/redraw/cooldown behavior against mocked state, not just checking the right function names appear in source.
- [ ] **Root cause of the underlying network flakiness itself, not fixed here.** Two live Sentry issues explain why the fetch sometimes fails on this exact path (both pre-date this fix — 2026-08-18 and 2026-08-27 — so not caused by any recent deploy): `EQ-FIELD-19` (AbortError on the tenant-config routing fetch) and `EQ-FIELD-16` (Failed to fetch inside the sign-in token mint), both tagged to the Shell-embedded SKS session. Spun off separately — Royce started it running in its own local session (`task_0fdf3a24`), in progress, not yet reported back. _(added 2026-09-02)_
...
*A PostHog traffic review for 2026-09-02 (framed as SKS's "first day live") found the raw traffic data didn't actually show a launch-day spike — the real signal was compositional: 42% of Field's active users that day had never appeared in the product before (vs ~8-10% on Shell/Cards), which Royce confirmed reads as genuine workforce adoption since Field is what every SKS worker touches day-to-day, not just admins. Two real bugs surfaced from the same investigation and shipped: (1) Field's crew-table sync failures were invisible to `sync_degraded` telemetry — root cause was structural (crew tables load in the same wave as `people`, wrapped in a swallow-catch), fixed in PR #885, live. (2) A follow-up audit for the same "silent-swallow" bug shape found `copyLastWeek()` (Copy Last Week) could report "saved ✓" even when every write failed — fixed in PR #890 to match the already-correct sibling `applyBatch()` pattern, but Royce deliberately held it back unmerged after checking real usage (2 uses ever, one a risky 172-cell blanket fill) — making a rarely-used bulk action safer risked encouraging more use of exactly the pattern he's worried about (automation hiding a stale value behind a "filled" cell). Separately, the same conversation's screenshot review of Weekly Roster's summary card led to PR #896 (below). Full detail: `eq/changelog/eq-field.md`, `sessions/2026-09-02.md`.*
...
- [ ] **Field's `sync_degraded` root cause: reasoned from code, not measured live** — PR #885's investigation concluded timeout/RLS/index are all fine and the 12 daily incidents read as ordinary connectivity blips, but couldn't pull live Supabase query logs for the actual incident window (MCP connection failed mid-session). If `sync_degraded` volume grows, close that live-measurement gap before assuming it's still just network noise. _(added 2026-09-02)_
