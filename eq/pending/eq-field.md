...
- [ ] **One further call site confirmed still open, found and independently re-verified against the live merged code after #833 landed**: `renderSchedule`'s own direct site-lead/internal-contacts lookup (`st = STATE.sites.find(x => x.abbr === s)`, now `roster.js` ~line 1456 post-merge) is a 6th call site #833 did not cover — it feeds `site_lead`/`site_lead_phone` and the "Ask for"/"Backup" internal-contacts block (`_siteInternalContactsHtml`) on a worker's own My Schedule day card. A MOD10-typed cell shows the correct name/address/job-number now, but still shows no site-lead or emergency contact for that day. Same fix shape as #833 (route through `_resolveSiteAbbr`), spawned as `task_1007afb5`. _(added 2026-08-30)_
- [ ] **Separate, lower-priority follow-up also spawned (`task_18045ada`), Royce has already started it in a parallel session**: `dashboard.js`/`core-bundle-b1.js`/`trial-dashboard.js` have their own separate, underscore-prefixed `_getSiteName` with the identical abbr-only gap — different file, generated bundle, lower-stakes aggregate/map view rather than a worker-facing surface. _(added 2026-08-30)_
