...
## eq-field: Weekly Roster "By Crew" gap chips + Edit Roster name/site filter (2026-09-01)
*Royce, screenshot of the SKS Weekly Roster "By Crew" view: "I find this view useless - what are our options?" Read `_computeRosterOverview`/`_renderRosterOverviewCard` before proposing anything — the card already computed the coverage gap (`totalActive` vs `onCount`) but never named who made it up. Presented 3 directions via `AskUserQuestion`; Royce picked "lead with gaps." Mid-turn, separately asked whether Edit Roster could filter by site/name — it had neither, unlike the read-only Weekly Roster page.*

- [x] **By Crew card now shows a "Not rostered today (N)" chip row** naming who's active, not on leave, and has no site code today — above the site list, which is unchanged. Deliberately did not fold in the "Unassigned(27)" Team-membership chip (`personInActiveTeam()` reads *filter* state, `STATE.teamFilters` — a person can have no Team and still be legitimately rostered today, a different concept). Left Royce's 2026-07-31 "don't list empty sites" rule untouched — that's about sites, this is about people.
- [x] **Edit Roster gets its own name search + site filter** (`#editor-search`/`#editor-site`) — computed independently in `renderEditor()` rather than reusing the read-only page's `getRosterPeopleForOther()`/`_rosterApplySearchSiteSort()`, which read that OTHER page's inputs; reusing them as-is would have silently filtered Edit Roster's "Other" group bucket by stale/wrong values while every other group filtered correctly.
- [x] **`roster.js` crossed its 1900-line CI ceiling as a side effect of the above two** — extracted the whole Roster Overview card block into new `scripts/roster-overview.js` (same precedent as the existing `roster-overview-map.js`/`week-picker.js` splits), rather than raising the ratchet; `roster.js` back to 1774 lines. eq-field [PR #860](https://github.com/eq-solutions/eq-field/pull/860) (v3.5.627), squash-merged on explicit "merge." Verified via `eslint` + `tests/roster-adapter.test.js`/`roster-cell-picker.test.js`/`roster-rules.test.js` (all green), `check-cache-busters.mjs`/`build-bundles.mjs --check` (clean), and by loading the real shipped source into a live browser tab via fetch+eval against fixture data plus real DOM interaction (screenshotted) — full local boot isn't reachable in this sandbox, `loadTenantConfig()` now requires the `tenant-config` Netlify Function even for `?tenant=demo`, which a bare static server 404s on. Confirmed live on `field.eq.solutions/sw.js` (v3.5.627) after merge.
...
- [ ] **Not click-tested live by a person** — no SKS/Core credentials in this environment, verified via fixture-driven browser rendering instead (see above). Worth a real pass: SKS supervisor opens Weekly Roster, confirms the gap chips read sensibly against real data; opens Edit Roster, confirms the new search/site filter narrows correctly. _(added 2026-09-01)_
...
- [ ] **Simon Bramall (relayed via Royce) has asked to reconcile all *future*-dated sks-nsw-labour entries into EQ Field too** — this session's copy only covered historical/past dates. Not started as of this close; first step is a list of what's already entered on each side. _(added 2026-09-01)_
...
*Follow-on to the removal-bug fix above, both same-day requests from Royce on the same live screenshot. (1) v3.5.623/PR #854 — "with all that space on the right hand side can you show a summary of whos selected": a `/decide` pass upgraded the first-draft design (a read-only mirror list) to removable chips, since a passive mirror still requires hunting the checkbox list to actually remove someone. Page-only (the modal opened from Roster/Contacts/Schedule/Timesheets is untouched); page widens 640→960px only while editing; each chip reads and writes the same checkbox `saveEditTeam()` reads, pinned to the page's own container even if the modal happens to be left open from an earlier navigation (a real edge case, covered by its own test). (2) v3.5.624/PR #856 — "clicking anywhere within the teams row brings up the edit section": the whole row now opens edit; the old "✎ Edit" button (redundant once the row does the same thing) is gone, replaced by a small "Editing" label; Delete stays separate via `event.stopPropagation()`; view-only users get no click affordance at all. Both verified via a 35-scenario local harness against the real, unmodified file (raw `file://` doesn't work for this app — needs a real static server) plus real mouse-click verification in the browser pane with the app's actual CSS loaded, not just assertions. Neither click-tested live on SKS — no Core/SKS credentials in this environment. Full detail: `eq/changelog/eq-field.md`, `sessions/2026-09-01.md`.*
...
- [ ] **This session hit the same cache-buster-tag CI drift twice more** (app-state.js on #854, base.css — a first-time CSS-file variant — on #856), despite the exact fix being documented in Claude's own memory and already applied once earlier the same session. Spawned `task_7499b5e4`: wire `check-cache-busters.mjs` into a local pre-push hook (mirroring this repo's existing brief-gate/reflection-gate hooks) so it's enforced mechanically instead of relying on remembering to run it. _(added 2026-09-01)_
