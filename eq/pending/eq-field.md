...
- [ ] **Separate, unrelated live bug noticed in passing, spawned as its own task** — Sentry `ReferenceError: _isPhoneViewport is not defined` (eq-field, 1 event 2026-09-08), sitting in the exact `timesheets.js` region this session was editing but not investigated (out of scope). Royce started `task_491cd965` in a separate local session; running independently, not yet reported back. _(added 2026-09-09)_
