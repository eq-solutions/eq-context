...
*Royce reported PR #720's fix "still shows everyone" — traced live rather than assumed: the new scoped fetch fails open on SKS, not just for the 'eq' tenant it was meant for. Root-caused against Jordan Sample's real data (linked, zero `apprentice_profiles` rows), then audited login/onboarding status across every real signal (ehow `audit_log`, jvkn `auth.users`, live Sentry issues) at Royce's request.*

- [x] **Real gap, not the 'eq'-tenant edge case it looked like.** `loadApprenticeData()`'s scoped-fetch failure (expired/invalid token, network blip, function error) fell through to the pre-fix unscoped legacy branch meant only for the 'eq' tenant — on SKS this silently showed the full company roster again the instant the scoped call had any hiccup. Fixed to fail closed on SKS (same as "no resolvable identity"). eq-field [PR #721](https://github.com/eq-solutions/eq-field/pull/721) (v3.5.518), squash-merged, confirmed live via `field.eq.solutions/sw.js`. CI caught two real issues on the way through: the file was pushed 20 lines over its 1550-line grandfather cap (trimmed comments, no lint-config change) and a missed `app-state.js` cache-buster bump (the same recurring bug class this repo has hit before).
- [x] **ehow's `audit_log` is not a reliable login-activity source — jvkn's `auth.users` is.** ehow logs whatever display name Shell had at sign-in (usernames, bare UUIDs for brand-new accounts), not the Field contact name, so a first pass wrongly read "zero apprentices have ever logged in." Re-ran against jvkn's `auth.users`/`org_memberships` (real UUIDs, `last_sign_in_at`) and found every apprentice has in fact logged in — corrected before it was acted on.
- [x] **Full onboarding/login audit, Royce's request.** 60 active SKS memberships: exactly one account fully confirmed and never signed in (Mark Brame — confirmed by Royce as expected, he's GM and hasn't needed to log in yet) and one signed-in account with no linked worker profile (traced to a `licence_type: "Test"` QA account, not a real person). One already-resolved identity-collision merge found in passing (`william_brown_identity_merge`, handled 2026-08-17). No evidence of genuine failed-login attempts in either DB — that telemetry doesn't exist at the DB level for Core-only auth.
- [x] **Live Sentry sweep surfaced a real, still-recurring bug** — `EQ-CARDS-1C`, a permission-denied error blocking brand-new Cards signups, last seen hours before this session. Root-caused (not guessed) to a session-validation race: the auto-provision RPC could fire before the client's Supabase session had caught up with a fresh OTP verify. Spun off as a background task; came back as [eq-cards PR #270](https://github.com/eq-solutions/eq-cards/pull/270), merged by Royce (auth-flow change, held for his explicit approval per standing rule). Corrected the initial blast-radius read too — 9 of the 10 occurrences traced to one already-resolved duplicate account, not a broad outage.
- [x] **Two more real bugs found investigating EQ-CARDS-1C, fixed in the same pass.** Self-joined apprentices/labour-hire workers were landing as `employment_type: "Direct"` because `public.workers.role` was never written on any of the 3 claim paths (Cards phone-OTP, desktop email+PIN, Cards-native RPC) — [eq-shell PR #1448](https://github.com/eq-solutions/eq-shell/pull/1448), merged, live (2 of 3 paths auto-deploy; the third needed a hand-applied jvkn migration — checked live and it was already applied, byte-for-byte match, no action needed). Separately, a real apprentice's self-service actions (Edit Goals, Ask for Feedback, "How am I going?") were silently not rendering because 4 checks were still keyed to the dead standalone-PIN identity — [eq-field PR #722](https://github.com/eq-solutions/eq-field/pull/722) (v3.5.519), merged, live.
- [x] **`EQ-SHELL-T` ("auth-stall: verify-timeout") checked, not a live problem.** Initial read ("1 user impacted") was wrong — no user-identifying field exists on this event type at all, so that's Sentry's default anonymous bucket, not one stuck person. It's a self-instrumented "slow session" warning (not a crash), and its sibling event (`session-spinner-timeout`) is already marked resolved. Left alone.
- [ ] **Latent bug found, not yet fixed**: `verify-pin.js`'s token-refresh (`verify-token` re-mint, used on every remember-me page-load restore) explicitly passes `shell_user_id` as `undefined`, dropping it from the refreshed token. Doesn't explain this session's bug (still fails closed correctly when it happens) but is a real gap worth its own look — a caller's server-attributable identity silently disappears on every session refresh. _(added 2026-08-18)_
