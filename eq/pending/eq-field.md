...
## eq-field: field_people SEC-33 visibility regression — directory view shipped, consumer rewiring still open (2026-08-27)
*eq-shell's SEC-33 fix (migration 0256, applied 2026-08-21 to ehow+zaap) added a RESTRICTIVE own-row-or-manager read policy on `app_data.staff` — correct, it protects DOB/emergency-contact/pay-rate PII. `app_data.field_people` (security_invoker=on) silently inherited it too. Confirmed live: 88 of 110 SKS staff (everyone who isn't a manager/supervisor) got back at most their own row, tenant-wide, since 2026-08-21 — and `STATE.people` is core boot data read by 42 files (roster, timesheets, leave, safety, dashboard, calendar...), not just the roster screen. Same root cause as `field_managers`'s identical collapse (eq-field PR #813, fixed same day by a concurrent session responding to a live worker complaint from Brendan Drinkwater), but deliberately not fixed the same mechanical way, since `field_people`'s column list genuinely carries PII.*

- [x] **`app_data.field_people_directory` shipped** — new view, `security_invoker=false` with its own inline `tenant_id` filter (same shape as the `field_managers` exception), exposing only a Royce-approved coworker-safe column subset (name, trade, licence, group, agency, on_roster, etc.) — excludes DOB, email, phone, emergency contact, PIN, rating, TAFE fields. `field_people` itself untouched, still `security_invoker=on`. Applied live to both ehow and zaap, verified via JWT simulation both directions (a real non-supervisor worker now gets the full 74-person roster from the new view; still gets just their own row from `field_people`; cross-tenant isolation intact; sensitive columns structurally absent, not just filtered). eq-field [PR #814](https://github.com/eq-solutions/eq-field/pull/814), squash-merged on explicit "merge", CI green.
- [ ] **Part 2, not started: repoint `roster.js` and the other `STATE.people` consumers at the new directory view for coworker display**, while keeping a worker's own profile-edit screens (DOB, emergency contact) reading the full `field_people` row. Deliberately deferred rather than rushed in the same pass — needs a per-consumer check across the 42 files that read `STATE.people`, not a global swap, to avoid breaking a worker's ability to see/edit their own sensitive fields. This is the actual user-visible fix; part 1 alone changes nothing on screen yet. _(added 2026-08-27)_
...
## eq-field: Weekly Supervisor Digest → Email Templates + test-send auth fix — closed, one click-through remaining (2026-08-26/27)
*Consolidated the digest's messy Supervision-page config (21-manager checklist + section toggles) into Manage → Email Templates, same governed surface the 3 leave emails already use, and added a "Send test to myself" safety net (PR #791, v3.5.573). Royce's first real click hit "unauthorized" — fixed the same session (PR #802, v3.5.578): two independent gaps, the edge function's new code was never actually deployed (Supabase edge functions ship separately from Netlify/git) and the Netlify function's credential didn't match what the edge function's auth check compares against. A THIRD gap found the next day (2026-08-27): the corrected credential (`DIGEST_CRON_SECRET`) had silently failed to save the first time — a Netlify MCP tool call reported "Environment variable upserted" but a follow-up read came back "Not Found". Set directly via the raw Netlify API instead, redeployed, and verified live via a credential-free probe: the function's response changed from `500 Server missing config` to `401 Not authenticated`, proving it now reaches the real auth check. Full root-cause detail in `eq/changelog/eq-field.md`'s 2026-08-26/27 entries.*

- [ ] **Real click-through by Royce still the closing verification step.** All three gaps verified fixed (auth handshake reaches the real session check, edge function redeployed and confirmed at v14, the credential itself confirmed actually saved this time — not just reported saved), but nobody has actually clicked "Send test to myself" on Manage → Email Templates and confirmed an email lands in an inbox. Two minutes, whenever convenient. _(added 2026-08-26, re-verified 2026-08-27)_
