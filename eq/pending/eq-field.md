...
*Continuing the RLS sweep from 2026-08-18's apprentice/timesheets work — checked the 5 safety/site-report tables plus site_audits' own child table, and separately re-verified whether the timesheets/leave WRITE-side gap (distinct from the READ-side fix shipped 2026-08-18 via PR #719) had actually landed.*

- [x] **Real finding**: `prestarts`/`toolbox_talks`/`incidents`/`site_diaries`/`site_audits`/`site_audit_items` enforced tenant/org scoping only, no role check — any authenticated SKS session, including apprentice/labour_hire, could INSERT/UPDATE/DELETE any prestart, toolbox talk, incident, site diary, site audit, or audit item directly via the API, bypassing the app's own manager/supervisor-only create+submit gate entirely. Read was never affected. Fixed with a manager/supervisor-only role check on write. eq-field [PR #728](https://github.com/eq-solutions/eq-field/pull/728), squash-merged. Migration applied live to ehow via Supabase MCP (Royce's explicit go) — post-apply verified: all 6 tables show their original permissive policy plus the new role-gated write policies.
- [x] **Checked whether the timesheets/leave WRITE-side fix had actually shipped, and found it hadn't — plus a substrate correction.** The migration's own file claims it "dispatches via eq-shell's One Pipe" (`tenant-migrate.yml`). That claim doesn't hold up: the One Pipe only reads migration files from eq-shell's own repo (a different schema, a different numbering scheme, applied fleet-wide) — it has no way to see a file sitting in eq-field's own `supabase/migrations/`. Every migration in this lineage claiming "dispatches via the One Pipe" (the 2026-08-16 read/write pair, the 2026-08-19 actor-identity fix, and this session's own site-reports fix) has actually gone in by hand, one Supabase MCP call at a time, with an explicit go each time — the file comment describes a pipe that doesn't reach these files, not what's actually happening. Doesn't change anything already applied; future sessions shouldn't expect "merge to main" to ship these on its own.
- [x] **Drafted the corrected timesheets/leave write-side migration** — re-points `20260816_timesheets_leave_own_crew_write.sql`'s own-row/crew write scoping at the same real-identity fix (`eq__caller_actor_staff_id`) the read side already uses live. Committed to branch `claude/timesheets-leave-write-actor-identity`, NOT merged, NOT applied.
- [ ] **Dispatch the timesheets/leave write-side migration once the unlinked-staff population clears.** Re-checked live 2026-08-19: 38 of 99 SKS staff still have no linked login (31 of them plain workers, not supervisors) — applying today would lock those 31 out of saving their own timesheet or leave request on day one. Same call Royce made on this exact blocker on 2026-08-16 — reconfirmed, not re-decided. Branch is ready (`claude/timesheets-leave-write-actor-identity`); needs a fresh explicit go at the moment the population actually clears, not assumed from a lower count alone. _(added 2026-08-19)_
...
- [ ] **app_config PIN key-scoping** — hygiene (PINs gate nothing now, low-priority). **Re-verified 2026-08-19: `anon` grant is gone.** `information_schema.table_privileges` on ehow `public.app_config` now lists only `postgres`/`service_role`/`authenticated`; `has_table_privilege('anon', 'public.app_config', 'SELECT')` returns `false` — contradicts the 2026-08-17 note above ("still anon-readable") and a same-day third-hand claim (relayed via a subagent from an unrelated eq-field session) that it currently grants public SELECT and exposes a TAFE API token. Could not reproduce that claim against live state — either the grant was tightened between 2026-08-17 and now (by PR #728 today, or something else), or the relay was inaccurate; not chased further. Worth a direct re-check before treating this as either "fixed" or "urgent." _(added 2026-06-30, corrected 2026-08-19)_
---
