...
*Royce: "jessica robsinson leave shows on the calendar but not in the roster (October)." Investigated end-to-end against live ehow data before touching code.*

- [x] **Root cause #1: Roster/Editor/Contacts/Timesheets silently hide anyone excluded by the active Team-pill filter, with only a highlighted pill as the cue — Calendar has no such filter at all.** Jessica is active, on-roster, on CT Team, with a genuine approved 2026-10-23→30 leave request; the filter (auto-seeded to a supervisor's own crew, or a sticky explicit pick persisted in localStorage) was hiding her whole row. Fix: a banner ("Showing <team> only — N people hidden · View All") now appears whenever the filter is actively hiding someone. eq-field [PR #886](https://github.com/eq-solutions/eq-field/pull/886) (v3.5.646), merged on explicit "merge it", confirmed live via `field.eq.solutions/sw.js`.
- [x] **Root cause #2: Edit Roster shows the raw schedule cell (by design, so it's actually editable) with no leave awareness at all — a day covered by approved leave read as a plain empty cell, indistinguishable from genuinely unrostered.** Worse, the "needs filling in" empty-cell flag fired on those days too — its own leave-check (`isOnLeaveAllWeek`) tested raw schedule cells, which stopped being written with leave codes back in v3.5.281, so it had been structurally dead ever since. Royce typed a real code onto Jessica's Friday because the page gave him no way to know she was on leave. Fix: `renderEditor()` now reuses `approvedLeaveCode()` (same function Weekly Roster/Calendar call) to suppress the false nag and show a soft amber tint + the leave code as placeholder + a tooltip, which survives even if a real value already sits on that day. eq-field [PR #888](https://github.com/eq-solutions/eq-field/pull/888) (v3.5.648) — **CI green, deploy preview clean, NOT merged.** No "merge" instruction given this session. _(added 2026-09-02)_
- [ ] **4th surface found with the identical blind spot, not fixed: Batch Fill (`scripts/batch.js` `applyBatch()`).** Its people picker shows name + phone only, no schedule/leave context; its "skip cells that already have something" option checks the raw cell, which — same as Edit Roster before the fix above — is never populated for someone on leave, so it doesn't protect leave days either. Arguably higher risk than the single-cell case since it's a bulk write (one click can silently overwrite several people's approved leave across several days at once). Confirmed via direct code read, not speculative. Royce: "enough" — explicitly not building this now. _(added 2026-09-02)_
