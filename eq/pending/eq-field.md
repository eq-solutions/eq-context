...
- [x] **`PDFViewerApplication.open()` had no `.catch()`.** Proved live — against both production's deployed pdf.js and the edited copy via a throwaway local harness — that `open()` can reject. Any real fetch failure (expired signed URL, network blip, a bad file from eq-shell's new conversion pipeline) left the "Loading document…" spinner running forever with no error and no way back. Fixed: the poll stops and a plain failure message replaces the spinner. eq-field [PR #823](https://github.com/eq-solutions/eq-field/pull/823) (v3.5.593), squash-merged on explicit "merge", confirmed live via `field.eq.solutions/sw.js`.
...
- [ ] **Documents to Sign error state still has no functional fallback if a load failure is persistent, not transient** — "go back and try again" just re-runs the same viewer path, which fails the same way again. No way back to the old raw-file/`window.open()` path once a document has a PDF twin (`pdf_status='ready'`). Spawned as a background task (`task_445c878c`); Royce has already started it in a separate session. _(added 2026-08-28)_
