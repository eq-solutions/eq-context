...
- **Still open, not a code gap:** Madagins' own EQ Field data plane (`app_data.staff` on its dedicated project) is still 0 rows. All 5 known people (Royce, Aditi, Michelle, Nelson Sareto, Conor Horgan) have Shell-level tenant membership — confirmed live, all added within the first 90 minutes of the tenant's creation — but Shell membership doesn't create a Field staff record on its own. Everyone can likely reach Shell; nobody has an actual Field roster record yet.
...
1. ~~Settle Madagins' live state (the four gaps above) in one sequential pass~~ — **done**, see `## Resolution`. Remaining: create Field staff records for the 5 people with Shell access but no roster record.
