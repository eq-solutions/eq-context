...
While pulling this sprint doc's template, found the shared `C:\Projects\eq-context` checkout was **41 commits behind `origin/main`** with three files (`digest.md`, `suite-state.md`, this repo's own sprint-doc template) sitting mid-merge-conflict (`git status` showed `UU` — a `git stash pop` that had conflicted and was never finished, no `MERGE_HEAD` tracked). A repo-level guard (`guard.js`'s `stale-main-gate`, added after a 2026-08-08/09 incident with the same shape) correctly blocked a direct commit on that stale `main` — but its worktree-detection regex only matches paths containing `/worktrees/` or ending `-wt`, so a worktree created anywhere else (e.g. the session scratchpad directory) gets misclassified as "not a worktree" and blocked too. Worked around by relocating to the `<repo>\.claude\worktrees\<name>` convention every other repo already used this session.

**Both spun off as a background task, since fixed** — resolved via a fresh isolated clone, `main` fast-forwarded, working tree clean. See `sessions/2026-08-14.md`.

**Correction (2026-08-14, after the fact):** this entry originally also claimed `sessions/2026-08-03.md`, `sessions/2026-08-13.md`, and `system/failures.md` had literal conflict markers "already committed into history." That was wrong — a sloppy `git grep` match on the marker *text*, not verified against where it actually appeared. All three uses are prose, in past tense, narrating an *earlier* conflict-marker incident that a concurrent session had already resolved at the time each entry was written — not a currently-broken file. Should have checked for markers at line-start before calling it corruption. No fix was needed for those three; none was made.
