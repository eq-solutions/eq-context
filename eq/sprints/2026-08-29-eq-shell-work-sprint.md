...
1. **~~33~~ 24 endpoints missing the shared permission-check helper** — audit complete: the 33 figure was stale (6 of the originally-flagged examples were already fixed independently), real count 24. **11 of 24 done** — the highest-stakes ones: 8 platform-admin routes (led by `admin-tenants.ts`'s permanent tenant-delete) + 3 self-scoped credential-set routes (a deactivated user could otherwise still set their own PIN/email/prefs). [eq-shell#1682](https://github.com/eq-solutions/eq-shell/pull/1682), merged + live. 13 remain.
...
- ~~**33 data-changing endpoints don't use the shared permission-check helper**~~ — **audit complete, real count 24, 11 fixed.** The highest-stakes 11 (8 platform-admin routes incl. permanent tenant-delete, 3 self-scoped credential-set routes): [eq-shell#1682](https://github.com/eq-solutions/eq-shell/pull/1682), merged + live. 13 remain, tracked in Wave 4 above. 2026-08-15 → 2026-08-30.
