...
1. **33 endpoints missing the shared permission-check helper** — audit running now (background agent). Confirmed first: the fix that was supposed to reach all 92 mutating endpoints already covers 59 of them automatically (the active-account check moved into `requirePerm()` itself) — the genuinely open count is the 33 that never call `requirePerm` at all.
...
4. ~~**Function-grant safety net's `app_data` blind spot**~~ — **done, but bigger than described.** [eq-shell#1680](https://github.com/eq-solutions/eq-shell/pull/1680), merged. Turned out to be two gaps: zaap/ehow were also running an outdated, more dangerous version of the underlying trigger (the exact failure shape behind the eq-cards #0111 outage) — jvkn had already been quietly upgraded past it, live, undocumented. Ported that fix to the tenant planes too, alongside adding `app_data`. **Migration built, not yet dispatched to live zaap/ehow** — that's still a separate step given what it touches.
