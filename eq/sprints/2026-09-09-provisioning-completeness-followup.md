...
2. Ordering — **`0257` is fully closed out.** [PR #1843](https://github.com/eq-solutions/eq-shell/pull/1843) (`27006acf`) guards its unconditional REVOKE behind an existence check, live-verified against ehow via `BEGIN...ROLLBACK` before merge (guard short-circuits cleanly on nonexistent tables; full body against ehow's real schema reproduces the original's exact `anon: SELECT`-only result). Merged via admin-override past an unrelated, confirmed-pre-existing jvkn control-plane drift failure on the required schema-drift check (`task_98d5e636` tracking that separately — not this migration's doing). **`--reconcile-ledger` dispatched fleet-wide** ([run 34334292417](https://github.com/eq-solutions/eq-shell/actions/runs/34334292417), success) — `0257`'s checksum re-stamped on all 3 active tenants (eq, sks, madagins), ledger-only, no schema/data touched. Bonus finding: madagins had **39 duplicate ledger rows** (the `NNNN`/`NNNN.sql` two-runner artifact) deduped in the same pass — explains the 365-vs-339 row-count oddity found earlier. The internal 0308-vs-0309 dependency (this pair's own separate ordering issue) is still open — `0260` remains correctly out of scope, ehow-only.
...
- **`public.nominations` on zaap — RLS enabled, zero policies, live silent lockout, not
  ...
  nothing back. 0 rows at the time, so no data had actually been hidden yet. Fixed:
  [eq-shell#1878](https://github.com/eq-solutions/eq-shell/pull/1878) (migrations `0316`/`0317`
  — the second file also revoked a superfluous, non-exploitable `anon` grant on madagins's
  `nomination_clashes` view found in the same pass). Merged via admin override past an
  unrelated, confirmed-pre-existing control-plane drift-check regression. Dispatched
  fleet-wide, live-verified post-dispatch on all 3 planes.
- **`public.field_job_number_overrides` on madagins — full CRUD `anon` grant**, found while
  directly re-checking this table's literal substitution (which was itself correct). Not
  currently exploitable (the table's only policy targets `authenticated` only, so RLS denied
  anon regardless), but broader than the `nomination_clashes` view cleanup — a real write
  grant on a populated tenant table, and ehow's own copy has no such grant. Fixed:
  [eq-shell#1893](https://github.com/eq-solutions/eq-shell/pull/1893) (migration `0318`),
  merged and dispatched 2026-09-13, live-verified — madagins's grant list now matches ehow's.
- **Everything else checked clean, no fix needed**: `tender_enrichment`, `tenders`,
  `apprentice_profiles`, `tender_import_runs`, `tender_review_decisions` (correct on every
...
| 1b | Legacy-baseline migrations (0308/0309, renumber to 0311/0312 — re-verify at land time) | **`0257` fully done** — [PR #1843](https://github.com/eq-solutions/eq-shell/pull/1843) merged (`27006acf`) + `--reconcile-ledger` dispatched clean on all 3 tenants. 3 more decisions needed from you on the pair itself (scoping now covers 2 files, internal 0308-vs-0309 ordering, 2 missing `service.*` objects, 1 dead function to keep-or-drop) | — |
