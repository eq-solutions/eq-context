...
- v3.5.315 (PR #476, open) — mobile drawer icons → consistent Lucide. The slide-up hamburger menu mixed geometric Unicode (◈ ⬡ ◉ ◎) + colour emoji (☎ 🏖 🛡 👥) — misleading on a phone (Supervision=telephone, Leave=beach/boat, Contacts=dot). All 21 `.d-icon` swapped for inline Lucide SVGs (new `.d-icon svg` rule mirrors `.mnav-icon svg`); `stroke:currentColor` so icons tint with row state. Manager-lock toggles lock⇄lock-open via innerHTML. Browser-verified on preview; awaiting merge.
