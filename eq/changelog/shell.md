...
- Carried #681's `eq_update_staff` fix forward as migration **0172** (renumbered from 0165; #681's client-side `job_title` changes already reached main separately, so only the RPC hardening was carried — #681 closed as superseded). The RPC now `COALESCE`s email/phone/trade/level/employment_type against existing values instead of nulling them on a partial update. **Applied live 2026-07-11 on all 3 tenant planes** via the tenant-migrate One Pipe (Royce-approved production gate, `allow_checksum_drift=true`); verified ledger + COALESCE'd function on ehow/zaap/nxoj. The RPC has no live callers (defensive).
